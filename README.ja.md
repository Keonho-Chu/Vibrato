...
<p align="center">
  <img src="assets/hero.png" alt="Vibrato 自律型コーディングエージェントのヒーローイラスト" width="100%" />
</p>
...
<p align="center">
  ...
  <a href="#なぜ-vib-rato-なのか">なぜ</a> ·
  <a href="#手持ちのコーディングプランで">コーディングプラン</a> ·
  <a href="#スマホで答える">スマホ</a> ·
  <a href="#変更の前に計画">ワークフロー</a> ·
  <a href="#トークンを節約">トークン節約</a> ·
  <a href="#openclaw--hermes--grokbot--自分の-bot-に-vib-を動かさせる">コントローラ</a> ·
  <a href="#paseoorcat3-code-の中で-vib-を動かす">エージェントシェル</a> ·
  <a href="#ドキュメント">ドキュメント</a>
</p>

**すでに持っているサブスクリプションでログインし、ファイルが 1 つ変わる前に計画し、証拠とともに実行する — エージェントからの質問にはターミナル・スマホ・自前の bot、どこからでも答えられます。**

Vibrato（`vib`）は外付けのコーディングエージェントハーネスです。任意のリポジトリやワークツリーに放り込むだけ。追加の API 課金なし。トークン単価の不安なし。ターミナルの張り付きも不要。

> Vibrato は実験的なベータ段階のプロジェクトです。粗い部分が残っている可能性があるため、重要な作業では出力を検証してから利用してください。
>
...
| 問題 | 何が起きるか | Vibrato の解決策 |
| :--- | :--- | :--- |
| 別建ての API 課金 | プラン料金*に加えて*トークン従量の API 費用 | すでに払っているコーディングプラン（Claude または Codex）で `/login`、または自前ホストの vLLM/SGLang エンドポイントを使ってトークン課金自体をなくす |
...
vib ultragoal create-goals --brief-file <承認済みの計画>
```
...
```sh
vib                                # 現在のチェックアウトで実行
vib --tmux                         # tmux ベースのリーダーセッション
vib --tmux --worktree my-task      # リスクの高い作業向けの隔離ワークツリー
vib @screenshot.png "何を変えるべき？"   # 画像入力
```

Nightly チャンネル: `sh vib-install.sh --channel nightly`（上でダウンロードしたタグ付きインストーラ）。インストールマトリクス全体、Windows 設定、更新チャンネル、シェル補完: [docs/install.md](docs/install.md)。Bun が必要なのはソースからのビルドだけです。
...
<p align="center">
  <img src="assets/coding-plans-banner.png" alt="Vibrato が対応するプロバイダ: Claude、ChatGPT/Codex、そして自前ホストの vLLM/SGLang エンドポイント" width="100%" />
</p>

Vibrato が選択肢として公開するプロバイダはちょうど 4 つです: Claude、OpenAI Codex、vLLM、SGLang。一度ログインすれば、すでに払っているサブスクリプションで Vibrato が動きます。OAuth ベースの 2 プランはセッション内で `/login` を実行してください:
...
vLLM と SGLang は自前ホスト型 OpenAI 互換ランタイムです — サブスクリプションも OAuth も不要で、API キーは任意です（認証なしのローカルサーバーなら空欄のままで構いません）。使えるプロバイダが一つもない状態で初めて対話的に起動すると、Vibrato はプロバイダオンボーディングメニューを自動的に開きます。最初の項目 **Connect a vLLM endpoint** はサーバー URL（デフォルト `http://127.0.0.1:8000/v1`）と API キー（認証なしのローカル vLLM なら空欄）だけを尋ね、その後サーバーからモデルを直接検出します。2 番目の項目は SGLang（`http://127.0.0.1:30000/v1`）に対して同様に動作します。同じメニューはいつでも `/provider` から開けます。

CLI での等価コマンド:

```sh
vib setup provider --preset vllm --base-url http://HOST:8000/v1     # キーは VLLM_API_KEY から
vib setup provider --preset sglang --base-url http://HOST:30000/v1  # キーは SGLANG_API_KEY から
```

`http://` のみのベース URL は、localhost、loopback、プライベートネットワークホスト（`10/8`、`172.16/12`、`192.168/16`、link-local、`.local`/`.internal`/`.lan` 名、素の LAN ホスト名）でのみ受け付けられます。公開ホストには引き続き `https://` が必要です。

<details>
<summary><strong>デフォルトの 4 つの先へ: カスタムエンドポイントと高度なルーティング</strong></summary>

その他の内蔵プロバイダ（OpenAI API、Google、Bedrock、OpenRouter、xAI、Mistral、MiniMax、GLM、Kimi、Cursor、Copilot、OpenCode Go、Ollama/LM Studio などのローカルランタイム、その他多数）は `/login`、`/model`、プリセットピッカーからは外れますが、トランスポート自体は引き続き同梱されています。`models.yml` に自分の id でエンドポイントを登録すれば——隠されたプロバイダの API 形式を再利用する場合も含めて——引き続き完全に選択可能で、プロバイダごとに複数アカウントを使用量ベースでプールでき、モデルプリセット/プロファイルでエージェントロールごとに混ぜて使えます。チームの資格情報は引き続き auth ブローカー/ゲートウェイで集中管理できます。
...
<p align="center">
  <img src="assets/telegram-mobile-hero.png" alt="Vibrato モバイル回答のヒーローイラスト" width="100%" />
</p>
...
- **設定は一度だけ** — 実行中セッションの `/settings` → Notifications から、またはヘッドレスで `vib notify setup|status|health|test|recovery`。トークンは入力時にマスクされ、以後表示されません。
- **`vib daemon`** が bot トークンごとに安全な long-poll 所有者を 1 つ維持し、新しいセッションが Telegram 409 競合なしにクリーンに接続します。
...
Vibrato は Claude Code / Codex の `SKILL.md` ファイル規約を使いますが、ランタイムスキルを直接読み込むのは正規の Vibrato 場所だけです。設定は不要です:
...
Claude Code と Codex のスキルディレクトリはインポートソースに過ぎません。`vib skills discover` が正確なコピーコマンドとともに表示するため、スキルを正規の `.vib` 場所へコピーしてから、セッションで `/skill:my-skill` として呼び出してください。スコープの信頼は `skills.trustProjectSkills` / `skills.trustUserSkills` で明示され（どちらもデフォルトで有効）、`skills.enabled` がマスタースイッチです。上記の同梱ワークフロースキル 4 つはディスク上のスキルで置き換えられません。場所、優先順位、診断については [docs/skills.md](docs/skills.md) を参照してください。

## デフォルトテーマ

デフォルトのダーク TUI アイデンティティは LIG System の CI を適用した lig-blue テーマで、ライト外観のターミナルは同梱の lig-white テーマがデフォルトです。明示的なテーマ設定が常に優先されます。

---

## トークンを節約

Vibrato はトークン請求の両側を最適化します:
...
デフォルトのダーク TUI アイデンティティは LIG System の CI を適用した lig-blue テーマで、ライト外観のターミナルは同梱の lig-white テーマがデフォルトです。カタログ全体と `theme.dark` / `theme.light` の設定は [docs/theme.md](docs/theme.md) を参照してください。

## OpenClaw / Hermes / Grokbot / 自分の bot に Vibrato を動かさせる

OpenClaw、Hermes、Grokbot、Discord bot、cron スクリプトなど、どのような外部コントローラでも、ブローカーにバインドされた **SDK session CLI** と同梱の
[`sdk-skills/`](https://github.com/Keonho-Chu/Vibrato/tree/main/sdk-skills) 手順
（`vib-sdk-discover` · `vib-sdk-operate` · `vib-sdk-author`）を通じて本物の Vibrato セッションを動かせます。永続的な turn と資格情報を含まない JSON を使い、ターミナルスクレイピングは行いません。
...
[Coordinator MCP ブリッジ](docs/hermes-mcp-bridge.md)（`vib mcp-serve coordinator`、`vib setup hermes` でインストール）が、その形の委任ツールを提供します。
...
## Paseo・Orca・T3 Code の中で Vibrato を動かす

むき出しのターミナルではなくデスクトップ/モバイルのエージェントシェルを使いたい場合、Vibrato は代表的な 3 つに接続できます。ただしサポートの水準は率直に異なります。
...
<td>Vibrato 自身がインストールするネイティブ ACP プロバイダ。モデルカタログ、Default/Plan モード、thinking レベル、実際の権限プロンプト、所有するサブエージェントも停止できるキャンセル、モバイル制御。</td>
<td><code>vib setup paseo</code><br/><sub>その後 <code>paseo daemon restart</code></sub></td>
</tr>
<tr>
<td align="center">
...
<td>Vibrato はカスタム CLI エージェントとして動作し、セッションごとにワークツリーを分けます。Orca の差分レビュー、ターミナル分割、SSH ワークツリー、モバイルコンパニオンを使えます。使用量追跡とアカウントのホットスワップはまだありません。</td>
<td><strong>Settings → Agents</strong><br/>コマンドに <code>vib</code> を追加</td>
</tr>
<tr>
<td align="center">
...
<td>T3 Code は現在 Codex・Claude・Cursor・Grok・OpenCode のハーネスだけを提供しており、Vibrato ハーネスはまだ upstream にありません。現時点では横に並べて実行し、ネイティブプロバイダは <a href="https://github.com/pingdotgg/t3code/discussions/7290">upstream に提案</a>済みです。</td>
...
```sh
vib setup paseo            # ACP プロバイダエントリを書き込み、バックアップするが、デーモンは決して再起動しない
paseo daemon restart
paseo provider ls          # vib が `available` と表示される必要がある
paseo run --provider vib --cwd /path/to/repo "your prompt"

vib setup paseo --check    # pass / stale / drift。機械可読な --json も利用可能
vib setup paseo --remove   # Vibrato 自身が作成したキーだけをロールバック
```

Orca は 1 フィールドだけです。Vibrato をインストールして（[docs/install.md](docs/install.md) を参照）、引数なしのコマンド `vib` を持つカスタムエージェントを追加します。Orca は権限バイパスフラグを公開するエージェントにそのフラグを事前入力しますが、Vibrato には設計上そのようなフラグはありません。引数は空のままにして、Vibrato 自身の承認ゲートを維持してください。
...
- [カスタマイズの権限・インポート・信頼](docs/customization.md) · [スキル](docs/skills.md) · [フック](docs/hooks.md) · [スタンドアロン MCP](docs/standalone-mcp.md) · [プラグインバンドル](docs/vib-plugins.md)
...
デフォルトのダーク TUI アイデンティティは LIG System の CI を適用した lig-blue テーマ。ライト外観のターミナルは同梱の lig-white テーマがデフォルトです。切り替えや自作は[テーマ](docs/theme.md)へ。

## SDK 拡張

### ローカルカスタマイズ: `/extensions`

対話セッションでは、`/extensions` が主要なカスタマイズ設定画面です。プロジェクト（`<project>/.vib/`）とユーザー全体（`~/.vib/agent/`）のスコープにわたるスキル、フック、MCP を設定し、状態/出所の診断、有効化/無効化/削除、Claude Code/Codex からのガイド付きインポート（正規化プレビュー、明示的な確認、衝突時のスキップ/リネーム/上書きポリシー、ロールバック可能な原子的書き込み）を提供します。非対話的な設定では、MCP サーバーに `vib mcp`、Claude Code/Codex のインポートに `vib migrate` を使います。

### スキルの移行と同梱スキルの検査

ワークフローを Vibrato に移す場合は、インストールまたは上書きする前に同梱のデフォルトを検査してください:
...
`vib setup defaults` は同梱の Vibrato ワークフロースキル 4 つをユーザーの `.vib` ディレクトリにインストールし、デフォルトでは既存のローカルファイルを保持します。`--check` が欠落または差異のあるファイルを報告したら、まず `vib skills read <name>` で埋め込みコピーと比較してください。ローカルのデフォルトワークフロースキルファイルを意図して置き換える場合にだけ `vib setup defaults --force` を使います。

## 既存のエージェントや bot と併用する

| ツールまたは bot | 推奨する Vibrato コマンド | 境界 |
| --- | --- | --- |
| Codex CLI | `vib --tmux --worktree <name>` または `vib` | `--worktree` は Vibrato 管理の隣接ワークツリーに名前を付けます。既存のパスでは先にそこへ `cd` してください。 |
| Claude Code | `vib --tmux` または `vib --tmux --worktree <name>` | Vibrato は Claude Code の拡張にはなりません。 |
| OpenCode | `vib` または `vib --tmux` | 現時点では外部ランナーのワークフローのみです。 |
| Claw Code | `vib --tmux --worktree <name>` | Vibrato は Claw Code にインストールされたり、置き換えたりしません。 |
| [Paseo](https://paseo.sh) | `vib setup paseo` | Vibrato は自身を ACP プロバイダとして登録し、`--remove` で自身の変更だけを戻します。Paseo 自身の設定ファイルは Paseo が所有します。 |
| [Orca](https://onorca.dev) | カスタムエージェントコマンドとして `vib` | Orca は自身のワークツリーターミナルで Vibrato を起動し、Vibrato は自身の承認ゲートを維持します。 |
| [T3 Code](https://t3.codes) | まだなし — 実験的 | upstream に Vibrato ハーネスはありません（[提案](https://github.com/pingdotgg/t3code/discussions/7290)）。ドライバーが届くまで Vibrato を横に並べて実行してください。 |
| 外部コントローラ / bot | Coordinator MCP、`vib sdk session`、または構成済みのマネージドアダプタ | 外部コントローラは、スクロールバックや直接エンドポイント転送ではなく、ブローカーにバインドされ資格情報を含まない表面を使います。ホスト中立の `vib-sdk-*` スキルは `vib sdk session` を組み合わせ、Coordinator 統合をインストールしません。 |
...
- [vib-remote](https://github.com/kogangdon/vib-remote) — Discord からリモートホスト上の許可リスト済み Vibrato セッションを制御する実運用の SDK 拡張です。
- [oh-my-vib-rato](https://github.com/devswha/oh-my-vib-rato) — 追加のワークフロースキルとスラッシュコマンドをインストールするコミュニティプラグインマーケットプレイスです。
- [vib-agy-skill](https://github.com/jkf87/vib-agy-skill) — Antigravity CLI を通じたビジョン/OCR、画像生成、Gemini print-mode のワンショットワークフローを統合するサードパーティのコミュニティ Vibrato スキルです。
- [Vibrato マルチベンダーセットアップガイド](https://github.com/project820/vib-multivendor-setup-guide) — マルチベンダー Vibrato 環境向けのロールベースプロバイダプロファイルとインストール可能なモデルバンドルです。現在の 4 プロバイダ許可リストより前に書かれたガイドです。Claude/Codex/vLLM/SGLang 以外のロールは、対応するカスタム `models.yml` エントリで自分で再現してください。

## 設定

プロバイダの再試行予算は `~/.vib/config.yml` にあります:
...
対話的な起動では、デフォルトでバックグラウンドから新しい Vibrato バージョンを GitHub Releases で確認します。この確認は通知専用で変更を加えません。Vibrato が起動中に自分自身をインストールまたは置き換えることはありません。対応プラットフォームでは `vib update` が一致する GitHub リリースバイナリをダウンロードして原子的に置き換えます（パッケージマネージャの shim は上書きしません）。ソースチェックアウトと `dev:link` 実行ファイルはそのチェックアウト経由で更新する必要があり、`vib update` はそれらの自己上書きを拒否します。未対応プラットフォームでは文書化されたインストーラを再実行してください。

起動時の確認を無効にするには `vib config set startup.checkUpdate false` を実行します。ネットワーク障害は起動を妨げないよう無視されます。

### あわせて読む資料

- [Vibrato マルチベンダーセットアップガイド](https://github.com/project820/vib-multivendor-setup-guide) — Anthropic、OpenAI/Codex、Google/Gemini、xAI/Grok、opencode-go にわたるロール別のプロバイダ/プロファイル選択のためのコミュニティガイドです。そのプリセットは同梱のデフォルトではなくユーザーレベルの設定ガイダンスとして扱い、導入前に自分の環境でモデルの可用性とプロバイダ認証を確認してください。Google/Gemini、xAI/Grok、opencode-go は Vibrato のデフォルトのプロバイダ選択からは隠されているため、それらのロールは自分の `models.yml` カスタムプロバイダエントリでのみ再現できます。

## TUI アイデンティティ

デフォルトのダーク TUI アイデンティティは LIG System の CI を適用した lig-blue テーマで、ライト外観のターミナルは同梱の lig-white テーマがデフォルトです。追加の同梱移行テーマ `claude-code`、`codex`、`opencode` はそれらのツールの見た目を再現しており、目が慣れた環境へ移りやすくするため Settings または `/theme` から選べます。明示的なユーザーテーマ設定が常に優先されます。
...
| テーマ | 見た目 | 向いている用途 |
| --- | --- | --- |
| `lig-blue` | ダークのデフォルト: LIG Innovative Blue のステータス行、白と LIG Futuristic Gray の文字、中立的な `◆` マーク。 | ダークターミナルでの LIG System CI。 |
| `lig-white` | 白地に LIG Innovative Blue の見出し・リンク・ステータス行。 | ライトターミナルまたは OS の外観での LIG System CI。 |
| `red-claw` | 温かい red-claw のアクセントと甲殻類シンボルを持つ旧ダークパレット。 | CI 適用前の見た目を好む場合。 |
| `blue-crab` | 旧・明るいターミナル向けの青いパレット。 | CI 適用前のライトターミナルの見た目。 |
| `claude-code` | テラコッタとピンクのハイライトを備えた Claude Code 風ダークパレット。 | Vibrato 内で維持する Claude Code の慣れ。 |
...
```sh
vib customize doctor         # 人間が読める出所と対処方法
vib customize doctor --json # CI/設定ツール向けの安定した JSON
```

`vib customize doctor` は唯一の読み取り専用トラブルシューティング表面です。検出されたすべてのカスタマイズ、出所の規約とスコープ（`vib`、Claude プロジェクト、Codex プロジェクト、プラグイン、明示的な設定）、実効優先順位とシャドーイング、loaded/enabled/disabled/quarantined/rejected/stored-only の状態、制限された理由コード、修復コマンド、信頼要件、再起動/新規セッションが必要かどうかを報告します。資格情報、エンドポイントトークン、認証ヘッダ、安全でない生の設定ダンプは決して表示しません。
...
bun run dev:link       # グローバルの `vib` がこのチェックアウトのソースを実行
