# Analyze Me with Vibrato

Use this prompt for meetup icebreakers where each participant asks Vibrato to introduce them from their own local Vibrato usage history.

The prompt is designed to be repeatable: it tells Vibrato what local artifacts to inspect, what patterns to extract, how to avoid leaking secrets, and how to turn the analysis into a short spoken self-introduction.

## Full prompt

```text
~/.vib 에 있는 내 Vibrato 사용내역을 바탕으로, Vibrato 밋업 아이스브레이킹용 “Vibrato가 보는 나” 자기소개 글을 작성해줘.

목표:
- 내 실제 Vibrato 사용패턴을 분석해서, 내가 어떤 개발자/빌더/운영자인지 소개하는 글을 써줘.
- 단순 통계 나열이 아니라, 사용 습관과 관심사에서 드러나는 성향을 해석해줘.
- 밋업에서 2~4분 정도 읽을 수 있는 분량으로 작성해줘.
- 너무 딱딱한 리포트 말고, 사람 소개글처럼 재미있고 선명하게 써줘.
- 과장하거나 없는 사실을 만들지 말고, 실제 ~/.vib 기록에서 관찰된 패턴만 근거로 삼아줘.

분석 지시:
1. ~/.vib 디렉터리 구조를 먼저 확인해줘.
2. 가능한 경우 아래의 안전한 메타데이터 중심 자료만 분석해줘:
   - ~/.vib/agent/history.db 의 집계값
   - ~/.vib/agent/sessions/**/*.jsonl 의 세션 메타데이터(세션 제목, timestamp, cwd, 메시지 수, tool-call 수, 파일 크기, subagent/task 이름)
   - ~/.vib 내부의 추가 파일은 사용패턴 집계에 꼭 필요하고 민감정보가 없다고 판단되는 경우에만 읽어줘.
   - 기본적으로 ~/.vib/logs/*, auth/config/credential 파일, env dump, raw tool-result body, raw prompt body, secret-like 값은 읽지 마.
...
5. 레포지토리와 주제 다양성을 꼭 반영해줘:
   ...
   - Vibrato core, 개인 프로젝트, 연구/quant, infra, UI, automation, image/media 등 주제 범위가 보이면 묶어서 설명해줘.
6. 민감정보는 절대 노출하지 마:
...
```text
~/.vib 사용내역을 분석해서 밋업 아이스브레이킹용 “Vibrato가 보는 나” 자기소개 글을 써줘.

반드시 실제 ~/.vib 기록을 읽되, 안전한 메타데이터와 집계값 중심으로 근거 기반 작성해:
...
민감정보는 읽지도 출력하지도 마. API key, 토큰, private credential, 개인 secret, 긴 원문 프롬프트, raw tool-result body, ~/.vib/logs/*, auth/config/env dump는 기본적으로 건너뛰고, 필요한 경우에도 안전한 집계값과 짧은 paraphrase만 써.
...
## Optional: VibratoTI prompt

A meetup host can also turn the same analysis into a playful, MBTI-like “VibratoTI” result. This is only an icebreaker taxonomy, not a psychological assessment.

```text
~/.vib 사용내역을 안전한 메타데이터와 집계값 중심으로 분석해서, 밋업 아이스브레이킹용 “VibTI”를 만들어줘.

목표:
- MBTI처럼 4글자 코드와 타입명을 만들되, 실제 성격검사가 아니라 Vibrato 사용패턴 기반의 재미있는 작업 스타일 분류로 작성해.
- 실제 ~/.vib 기록에서 확인한 사용패턴만 근거로 삼아줘.
- 민감정보는 읽지도 출력하지도 마. API key, 토큰, private credential, 개인 secret, 긴 원문 프롬프트, raw tool-result body, ~/.vib/logs/*, auth/config/env dump는 기본적으로 건너뛰고, 안전한 집계값과 짧은 paraphrase만 써.
...
## 타입 설명
밋업에서 1분 정도 읽을 수 있게, 이 사람이 Vibrato를 어떻게 쓰는 사람인지 재미있게 설명해줘.
