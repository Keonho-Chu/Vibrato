...
1. **Custom providers** — point Vibrato at any OpenAI/Anthropic-compatible endpoint, proxy, or local runtime through `~/.vib/agent/models.yml`.
...
## Custom providers (`~/.vib/agent/models.yml`)
...
`vib setup provider --preset` ships two ready-made presets, `vllm` and `sglang`, both requiring `--base-url` and both backed by live model discovery against the server's `/v1/models`. Every other coding-plan preset that used to render a provider block for you (GLM, MiniMax, Alibaba Token Plan, ClinePass, Command Code GOAT, and the rest) has been retired from the preset picker along with its built-in provider; hand-write the equivalent `providers:` block above instead. See [`models.md` → Coding-plan provider presets](./models.md#coding-plan-provider-presets).
...
Vibrato treats stored OAuth credentials as an account pool. API-key sources can be visible in the same inventory, but the account-pinning and selective-removal UX is OAuth-only.

### `/login`: provider picker, account picker, and session scope

- Bare `/login` opens the OAuth provider picker, now limited to the two allowlisted OAuth providers (`anthropic`, `openai-codex`/`openai-codex-device`; see [Supported providers](./models.md#supported-providers)). `/login <provider>` targets one provider directly, including a hidden built-in provider reproduced under a custom `models.yml` id. vLLM and SGLang are not OAuth providers and are not part of this picker — configure them through `/provider` instead.
...
- A session selection is recorded with the session's credential scope, so it does not silently change another live session. Use `vib accounts pin ... --persistent` when the intent is global for future sessions.
- With a direct broker configured, the same OAuth login path uses the broker's remote write hooks; the refresh token remains on the broker. `vib auth-broker login <provider>` is the broker-host CLI flow when the operator is working on the broker host instead.
...
A direct-broker client has no local hard-removal targets for individual rows, so `/logout` cannot selectively remove a broker row. Use `vib auth-broker logout <provider>` on the broker host for provider-wide removal. The separate `vib accounts logout` command is explicitly local-only and refuses to run when broker mode is configured.
...
### `vib accounts` command grammar

The command is intentionally payload-free and has four actions:

```text
vib accounts list [--json]
vib accounts check [<provider>] [--json]
vib accounts pin <provider> <selector> --persistent [--json]
vib accounts pin <provider> --clear --persistent [--json]
vib accounts logout <provider> --account <id|email> [--json]
vib accounts logout <provider> --all [--json]
```
...
API-key rows remain visible and checkable so operators can see which source is selected, but they are not part of OAuth account pooling, cannot be pinned by `vib accounts pin`, and cannot be removed by `vib accounts logout`.

### `vib accounts --json` output and failure contract

With `--json`, `vib accounts` writes exactly one JSON document to stdout. A successful action uses an `{ "ok": true, ... }` envelope. An action-level failure uses `{ "ok": false, "error": { "code": "...", "message": "..." } }`, sets a nonzero exit status, and never writes a second stdout document. Diagnostics written to stderr are sanitized and bounded; they must not contain credential payloads, stacks, or unredacted provider responses. A completed `check` may still use an `ok: true` envelope with per-account failed/unknown check rows; its nonzero status reports those probe results rather than a command-format failure.

## Persistent auth configuration

The canonical configuration is nested YAML in the global `~/.vib/agent/config.yml` (or the configured agent directory):
...
- `auth.broker.url` and `auth.broker.token` select direct-broker mode. `VIB_AUTH_BROKER_URL` and `VIB_AUTH_BROKER_TOKEN` take precedence over the nested values; nested values may be literal strings or trusted `$ENV_NAME` references. Resolution order remains explicit env → nested value (after indirection) → the owner-only `<config-dir>/auth-broker.token` file, so an unresolved nested token does not displace the token-file authority. A configured URL without a resolvable token is a hard error; Vibrato does not silently fall back to local SQLite. An absent config file leaves broker mode disabled, while malformed or unreadable global startup-auth config fails closed with a typed `StartupAuthConfigError`.
- `auth.credentialRankingMode` is `balanced` (default) or `earliest-reset`. `VIB_CREDENTIAL_RANKING_MODE` takes precedence over the nested setting.
...
| Capability | Local SQLite client | Direct-broker Vibrato client | Auth-gateway service |
| --- | --- | --- | --- |
| Credential writer | Local `agent.db` | Broker host's `agent.db` | Broker host's `agent.db` (gateway is a broker client) |
| OAuth login/add | `/login` picker and **Add new account** write locally | `/login` picker and **Add new account** write through the broker; `vib auth-broker login` also works on the broker host | No login picker; use `vib auth-broker login` on the broker host |
| Logout/removal | `/logout` can remove one local OAuth row or all local OAuth rows; `vib accounts logout` selects by `--account` or `--all` | Selective local removal is unavailable; use provider-wide `vib auth-broker logout` on the broker host; `vib accounts logout` refuses in broker mode | No account-removal API; mutate the broker directly |
| Inventory/check | `/usage` and `vib accounts list` are cache-only; `/usage check` and `vib accounts check` probe sequentially | Same client presentation/check contract; expired OAuth refreshes route through the broker | One gateway instance owns one explicit provider scope; `vib auth-gateway check` probes only that provider and `GET /v1/usage` serves scope-filtered cached usage |
| Pin/ranking | Session pins/AUTO plus global `vib accounts pin --persistent` and ranking mode | Same Vibrato session/global controls; the broker remains the credential writer | The service has no TUI session scope; broker clients/operators control pins and ranking |
...
`/usage`, `vib accounts list`, and the account picker render the current inventory plus retained health/usage observations. They do not probe providers merely to render a page. Broker-backed clients may receive a background snapshot/metadata update. An explicit check returns fresh per-row results and retains safe health state; it does not change the cache-only contract of the plain commands.

## Secret and log safety

- Account inventory, `/usage`, `/usage check`, and `vib accounts --json` intentionally omit credential payloads and raw provider response bodies. Error reasons are bounded and scrub credential-shaped strings.
