...
explains how Vibrato relates to Claude Code and OpenAI Codex layouts without making
the per-surface references repeat one another.

## The authority boundary

Vibrato has two canonical persistence scopes:

- **Project:** `<project>/.vib/` (the repository's project root, or the opened
  project directory when there is no repository root).
- **User:** `~/.vib/agent/` (the canonical user agent directory; the configured
  home-relative Vibrato config root and its legacy skill roots are described in
  [Skills](./skills.md)).

These `.vib` scopes are the long-term Vibrato authority. A normal standalone session
...
used to invent another Vibrato configuration scope.
...
the chosen `.vib` destination. Import does not edit the source files. MCP and
...
native `.vib/hooks/` modules to `ExtensionRunner`. Claude/Codex directory hook
...
`.vib/hooks/` copy and its provenance boundary.
...
| Surface | Native Vibrato project | Native Vibrato user | Claude Code project source | Claude Code user source | Codex project source | Codex user source | Vibrato treatment |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **MCP** | `<project>/.vib/mcp.json` | `~/.vib/agent/mcp.json` | `<project>/.mcp.json` for the #4492 import transaction; the doctor also reports `.claude/mcp.json` and `.claude/.mcp.json` convention candidates | `~/.claude.json` for the import transaction | `<project>/.codex/config.toml`, `[mcp_servers.<name>]` | `~/.codex/config.toml`, `[mcp_servers.<name>]` | Only native `.vib` MCPs autoload in ordinary standalone sessions. Import adapters normalize bounded JSON/TOML entries, validate them, and write native `mcp.json`. |
| **Skill** | `<project>/.vib/skills/<name>/SKILL.md` | `~/.vib/agent/skills/<name>/SKILL.md` | `<project>/.claude/skills/<name>/SKILL.md` | `~/.claude/skills/<name>/SKILL.md` | `<project>/.codex/skills/<name>/SKILL.md` | `~/.codex/skills/<name>/SKILL.md` | Native `.vib` skills are loaded by Vibrato. Claude/Codex skills are import candidates; they are not loaded directly into a Vibrato session. |
| **Hook** | `<project>/.vib/hooks/pre|post/<file>` | `~/.vib/agent/hooks/pre|post/<file>` | `<project>/.claude/hooks/pre|post/<file>` | `~/.claude/hooks/pre|post/<file>` when explicitly selected for import | `<project>/.codex/hooks/pre-<tool>.ts` / `post-<tool>.ts` | `~/.codex/hooks/pre-<tool>.ts` / `post-<tool>.ts` when explicitly selected for import | Ordinary sessions execute only canonical native `.vib` directory hooks. Claude/Codex layouts are explicit import and diagnostic sources; accepted imports are normalized to canonical `pre`/`post` phases. Codex-managed `hooks.json` remains Codex-owned. |
...
forms. Host-specific files surfaced by `vib customize doctor` remain
...
1. Open the project or global `.vib` dashboard scope.
...
A project import writes beneath `<project>/.vib/`; a user import writes beneath
`~/.vib/agent/`. A project source does not become user configuration, and a
...
For non-interactive MCP/skill-only migration, `vib migrate --from
...
preview. Direct native MCP registration uses `vib mcp add`; see the linked MCP
reference.
...
- **Skills:** project `.vib/skills` wins over user scope. Within project scope,
  ...
  legacy root and historical `~/.vib/skills` root. Duplicate names are
...
- **Hooks:** native project hooks win over the same native user hook. The
  capability registry gives the native Vibrato provider precedence over the Claude
...
- **MCP:** a native project server wins over a native user server with the same
  ...
  [Vibrato plugin bundles](./vib-plugins.md). Claude/Codex MCP files do not enter
...
and the winner rather than silently deleting the loser. `vib customize doctor`
...
- `skills.trustProjectSkills` controls project `.vib/skills`.
- `skills.trustUserSkills` controls user `.vib` and legacy user roots.
...
uses a hook directory. They execute as code in the Vibrato process and the normal
...
an operating-system sandbox; see [Vibrato plugin bundles](./vib-plugins.md).

Codex's managed `hooks.json` is a different authority. `vib setup hooks` writes
managed `UserPromptSubmit` and `Stop` entries that invoke
`vib codex-native-hook`; Codex owns their scheduling, timeout, cancellation,
environment, and command logging. Vibrato does not claim Claude named settings-hook
...
1. **Preview is read-only.** It scans only the selected product/scope and
   ...
   serialization-safe preview. Building a preview does not create `.vib`
   files.
...
- `vib customize doctor --json` is read-only and never emits credentials,
  endpoint tokens, auth headers, environment values, or unsafe raw config.
  `vib mcp list` and the dashboard use the same redacted-display posture.
...
| `imported` | A native skill retains an import provenance marker. | Treat `.vib` as the authority; inspect the source only for comparison. |
...
| `quarantined` | A plugin surface failed its integrity or security policy. | Follow the plugin quarantine detail; do not bypass it by copying foreign files into `.vib`. |
...
foreign file exists; it is not evidence that Vibrato loaded it.
...
- [Vibrato plugin bundles](./vib-plugins.md) — loose `.vib` customization versus
...
[#4492](https://github.com/Keonho-Chu/Vibrato/pull/4492), resolving the
umbrella customization behavior tracked by [#4291](https://github.com/Keonho-Chu/Vibrato/issues/4291).
