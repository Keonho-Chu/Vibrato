...
| `VIB_NATIVE_VARIANT`          | Forces the x64 native addon variant; accepts only `modern` or `baseline`, and ignores anything else. Unset or blank falls through to the pre-rebrand `PI_NATIVE_VARIANT`, then to CPU detection. See [natives architecture](./natives-architecture.md). |
