...
| --- | --- | --- |
| SDK session CLI | `vib sdk session list|inspect|send|status|tail` or `raw control|query|global` | A local script needs bounded, credential-free session operations. |
| Coordinator MCP | `vib mcp-serve coordinator` | A controller needs multi-session orchestration, durable reports, or worktree-scoped lifecycle operations. |
...
| ACP | `vib --mode acp` or `vib acp` | An editor or ACP-compatible client supplies the session frontend. |

`--mode rpc`, `--mode rpc-ui`, `--mode bridge`, and `vib sdk serve` have been removed;
...
Owned termination is an explicit opt-in: `_meta.vib.abortScope: "owned"` on the
`session/cancel` notification, or `VIB_ACP_ABORT_SCOPE=owned` in the agent environment as
...
Add Vibrato through Air's **Add Custom Agent** action, then configure the Air-managed `acp.json`. With only `["acp"]`, Air shows Vibrato's existing model list. Add `--mpreset <id>` only when the Air model selector should show the available Vibrato preset list and create new sessions with that preset.
...
`always-allow` gives the agent permission to execute gated tools, including shell commands, without an Air approval prompt. Omit `VIB_ACP_PERMISSION_MODE` or set it to `prompt` when manual approval is required. Start a new Air task after changing `acp.json`; restart Air if it reuses an already-running agent process.

Air supplies MCP servers through ACP session requests. Vibrato accepts client-supplied stdio, HTTP, and SSE definitions for new sessions and offline resume. Do not add `--mcp-config` to the ACP command: that CLI option is intentionally unsupported for broker-backed ACP. A live session's MCP configuration is immutable; reconnect declarations from Air attach to the existing configuration instead of attempting to replace it. Close or resume the offline session to change its MCP configuration.
...
[Paseo](https://github.com/getpaseo/paseo) registers Vibrato as a generic ACP provider through its custom provider configuration. Add this entry to `$PASEO_HOME/config.json` (default `~/.paseo/config.json`); Paseo then lists **Vibrato** in its provider picker with Vibrato's model catalog and Default/Plan modes:
...
Vibrato's ACP session configuration carries the spec-defined `category` on the Mode, Model, and Thinking select options (`mode`, `model`, `thought_level`), which lets ACP clients such as Paseo discover models and thinking levels without provider-specific metadata. The model catalog is filtered to providers with usable stored credentials (`providers.list/active`), falling back to the full catalog on session hosts that do not expose that query.

Model profiles also appear in the ordinary **Model** picker as synthetic entries under the reserved namespace, e.g. `vib-rato/codex-eco` (displayed with the profile label, such as "Codex Eco"). Selecting one through the ACP `Model` select immediately switches the live session to the full profile without persisting `modelProfile.default`; persistence remains an explicit `/model` TUI choice or `vib --mpreset codex-eco --default`. Only profiles whose providers have usable stored credentials are selectable; synthetic rows are already availability-filtered by the session host, so the Q29 active-provider filter never drops them. An unavailable-but-active profile stays visible as the current readback and, if selected, fails with the existing authentication-required error. The separate ACP startup `--mpreset`/Q27 `Preset` select is likewise session-scoped and non-persistent.

Sessions launched through an ACP client (e.g. `paseo run --provider vib/...`) are broker-managed and appear in ACP `session/list`, so Paseo's import flow can attach them. Interactive `vib` sessions host their own SDK endpoint and are not broker-registered, so they are not listed by ACP clients; use the Vibrato SDK/notifications surface to control those sessions.
...
commit `47dc1c56b20da3c248a4a1b5c5106f52e65e6594` against `vib --mode acp`
...
for the tested Air and Vibrato builds, attach only redacted logs, and record the
