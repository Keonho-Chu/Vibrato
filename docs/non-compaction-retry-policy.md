...
7. For usage-limit errors, call auth storage (`markUsageLimitReached(...)`); if credential switching succeeds, force delay to `0`, otherwise use the applicable backoff. A `quota` failure against an API-key pool skips this entirely and records a session-side hold instead, unless `retry.rotateCredentialsOnQuota` is enabled — see "Daily-quota holds" below.
...
- No credential movement for an API-key pool, this turn or any later one. Every API key stored for a provider is used against that provider's one baseUrl, so reaching the next key retries the same gateway allowance under a different identity and bypasses the gateway's audit and quota boundary. `#markFailedCredential` therefore leaves the stored row completely untouched: marking it `markUsageLimitReached` would not rotate immediately, but a blocked row is skipped by API-key selection, which hands the NEXT turn to the second key. The reset instant is recorded on the session instead and read back by `#quotaRetryableAtMs()`. `retry.rotateCredentialsOnQuota` (default `false`, no settings UI) restores the old behavior for a deployment that genuinely owns several independent quotas behind one provider id.
- OAuth pools are out of scope and unchanged. Several OAuth rows are several accounts the operator owns, and switching between them when one subscription hits its usage limit is an existing feature rather than a gateway-boundary bypass.
- The failed selector is suppressed until the reset instant, with a reason recorded alongside the window and readable through `ModelRegistry.getSelectorSuppressionReason(...)`. That accessor deliberately does not consume the one-shot `expired` observation the revert policy above depends on. The surfaced error names the daily limit and its reset time rather than leaving a bare 429 that reads like a rejected key.
