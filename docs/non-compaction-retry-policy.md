...
7. For usage-limit errors, call auth storage (`markUsageLimitReached(...)`); if credential switching succeeds, force delay to `0`, otherwise use the applicable backoff. A `quota` failure on an API-key credential skips this entirely and records a session-side hold instead, unless `retry.rotateCredentialsOnQuota` is enabled; OAuth credentials keep this path — see "Token-limit holds" below.
7a. A `quota` failure whose Retry-After is at least 60 s is a token-limit hold: the failed selector is suppressed until the reset instant with a reason, the same model is not retried, and a managed chain advances at most one entry instead of spending its per-entry budget.
8. Eligible ordered role-array fallback chains advance on entry-budget exhaustion. A selected fallback entry remains sticky until the head selector's suppression cooldown expires — a rate-limit cooldown, or since issue #14 a token-limit hold — when `retry.fallbackRevertPolicy: cooldown-expiry` probes it again on a new turn.
...
- `retry.rotateCredentialsOnQuota` (default `false`) — operator escape hatch that re-enables API-key switching on `quota` failures; OAuth pools ignore it and switch either way. See "Token-limit holds"
...
A `quota` trigger carrying a Retry-After of at least 60 s (`QUOTA_TERMINAL_RETRY_AFTER_MS`) is an allowance spent for the upstream's current quota window, not congestion. Capping such a hint at `retry.maxDelayMs` would re-issue the request seconds after a gateway asked for hours, so the hold is terminal for the failed model instead:

The window is the upstream's business, not this policy's. The Vibrato gateway counts its token limit per window of `VUG_QUOTA_WINDOW_HOURS`, an operator setting that defaults to 24 hours and accepts any divisor of a day, with windows cut at that gateway's own configured timezone offset. Its wire contract keeps the `daily_token_limit` code and `x-vug-daily-*` header names for stability even when the window is shorter than a day. Nothing here reads the window length: the Retry-After alone decides, so every window from three hours to a day lands the same way. User-facing strings are therefore window-neutral ("token limit reached") rather than claiming a daily reset the operator may not have configured. Connecting to such a gateway, and the headers it reports back, are covered in [Gateway connections](./models.md#gateway-connections).
...
- The 60 s threshold has one deliberately uncovered shape: a per-minute token bucket that reports a quota code rather than a rate-limit one. Its Retry-After sits at or just above 60 s, so the turn ends instead of waiting out a roughly one-minute window. That is the accepted cost of not retrying a genuinely spent quota window; a provider in that shape should report a rate-limit code, which keeps its ordinary retry budget.
