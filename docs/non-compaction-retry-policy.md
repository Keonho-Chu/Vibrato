...
The window is the upstream's business, not this policy's. The Vibrato gateway counts its token limit per window of `VUG_QUOTA_WINDOW_HOURS` (24 by default, 3 in production, resetting at KST 00:00, 03:00 and so on), and its wire contract keeps the `daily_token_limit` code and `x-vug-daily-*` header names for stability. Nothing here reads the window length: the Retry-After alone decides, so every window from three hours to a day lands the same way. User-facing strings are therefore window-neutral ("token limit reached") rather than claiming a daily reset the operator may not have configured.
