...
7. For usage-limit errors, call auth storage (`markUsageLimitReached(...)`); if credential switching succeeds, force delay to `0`, otherwise use the applicable backoff. A `quota` failure skips this entirely and records a session-side hold instead, unless `retry.rotateCredentialsOnQuota` is enabled — see "Daily-quota holds" below.
...
- No credential movement at all, this turn or any later one. Every credential stored for a provider is used against that provider's one baseUrl, so reaching the next one retries the same allowance under a different identity and bypasses the gateway's audit and quota boundary. `#markFailedCredential` therefore leaves the stored row completely untouched: calling `markUsageLimitReached` would not rotate immediately, but a blocked row is skipped by selection, which hands the NEXT turn to the second identity. The reset instant is recorded on the session instead (`#recordQuotaHold`) and read back by `#quotaRetryableAtMs()`. It is computed by `AuthStorage.getUsageLimitResetAtMs(...)`, which is the same computation `markUsageLimitReached` uses, so a subscription usage report still raises a bare Retry-After to the real reset.
- This covers OAuth pools too, which means multi-account switching on a subscription usage limit no longer happens while a hold is active. `retry.rotateCredentialsOnQuota` (default `false`, no settings UI) restores the previous behavior — both API-key switching and multi-account OAuth switching — for a deployment that genuinely owns several independent quotas behind one provider id.
