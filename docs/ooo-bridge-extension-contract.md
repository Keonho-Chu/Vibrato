...
Vibrato exposes the `ooo` bridge through the existing extension input-event surface. It is not a default workflow skill, hook, slash command, or built-in agent.
...
```ts
import { createOuroborosOooBridge } from "@vib-rato/coding-agent/extensibility/extensions";

export default function activate(vib) {
  vib.on("input", createOuroborosOooBridge());
}
```
...
- `ooo interview [topic]` starts `ouroboros_interview` through a lazily connected `ouroboros mcp serve --runtime vib` stdio server.
...
- Other exact-prefix `ooo ...` commands run `ouroboros dispatch --runtime vib <full-input>` through `createExactPrefixCommandBridge()`.
...
This path is verified against [Q00/ouroboros `v0.50.7`](https://github.com/Q00/ouroboros/releases/tag/v0.50.7). Install its MCP profile at the exact version, then configure Vibrato:

```bash
uv tool install 'ouroboros-ai[mcp]==0.50.7'
ouroboros setup --runtime vib
```
...
### Verified Vibrato bridge installation

Ouroboros setup installs its own managed Vibrato bridge. Replace it with the standalone Vibrato bridge from immutable commit `4311fefd49e9c6781c4d1111b8dd3f758e7d8974`, whose example file has SHA-256 `2b0e1e25ac145331f112da629076875542db6f6e63c3c17adcd6770a4dcaf7bd`:

```bash
curl -fL https://raw.githubusercontent.com/Keonho-Chu/Vibrato/4311fefd49e9c6781c4d1111b8dd3f758e7d8974/packages/coding-agent/examples/extensions/ooo-bridge.ts -o /tmp/vib-ooo-bridge.ts
shasum -a 256 /tmp/vib-ooo-bridge.ts
mkdir -p "${HOME}/${VIB_CONFIG_DIR:-.vib}/agent/extensions/ouroboros-ooo-bridge" && cp /tmp/vib-ooo-bridge.ts "${HOME}/${VIB_CONFIG_DIR:-.vib}/agent/extensions/ouroboros-ooo-bridge/index.ts"
```

The `shasum` output must match the published example digest before the copy. The example has no runtime imports: it obtains the bundled bridge helper from the injected extension API, so the copied file works in compiled Vibrato binaries without extension-local `node_modules`. For project-only installation, copy the same verified file to `.vib/extensions/ouroboros-ooo-bridge/index.ts`. Start a new Vibrato session after installation, then run:
...
- `/skill:deep-interview` is Vibrato's bundled native interview workflow. It includes Ouroboros-inspired behavior but does not invoke the external CLI.
- `ooo interview` is the external integration. It calls Ouroboros's MCP interview tool, renders each question in Vibrato, correlates ordinary answers by Ouroboros session ID, and stops claiming input when the interview completes.

The canonical install location is the agent extensions directory discovered by the native Vibrato provider:

- user-level: `$HOME/${VIB_CONFIG_DIR:-.vib}/agent/extensions`
- project-level: `<cwd>/.vib/extensions`
...
`VIB_CONFIG_DIR` selects the **home-relative** config directory name: the config root is `<home>/<VIB_CONFIG_DIR>`, defaulting to `~/.vib`. It does not select a project directory — the project-level path is the constant `.vib` (`discovery/helpers.ts`, `getProjectAgentDir()`), so `VIB_CONFIG_DIR` never moves it. `VIB_CODING_AGENT_DIR` overrides the agent directory **path** rather than naming one under `$HOME`; it is resolved with `path.resolve`, so an absolute value is used as-is and a relative value is resolved against the current working directory.

Discovery is the exception to that second override. The native provider builds its user-level root from `VIB_CONFIG_DIR` alone (`<home>/<config-dir>/agent`) and never consults `getAgentDir()`, so an operator who sets `VIB_CODING_AGENT_DIR` moves the agent directory for the rest of the product but **not** for extension, skill, rule, or hook discovery.
