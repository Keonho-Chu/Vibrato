...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/vib-rato` → `@vib-rato/coding-agent`
  - `@mariozechner/pi-agent-core` → `@vib-rato/agent-core`
  - `@mariozechner/pi-tui` → `@vib-rato/tui`
  - `@mariozechner/pi-ai` → `@vib-rato/ai`
...
- In `packages/coding-agent`, use `logger` from `@vib-rato/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@vib-rato/natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@vib-rato/natives` | Native implementation with a small TS wrapper |
...
| `pkg.pi` manifest field       | `pkg.vib` preferred; fallback to `pkg.pi` remains |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@vib-rato/natives`
