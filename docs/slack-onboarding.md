...
local Vibrato sessions continue to own loopback SDK endpoints, and Slack provides a
...
`vib notify setup slack` is non-interactive. It requires these flags:
...
channel come from `vib notify setup` alone, and the operator supplies only a
...
1. **Prepare.** Start the session prepared. A manually started session opts in
   with `VIB_NOTIFY_BIND_EXISTING_THREAD=1` in its environment; a broker
...
2. **Bind.** `vib notify bind-thread --session-id <id> --thread-ts <root>`
   ...
   success.
3. **Activate.** `vib notify activate-thread --session-id <id>` asks
...
existing global (`notifications.enabled`, `VIB_NOTIFICATIONS=0`) and per-session
...
`vib_coordinator_start_session` with `prepare_existing_thread: true` (which
rejects an initial prompt and returns the session at state `prepared`), the
unchanged `vib notify bind-thread` command, and
`vib_coordinator_activate_session`. The Coordinator never writes the mapping
...
`vib_coordinator_send_prompt` until it is activated.
...
it in the daemon's own owner-only command directory. Vibrato trusts same-UID local
