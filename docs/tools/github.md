...
- Worktree path is `path.join(getWorktreesDir(), encodeRepoPathForFilesystem(primaryRepoRoot), localBranch)`, where `getWorktreesDir()` is `~/.vib/wt`; effective path is `~/.vib/wt/<encoded-primary-repo-root>/pr-<number>`.
...
- The branch push metadata is persisted with `git config` under the repository's shared `.git/config` as:
  ...
  - `branch.pr-<number>.vibPrHeadRef`
  - `branch.pr-<number>.vibPrUrl`
  - `branch.pr-<number>.vibPrIsCrossRepository`
  - `branch.pr-<number>.vibPrMaintainerCanModify`
...
- Filesystem
  ...
  - `pr_checkout` may create directories under `~/.vib/wt/<encoded-primary-repo-root>/` and add git worktrees there.
...
- Local validation errors throw `ToolError`, including:
  ...
  - `pr_push` on a branch without `vibPrHeadRef` metadata
