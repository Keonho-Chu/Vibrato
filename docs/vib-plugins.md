# Vibrato Plugin Bundles

Vibrato supports two distinct plugin families. Do not confuse them:

1. **Legacy marketplace / npm plugins** (`packages/coding-agent/src/extensibility/plugins`) — installed through the existing `vib plugin install <marketplace-ref|npm-spec>` marketplace/npm flows. Unchanged by this system.
2. **Vibrato plugin bundles** — directories whose root contains a **`vibrato-plugin.json`** manifest (`kind: "vib-rato-plugin"`). These *extend* existing Vibrato capabilities and are the subject of this document.

A Vibrato plugin bundle may only **extend** existing skills/agents — it can never register a new top-level skill, slash-command, command, or agent. Vibrato exposes exactly four default workflow skills (`autoresearch`, `deep-interview`, `ralplan`, `ultragoal`) and four role agents (`executor`, `architect`, `planner`, `critic`); bundles add sub-skills/appendices/tools/hooks/MCPs to those existing parents only.

## Loose customization vs plugin bundles

A plugin manifest is **never required** to add one local MCP, hook, skill, or extension. Native loose customization has one canonical destination in each scope: `<project>/.vib/` for project-local configuration and `~/.vib/agent/` for user-global configuration. Claude Code and Codex layouts are explicit import sources for `/extensions` ([#4291](https://github.com/Keonho-Chu/Vibrato/issues/4291)); they are not parallel runtime authorities. Reach for a bundle only when you want a versioned, distributable package of several surfaces with atomic install/update/uninstall, hashing, quarantine, and collision ownership.

| You want… | Loose surface (no manifest) | Plugin bundle (`vibrato-plugin.json`) |
|-----------|-----------------------------|-------------------------------------|
| **One local MCP server** | `mcpServers` map in `<project>/.vib/mcp.json` or `~/.vib/agent/mcp.json`; supports `command`/`args`/`env`/`cwd`/`url`/`headers`/`auth`/`oauth`/`type` | `mcps` array (or the `mcpServers` alias — see below); no `env`/`auth`/`oauth`/`headers` |
| **One local hook** | `<project>/.vib/hooks/pre/<tool>.ts` / `post/<tool>.ts`, or the same layout under `~/.vib/agent/hooks/` | `hooks` array of constrained `{ name, event, target?, phase?, path }` entries |
| **One local skill** | `.vib/skills/<name>/SKILL.md` (project) / `~/.vib/agent/skills/<name>/SKILL.md` (user) | `subskills` entries bound to an existing protected parent (`binds_to`/`phase`/`activation_arg`) |
| **One local extension** | `<project>/.vib/extensions/<name>/` or `~/.vib/agent/extensions/<name>/`, with its extension manifest + entry | Bundles have no extension surface; extensions are loose-only |
...
| **A brand-new top-level workflow skill** | ✅ Loose `.vib/skills/<name>/SKILL.md` (never via a bundle) | ❌ Forbidden (`forbidden_surface`) — bundles may only extend the four protected workflow skills |
...
Import is a transaction, not discovery precedence: `/extensions` selects Claude Code or Codex plus project-local or user-global scope, previews the normalized result, then writes the accepted configuration into the selected canonical `.vib` scope. Import UI and transaction behavior belong to #4291; this bundle contract neither activates foreign layouts at runtime nor implements that UI.

## Manifest (`vibrato-plugin.json`)
...
- `skills` (top level) — bundles may only EXTEND the four protected workflow skills; use `subskills` or the loose `.vib/skills/<name>/SKILL.md` surface. Never silently replaced by a bundle (`forbidden_surface`).
...
- `commands` / `slash-commands` (Claude Code plugin.json) — bundles cannot register slash commands; use the loose `.vib/commands/<name>` surface (`forbidden_surface`).
- `hooks` written as a Claude Code plugin.json event-keyed map or `{ matcher, hooks, source }` entries — cannot be preserved as constrained Vibrato bundle hooks; use canonical `{ name, event, target?, phase?, path }` or the loose `.vib/hooks/pre|post/<tool>.ts` surface (`invalid_manifest`).
- `mcpServers` entries using `env`/`auth`/`oauth`/`headers`/`enabled`/`timeout`/`autoload`/`noInheritEnv`, or fields incompatible with the selected transport — the bundle runtime cannot preserve these semantics end to end. Move the server to canonical loose `.vib/mcp.json` (directly or through the `/extensions` import flow in #4291) or drop the field (`unsupported_surface`).
...
```sh
vib plugin install <path|git-url|tarball> --user      # install into the user root
vib plugin install <path|git-url|tarball> --project   # install into the project root
```

Exactly one of `--user` / `--project` is required for Vibrato plugin bundles (there is no default root). A source containing `vibrato-plugin.json` is classified as a Vibrato bundle and routed to the bundle installer **before** the marketplace/npm path; non-bundle sources fall through to the legacy flow.
...
- **Appendices** render as lower-authority, delimited `<vib-plugin-system-appendix>` / `<vib-plugin-agent-appendix>` blocks appended after the base/project prompt; size-capped (8 KiB/appendix, 32 KiB total) fail-closed; content is escaped and control-char sanitized. They can never override base/developer instructions.
...
- **Tier-2 activation** (full body): on explicit activation (e.g. `deep-interview --autoresearch`) or an agent's contextual choice, the full sub-skill body is injected as a `<vib-subskill>` block at the matching phase.
...
- Install, force update (`vib plugin upgrade`), enable/disable, uninstall, quarantine, and hash-drift are covered by the lifecycle suite; the registry records everything required for them (per-surface IDs + copied-file ownership).
