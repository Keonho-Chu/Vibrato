# Human-owned Vibrato tmux sessions

A tmux-hosted Vibrato TUI is a **human-only terminal surface**. It is not an external control or viewing API.
...
```sh
./scripts/vib-session/create.sh <session-name> <worktree-path>
```
...
- The Vibrato SDK for authenticated lifecycle, control, and query operations.
