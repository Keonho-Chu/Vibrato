...
plugin_target="${streamdeck_root}/Plugins/dev.vibrato.streamdeck.sdPlugin"
profile_source="${root}/profile"
profile_target="${streamdeck_root}/ProfilesV3"
profile_name="Vibrato-cmux-Control-$(date +%Y%m%d-%H%M%S).streamDeckProfile"

command -v vib >/dev/null || { print -u2 'vib is not on PATH'; exit 1; }
...
if [[ -d "$plugin_target" ]]; then
  /usr/bin/ditto -c -k --sequesterRsrc --keepParent "$plugin_target" "${streamdeck_root}/ManualBackups/dev.vibrato.streamdeck-before-install-$(date +%Y%m%d-%H%M%S).zip"
fi
rm -rf "$plugin_target"
...
{"Name":"Vibrato cmux Control","Pages":{"Current":"c8f6a2a1-6db4-49f4-9e3c-4e6a7ed88931","Default":"c8f6a2a1-6db4-49f4-9e3c-4e6a7ed88931","Pages":["c8f6a2a1-6db4-49f4-9e3c-4e6a7ed88931","e1c9d4c3-8fd6-4b16-b05e-607c9faab153"]},"Version":"3.0"}
JSON
