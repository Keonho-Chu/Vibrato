...
function readServerState(html: string): Record<string, unknown> {
	...
	expect(document.querySelector("[data-marker='VIB_SCRIPT_DATA_MARKER']")).toBeNull();
...
describe("OAuthCallbackFlow callback script data", () => {
	it.each([
		...
		const injected = `${terminator}<script data-marker=VIB_SCRIPT_DATA_MARKER>VIB_INJECTED_SCRIPT_NODE</script>`;
		...
		expect(html).not.toContain("<script data-marker=VIB_SCRIPT_DATA_MARKER>");
		expect(html).toContain(`${terminator.replace("<", "\\u003c")}\\u003cscript data-marker=VIB_SCRIPT_DATA_MARKER>`);
	...
	it.each([
		...
		const injected = `${metasequence}</ScRiPt><script data-marker=VIB_SCRIPT_DATA_MARKER>VIB_INJECTED_SCRIPT_NODE</script>`;
		const { response, html, outcome } = await runCallback(
			new URLSearchParams({ error: "access_denied", error_description: injected }),
		);
