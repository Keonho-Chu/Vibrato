...
import { buildKimiCommonHeaders } from "@vib-rato/ai/utils/oauth/kimi";
...
beforeEach(() => {
	...
	previousAgentDir = process.env.VIB_CODING_AGENT_DIR;
	const tempAgentRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vib-kimi-headers-"));
...
afterEach(() => {
	vi.restoreAllMocks();
	const dir = process.env.VIB_CODING_AGENT_DIR;
	if (dir?.includes("vib-kimi-headers-")) fs.rmSync(path.dirname(dir), { recursive: true, force: true });
	if (previousAgentDir === undefined) delete process.env.VIB_CODING_AGENT_DIR;
	else process.env.VIB_CODING_AGENT_DIR = previousAgentDir;
});
...
describe("kimi common headers", () => {
	it("sanitizes non-ASCII and control characters from OS-derived header values", () => {
		...
		const deviceIdPath = path.join(process.env.VIB_CODING_AGENT_DIR ?? "", "kimi-device-id");
		expect(fs.readFileSync(deviceIdPath, "utf-8").trim()).toBe(headers["X-Msh-Device-Id"]);
		expect(deviceIdPath).toContain("vib-kimi-headers-");
