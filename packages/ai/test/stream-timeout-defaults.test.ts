...
const ENV_KEYS = [
	...
	"VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS",
...
describe("getStreamIdleTimeoutMs(fallbackMs)", () => {
	...
	it("honors the documented VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS override", () => {
		Bun.env.VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS = "77";
		expect(getStreamIdleTimeoutMs(300_000)).toBe(77);
	});

	it("resolves Vibrato-first: VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS wins over legacy PI_STREAM_IDLE_TIMEOUT_MS", () => {
		Bun.env.VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS = "77";
		Bun.env.PI_STREAM_IDLE_TIMEOUT_MS = "42";
		expect(getStreamIdleTimeoutMs(300_000)).toBe(77);
	});

	it("treats VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS=0 as a watchdog disable", () => {
		Bun.env.VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS = "0";
		expect(getStreamIdleTimeoutMs(300_000)).toBeUndefined();
	});
});

describe("getOpenAIStreamIdleTimeoutMs()", () => {
	it("honors the documented VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS first", () => {
		Bun.env.VIB_OPENAI_STREAM_IDLE_TIMEOUT_MS = "88";
