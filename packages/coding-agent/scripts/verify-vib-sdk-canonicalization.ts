...
const repoRoot = process.env.VIB_SDK_CANONICALIZATION_SCAN_ROOT
	? path.resolve(process.env.VIB_SDK_CANONICALIZATION_SCAN_ROOT)
	: path.resolve(import.meta.dir, "..", "..", "..");
const scannerPath = "packages/coding-agent/scripts/verify-vib-sdk-canonicalization.ts";
const packageManifestPath = "packages/coding-agent/package.json";
const retiredPythonRpcPackagePath = "python/vib-rpc/";
const bridgeClientPackageManifestPath = "packages/bridge-client/package.json";
const bridgeClientPackageName = "@vib-rato/bridge-client";
const bridgeOrUnattendedImportPattern =
	/(?:\b(?:import|export)\s+(?:type\s+)?(?:[^"'`;]*?\s+from\s+)?|\bimport\s*\(\s*)["'][^"']*(?:(?:^|\/)unattended)(?:["'/]|$)/g;
const bridgeClientImportPattern = /(?:\bfrom\s*|\bimport\s*\(\s*)["'](@vib-rato\/bridge-client[^"']*)["']/g;
...
const pythonVibRpcImportPattern = /^\s*(?:from\s+vib_rpc(?:\.|\s)|import\s+vib_rpc(?:\.|\s|,|$))/gm;
...
const retiredTmuxMachineBusPaths = new Set(["scripts/vib-session/prompt.sh", "scripts/vib-session/tail.sh"]);
const machineTmuxDocumentationPaths = new Set([
	"docs/vib-session-clawhip-routing.md",
	"packages/coding-agent/src/setup/hermes/templates/operator-instructions.v1.md",
]);
const machineTmuxDocumentationPattern =
	/(?:scripts\/vib-session\/(?:prompt|tail)\.sh|\b(?:load-buffer|paste-buffer|send-keys|capture-pane|pipe-pane)\b|(?:\.\/)?(?:scripts\/)?vib-session\/create\.sh(?:[ \t]+(?:"[^"\n]*"|'[^'\n]*'|[^\s]+)){3})/g;
...
function legacyPythonRpcViolations(file: string, contents: string): string[] {
	...
	if (isPython(file)) {
		for (const match of contents.matchAll(pythonVibRpcImportPattern)) {
			violations.push(`${file}:${lineNumber(contents, match.index ?? 0)}: imports removed vib_rpc Python client`);
		}
	...
	if (isPythonDistributionMetadata(file)) {
		for (const match of contents.matchAll(/\bvib-rpc\b/gi)) {
			violations.push(
				`${file}:${lineNumber(contents, match.index ?? 0)}: declares removed vib-rpc distribution metadata`,
			);
		}
	}
	return violations;
...
function bridgeClientOwnershipViolations(file: string, contents: string): string[] {
	...
	for (const match of contents.matchAll(relativeImportPattern)) {
		...
		if (
			/(?:^|\/)(?:coding-agent|agent-session|sdk\/(?:host|session)|session)(?:\/|$)|@vib-rato\/coding-agent/.test(
				specifier,
			)
		) {
			violations.push(
...
const teamRuntimeTmuxPath = "packages/coding-agent/src/vib-runtime/team-runtime.ts";
const teamWorkersTmuxPath = "packages/coding-agent/src/vib-runtime/team-workers.ts";

const coordinatorMcpRoot = "packages/coding-agent/src/coordinator-mcp/server.ts";
function isPublishedVibSessionShellHelper(file: string): boolean {
	return file.startsWith("scripts/vib-session/") && file.endsWith(".sh");
}
...
function shellVibBinaryReferenceViolations(contents: string): ShellRange[] {
	...
	const references = /(["']?)\$(?:VIB_BIN|VIB_SESSION_VIB_BIN|\{(?:VIB_BIN|VIB_SESSION_VIB_BIN)\})\1/g;
	...
	for (const reference of normalizedContents.matchAll(references)) {
		...
		if (/(?:^|[\s"'])VIB_SESSION_VIB_BIN\s*=\s*$/.test(before)) continue;
...
function tmuxCreateStartupViolations(file: string, contents: string): string[] {
	if (file !== "scripts/vib-session/create.sh") return [];
	const violations: string[] = [];
	...
	const canonicalCommand = /^\s*command\s*=\s*\[\s*os\.environ\[\s*["']VIB_SESSION_VIB_BIN["']\s*\]\s*\]\s*$/gm;
	...
	if (
		...
		violation(
			...
			"human-only tmux owner must launch exactly VIB_SESSION_VIB_BIN with zero startup arguments",
		);
	}
	const canonicalInteractivePopenAssignment =
		/^\s*child\s*=\s*subprocess\.Popen\(\s*command\s*(?:,\s*cwd\s*=\s*os\.environ\[\s*["']VIB_SESSION_WORKDIR["']\s*\])?\s*\)\s*$/;
	const expectedLifecycleCallsites = [
		{
			operation: "terminal observer",
			pattern:
				/\bcompleted\s*=\s*subprocess\.run\(\s*\[\s*os\.environ\[\s*["']VIB_SESSION_VIB_BIN["']\s*\]\s*,\s*["']--internal-tmux-owner-isolation["']\s*\]\s*,/g,
		},
		{
			operation: "owner-isolation plan",
			pattern:
				/\bPLAN_RESPONSE\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$PLAN_LINE"\s*\|\s*"\$VIB_BIN"\s+--internal-tmux-owner-isolation\s*\)"/g,
		},
		{
			operation: "post-spawn proof",
			pattern:
				/\bPOST_SPAWN_RESPONSE\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$PLAN_LINE"\s*\|\s*"\$VIB_BIN"\s+--internal-tmux-owner-isolation\s*\)"/g,
		},
		{
			operation: "generation publication",
			pattern:
				/\bGENERATION_PUBLISH_RESPONSE\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$GENERATION_PUBLISH_REQUEST"\s*\|\s*"\$VIB_BIN"\s+--internal-tmux-owner-isolation\s*\)"/g,
		},
		{
			operation: "terminal monitor",
			pattern:
				/\bif\s+verdict\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$request"\s*\|\s*timeout\s+"[^"]+"\s+"\$VIB_SESSION_VIB_BIN"\s+--internal-tmux-owner-isolation\s*\)"\s*;\s*then/g,
		},
	];
	...
	for (const match of contents.matchAll(pythonCommandAssignment)) {
		...
		violation(match.index ?? 0, "human-only tmux owner must not construct or wrap a non-lifecycle Vibrato argv");
	}

	const allowedVibEnvironmentReferences: ShellRange[] = canonicalCommands.map(match => ({
		start: match.index ?? 0,
		end: (match.index ?? 0) + match[0].length,
	}));
	const allowedCommandReferences = [...allowedVibEnvironmentReferences];
	...
	for (const match of contents.matchAll(subprocessCalls)) {
		...
		const isLifecycleCall =
			/^\s*\[\s*os\.environ\[\s*["']VIB_SESSION_VIB_BIN["']\s*\]\s*,\s*["']--internal-tmux-owner-isolation["']\s*\]\s*$/.test(
				argument.text,
			);
		...
		if (isLifecycleCall) {
			...
			allowedVibEnvironmentReferences.push({ start: argument.start, end: argument.start + argument.text.length });
			continue;
		}
		if (/\bcommand\b|os\.environ\[\s*["']VIB_SESSION_VIB_BIN["']\s*\]/.test(argument.text)) {
			violation(
				match.index ?? 0,
				"human-only tmux owner must invoke Vibrato only with zero interactive argv or the exact owner-isolation lifecycle argv",
			);
		}
	}
	for (const match of contents.matchAll(/\[\s*["']vib["'](?:\s*,|\s*\])/g)) {
		violation(match.index ?? 0, "human-only tmux owner must not launch a literal vib executable");
	}
	for (const match of contents.matchAll(/(?:^|\n)\s*(?:export\s+)?[A-Za-z_][A-Za-z0-9_]*\s*=\s*["']vib["']\s*$/gm)) {
		violation(match.index ?? 0, "human-only tmux owner must not alias or wrap the vib executable");
	}
	for (const match of contents.matchAll(
		/(?:^|\n)\s*(?:alias\s+[A-Za-z_][A-Za-z0-9_]*\s*=\s*['"]?vib\b|(?:function\s+)?[A-Za-z_][A-Za-z0-9_]*\s*\(\s*\)\s*\{[^\n]*\bvib\b)/gm,
	)) {
		violation(match.index ?? 0, "human-only tmux owner must not alias or wrap the vib executable");
	}
	for (const match of contents.matchAll(
		/(?:^|[|;&({]\s*|\b(?:command|exec|env|sudo|nice|nohup)\s+)["']?vib["']?(?=\s|$)/gm,
	)) {
		violation(match.index ?? 0, "human-only tmux owner must not launch a literal vib executable");
	}
	for (const match of contents.matchAll(
		/\benv(?:\s+(?:[A-Za-z_][A-Za-z0-9_]*=(?:"[^"]*"|'[^']*'|[^\s]+)|"[^"]*"|'[^']*'))*\s+["']?vib["']?(?=\s|$)/gm,
	)) {
		violation(match.index ?? 0, "human-only tmux owner must not launch a literal vib executable");
	}
	...
	if (interactiveLaunches !== 1) {
		violation(0, "human-only tmux owner must launch exactly one zero-argv interactive Vibrato process");
	}
	for (const match of contents.matchAll(/\bcommand\b/g)) {
		...
		if (!allowedCommandReferences.some(range => offset >= range.start && offset < range.end)) {
			violation(offset, "human-only tmux owner must not construct or wrap a non-lifecycle Vibrato argv");
		}
	}
	...
	for (const match of contents.matchAll(/os\.environ\[\s*["']VIB_SESSION_VIB_BIN["']\s*\]/g)) {
		const offset = match.index ?? 0;
		if (!allowedVibEnvironmentReferences.some(range => offset >= range.start && offset < range.end)) {
			violation(offset, "human-only tmux owner must not construct or wrap a non-lifecycle Vibrato argv");
		}
	}
	...
	const shellVibInvocations =
		/(?:^|[|;&(]\s*|\b(?:command|exec)\s+|\btimeout\s+(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s]+)\s+)(["']?)\$(?:VIB_BIN|VIB_SESSION_VIB_BIN|\{(?:VIB_BIN|VIB_SESSION_VIB_BIN)\})\1([^\r\n;|&)]*)/gm;
	for (const match of normalizedContents.matchAll(shellVibInvocations)) {
		...
		if (
			...
			violation(invocationStart, "human-only tmux owner invokes Vibrato with a non-lifecycle startup argument");
		}
	}
	for (const reference of shellVibBinaryReferenceViolations(contents)) {
		violation(reference.start, "human-only tmux owner must not construct or wrap a non-lifecycle Vibrato argv");
	}
	for (const match of contents.matchAll(/\bVIB_SESSION_FLAGS\b/g)) {
...
function exactTeamRuntimeSendKeysRanges(contents: string): ShellRange[] {
	...
	const continuation = /async\s+function\s+continueStalledVibTeamWorkers\s*\([^)]*\)\s*:\s*Promise<void>\s*\{/.exec(
		contents,
	);
	const monitor = /(?:export\s+)?async\s+function\s+monitorVibTeam\s*\([\s\S]*?\)\s*:\s*Promise<[^>]+>\s*\{/.exec(
		contents,
	);
	...
	const authorityChecks = executorBody.match(/assertVibTmuxMutationAuthoritySync\(authority\)/g) ?? [];
	if (
		!literalSend ||
		!keySend ||
	...
	const continuationCalls = [...monitorBody.matchAll(/await\s+continueStalledVibTeamWorkers\s*\([^;]*\)\s*;/g)];
	const reconcileCalls = [...monitorBody.matchAll(/await\s+reconcileVibTeamStaleClaimsUnlocked\s*\([^;]*\)\s*;/g)];
	if (
...
async function scan(): Promise<string[]> {
	...
	for (const file of files) {
		...
		if (file.startsWith(retiredPythonRpcPackagePath)) {
			violations.push(`${file}: retired Python vib-rpc package source survived`);
		}
		...
		if (isPublishedVibSessionShellHelper(file)) {
			const contents = await Bun.file(path.join(repoRoot, file)).text();
		...
		for (const match of contents.matchAll(/\bVIB_BRIDGE_[A-Z0-9_]*\b/g)) {
			violations.push(
		...
		if (isProductionTypeScriptOrJavaScript(file)) {
			...
			if (
				...
				!/^export const VIB_TEAM_CONTINUATION_PROMPT =\n\t"Continue only your current claimed Vibrato team task\. Re-read current Vibrato team state; do not replay prior output; report status\.";$/m.test(
					contents,
				)
			)
				violations.push(
...
async function runSelfTestFixture(
	...
	const fixture = await fs.mkdtemp(path.join(os.tmpdir(), "vib-sdk-canonicalization-"));
	try {
		...
		const scanner = Bun.spawn([process.execPath, import.meta.path], {
			cwd: repoRoot,
			env: { ...process.env, VIB_SDK_CANONICALIZATION_SCAN_ROOT: fixture },
			stdout: "pipe",
			stderr: "pipe",
			timeout: timeoutMs,
		});
...
async function selfTest(): Promise<void> {
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/rpc-compat-mode.ts": 'Bun.spawnSync(["vib", "--mode", "rpc-compat"]);\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/bridge-compat-mode.ts":
				'Bun.spawnSync(["vib", "--mode", "bridge-compat"]);\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/aliased-retired-modes.ts":
				'const rpcMode = "rpc";\nlet bridgeMode = "bridge-compat";\nvar unattendedMode = "unattended";\nBun.spawnSync(["vib", "--mode", rpcMode]);\nBun.spawnSync(["vib", "--mode", bridgeMode]);\nBun.spawnSync(["vib", "--mode", unattendedMode]);\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	...
	await runExactRetiredModeFixture(
		...
		'const launch = () => Bun.spawnSync(["vib", "--mode", mode]);\nconst mode = "rpc";\nlaunch();\n',
		1,
		[retiredModeDiagnostic(retiredModeFixturePath("closure-forward-const.ts"), 1)],
	);
	await runExactRetiredModeFixture(
		"safe-block-shadow.ts",
		'const mode = "rpc"; { const mode = "acp"; Bun.spawnSync(["vib", "--mode", mode]); }\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"parameter-shadow.ts",
		'const mode = "rpc"; function launch(mode: string) { Bun.spawnSync(["vib", "--mode", mode]); } launch("acp");\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"catch-shadow.ts",
		'const mode = "rpc"; try { throw "acp"; } catch (mode) { Bun.spawnSync(["vib", "--mode", mode]); }\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"destructure-shadow.ts",
		'const mode = "rpc"; const { mode: shadow } = { mode: "acp" }; Bun.spawnSync(["vib", "--mode", shadow]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"sibling-shadow.ts",
		'{ const mode = "rpc"; void mode; } { const mode = "acp"; Bun.spawnSync(["vib", "--mode", mode]); }\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"direct-tdz.ts",
		'Bun.spawnSync(["vib", "--mode", mode]); const mode = "rpc";\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"pre-use-safe-to-retired.ts",
		'let mode = "acp"; mode = "rpc"; Bun.spawnSync(["vib", "--mode", mode]);\n',
		1,
		[retiredModeDiagnostic(retiredModeFixturePath("pre-use-safe-to-retired.ts"))],
	);
	await runExactRetiredModeFixture(
		"pre-use-retired-to-safe.ts",
		'let mode = "rpc"; mode = "acp"; Bun.spawnSync(["vib", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"later-safe-write.ts",
		'let mode = "rpc"; Bun.spawnSync(["vib", "--mode", mode]); mode = "acp";\n',
		1,
		[retiredModeDiagnostic(retiredModeFixturePath("later-safe-write.ts"))],
	);
	await runExactRetiredModeFixture(
		"later-retired-write.ts",
		'let mode = "acp"; Bun.spawnSync(["vib", "--mode", mode]); mode = "rpc";\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"branch-write.ts",
		'let mode = "acp"; if (condition) mode = "rpc"; Bun.spawnSync(["vib", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"compound-write.ts",
		'let mode = "rpc"; mode += "-compat"; Bun.spawnSync(["vib", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"update-write.ts",
		'let mode = "rpc"; mode++; Bun.spawnSync(["vib", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"dynamic-member-controls.ts",
		'const dynamicMode = process.env.MODE; const state = { mode: "rpc" }; Bun.spawnSync(["vib", "--mode", dynamicMode]); Bun.spawnSync(["vib", "--mode", state.mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"typed-acp-control.ts",
		'const mode: string = "acp"; Bun.spawnSync(["vib", "--mode", mode]);\n',
		0,
		[],
	);
	...
	await runExactRetiredModeFixture(
		"cycle.ts",
		'const a = b; const b = a; Bun.spawnSync(["vib", "--mode", a]);\n',
		0,
		[],
		{ timeoutMs: 2_000 },
	);
	await runExactRetiredModeFixture("deduplicated-direct-mode.ts", 'Bun.spawnSync(["vib", "--mode", "rpc"]);\n', 1, [
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/package.json":
				'{"exports":{"./commands/vib-runtime-bridge":"./src/bridge.ts","./unattended":"./src/unattended.ts"}}\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/package.json":
				'{"exports":{"./modes/rpc/*":null,"./commands/vib-runtime-bridge":null,"./unattended":null,"./modes/shared/agent-wire/*":null}}\n',
		},
		0,
	);
	await runSelfTestFixture(
	...
	await runSelfTestFixture(
		{
			"package.json": '{"catalog":{"@vib-rato/bridge-client":"0.10.1"}}\n',
			"packages/bridge-client/package.json": '{"name":"@vib-rato/bridge-client"}\n',
			"packages/bridge-client/src/index.ts": "export class SdkClient {}\nexport type SdkClientOptions = {};\n",
			"packages/coding-agent/package.json": '{"dependencies":{"@vib-rato/bridge-client":"catalog:"}}\n',
			"packages/coding-agent/src/sdk/client/client.ts":
				'export { SdkClient } from "@vib-rato/bridge-client";\nexport type { SdkClientOptions } from "@vib-rato/bridge-client";\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/bridge-client/package.json":
				'{"name":"@vib-rato/bridge-client","dependencies":{"unsafe":"1.0.0"}}\n',
		},
		1,
		"declares unsupported bridge-client package metadata",
	);
	...
	await runSelfTestFixture(
		{
			"packages/bridge-client/src/client.ts":
				'import { SdkClient } from "@vib-rato/coding-agent";\nvoid SdkClient;\n',
		},
		1,
		"bridge-client imports coding-agent or AgentSession authority",
	);
	await runSelfTestFixture(
		{ "packages/renamed-workspace/package.json": '{"name":"@vib-rato/bridge-client"}\n' },
		1,
		"declares unsupported bridge-client package metadata",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { BridgeClient } from "@vib-rato/bridge-client";\nvoid BridgeClient;\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { handshake } from "@vib-rato/bridge-client";\nvoid handshake;\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { commands } from "@vib-rato/bridge-client";\nvoid commands;\n',
		},
		1,
		"imports historical bridge-client protocol surface",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { SdkClient } from "@vib-rato/bridge-client/commands";\nvoid SdkClient;\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts": 'import { SSE } from "@vib-rato/bridge-client";\nvoid SSE;\n',
		},
		1,
		"imports historical bridge-client protocol surface",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts": 'import { control } from "@vib-rato/bridge-client";\nvoid control;\n',
		},
		1,
		"imports historical bridge-client protocol surface",
	);
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/neutral-name.ts": 'Bun.spawnSync(["vib", "--mode", "rpc"]);\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/neutral-expect.ts":
				'expect(Bun.spawnSync(["vib", "--mode", "rpc"])).toBeDefined();\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	...
	await runSelfTestFixture(
		{ "scripts/vib-session/prompt.sh": "#!/usr/bin/env bash\n" },
		1,
		"retired tmux machine prompt or viewing helper survived",
	);
	await runSelfTestFixture(
		{
			"scripts/vib-session/unsafe.sh":
...
command = [os.environ["VIB_SESSION_VIB_BIN"]]
child = subprocess.Popen(command)
completed = subprocess.run([os.environ["VIB_SESSION_VIB_BIN"], "--internal-tmux-owner-isolation"], input="{}")
PLAN_RESPONSE="$(printf '%s\\n' "$PLAN_LINE" | "$VIB_BIN" --internal-tmux-owner-isolation)"
POST_SPAWN_RESPONSE="$(printf '%s\\n' "$PLAN_LINE" | "$VIB_BIN" --internal-tmux-owner-isolation)"
GENERATION_PUBLISH_RESPONSE="$(printf '%s\\n' "$GENERATION_PUBLISH_REQUEST" | "$VIB_BIN" --internal-tmux-owner-isolation)"
if verdict="$(printf '%s\\n' "$request" | timeout "1s" "$VIB_SESSION_VIB_BIN" --internal-tmux-owner-isolation)"; then
  true
fi
`;
	await runSelfTestFixture({ "scripts/vib-session/create.sh": canonicalCreateFixture }, 0);
	await runSelfTestFixture(
		{ "scripts/vib-session/create.sh": `${canonicalCreateFixture}\nsubprocess.Popen(["vib"])\n` },
		1,
		"must not launch a literal vib executable",
	);
	await runSelfTestFixture(
		{ "scripts/vib-session/create.sh": `${canonicalCreateFixture}\nlauncher="vib"\n"$launcher"\n` },
		1,
		"must not alias or wrap the vib executable",
	);
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": `${canonicalCreateFixture}\nrunner(){ vib --internal-tmux-owner-isolation; }\nrunner\n`,
		},
		1,
		"must not alias or wrap the vib executable",
	);
	await runSelfTestFixture(
		{ "scripts/vib-session/create.sh": `${canonicalCreateFixture}\n"$VIB_BIN" --internal-tmux-owner-isolation\n` },
		1,
		"invokes Vibrato with a non-lifecycle startup argument",
	);
	await runSelfTestFixture(
		{ "scripts/vib-session/create.sh": `${canonicalCreateFixture}\nVIB_SESSION_FLAGS=unsafe\n` },
		1,
		"exposes caller-provided startup arguments",
	);
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": canonicalCreateFixture.replace(
				'[[ $# -eq 2 ]] || { echo "Usage: $0 <session-name> <worktree-path>" >&2; exit 2; }',
				"[[ $# -eq 2 ]] || true",
			),
		},
	...
	await runSelfTestFixture(
		...
		1,
		"must launch exactly VIB_SESSION_VIB_BIN with zero startup arguments",
	);
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": canonicalCreateFixture.replace(
	...
	await runSelfTestFixture(
		...
		1,
		"must not construct or wrap a non-lifecycle Vibrato argv",
	);
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": canonicalCreateFixture.replace(
	...
	await runSelfTestFixture(
		...
		1,
		"must invoke Vibrato only with zero interactive argv or the exact owner-isolation lifecycle argv",
	);
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": canonicalCreateFixture.replace(
	...
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": `${canonicalCreateFixture}\nlaunch = [os.environ["VIB_SESSION_VIB_BIN"], "--file", "task.md"]\nsubprocess.Popen(launch)\n`,
		},
		1,
		"must not construct or wrap a non-lifecycle Vibrato argv",
	);
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": canonicalCreateFixture.replace(
				'[os.environ["VIB_SESSION_VIB_BIN"], "--internal-tmux-owner-isolation"]',
				'[\n  os.environ["VIB_SESSION_VIB_BIN"],\n  "--internal-tmux-owner-isolation",\n  "--file",\n  "task.md",\n]',
			),
		},
		1,
		"must invoke Vibrato only with zero interactive argv or the exact owner-isolation lifecycle argv",
	...
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": canonicalCreateFixture.replace(
				'"$VIB_BIN" --internal-tmux-owner-isolation',
				'command -p "$VIB_BIN" --file task.md',
			),
		},
		1,
		"must not construct or wrap a non-lifecycle Vibrato argv",
	...
	await runSelfTestFixture(
		{
			"scripts/vib-session/create.sh": canonicalCreateFixture.replace(
				'"$VIB_BIN" --internal-tmux-owner-isolation',
				'exec -a vib "$VIB_BIN" --file task.md',
			),
		},
		1,
		"must not construct or wrap a non-lifecycle Vibrato argv",
	...
	const realCreate = await Bun.file(path.join(repoRoot, "scripts/vib-session/create.sh")).text();
	await runSelfTestFixture({ "scripts/vib-session/create.sh": realCreate }, 0);
	await runSelfTestFixture(
		{ "scripts/vib-session/watch-output.sh": "#!/usr/bin/env bash\n$TMUX_BIN \\\n  pipe-pane -t session 'sink'\n" },
		1,
		"published shell helper performs tmux machine prompt injection or pane viewing",
	);
	await runSelfTestFixture(
		{ "docs/vib-session-clawhip-routing.md": "$TMUX_BIN \\\n  pipe-pane -t owner 'sink'\n" },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
		{
			"docs/vib-session-clawhip-routing.md": './scripts/vib-session/create.sh bot /repo \\\n  --print "task"\n',
		},
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	...
	await runSelfTestFixture(
		{ "docs/vib-session-clawhip-routing.md": "scripts/vib-session/tail.sh visible output\n" },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
		{ "docs/vib-session-clawhip-routing.md": "tmux pipe-pane -t owner 'sink'\n" },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
		{ "docs/vib-session-clawhip-routing.md": './scripts/vib-session/create.sh bot /repo --print "task"\n' },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/vib-runtime/team-runtime.ts":
	...
	const args =
		operation.type === "literal-send"
			...
			: operation.type === "key-send"
				...
				: [];
	assertVibTmuxMutationAuthoritySync(authority);
	const result = Bun.spawnSync(args);
	assertVibTmuxMutationAuthoritySync(authority);
	return result;
}
async function continueStalledVibTeamWorkers(): Promise<void> {
	...
	const continuationPrompt = "Continue only your current claimed Vibrato team task. Re-read current Vibrato team state; do not replay prior output; report status.";
	const revalidationReason = await validateVibContinuationEligibility(dir, config, worker);
	...
	const dispatch = vibTeamRuntimeTestSeams?.continuationTmuxDispatch
		? vibTeamRuntimeTestSeams.continuationTmuxDispatch(config.tmux_command, args)
		: (() => {
...
async function monitorVibTeam(): Promise<void> {
	await withVibTeamTaskMutation(taskStore(dir), async capability => {
		await continueStalledVibTeamWorkers();
		await reconcileVibTeamStaleClaimsUnlocked(workerOrchestrationRuntime, teamName, dir, config, env, capability);
	});
}
`;
	await runSelfTestFixture(
		{ "packages/coding-agent/src/vib-runtime/team-runtime.ts": canonicalTeamRuntimeSendKeysFixture },
		0,
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/vib-runtime/team-runtime.ts": canonicalTeamRuntimeSendKeysFixture.replace(
				"\tassertTeamTmuxMutationPreproof(config, operation);\n",
				"",
			),
		},
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/vib-runtime/team-runtime.ts": canonicalTeamRuntimeSendKeysFixture.replaceAll(
				"worker.pane_id",
				'"%99"',
			),
		},
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/bridge-env.ts":
				"const endpoint = process.env.VIB_BRIDGE_ENDPOINT;\nvoid endpoint;\n",
		},
		1,
		"forbidden bridge environment reference VIB_BRIDGE_ENDPOINT",
	);
	await runSelfTestFixture(
		{
	...
	await runSelfTestFixture(
		{ "python/vib-rpc/src/vib_rpc/client.py": "class RpcClient: pass\n" },
		1,
		"retired Python vib-rpc package source survived",
	);
	await runSelfTestFixture(
		{ "python/robovib/src/controller.py": "from vib_rpc import RpcClient\n" },
		1,
		"imports removed vib_rpc Python client",
	);
	await runSelfTestFixture(
		{ "scripts/legacy-controller.py": 'subprocess.run(["vib", "--mode", "rpc"])\n' },
		1,
		"invokes removed --mode rpc",
	);
	await runSelfTestFixture(
		{ "python/robovib/pyproject.toml": '[project]\ndependencies = ["vib-rpc>=0.1.0"]\n' },
		1,
		"declares removed vib-rpc distribution metadata",
	);
	await runSelfTestFixture(
		{ "python/robovib/src/controller.py": 'client.negotiate_unattended(actor="legacy")\n' },
		1,
	...
	await runSelfTestFixture({ "artifacts/closure-report.json": '{"legacy":"vib --mode rpc"}\n' }, 0);
	await runSelfTestFixture(
...
try {
	...
	if (violations.length > 0) {
		process.stderr.write(`Vibrato SDK canonicalization violations found:\n${violations.join("\n")}\n`);
		process.exit(1);
	}
	process.stdout.write(
		`Vibrato SDK canonicalization verification passed (${sanctionedServerHosts.size} sanctioned server hosts).\n`,
	);
} catch (error) {
