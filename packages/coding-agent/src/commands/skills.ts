...
export default class Skills extends Command {
	static description = "Inspect bundled Vibrato workflow skills and discover custom filesystem skills";
	...
	static examples = [
		...
		"# Read an embedded workflow skill without requiring .vib files\n  vib skills read ultragoal",
		"# Machine-readable embedded skill content\n  vib skills read ralplan --json",
		"# Show filesystem-discovered skills (project and user) with diagnostics\n  vib skills discover",
		"# Show only project-scope skills (project .vib/skills locations)\n  vib skills discover --source project --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Skills);
		if (!args.action) {
			renderCommandHelp("vib", "skills", Skills);
			return;
		}
