...
import { BundledDefaultContentError, readBundledContentSync } from "./vib-defaults";
import type { BundledVibSkillCatalogEntry } from "./vib-skills.generated";
...
describe("bundled default content", () => {
	test("unreadable source throws a typed contextual error", () => {
		const entry = {
			...
			loadContent: async () => "",
		} as BundledVibSkillCatalogEntry;
		...
		try {
			...
			expect((error as Error).message).toContain("Unable to read bundled Vibrato definition");
		}
	});
});
