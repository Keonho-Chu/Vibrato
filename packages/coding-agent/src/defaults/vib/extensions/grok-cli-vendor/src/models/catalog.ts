...
export function supportsReasoningEffort(modelId: string): boolean {
  ...
  // Effort-capable id not listed in VIB_GROK_CLI_MODELS env list — still honor prefix (avoids spurious 400s).
  return true;
}

// ─── VIB_GROK_CLI_MODELS env override ─────────────────────────────────────

/**
 * Resolve the active model list. If `VIB_GROK_CLI_MODELS` is set,
 * it filters/reorders the fallback list; unknown IDs get sensible defaults.
 */
export function resolveModels(): GrokCliModelConfig[] {
  const env = (process.env.VIB_GROK_CLI_MODELS || '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
