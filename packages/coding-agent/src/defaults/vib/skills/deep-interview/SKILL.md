...
- Use the active Vibrato workflow/state CLI as the only sanctioned writer for `.vib/` interview artifacts; do not edit `.vib/` directly without an explicit force override.
...
- Auto-mode architects are read-only: no code edits, no `.vib/` mutation, no workflow chaining, no formatters, and no execution delegation.
...
If this raw bundled skill is loaded by Vibrato's native skill loader through `/skill:deep-interview`, do not treat that path as permission to skip rendered Vibrato setup. The user-facing invocation is `/skill:deep-interview`; do not recommend or advertise CLI bridge commands as the deep-interview entrypoint. Regardless of invocation path, Phase 0 below remains blocking and must resolve `vib.deepInterview.ambiguityThreshold` from pre-resolved native state or settings before any announcement, state write, question, or ambiguity score.

## Corrupt current-session state recovery

When deep-interview detects its own current-session state is corrupt, tampered, unreadable, or stale on resume, run `vib deep-interview clear --force` before reseeding or restarting. Scope the clear to the current session via `--session-id` or `VIB_SESSION_ID`; it clears only deep-interview state for that session and never clears other skills or sessions.

## Phase 0: Resolve Ambiguity Threshold (blocking prerequisite)

Complete this phase before Phase 1, before brownfield exploration, before Vibrato state persistence, before Round 0, and before any ambiguity scoring. Do not continue if the resolved threshold and source are unknown.

1. **Prefer pre-resolved native state**:
   - First inspect active deep-interview state with `vib deep-interview read --json`.
...
2. **Only if native state lacks a resolved threshold, read threshold settings in the runtime precedence order** (one shared resolver; first valid value wins):
   ...
   2. user `<agentDir>/config.yml` (normally `~/.vib/agent/config.yml`, honoring `VIB_CODING_AGENT_DIR`/`PI_CODING_AGENT_DIR`; XDG variables apply only to categorized data/state/cache subdirectories, never to the workflow config path)
   3. built-in default (`0.05` before resolution-flag fallback)
   - `config.yml` uses the nested (schema) form - `vib: { deepInterview: { ambiguityThreshold } }`. Project configuration beats user configuration. The reported `threshold_source` is the canonical path of the winning file, or the default. `config.yml` is the ONLY settings surface: the legacy `settings.json` files' workflow keys are retired: the config-root `~/.vib/settings.json` is migrated once into the default global agent `config.yml` and its source removed, while the project `.vib/settings.json` is retained for non-workflow settings (only its workflow keys are migrated into project `.vib/config.yml` and no longer read unless a migration target is absent - a migration that could not publish (e.g. a read-only `.vib`) leaves the retained legacy value effective as the previously configured override until it can publish). Invalid optional settings files continue to the next layer or the default (tolerant) — never a failed `Read`; do not probe arbitrary ancestor candidates.
3. **Resolve threshold and source**:
   ...
   - Set these run variables exactly: `<resolvedThreshold>`, `<resolvedThresholdPercent>`, and `<resolvedThresholdSource>` (for example `./.vib/config.yml`, `<agentDir>/config.yml` such as `~/.vib/agent/config.yml`, or the selected mode default).
...
5. **Carry threshold source forward mechanically**:
   ...
   - Include `threshold_source` in the first `vib deep-interview write` payload and preserve it on later state updates; do not edit `.vib/_session-{sessionid}/state` files directly unless an explicit force override is active.
...
Run this gate after the Phase 0 threshold marker and before Phase 1, brownfield exploration, `vib deep-interview write`, Round 0, ambiguity scoring, or spec writing.
...
1. **Stop deep-interview immediately**:
   - First inspect current-session state with `vib deep-interview read --json` (include `--session-id <current-session-id>` when available).
   - Clear through `vib deep-interview clear --force --json` only when the state is a newly seeded empty interview: no recorded `rounds`, no `spec_path`, no `handoff_from`, no final/pending spec, and no user-confirmed topology.
...
3.7. **Artifact path discipline**:
   - Final specs MUST resolve to `.vib/_session-{sessionid}/specs/deep-interview-{slug}.md` exactly.
   - Write final specs and all ephemeral interview artifacts through the active Vibrato workflow/state CLI when available.
   - Direct `.vib/` file edits are forbidden unless an explicit force override is active; do not use `write`, `edit`, or `ast_edit` against `.vib/_session-{sessionid}/specs`, `.vib/_session-{sessionid}/plans`, `.vib/_session-{sessionid}/state`, or other `.vib/` paths during normal workflow operation.
   - Preferred: pass the spec markdown **inline** to the native deep-interview write command (`--write … --spec "<markdown>"`) — no scratch file is needed. The CLI is the only sanctioned writer for `.vib/_session-{sessionid}/specs`.
   - Only if a spec is too large to pass inline, stage it with the `write` tool to a system temp directory (`os.tmpdir()`/`$TMPDIR`, `/tmp`, `/var/tmp`) outside the project tree, then pass that path to `--spec`. The planning phase-boundary block tolerates these neutral temp writes; never stage interview artifacts inside the repo or under `.vib/`, and do not improvise repo-relative scratch files.
   - When staging via bash instead of the `write` tool, a heredoc into a neutral temp path (`cat > /tmp/spec.md <<'EOF' … EOF`) is also tolerated; quote the heredoc delimiter (`<<'EOF'`) so the document body stays inert. Never stage into the repo or `.vib/`, and prefer the `write` tool over bash for large bodies.

4. **Initialize state** via `vib deep-interview write --input '<json>'`:
...
1. **Generate the specification** using opus model with the prompt-safe transcript. If the full interview transcript or initial context is too large, include the summary plus all concrete decisions, acceptance criteria, unresolved gaps, and ontology snapshots; never overflow the prompt with raw oversized context.
   ...
   - Apply the self-proofread once (DIPP-5) to newly generated spec prose before persistence, including generated natural-language table cells such as coverage notes, while preserving transcript answers, quoted/source text, code identifiers, file paths, commands, JSON/settings keys, table structure/fixed labels, and `.vib/_session-{sessionid}/specs/deep-interview-{slug}.md` unchanged.
2. **Write the final spec through the workflow CLI**: persist the artifact at `.vib/_session-{sessionid}/specs/deep-interview-{slug}.md`
   - Always use this exact final spec path. Prefer passing the spec markdown **inline** as the `--spec` value; only when it is too large to pass inline, stage it as a file in a system temp directory (`os.tmpdir()`/`$TMPDIR`, `/tmp`, `/var/tmp`) outside the project tree and pass that path — never write scratch specs to the repo root, the project tree, or `.vib/`.
   - Use the native deep-interview write command with `--write --stage final --slug {slug} --spec <markdown-or-path> [--json]` for artifact and state persistence; direct `.vib/` file edits are forbidden unless an explicit force override is active.
...
That command persists `.vib/_session-{sessionid}/specs/deep-interview-{slug}.md`, seeds ralplan in deliberate mode, and performs the safe deep-interview → ralplan state handoff. Skipping spec persistence leaves the Phase 5 chain blocked by design.
...
- Preserve the Vibrato `ask` tool path for native interaction; do not introduce parallel structured-question transport into this skill
...
- Use `vib deep-interview write` / `vib deep-interview read` for interview state persistence; the initial and subsequent deep-interview state payloads must include `threshold_source` alongside `threshold`; do not edit `.vib/_session-{sessionid}/state` directly without force override. For incremental scoring/maintenance updates, prefer the staged-transition verbs (`stage --for <transition> --input '<json>'`, `check`, `apply`, `discard`) — stage only the current delta (one round record by `round_key`, changed facts, or changed fields), never the whole transcript; `write` is incremental by default and replaces only with an explicit `--reset`; the session is inherited from `VIB_SESSION_ID`, revision CAS is runtime-owned, and the effective `current_ambiguity` is derived and clamped by the CLI at `apply`/`write` — read it from the command output rather than setting it yourself.
- Use the Vibrato workflow CLI to save the final spec at `.vib/_session-{sessionid}/specs/deep-interview-{slug}.md` exactly; do not use `write`, `edit`, or `ast_edit` directly on `.vib/` paths without force override.
- Use public Vibrato workflow entrypoints to bridge to ralplan or ultragoal only after explicit execution approval — never implement directly. Implementation handoff defaults to ultragoal.
...
- [ ] Spec persisted to `.vib/_session-{sessionid}/specs/deep-interview-{slug}.md` exactly via the Vibrato CLI (no direct `.vib/` edits without force override), covering every active topology component plus goal/constraints/acceptance criteria/clarity/ontology/transcript
...
Optional settings in `.vib/config.yml`:
...
If interrupted, run `/skill:deep-interview` again. The skill resumes from Vibrato workflow state via `vib deep-interview read`; do not read or edit `.vib/_session-{sessionid}/state` files directly unless an explicit force override is active.

## Approval-Gated Pipeline: deep-interview → ralplan → pending approval

See the Phase 5b "Approval-Gated Refinement Path" diagram for the full flow. In short: interview → spec at `.vib/_session-{sessionid}/specs/deep-interview-{slug}.md` → user selects "Refine with ralplan consensus" → `/skill:ralplan` (Planner/Architect/Critic consensus, plan written to `.vib/_session-{sessionid}/plans/`) → stop at `pending approval`. Execution is always a separate approval-gated step; deep-interview and ralplan never auto-invoke ultragoal just because a spec or plan exists.
