...
source: "forked from upstream ultragoal skill and rebranded for Vibrato"
---

# Ultragoal Workflow

Use when the user asks for `ultragoal`, `create-goals`, `complete-goals`, durable multi-goal planning, or sequential execution over Vibrato goal mode.
...
- `.vib/_session-{sessionid}/ultragoal/brief.md`
- `.vib/_session-{sessionid}/ultragoal/goals.json`
- `.vib/_session-{sessionid}/ultragoal/ledger.jsonl` (checkpoint and structured steering audit events)

Existing aggregate plans with the legacy enumerated objective are migrated to the stable pointer objective on read, persisted to `goals.json`, retained in `vibObjectiveAliases` for already-active hidden goal reconciliation, and audited with an `aggregate_objective_migrated` ledger entry.
- **Nudge budget setting** — the per-story give-up budget
  (`vib.ultragoal.nudgeBudget`, default **10**, non-negative integer) is read
  through one shared resolver in this exact order (first valid value wins):
  1. project `.vib/config.yml`
  2. user `<agentDir>/config.yml` (normally `~/.vib/agent/config.yml`, honoring
     `VIB_CODING_AGENT_DIR`/`PI_CODING_AGENT_DIR`; XDG applies only to categorized data/state/cache subdirs, never the workflow config path)
  3. built-in default
  `config.yml` uses the nested (schema) form - `vib: { ultragoal: { nudgeBudget } }`.
  ...
  are retired: the config-root `~/.vib/settings.json` is migrated once into the
  default global agent `config.yml` and its source removed, while the project
  `.vib/settings.json` is retained for non-workflow settings (only its workflow
  keys are migrated into project `.vib/config.yml` and no longer read unless a migration target is absent - a migration that could not publish (e.g. a read-only `.vib`) leaves the retained legacy value effective as the previously configured override until it can publish). Invalid optional settings
  files continue to the next layer or the default (tolerant).

## Corrupt current-session state recovery

When ultragoal detects its own current-session state is corrupt, tampered, unreadable, or stale on resume, run `vib state clear --force --mode ultragoal` before reseeding or restarting. Scope the clear to the current session via `--session-id`, the command payload, or `VIB_SESSION_ID`; it clears only ultragoal state for that session and never clears other skills or sessions.

## Always-used command examples

Use these exact `vib ultragoal` commands before spending tool calls rediscovering syntax:

```sh
vib ultragoal status
vib ultragoal status --json
vib ultragoal create-goals --brief "<brief>"
vib ultragoal create-goals --brief-file <path>
vib ultragoal complete-goals
vib ultragoal complete-goals --retry-failed
vib ultragoal quality-gate source-hash --json
vib ultragoal quality-gate validate --quality-gate-json <quality-gate-json-or-path> [--goal-id <id>] [--json]
vib ultragoal checkpoint --goal-id <id> --status complete --evidence "<evidence>" --quality-gate-json <quality-gate-json-or-path>
vib ultragoal checkpoint --goal-id <id> --status failed --evidence "<blocker/evidence>"
vib ultragoal record-review-blockers --goal-id <id> --title "Resolve final review blockers" --objective "<blocker-resolution objective>" --evidence "<review findings>"
```
...
2. Run one of:
   - `vib ultragoal create-goals --brief "<brief>"`
   - `vib ultragoal create-goals --brief-file <path>`
   - `cat <brief> | vib ultragoal create-goals --from-stdin`
   - `vib ultragoal create-goals --vib-goal-mode per-story --brief "<brief>"` only when one Vibrato goal context per story is explicitly preferred
3. Inspect `.vib/_session-{sessionid}/ultragoal/goals.json` and refine if needed.
...
Loop until `vib ultragoal status` reports all goals complete:

1. Run `vib ultragoal complete-goals`.
2. Read the printed handoff.
3. Call `goal({"op":"get"})`.
4. If no active Vibrato goal exists, call `goal({"op":"create","objective":"<printed payload objective>"})` with the printed payload. In aggregate mode, if the same aggregate objective is already active, continue the current Vibrato story without creating a new Vibrato goal. If `goal({"op":"get"})` shows a stale dropped goal (status `"dropped"`) and a new aggregate must start, no extra cleanup is needed — `goal({"op":"create"})` succeeds directly. If a previous aggregate is still active and you genuinely need a fresh start in the same session, call `goal({"op":"drop"})` first, then `goal({"op":"create"})`.
5. Complete the current Vibrato story only.
...
8. Checkpoint the durable ledger. Complete checkpoints require `--quality-gate-json` only:
   `vib ultragoal checkpoint --goal-id <id> --status complete --evidence "<evidence>" --quality-gate-json <quality-gate-json-or-path>`
   A successful complete checkpoint is story completion, not automatic run completion. Read the checkpoint output: when it prints `Next ultragoal goal: <id>`, continue that active story under the same aggregate Vibrato goal; when it prints `All ultragoal goals are complete`, the durable run is terminal. `vib ultragoal complete-goals` remains the supported manual next-story command if continuation output was missed.
9. If blocked or failed, checkpoint failure:
   `vib ultragoal checkpoint --goal-id <id> --status failed --evidence "<blocker/evidence>"`
10. For legacy per-story completed-goal blockers, preserve the non-terminal blocker with:
   `vib ultragoal checkpoint --goal-id <id> --status blocked --evidence "<completed legacy Vibrato goal blocks goal create in this thread>"`
11. Resume failed goals with `vib ultragoal complete-goals --retry-failed`.
...
- **`resolvable`** — anything the agent can act on: failing tests, missing implementation, a dependency to install, an ambiguous-but-inferable detail, investigation. **Never pause.** Exhaust autonomous resolution first: investigate, `vib ultragoal steer --kind add_subgoal --title "Investigate blocker" --objective "..." --evidence "..." --rationale "..."`, delegate an `executor`, or preserve the blocker durably with `vib ultragoal checkpoint --status blocked` / `vib ultragoal record-review-blockers` and keep scheduling the next goal.
...
```sh
vib ultragoal classify-blocker --classification human_blocked --evidence "<the specific human-only dependency>" [--goal-id <id>]
vib ultragoal record-critic-verdict --terminus pause --classification-event-id <eventId> --verdict OKAY --evidence "<terminal critic evidence>"
goal({"op":"pause"})
```
...
Use `vib ultragoal steer` when real findings or blockers prove the current story decomposition should change while the aggregate objective and constraints stay fixed. Steering is explicit-only and evidence-backed; broad natural-language requests are rejected instead of guessed.
...
```sh
vib ultragoal steer --kind add_subgoal --title "Investigate blocker" --objective "Validate the blocker and report evidence." --evidence "log/test output" --rationale "The blocker changes the safe execution order." --json
vib ultragoal steer --kind split_subgoal --goal-id G002 --replacements-json '[{"title":"Fix parser","objective":"Resolve parser blocker."},{"title":"Verify parser","objective":"Run focused parser verification."}]' --evidence "Implementation split found two separable risks" --rationale "Splitting keeps each sub-goal independently verifiable." --json
vib ultragoal steer --kind reorder_pending --order-json '["G003","G002"]' --evidence "Dependency order changed after investigation" --rationale "G003 must land before G002 can proceed safely." --json
vib ultragoal steer --kind revise_pending_wording --goal-id G002 --title "Clarify blocker story" --evidence "The current title hides the actual blocker" --rationale "Clear wording keeps the ledger auditable." --json
vib ultragoal steer --kind annotate_ledger --evidence "User changed release ordering at runtime" --rationale "The aggregate objective is unchanged, but the execution history needs an audit note." --json
vib ultragoal steer --kind mark_blocked_superseded --goal-id G004 --evidence "The blocked work is no longer required because replacement evidence covers it" --rationale "No replacement sub-goal is needed; superseding only the blocked sub-goal unblocks final completion without changing the aggregate objective." --json
```
...
- Do not edit the aggregate goal objective, original brief constraints, quality gates, or completion status. The aggregate objective is a stable pointer to `.vib/_session-{sessionid}/ultragoal/goals.json` and `.vib/_session-{sessionid}/ultragoal/ledger.jsonl`, not an enumeration of initial goal ids.
- Do not hard-delete goals, auto-complete work, weaken verification, or silently mutate `.vib/_session-{sessionid}/ultragoal`.
- Accepted and rejected attempts append structured audit entries to `.vib/_session-{sessionid}/ultragoal/ledger.jsonl`.
...
Ultragoal execution should use Vibrato's bundled role-agent roster when a durable story is large enough to benefit from delegation:
...
- A resumed subagent is still the same worker under the same contract: it must not mutate `.vib/_session-{sessionid}/ultragoal`, call goal tools, or absorb checkpoint/goal-state ownership, and review lanes (`architect`, `critic`) stay read-only when resumed.
...
The Ultragoal leader owns `.vib/_session-{sessionid}/ultragoal/goals.json` and `.vib/_session-{sessionid}/ultragoal/ledger.jsonl`. Role agents return implementation/review evidence; they do not checkpoint Ultragoal or mutate goal state.
...
- Worker agents **MUST NOT mutate `.vib/_session-{sessionid}/ultragoal`**, call goal tools, make checkpoint decisions, own integration, or own final verification. The Ultragoal leader keeps those responsibilities.
- Workers must not run `vib ultragoal checkpoint`: checkpoint authority stays with the leader after worker tasks are terminal. The leader checkpoints from worker evidence plus the current-session Vibrato goal snapshot, and performs no hidden goal mutation.
...
```sh
vib ultragoal create-goals --brief-file <path> --validation-batch-json '[{"schemaVersion":1,"batchId":"VB001","memberIds":["G001","G002","G003"],"finalGoalId":"G003"}]'
```
...
- It is a read-only detector+reporter over the active story's changed files only: it never edits code, writes files, mutates `.vib/`, checkpoints, calls goal tools, or spawns workflows.
...
9. If the joined findings contain any blocker, do **not** checkpoint `complete` and do **not** call `goal({"op":"complete"})`. Record **one consolidated blocker batch** for all findings from the whole cohort instead of one story per lane:
   ```sh
   vib ultragoal record-review-blockers --goal-id <id> --title "Resolve verification blockers" --objective "<blocker-resolution objective>" --evidence "<joined cohort findings>"
   ```
...
While an Ultragoal run is active, the `ask` tool is blocked for all agents. Record unresolved review decisions as durable blockers with `vib ultragoal record-review-blockers` instead of prompting interactively.
...
Validate before you checkpoint. `vib ultragoal quality-gate validate --quality-gate-json <json-or-path> [--goal-id <id>] [--json]` applies exactly the same rules as `checkpoint --status complete` (including deferred-vs-boundary gate selection and artifact existence checks) but is strictly read-only: it never touches `goals.json`, `ledger.jsonl`, or goal state. It exits non-zero with the full diagnostics list when invalid, so authoring a gate is one pass instead of an edit/retry loop. `--quality-gate-json` must include:
...
```
vib state ultragoal write --input '{"current_phase":"handoff"}' --json
```

The skill tool then dispatches `/skill:ralplan` or `/skill:deep-interview` same-turn and runs `vib state ultragoal handoff --to <ralplan|deep-interview> --json` in-process to atomically demote ultragoal, promote the callee, and sync both `.vib/_session-{sessionid}/state/skill-active-state.json` files. You do not need to run the handoff verb yourself.

## Constraints

- The shell command emits a model-facing handoff for the active Vibrato agent; it does not invoke any `/goal` slash-command and the agent loop must not depend on any `/goal` subcommand.
