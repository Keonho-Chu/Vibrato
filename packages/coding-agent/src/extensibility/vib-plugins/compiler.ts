...
import { parseFrontmatter, pathIsWithin } from "@vib-rato/utils";
...
async function readManifestJson(filePath: string): Promise<unknown> {
	...
	try {
		...
		throw new VibPluginLoadError("missing_file", `Missing Vibrato plugin manifest at ${filePath}`, {
	...
	try {
		...
		throw new VibPluginLoadError("invalid_manifest", `Invalid Vibrato plugin manifest JSON at ${filePath}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
}
...
async function resolveDeclaredFile(pluginRoot: string, rel: string): Promise<string> {
	...
	try {
		...
		throw new VibPluginLoadError("missing_file", `Missing Vibrato plugin file at ${resolved}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
	const realRoot = await fs.realpath(pluginRoot);
	if (!pathIsWithin(realRoot, real)) {
		throw new VibPluginLoadError("security_policy", `Vibrato plugin file escapes root via symlink: ${rel}`);
	}
	return resolved;
}
...
async function hashFile(
	...
	try {
		...
		throw new VibPluginLoadError("missing_file", `Missing Vibrato plugin file at ${absPath}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
	const digest = sha256(buf);
	if (declaredSha !== undefined && declaredSha.toLowerCase() !== digest) {
		throw new VibPluginLoadError("hash_mismatch", `Vibrato plugin file hash mismatch for ${rel}`);
	}
	return { sha256: digest, bytes: buf.byteLength };
}

function mcpConfigHash(entry: VibPluginMcpManifestEntry): string {
...
async function compileAppendix(
	pluginRoot: string,
	entry: VibPluginAppendixManifestEntry,
	field: string,
	...
	if (hasPath === hasContent) {
		throw new VibPluginLoadError(
			"invalid_appendix",
			`Invalid Vibrato plugin ${field}: exactly one of "path" or "content" is required`,
		);
	}
	if (hasContent) {
		const content = entry.content ?? "";
		if (content.trim().length === 0) {
			throw new VibPluginLoadError("invalid_appendix", `Invalid Vibrato plugin ${field}: inline content is empty`);
		}
		const digest = sha256(content);
		if (entry.sha256 !== undefined && entry.sha256.toLowerCase() !== digest) {
			throw new VibPluginLoadError("hash_mismatch", `Vibrato plugin ${field} content hash mismatch`);
		}
	...
	if (bytes === 0) {
		throw new VibPluginLoadError("invalid_appendix", `Invalid Vibrato plugin ${field}: file is empty`);
	}
 ...
 */
export async function compileVibPluginBundle(root: string): Promise<NormalizedVibPluginBundle> {
	const pluginRoot = path.resolve(root);
	const manifestPath = path.join(pluginRoot, VIB_PLUGIN_MANIFEST_FILENAME);
	...
	for (const rel of manifest.subskills) {
		...
		try {
			...
			throw new VibPluginLoadError("missing_file", `Missing Vibrato sub-skill file at ${abs}`, {
				cause: error instanceof Error ? error : undefined,
			});
		}
		...
		try {
			...
			throw new VibPluginLoadError("invalid_frontmatter", `Invalid Vibrato sub-skill frontmatter at ${abs}`, {
				cause: error instanceof Error ? error : undefined,
			});
		}
	...
	for (const hook of manifest.hooks) {
		...
		if (hook.event === "tool_call") {
			if (!hook.target) {
				throw new VibPluginLoadError(
					"invalid_hook",
					`Vibrato plugin hook "${hook.name}": tool_call requires a target`,
				);
			}
			if (!hook.phase) {
				throw new VibPluginLoadError(
					"invalid_hook",
					`Vibrato plugin hook "${hook.name}": tool_call requires a "before"/"after" phase`,
				);
			}
		}
		if (hook.phase && hook.event !== "tool_call" && hook.event !== "tool_result") {
			throw new VibPluginLoadError(
				"invalid_hook",
				`Vibrato plugin hook "${hook.name}": phase is only supported for tool_call/tool_result events`,
			);
		}
		if (hook.event === "tool_result" && hook.phase !== "after") {
			throw new VibPluginLoadError(
				"invalid_hook",
				`Vibrato plugin hook "${hook.name}": tool_result requires the after phase`,
			);
		}
		hooks.push({
	...
	for (const entry of manifest.mcps) {
		...
		if (entry.transport === "stdio") {
			if (!entry.command) {
				throw new VibPluginLoadError("invalid_mcp", `Vibrato plugin MCP "${entry.name}": stdio requires a command`);
			}
		} else if (!entry.url) {
			throw new VibPluginLoadError(
				"invalid_mcp",
				`Vibrato plugin MCP "${entry.name}": ${entry.transport} requires a url`,
			);
		}
	...
	const surfaces: NormalizedVibPluginSurfaces = {
		subskills,
		tools,
		hooks,
		mcps,
	...
	copiedFiles.unshift({
		relativePath: VIB_PLUGIN_MANIFEST_FILENAME,
		sha256: manifestHash,
