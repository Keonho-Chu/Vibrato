...
import { VibPluginLoadError, type VibPluginRegistryEntry, type VibPluginScope } from "./types";
...
async function resolveConstrainedHookFile(root: string, relativePath: string): Promise<string> {
	...
	if (rel.startsWith("..") || path.isAbsolute(rel))
		throw new VibPluginLoadError(
			"runtime_mismatch",
			`Vibrato plugin hook escapes its installed root: ${relativePath}`,
		);
	return fileReal;
}
...
export class ConstrainedPluginHookDescriptor {
	...
	async load(): Promise<ConstrainedPluginHook> {
		...
		if (typeof factory !== "function")
			throw new VibPluginLoadError("invalid_hook", "Plugin hook must export a default function");
...
async function loadOneHook(
	...
	try {
		...
		const code = error instanceof VibPluginLoadError ? error.code : "invalid_hook";
		return {
			hook: null,
			quarantine: {
...
export async function loadConstrainedPluginHooks(input: { cwd: string }): Promise<ConstrainedHookLoadResult> {
	const effective = await loadEffectiveVibPluginRegistry(input.cwd);
