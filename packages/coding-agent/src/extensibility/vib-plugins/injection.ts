...
import { type LoadedSubskillActivation, VIB_SUBSKILL_PARENT_AGENTS } from "./types";
...
export function wrapSubskillBlock(
	...
	return `\n\n---\n\n<vib-subskill plugin="${escapeAttribute(activation.plugin)}" name="${escapeAttribute(activation.subskillName)}" parent="${escapeAttribute(activation.parent)}" phase="${escapeAttribute(activation.phase)}" arg="${escapeAttribute(activation.activationArg)}">\n${sanitizePromptBody(body)}\n</vib-subskill>`;
}
...
export async function buildAgentSubskillInjection(input: {
	...
	if (!(VIB_SUBSKILL_PARENT_AGENTS as readonly string[]).includes(input.agentName)) return "";
	const resolvedSessionId = await resolveBoundarySessionId(input.cwd, input.sessionId);
...
import type { VibPluginRegistryEntry } from "./types";
...
function wrapAdvert(kind: "skill" | "agent", parent: string, lines: string[]): string {
	return `<vib-plugin-subskill-advertisement parent="${clampField(parent)}" kind="${kind}">\n${lines.join("\n")}\n</vib-plugin-subskill-advertisement>`;
}
...
function collectAdverts(entries: readonly VibPluginRegistryEntry[], parent: string, phase?: string): AdvertItem[] {
...
export function buildAgentSubskillAdvertisement(entries: readonly VibPluginRegistryEntry[], agentName: string): string {
