...
import {
	...
	type VibPluginQuarantineEntry,
	type VibPluginRegistryEntry,
	type VibPluginScope,
} from "./types";

/**
 * Pure fingerprint and reconciliation helpers for the Vibrato bundle lifecycle.
...
export function bundleIdentity(scope: VibPluginScope, name: string): VibBundleIdentity {
	return { kind: VIB_BUNDLE_KIND, scope, name };
}

export function identityKey(identity: VibBundleIdentity): string {
	return `${identity.kind}\u0000${identity.scope}\u0000${identity.name}`;
}

export function identityEquals(a: VibBundleIdentity, b: VibBundleIdentity): boolean {
	return a.kind === b.kind && a.scope === b.scope && a.name === b.name;
}

/** All stable surface IDs of a surface set, sorted and de-duplicated. */
export function surfaceIdsOf(surfaces: NormalizedVibPluginSurfaces): string[] {
	const ids = [
 ...
 */
export function targetFingerprint(entry: VibPluginRegistryEntry): string {
 ...
 */
export function baselineFingerprint(entry: VibPluginRegistryEntry): string {
	const source = [
		entry.source.kind,
		entry.source.uri,
		entry.source.ref ?? "",
...
export function candidateFingerprint(scope: VibPluginScope, bundle: NormalizedVibPluginBundle): string {
 ...
 */
export function activationFingerprint(entries: readonly VibPluginRegistryEntry[]): string {
	const parts = entries
		.filter(e => e.enabled)
		.map(e => {
...
export function reconcileEnablement(
	...
	candidateQuarantine: readonly VibPluginQuarantineEntry[] = [],
