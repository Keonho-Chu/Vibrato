...
import {
	...
	type VibBundleTransactionDecision,
	VibPluginSourceUnavailableError,
} from "./installer";
import {
...
import type {
	...
	VibBundleSafeSource,
	VibBundleSummary,
	VibBundleSurfaceSummary,
	VibInstallResult,
	VibLifecycleError,
	VibLifecycleResult,
	VibPluginRegistry,
	VibPluginRegistryEntry,
	VibPluginRegistrySource,
	VibPluginScope,
	VibReviewedUpdateToken,
	VibToggleResult,
	VibUninstallPreview,
...
/**
 * Vibrato bundle lifecycle service.
 *
 * This module is the ONLY policy and persistence writer for Vibrato bundles: fresh
...
export interface VibLifecycleContext {
	cwd: string;
}

function fail(code: VibLifecycleError["code"], message: string, recovery?: string): VibLifecycleError {
...
const UNSUPPORTED_UPDATE_REASON: Partial<Record<VibPluginRegistrySource["kind"], string>> = {};
...
export function redactSourceLocator(source: VibPluginRegistrySource): string {
...
function toSafeSource(source: VibPluginRegistrySource): VibBundleSafeSource {
	const unsupportedReason = UNSUPPORTED_UPDATE_REASON[source.kind];
	const safe: VibBundleSafeSource = {
		kind: source.kind,
...
function surfaceSummaries(entry: VibPluginRegistryEntry): VibBundleSurfaceSummary[] {
	...
	const rows: VibBundleSurfaceSummary[] = [
		...
		const summary: VibBundleSurfaceSummary = {
			...row,
...
export function toBundleSummary(entry: VibPluginRegistryEntry): VibBundleSummary {
 ...
 */
function storedSourceLocator(source: VibPluginRegistrySource): string {
...
async function readEffective(cwd: string): Promise<VibPluginRegistryEntry[]> {
...
export async function listVibBundles(ctx: VibLifecycleContext): Promise<VibBundleSummary[]> {
...
export async function getVibBundle(
	ctx: VibLifecycleContext,
...
function safeInstalledRoot(scope: VibPluginScope, cwd: string, pluginRoot: string): string | null {
...
function isUninstallableEntry(value: unknown, identity: VibBundleIdentity): value is VibPluginRegistryEntry {
...
function uninstallFailure(
	...
	const recovery =
		kind === "metadata"
			? `Repair the Vibrato ${identity.scope} registry, then retry vib plugin uninstall ${identity.name} --${identity.scope}`
			: `Check Vibrato plugin directory permissions, then retry vib plugin uninstall ${identity.name} --${identity.scope}`;
	return fail("invalid_target", `Could not uninstall Vibrato bundle "${identity.name}" because ${detail}`, recovery);
}
...
function resolveUninstallTarget(
	registry: VibPluginRegistry,
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
): VibLifecycleResult<{ entry: VibPluginRegistryEntry; root: string }> {
...
async function readUninstallRegistry(
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
	read: (scope: VibPluginScope, cwd: string) => Promise<VibPluginRegistry>,
	onMalformed: (identity: VibBundleIdentity) => VibLifecycleError,
): Promise<VibLifecycleResult<VibPluginRegistry>> {
	try {
...
/**
 * What {@link uninstallVibBundle} would remove, resolved and validated the same
 ...
 * mutating {@link uninstallVibBundle} keeps mapping the same condition to
 * `invalid_target`, its historical result contract.
 */
export async function previewVibBundleUninstall(
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
): Promise<VibLifecycleResult<VibUninstallPreview>> {
...
export async function uninstallVibBundle(
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
): Promise<VibLifecycleResult<{ identity: VibBundleIdentity; summary: VibBundleSummary }>> {
...
function notInstalled(identity: VibBundleIdentity): VibLifecycleError {
	return fail(
		"not_installed",
		`Vibrato bundle "${identity.name}" is not installed in the ${identity.scope} scope`,
		`vib plugin install <source> --${identity.scope}`,
	);
}
/**
 ...
 * load error that callers treat as "Vibrato does not own this scope right now"; the
 * preview must classify the same way instead of claiming the target.
 */
function registryUnreadable(identity: VibBundleIdentity): VibLifecycleError {
	return fail(
		"registry_unreadable",
		`Could not read the Vibrato ${identity.scope} plugin registry while resolving "${identity.name}"`,
		`Repair the Vibrato ${identity.scope} registry (vib plugin doctor --fix), then retry`,
	);
}

function alreadyInstalled(name: string, scope: VibPluginScope): VibLifecycleError {
	return fail(
		"already_installed_use_upgrade",
		`Vibrato bundle "${name}" is already installed in the ${scope} scope`,
		`vib plugin upgrade ${name} --${scope}`,
	);
}
...
async function withSourceAvailability<T>(
	identity: VibBundleIdentity,
	run: () => Promise<VibLifecycleResult<T>>,
): Promise<VibLifecycleResult<T>> {
	try {
		return await run();
	} catch (error) {
		...
		if (error instanceof VibPluginSourceUnavailableError) {
			return {
				ok: false,
				error: fail(
					"source_unavailable",
					`The stored source for Vibrato bundle "${identity.name}" could not be resolved`,
					`vib plugin install <source> --${identity.scope}`,
				),
			};
		}
		if (error instanceof VibPluginLoadError) {
			return {
				ok: false,
				error: fail(
					"invalid_target",
					`Stored source for Vibrato bundle "${identity.name}" is no longer a valid plugin target`,
					`vib plugin install <source> --${identity.scope}`,
				),
			};
		}
...
function isLocalDirectorySource(source: string): boolean {
	...
	// place, so its manifest is not at `<source>/vibrato-plugin.json`.
...
async function declaredBundleName(source: string): Promise<string | undefined> {
	...
	try {
		...
		const manifestPath = path.join(source, VIB_PLUGIN_MANIFEST_FILENAME);
 ...
 */
export async function installVibBundle(
	ctx: VibLifecycleContext,
	scope: VibPluginScope,
	source: string,
): Promise<VibLifecycleResult<VibInstallResult>> {
	...
	let result: Awaited<ReturnType<typeof runVibBundleTransaction>>;
	try {
		result = await runVibBundleTransaction(source, {
			scope,
			cwd: ctx.cwd,
			decide: async ({ existing, candidate }): Promise<VibBundleTransactionDecision> => {
				if (existing) {
					return {
						kind: "abort",
						error: fail(
							"already_installed_use_upgrade",
							`Vibrato bundle "${existing.name}" is already installed in the ${scope} scope`,
							`vib plugin upgrade ${existing.name} --${scope}`,
						),
					};
				}
		...
		if (error instanceof VibPluginSourceUnavailableError) {
			throw new VibPluginLoadError("missing_file", "Vibrato plugin source directory not found");
		}
		throw error;
	}
 ...
 */
export async function previewVibBundleUpdate(
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
	...
	if (!safeSource.updatable) {
		return {
			...
			error: fail(
				"source_unsupported",
				`Vibrato bundle "${identity.name}" was installed from a ${entry.source.kind} source that cannot be re-resolved`,
			),
		};
	}
	...
	return await withSourceAvailability(identity, async () =>
		resolveVibBundleCandidate(storedSourceLocator(entry.source), async ({ bundle }) => {
			...
			const token: VibReviewedUpdateToken = {
				identity,
 ...
 * zero mutation.
 */
export async function applyVibBundleUpdate(
	ctx: VibLifecycleContext,
	...
	return await withSourceAvailability<VibUpdateApplyResult>(identity, async () => {
		const result = await runVibBundleTransaction(storedSourceLocator(entry.source), {
			scope: identity.scope,
			cwd: ctx.cwd,
			decide: async ({ existing, effective, bundle, candidate }): Promise<VibBundleTransactionDecision> => {
				...
				const next: VibPluginRegistryEntry = {
					...candidate,
...
async function mutateEntry(
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
	mutate: (entry: VibPluginRegistryEntry) => VibLifecycleResult<VibPluginRegistryEntry | null>,
 ...
 */
export async function setVibBundleEnabled(
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
	enabled: boolean,
): Promise<VibLifecycleResult<VibToggleResult>> {
	return await mutateEntry(ctx, identity, entry => {
		if (enabled && (entry.quarantine?.length ?? 0) > 0) {
			return {
				ok: false,
				error: fail("quarantined", `Vibrato bundle "${identity.name}" is quarantined and cannot be enabled`),
			};
		}
...
export async function setVibBundleSurfaceEnabled(
	ctx: VibLifecycleContext,
	identity: VibBundleIdentity,
	surfaceId: string,
	enabled: boolean,
	...
	return await mutateEntry(ctx, identity, entry => {
		if (!surfaceIdsOf(entry.surfaces).includes(surfaceId)) {
			return {
				ok: false,
				error: fail("surface_unknown", `Vibrato bundle "${identity.name}" has no surface "${surfaceId}"`),
			};
		}
...
export async function currentActivationFingerprint(ctx: VibLifecycleContext): Promise<string> {
