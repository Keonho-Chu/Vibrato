...
async function readRequiredText(filePath: string, kind: "sub-skill" | "tool"): Promise<string> {
	try {
		...
		throw new VibPluginLoadError("missing_file", `Missing Vibrato plugin ${kind} file at ${filePath}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
}
...
export async function loadVibPlugin(root: string): Promise<LoadedVibPlugin> {
	...
	for (const rel of manifest.subskills) {
		...
		try {
			...
			throw new VibPluginLoadError("invalid_frontmatter", `Invalid Vibrato sub-skill frontmatter at ${filePath}`, {
				cause: error instanceof Error ? error : undefined,
			});
		}
...
export async function loadVibPlugins(roots: readonly string[]): Promise<LoadedVibPlugin[]> {
	const plugins: LoadedVibPlugin[] = [];
	for (const root of roots) {
		plugins.push(await loadVibPlugin(root));
	}
