import { loadEffectiveVibPluginRegistry } from "./registry";
import { type SessionQuarantine, validateSessionBundles, verifyEntryHashes } from "./session-validation";
import type { VibPluginRegistryEntry, VibPluginScope } from "./types";

/**
 * Observability for Vibrato plugin bundle surfaces, consumable by the extension
...
export interface PluginSurfaceRow {
	...
	sourceKind: VibPluginRegistryEntry["source"]["kind"];
...
function rowsForEntry(entry: VibPluginRegistryEntry, quarantinedIds: Map<string, string>): PluginSurfaceRow[] {
 ...
 * `vib customize doctor`) so reporting never persists registry migrations.
 */
export async function summarizeVibPluginObservability(
	cwd: string,
	options: { migrate?: boolean } = {},
): Promise<PluginObservabilitySummary> {
	const effective = await loadEffectiveVibPluginRegistry(cwd, options);
