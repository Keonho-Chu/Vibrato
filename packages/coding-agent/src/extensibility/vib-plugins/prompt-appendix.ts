...
import type { NormalizedAppendixSurface, VibPluginRegistryEntry, VibSubskillParentAgent } from "./types";
import { VibPluginLoadError } from "./types";
...
export function sanitizePromptBody(text: string): string {
	...
	// <developer>/<vib-subskill> tag that escapes the lower-authority block.
...
function assertAppendixDigest(bytes: Buffer, surface: NormalizedAppendixSurface, label: string): void {
	...
	if (actual.toLowerCase() !== surface.contentHash.toLowerCase()) {
		throw new VibPluginLoadError("runtime_mismatch", `Appendix hash drift at ${label}`);
	}
}
...
async function readAppendixBody(
	...
	try {
		...
		throw new VibPluginLoadError("runtime_mismatch", `Missing or unreadable appendix at ${surface.relativePath}`, {
...
export interface RenderPluginAppendixOptions {
	...
	beforeRead?: (entry: VibPluginRegistryEntry, surface: NormalizedAppendixSurface) => Promise<void>;
}
...
export interface RenderedPluginAppendices {
	...
	byAgent: Map<VibSubskillParentAgent, string>;
...
export async function renderPluginAppendices(
	...
	const byAgent = new Map<VibSubskillParentAgent, string[]>();
	...
	for (const entry of entries) {
		...
		for (const sa of entry.surfaces.systemAppendices) {
			...
			if (!body) continue;
			const block = `<vib-plugin-system-appendix plugin="${escapeAttr(entry.name)}" name="${escapeAttr(sa.name)}" sha256="${sa.contentHash}" authority="appendix-lower-than-system">\n${body}\n</vib-plugin-system-appendix>`;
		...
		for (const aa of entry.surfaces.agentAppendices) {
			...
			if (!body) continue;
			const block = `<vib-plugin-agent-appendix plugin="${escapeAttr(entry.name)}" agent="${escapeAttr(aa.agent)}" name="${escapeAttr(aa.name)}" sha256="${aa.contentHash}" authority="appendix-lower-than-agent">\n${body}\n</vib-plugin-agent-appendix>`;
			if (!admit(block)) continue;
