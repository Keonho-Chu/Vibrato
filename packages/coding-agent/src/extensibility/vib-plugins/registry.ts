...
import { migrateVibPluginEntries } from "./migration";
import { vibPluginProjectRoot, vibPluginUserRoot } from "./paths";
import { VibPluginLoadError, type VibPluginRegistry, type VibPluginRegistryEntry, type VibPluginScope } from "./types";
...
export function registryRootForScope(scope: VibPluginScope, cwd: string): string {
	return scope === "user" ? vibPluginUserRoot() : vibPluginProjectRoot(cwd);
}

export function registryPathForScope(scope: VibPluginScope, cwd: string): string {
	return path.join(registryRootForScope(scope, cwd), REGISTRY_FILENAME);
}

function emptyRegistry(scope: VibPluginScope): VibPluginRegistry {
 ...
 */
export function sortRegistryEntries(entries: VibPluginRegistryEntry[]): VibPluginRegistryEntry[] {
	const scopeRank = (scope: VibPluginScope): number => (scope === "user" ? 0 : 1);
...
async function readRegistryRaw(scope: VibPluginScope, cwd: string): Promise<VibPluginRegistry> {
	...
	try {
		...
		throw new VibPluginLoadError("invalid_manifest", `Corrupt Vibrato plugin registry at ${registryPath}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
	if (typeof parsed !== "object" || parsed === null || (parsed as VibPluginRegistry).version !== 1) {
		throw new VibPluginLoadError("invalid_manifest", `Unsupported Vibrato plugin registry shape at ${registryPath}`);
	}
	const registry = parsed as VibPluginRegistry;
	if (registry.scope !== scope)
		throw new VibPluginLoadError(
			"invalid_manifest",
			`Vibrato plugin registry scope mismatch at ${registryPath}: expected ${scope}`,
		);
	if (
		!Array.isArray(registry.plugins) ||
		registry.plugins.some(plugin => {
			if (!plugin || typeof plugin !== "object") return true;
			const entry = plugin as VibPluginRegistryEntry;
			return (
				entry.scope !== scope ||
				!entry.surfaces ||
		...
		throw new VibPluginLoadError(
			"invalid_manifest",
			`Invalid Vibrato plugin registry entries or scope at ${registryPath}`,
		);
	}
...
async function discoverLegacyEntries(
	...
	cwd: string,
	existing: readonly VibPluginRegistryEntry[],
): Promise<VibPluginRegistryEntry[]> {
		...
		try {
			...
			try {
				const manifest = JSON.parse(
					await fs.readFile(path.join(pluginRoot, "vibrato-plugin.json"), "utf8"),
			...
			const now = new Date().toISOString();
			const code = error instanceof VibPluginLoadError ? error.code : "missing_file";
			discovered.push({
				name,
				version,
				scope,
 ...
 */
export async function readEffectiveRegistryUnpersisted(scope: VibPluginScope, cwd: string): Promise<VibPluginRegistry> {
	const registry = await readRegistryRaw(scope, cwd);
	const discovered = await discoverLegacyEntries(scope, cwd, registry.plugins);
	const migrated = await migrateVibPluginEntries([...registry.plugins, ...discovered]);
...
export async function readRegistry(
	...
	return await withRegistryLock(scope, cwd, async () => {
		...
		const latestMigrated = await migrateVibPluginEntries([...latest.plugins, ...latestDiscovered]);
		if (latestMigrated.changed || latestDiscovered.length > 0) {
			const next: VibPluginRegistry = { ...latest, plugins: sortRegistryEntries(latestMigrated.entries) };
	...
	for (;;) {
		try {
			...
			if (Date.now() > deadline) {
				throw new VibPluginLoadError(
					"install_conflict",
					`Timed out acquiring Vibrato plugin registry lock at ${lockPath}; remove it manually if no install is running`,
				);
			}
...
export async function withRegistryLock<T>(scope: VibPluginScope, cwd: string, fn: () => Promise<T>): Promise<T> {
...
export async function writeRegistryUnlocked(
	...
	cwd: string,
	ownerScope: VibPluginScope = registry.scope,
): Promise<void> {
	if (registry.scope !== ownerScope)
		throw new VibPluginLoadError(
			"invalid_manifest",
			`Vibrato plugin registry scope mismatch: caller owns ${ownerScope}, registry declares ${registry.scope}`,
		);
	if (registry.plugins.some(entry => entry.scope !== ownerScope))
		throw new VibPluginLoadError(
			"invalid_manifest",
			`Vibrato plugin entry scope mismatch: caller owns ${ownerScope}`,
		);
	const registryPath = registryPathForScope(ownerScope, cwd);
	await fs.mkdir(path.dirname(registryPath), { recursive: true });
	const sorted: VibPluginRegistry = { ...registry, scope: ownerScope, plugins: sortRegistryEntries(registry.plugins) };
...
export async function updateRegistry(
	scope: VibPluginScope,
	cwd: string,
	mutator: (entries: VibPluginRegistryEntry[]) => VibPluginRegistryEntry[],
): Promise<VibPluginRegistry> {
	return await withRegistryLock(scope, cwd, async () => {
		const current = await readRegistry(scope, cwd, { migrate: false });
		const nextEntries = mutator([...current.plugins]);
		const next: VibPluginRegistry = { version: 1, scope, plugins: sortRegistryEntries(nextEntries) };
 ...
 * example `vib customize doctor`) pass `{ migrate: false }` so reporting never
 * writes to the registry.
 */
export async function loadEffectiveVibPluginRegistry(
	cwd: string,
	options: { migrate?: boolean } = {},
): Promise<VibPluginRegistryEntry[]> {
...
export function registryEntryFingerprint(entry: VibPluginRegistryEntry): string {
