...
import { loadEffectiveVibPluginRegistry, registryPathForScope } from "./registry";
import { type SessionQuarantine, type SessionValidationResult, validateSessionBundles } from "./session-validation";
import type { JsonSchema202012, NormalizedToolSurfaceV2, VibPluginRegistryEntry, VibPluginScope } from "./types";
...
export interface VibPluginToolDeclaration extends NormalizedToolSurfaceV2 {
	plugin: string;
...
async function resolveRuntimeFile(root: string, relativePath: string): Promise<string> {
	...
	if (!isWithin(rootReal, fileReal))
		throw new Error(`Vibrato plugin implementation escapes its installed root: ${relativePath}`);
	return fileReal;
}
/**
 ...
 */
export async function getVibPluginToolDeclarations(cwd: string): Promise<VibPluginToolDeclaration[]> {
	const entries = await loadEffectiveVibPluginRegistry(cwd);
	const declarations: VibPluginToolDeclaration[] = [];
	for (const entry of entries) {
		if (!entry.enabled || entry.migration?.status === "failed") continue;
		for (const surface of entry.surfaces.tools) {
			if (isV2Tool(surface))
				declarations.push({ ...surface, plugin: entry.name, scope: entry.scope } as VibPluginToolDeclaration);
		}
	}
	return declarations;
}
...
export async function serveVibPluginSchemas(cwd: string): Promise<Record<string, JsonSchema202012>> {
	const declarations = await getVibPluginToolDeclarations(cwd);
...
const registryScopes: VibPluginScope[] = ["user", "project"];
...
async function snapshotPluginFiles(entries: readonly VibPluginRegistryEntry[]): Promise<FileSnapshot[]> {
...
async function verifyEntryHashesCached(entry: VibPluginRegistryEntry): Promise<SessionQuarantine | null> {
	for (const file of entry.copiedFiles) {
		let abs: string;
		try {
...
export async function loadAlwaysOnPluginTools(input: {
	...
	declarations?: readonly VibPluginToolDeclaration[];
	...
	const loaded = await loadCustomTools(
		...
		async resolvedPath => {
			...
			if (!info?.implementationHash)
				throw new Error(`Unregistered or unhashed Vibrato tool import: ${resolvedPath}`);
			...
			if (path.resolve(finalPath) !== path.resolve(resolvedPath))
				throw new Error(`Vibrato tool path drifted before import: ${info.relativePath}`);
