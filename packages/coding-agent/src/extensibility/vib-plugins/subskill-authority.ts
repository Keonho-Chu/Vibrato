...
async function readVerifiedFile(
	...
	try {
		...
		throw new VibPluginLoadError("runtime_mismatch", `Missing or unreadable ${label} at ${relativePath}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
	if (!isWithin(rootReal, fileReal)) {
		throw new VibPluginLoadError("runtime_mismatch", `${label} escapes the installed plugin root: ${relativePath}`);
	}
	let bytes: Buffer;
	try {
	...
	if (actual.toLowerCase() !== expected.toLowerCase()) {
		throw new VibPluginLoadError("runtime_mismatch", `${label} hash drift at ${relativePath}`);
	}
...
async function tryVerifyFile(
	...
	try {
		...
		if (error instanceof VibPluginLoadError) return null;
		throw error;
	}
}
...
): VibPluginRegistryEntry | undefined {
...
export async function resolveValidatedActiveSubskill(input: {
	...
	const entries = await loadEffectiveVibPluginRegistry(input.cwd);
