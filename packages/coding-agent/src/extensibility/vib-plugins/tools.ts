...
export async function loadActiveSubskillTools(input: {
	...
	const result = await loadCustomTools(
		...
		async resolvedPath => {
			...
			if (!pair) throw new Error(`Unregistered Vibrato subskill tool import: ${resolvedPath}`);
	...
	for (const error of result.errors)
		logger.warn("Skipping Vibrato plugin sub-skill tool", { path: error.path, error: error.error });
	...
	for (const loadedTool of result.tools) {
		...
		if (reservedToolNames.has(name)) {
			logger.warn("Skipping Vibrato plugin sub-skill tool name because it conflicts with a reserved tool", { name });
			continue;
		}
		if (seenNames.has(name)) {
			logger.warn("Skipping duplicate Vibrato plugin sub-skill tool name", { name, path: loadedTool.path });
			continue;
		}
		seenNames.add(name);
