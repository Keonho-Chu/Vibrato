...
import { isKnownWorkflowState } from "../../vib-runtime/workflow-manifest";
...
import {
	...
	VIB_AGENT_SUBSKILL_PHASES,
	VIB_SUBSKILL_PARENT_AGENTS,
	VIB_SUBSKILL_PARENT_SKILLS,
	VibPluginLoadError,
	type VibPluginRegistryEntry,
	type VibSubskillParentAgent,
} from "./types";

function isParentSkill(value: string): value is CanonicalVibWorkflowSkill {
	return (VIB_SUBSKILL_PARENT_SKILLS as readonly string[]).includes(value);
}

function isParentAgent(value: string): value is VibSubskillParentAgent {
	return (VIB_SUBSKILL_PARENT_AGENTS as readonly string[]).includes(value);
}
...
export function validateBinding(fm: SubskillFrontmatter): void {
	...
	if (isParentSkill(parent)) {
		if (!isKnownWorkflowState(parent, fm.phase)) {
			throw new VibPluginLoadError("invalid_phase", `Invalid Vibrato sub-skill phase for ${parent}: ${fm.phase}`);
		}
		return;
	}

	if (isParentAgent(parent)) {
		if (!VIB_AGENT_SUBSKILL_PHASES[parent].includes(fm.phase)) {
			throw new VibPluginLoadError("invalid_phase", `Invalid Vibrato sub-skill phase for ${parent}: ${fm.phase}`);
		}
		return;
	}

	throw new VibPluginLoadError("invalid_parent", `Invalid Vibrato sub-skill parent: ${parent}`);
}
	...
	for (const binding of bindings) {
		...
		if (existing) {
			throw new VibPluginLoadError(
				"duplicate_arg",
				`Duplicate Vibrato sub-skill activation_arg for ${binding.parent}: ${binding.activationArg} (${existing.filePath}, ${binding.filePath})`,
			);
		}
	...
	for (const binding of bindings) {
		...
		if (existing) {
			throw new VibPluginLoadError(
				...
				`Duplicate Vibrato sub-skill parent/phase binding for ${binding.parent}/${binding.phase} (${existing.filePath}, ${binding.filePath})`,
			);
		}
...
export function validateInstallPlan(
	...
	effectiveEntries: readonly VibPluginRegistryEntry[],
): void {
	...
	for (const t of bundle.surfaces.tools) {
		if (toolNames.has(t.name)) {
			throw new VibPluginLoadError("duplicate_tool", `Vibrato plugin tool name collides: ${t.name}`);
		}
		toolNames.add(t.name);
	}
	for (const h of bundle.surfaces.hooks) {
		if (hookKeys.has(h.extensionId)) {
			throw new VibPluginLoadError("duplicate_hook", `Vibrato plugin hook collides: ${h.extensionId}`);
		}
		hookKeys.add(h.extensionId);
	}
	for (const m of bundle.surfaces.mcps) {
		if (mcpNames.has(m.name)) {
			throw new VibPluginLoadError("duplicate_mcp", `Vibrato plugin MCP name collides: ${m.name}`);
		}
		mcpNames.add(m.name);
	...
	for (const a of [...bundle.surfaces.systemAppendices, ...bundle.surfaces.agentAppendices]) {
		if (appendixIds.has(a.extensionId)) {
			throw new VibPluginLoadError("duplicate_appendix", `Vibrato plugin appendix collides: ${a.extensionId}`);
		}
	...
	for (const s of bundle.surfaces.subskills) {
		...
		if (subskillArgs.has(argKey)) {
			throw new VibPluginLoadError(
				"duplicate_arg",
				`Vibrato plugin subskill activation_arg collides for ${s.parent}: ${s.activationArg}`,
			);
		}
		if (parentPhases.has(phaseKey)) {
			throw new VibPluginLoadError(
				"duplicate_parent_phase",
				`Vibrato plugin subskill parent/phase collides: ${s.parent}/${s.phase}`,
			);
		}
