import { Container, type SelectItem, SelectList, Spacer, Text } from "@vib-rato/tui";
import {
	...
	type VibLifecycleContext,
} from "../../extensibility/vib-plugins/lifecycle";
import { identityEquals, identityKey } from "../../extensibility/vib-plugins/lifecycle-reconciliation";
import { findingsForBundle, type VibRuntimeSnapshotProvider } from "../../extensibility/vib-plugins/runtime-quarantine";
import type {
	VibBundleIdentity,
	VibBundleSummary,
	VibLifecycleResult,
	...
	VibUpdatePreview,
} from "../../extensibility/vib-plugins/types";
...
export interface VibBundleLifecyclePort {
	listVibBundles(ctx: VibLifecycleContext): Promise<VibBundleSummary[]>;
	getVibBundle(ctx: VibLifecycleContext, identity: VibBundleIdentity): Promise<VibLifecycleResult<VibBundleSummary>>;
	previewVibBundleUpdate(
...
export interface VibBundleSettingsDependencies {
	lifecycle?: VibBundleLifecyclePort;
	runtimeSnapshotProvider?: VibRuntimeSnapshotProvider;
	activationGeneration?: number;
}

export interface VibBundleSettingsCallbacks {
	onClose: () => void;
	onBundlesChanged?: () => void;
	onRenderRequested?: () => void;
}

export type VibBundleSettingsState =
	| "loading"
	| "error"
	| "empty"
	| "list"
...
const PRODUCTION_LIFECYCLE: VibBundleLifecyclePort = {
	listVibBundles,
	getVibBundle,
	previewVibBundleUpdate,
	applyVibBundleUpdate,
...
/** Scope-qualified, lifecycle-backed Settings UI for installed Vibrato bundles. */
export class VibBundleSettingsComponent extends Container {
	#input: SelectList | null = null;
	#bundles: VibBundleSummary[] = [];
	#focused: VibBundleIdentity | null = null;
	#preview: VibUpdatePreview | null = null;
	#state: VibBundleSettingsState = "loading";
	...
	constructor(
		cwd: string,
		private readonly callbacks: VibBundleSettingsCallbacks,
		private readonly dependencies: VibBundleSettingsDependencies = {},
	) {
		super();
		this.#context = { cwd };
		void this.#load();
	...
	readonly #context: VibLifecycleContext;

	get stateId(): VibBundleSettingsState {
		return this.#state;
	}
	...
	get #lifecycle(): VibBundleLifecyclePort {
	...
	async #load(): Promise<void> {
		...
		this.#render();
		try {
			this.#bundles = await this.#lifecycle.listVibBundles(this.#context);
			if (this.#disposed) return;
			this.#state = this.#bundles.length === 0 ? "empty" : "list";
		} catch {
			if (this.#disposed) return;
			this.#state = "error";
			this.#message = "Unable to load Vibrato bundles.";
		}
		this.#render();
	}
	...
	#render(): void {
		...
		this.addChild(new Text(theme.bold(theme.fg("accent", "  Vibrato Bundles")), 0, 0));
		...
		switch (this.#state) {
			...
			case "error":
				this.#renderActionList([
					{ value: "retry", label: "Retry", description: this.#message ?? "Unable to load Vibrato bundles." },
				]);
				break;
			case "empty":
				this.addChild(new Text(theme.fg("muted", "  No Vibrato bundles installed"), 0, 0));
				this.#renderActionList([]);
				break;
			case "list":
				this.#renderList();
	...
	async #reviewUpdate(): Promise<void> {
		...
		this.#message = null;
		try {
			const result = await this.#lifecycle.previewVibBundleUpdate(this.#context, bundle.identity);
	...
	async #applyUpdate(): Promise<void> {
		...
		await this.#mutate(preview.identity, "update-running", async () => {
			const result = await this.#lifecycle.applyVibBundleUpdate(this.#context, preview.token);
	...
	async #toggleBundle(enabled: boolean): Promise<void> {
		...
		await this.#mutate(bundle.identity, "mutation-in-flight-locked", async () => {
			const result = await this.#lifecycle.setVibBundleEnabled(this.#context, bundle.identity, enabled);
			if (this.#disposed || !this.#isFocused(bundle.identity)) return;
	...
	async #toggleSurface(surfaceId: string, enabled: boolean): Promise<void> {
		...
		await this.#mutate(bundle.identity, "mutation-in-flight-locked", async () => {
			const result = await this.#lifecycle.setVibBundleSurfaceEnabled(
				this.#context,
				bundle.identity,
				surfaceId,
				enabled,
	...
	#applyToggleResult(result: VibLifecycleResult<VibToggleResult>): void {
		if (!result.ok) {
	...
	#replaceSummary(summary: VibBundleSummary): void {
