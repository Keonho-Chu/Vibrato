import {
	...
	TERMINAL,
	type TUI,
	type VibratoGifFrame,
	type VibratoGifTimeline,
	type VibratoPixelFrames,
	visibleWidth,
 ...
 */
const petOverlayEmitterOwners = new WeakMap<TUI, VibratoPetWidget>();
...
function gifTimeline(steps: ReadonlyArray<readonly [PetFrameName, number]>): VibratoGifFrame[] {
	return steps.map(([name, delayMs]) => ({ name, delayMs }));
}

function burstGifTimeline(burst: PetBurst): VibratoGifTimeline {
...
function timelineSignature(frames: VibratoGifTimeline): string {
...
/** The vibrato pet pixel sprite and protocol-specific rendering lifecycle. */
export class VibratoPetWidget {
	#ui: TUI;
	#editor: CustomEditor;
	...
	#mode: PetMode = "off";
	#pixel: VibratoPixelFrames | undefined;
	...
	#itermProtocol = false;
	#itermLastSemantic = "";
	#itermOwner = `vibrato-pet-${Math.random().toString(36).slice(2)}`;
	#itermGeneration = 0;
	...
	#applyMode(mode: PetMode, mountComposer: boolean): void {
		...
		if (mode === this.#mode) {
			if (mode === "off") return;
			const nextProtocol = this.#forcedProtocol ?? VibratoPetWidget.pixelProtocol();
		...
		const protocol = this.#forcedProtocol ?? VibratoPetWidget.pixelProtocol();
	...
	#buildPixel(protocol: "sixel" | "kitty" | "iterm" | "text"): void {
		...
		this.#itermProtocol = false;
		this.#pixel = buildVibratoPixelFrames({
			protocol,
			skin,
	...
	#itermTimeline(now: number, working: boolean, flexing: boolean): VibratoGifTimeline {
	...
	async #submitIterm(
		...
		this.#itermLastSemantic = semantic;
		const cell = getCellDimensions();
		const gif = getVibratoPetGifCached({
