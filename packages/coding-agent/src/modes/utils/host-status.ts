 ...
 *
 *     ESC ] 777 ; notify;Terax;vib;<event> BEL
 *
 ...
 * any host that wants agent status can parse the stream without vib knowing it
 * exists.
 *
 * `notify;Terax;` is the wire identifier of the only protocol that currently
 * carries per-turn agent state, not a host check - the payload names vib as the
 * reporting agent.
 *
...
function marker(event: HostStatusEvent): string {
	return `\x1b]777;notify;Terax;vib;${event}\x07`;
}
