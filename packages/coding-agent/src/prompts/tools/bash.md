...
- `{{this}}`
{{/each}}
Use it only for sanctioned Vibrato workflow CLI persistence or state read/write/contract operations; the only per-command env override allowed is `VIB_RALPLAN_ARTIFACT` when paired with `vib ralplan --write … --artifact-env VIB_RALPLAN_ARTIFACT`, and all other shell command shapes are blocked before execution.
{{/when}}
</restricted-bash-mode>
{{/if}}
