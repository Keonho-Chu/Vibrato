...
import { getWorktreeDir, hashPath, logger, Snowflake } from "@vib-rato/utils";
import * as git from "../utils/git";

let nativeWorktreeBindings: typeof import("@vib-rato/natives") | undefined;

function nativeWorktree(): typeof import("@vib-rato/natives") {
	if (!nativeWorktreeBindings)
		nativeWorktreeBindings = require("@vib-rato/natives") as typeof import("@vib-rato/natives");
...
async function writeSyntheticTree(repoDir: string, baseTreeish: string, patches: readonly string[]): Promise<string> {
	const tempIndex = path.join(os.tmpdir(), `vib-task-index-${Snowflake.next()}`);
	try {
...
export async function commitToBranch(
	...
	const branchName = `vib/task/${taskId}`;
	...
	if (rootPatch.trim()) {
		...
		const tmpDir = path.join(os.tmpdir(), `vib-branch-${Snowflake.next()}`);
		try {
...
export async function mergeTaskBranches(
	...
	const didStash = await git.stash.push(repoRoot, "vib-task-merge");
