...
import { isEnoent, peekFile } from "@vib-rato/utils";
...
type SettingsReader = Pick<SettingsType, "get">;

function resolveGuardSettings(settings?: SettingsReader): SettingsReader {
...
export async function assertEditableFile(
	...
	const activeSettings = resolveGuardSettings(settings);
	if (!activeSettings.get("edit.blockAutoGenerated")) {
		return;
	}
 ...
 */
export function assertEditableFileContent(content: string, displayPath: string, settings?: SettingsReader): void {
	const activeSettings = resolveGuardSettings(settings);
	if (!activeSettings.get("edit.blockAutoGenerated")) {
		return;
	}
