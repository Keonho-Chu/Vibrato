/**
 * Native implementation of `vib autoresearch` — the session-scoped mission
 * runtime for the autoresearch workflow skill.
 *
 * Everything the runtime persists lives under
 * `<cwd>/.vib/_session-{vibSessionId}/autoresearch/` (`sessionAutoresearchDir`):
 ...
 * through the sanctioned `.vib/**` state-writer primitives, which structurally
 * refuse targets outside the project `.vib/` tree.
 *
...
export interface AutoresearchReadReceipt {
	...
	/**
	 * Vibrato session id the mission state was read from. Callers deriving
...
function renderAutoresearchHelp(): string {
	return [
		"Run native Vibrato Autoresearch workflow commands",
		"",
		"USAGE",
		"  $ vib autoresearch [--spec <path>] [--json] [goal...]",
		"  $ vib autoresearch intake --spec <path> [--json]",
		"  $ vib autoresearch read [--json]",
		"  $ vib autoresearch clear [--json]",
		"  $ vib autoresearch write --goal <goal> --mode <web|mixed|data> --slug <slug> [--deliverable <text>] [--constraint <text>] [--json]",
		"  $ vib autoresearch log-run --run-id <id> --status <status> --description <text> [--metric <number>] [--json]",
		"  $ vib autoresearch verdict --status-json <object> --evidence <text> --evaluator <id> [--caveat <text>] [--json]",
		"  $ vib autoresearch critic --status-json <object> --evidence <text> --evaluator <id> [--caveat <text>] [--json]",
		"",
		"INTAKE",
		...
		"",
		"STATE",
		"  Mission/ledger/verdict state persists under .vib/_session-{sessionid}/autoresearch/.",
		"  The global autoresearch store is not written.",
		"",
		"EXAMPLES",
		"  $ vib autoresearch --spec .vib/_session-abc/specs/deep-interview-my-mission.md --json",
		'  $ vib autoresearch "Optimize the tokenizer throughput"',
		"  $ vib autoresearch",
		"",
	].join("\n");
}
...
function assertOnlyAutoresearchFlags(args: readonly string[], allowed: readonly string[]): void {
	for (let index = 0; index < args.length; index += 1) {
		...
		if (!allowed.includes(arg)) throw new AutoresearchCommandError(2, `unknown flag for vib autoresearch: ${arg}`);
		if (arg !== "--json") index += 1;
	}
}
...
function extractPositionalGoal(args: readonly string[]): string {
	...
	for (const arg of args) {
		...
		if (arg.startsWith("-")) {
			throw new AutoresearchCommandError(2, `unknown flag for vib autoresearch: ${arg}`);
		}
		parts.push(arg);
	}
...
async function reconcileAutoresearchState(
	...
	try {
		...
		await syncSkillActiveState({
			...
			phase: options.phase ?? "research",
			sessionId: resolvedSessionId,
			source: "vib-autoresearch-native",
			hud: hud ?? {
				version: 1,
...
export async function runNativeAutoresearchCommand(
	...
	try {
		...
		if (verb === "handoff") {
			throw new AutoresearchCommandError(
				2,
				'unknown verb "handoff" — for spec intake use `vib autoresearch intake --spec <path>`; for workflow handoff invoke the next skill directly (/skill:<callee>) or run `vib state autoresearch handoff --to <callee>`',
			);
		}
		if (verb === "intake") {
