 ...
 * compute state paths, merge records, or write `.vib` files directly. All writes
...
async function persistEnvelope(
	...
	const writeResult = await writeGuardedWorkflowEnvelopeAtomic(statePath, payload, {
		...
		policy: "source",
		expectedRevision,
		receipt: { cwd, skill: "deep-interview", owner: "vib-runtime", command, sessionId, nowIso: now },
		audit: { category: "state", verb: "write", owner: "vib-runtime", skill: "deep-interview", sessionId },
	});
...
async function syncRecorderHud(
	...
	await syncSkillActiveState({
		...
		phase,
		sessionId,
		source: "vib-runtime-deep-interview-recorder",
...
export async function appendOrMergeDeepInterviewRound(
	...
	await persistEnvelope(cwd, statePath, envelope, options.sessionId, "vib deep-interview record-answer");
	try {
...
export async function enrichDeepInterviewRoundScoring(
	...
	inner.ambiguity_floor = breakdown;
	await persistEnvelope(cwd, statePath, envelope, options.sessionId, "vib deep-interview score-round");
	await syncRecorderHud(cwd, envelope, options.sessionId);
	return { record };
}
