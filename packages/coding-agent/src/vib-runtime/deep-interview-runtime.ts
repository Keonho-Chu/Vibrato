...
/**
 * Native implementation of `vib deep-interview`.
 *
 ...
 * (`[--trace] [--quick|--standard|--deep] <idea>`), seeds `.vib/state/deep-interview-state.json`, and
...
export function deepInterviewStatePath(cwd: string, sessionId?: string): string {
	const resolvedSessionId = sessionId?.trim() || process.env.VIB_SESSION_ID?.trim();
 ...
 * resolver: project `.vib/config.yml` > project `.vib/settings.json` > user
...
async function resolveConfiguredAmbiguityThreshold(
	...
	const resolution = await resolveWorkflowSetting(cwd, "vib.deepInterview.ambiguityThreshold", {
		defaultValue: DEFAULT_AMBIGUITY_THRESHOLD,
		parse: value => {
			if (typeof value !== "number" || !Number.isFinite(value) || value <= 0 || value > 1) {
				return {
					kind: "invalid",
					reason: "expected vib.deepInterview.ambiguityThreshold to be a number in (0, 1]",
				};
			}
...
async function resolveSpecWriteArgs(args: readonly string[], cwd: string): Promise<ResolvedDeepInterviewSpecWriteArgs> {
	...
	for (const arg of args) {
		...
		if (arg.startsWith("-") && !allowedFlags.has(arg)) {
			throw new DeepInterviewCommandError(2, `unknown flag for vib deep-interview --write: ${arg}`);
		}
	}
...
async function resolveDeepInterviewArgs(
	...
	for (const arg of args) {
		...
		if (arg.startsWith("-")) {
			throw new DeepInterviewCommandError(2, `unknown flag for vib deep-interview: ${arg}`);
		}
		ideaParts.push(arg);
	}
...
export async function persistDeepInterviewSpec(
	...
	await writeWorkflowEnvelopeAtomic(statePath, payload, {
		...
		receipt: {
			...
			skill: "deep-interview",
			owner: "vib-runtime",
			command: "vib deep-interview persist-spec-state",
			sessionId: resolved.sessionId,
			nowIso: createdAt,
		},
		audit: {
...
async function seedDeepInterviewState(cwd: string, resolved: ResolvedDeepInterviewArgs): Promise<string> {
	...
	await writeWorkflowEnvelopeAtomic(statePath, payload, {
		...
		receipt: {
			...
			skill: "deep-interview",
			owner: "vib-runtime",
			command: "vib deep-interview seed",
			sessionId: resolved.sessionId,
			nowIso: now,
		},
		audit: {
...
export async function runNativeDeepInterviewCommand(
	...
	try {
		...
		if (!resolved.idea) {
			throw new DeepInterviewCommandError(
				2,
				'vib deep-interview requires an idea, e.g. `vib deep-interview "<idea>"`.',
			);
		}
