...
import { resolveVibSessionForWrite, SessionResolutionError, writeSessionActivityMarker } from "./session-resolution";
...
/**
 * Staged JSON transitions for deep-interview state (`vib deep-interview stage|check|apply|discard`).
 *
 ...
 *   so no `--draft-id` is needed; the session resolves from `VIB_SESSION_ID` (or
 ...
 * (`--write`, `vib state handoff/clear`), never through a staged patch.
 */
...
async function readCurrentState(cwd: string, sessionId: string): Promise<CurrentState> {
	...
	if (read.kind === "corrupt") {
		throw new DeepInterviewStageError(
			...
			'repair or clear it with `vib state clear --force --mode deep-interview`, then re-seed with `vib deep-interview "<idea>"`',
		);
	}
	if (read.kind === "absent")
...
function computeMergedEnvelope(
	...
	try {
		...
		throw new DeepInterviewStageError(
			...
			"fix the payload and re-stage (`vib deep-interview discard` then `stage`)",
		);
	}
 ...
 * payload `session_id` (stage only), or `VIB_SESSION_ID`. Mutating verbs never
		...
		async () => {
			...
			if (existingDraft.kind === "valid") {
				throw new DeepInterviewStageError(
					...
					"apply it (`vib deep-interview apply`) or discard it (`vib deep-interview discard`) before staging again",
				);
			}
			...
			if (!current.exists && transition !== "initialize-context") {
				throw new DeepInterviewStageError(
					...
					'seed the interview first with `vib deep-interview "<idea>"` or stage --for initialize-context',
				);
			}
...
function requireDraftRead(read: DraftReadResult, sessionId: string): DeepInterviewStageDraft {
	if (read.kind === "absent") {
		throw new DeepInterviewStageError(
			...
			"stage one first: `vib deep-interview stage --for <transition> --input '<json>'`",
		);
	}
	if (read.kind === "corrupt") {
		throw new DeepInterviewStageError(
			...
			"discard it (`vib deep-interview discard`) and re-stage",
		);
	}
	return read.draft;
}
		...
		async () => {
			...
			if (draftIsStale(draft, current)) {
				...
				throw new DeepInterviewStageError(
					...
					"re-stage against current state: `vib deep-interview stage --for <transition> --input '<json>'`",
				);
			}
			const nowIso = new Date().toISOString();
			...
			try {
				const written = await writeGuardedWorkflowEnvelopeAtomic(statePath, merged, {
					...
					receipt: {
						...
						skill: "deep-interview",
						owner: "vib-runtime",
						command: `vib deep-interview apply (${draft.transition})`,
						sessionId,
						nowIso,
		...
		async () => {
			...
			if (pendingDraft.kind === "valid") {
				throw new DeepInterviewStageError(
					...
					"apply it (`vib deep-interview apply`) or discard it (`vib deep-interview discard`) first",
				);
			}
			const current = await readCurrentState(cwd, sessionId);
			const nowIso = new Date().toISOString();
			...
			const written = await writeGuardedWorkflowEnvelopeAtomic(statePath, merged, {
				...
				receipt: {
					...
					skill: "deep-interview",
					owner: "vib-runtime",
					command: `vib deep-interview write${reset ? " --reset" : ""}`,
					sessionId,
					nowIso,
...
async function syncStageHud(cwd: string, sessionId: string, envelope: Record<string, unknown>): Promise<void> {
	try {
		...
		await syncSkillActiveState({
			...
			phase,
			sessionId,
			source: "vib-deep-interview-native",
...
/** Thin lifecycle passthroughs: same runtime plumbing, vib deep-interview surface. */
			...
			error instanceof DeepInterviewStageError
				...
				: error instanceof SessionResolutionError
					? new DeepInterviewStageError(
							"DI_STAGE_SESSION_REQUIRED",
							error.message,
							"pass --session-id, set VIB_SESSION_ID, or include session_id in the staged payload, then retry",
						)
					: undefined;
		if (staged) {
			const body = json
