/**
 * Official GC of already-quarantined empty `.vib-delete-*` receipts.
...
import { exactUnlinkDirect, type NativeExactUnlinkResult } from "@vib-rato/natives";

export const EMPTY_DELETE_PREFIX = ".vib-delete-";
const EMPTY_DELETE_RECEIPT_PATTERN =
	/^\.vib-delete-session-state-lock-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.json$/u;
				...
				try {
					if (!record.identity) {
						...
						const result = unlink(record.path, {
							...record.identity,
							quarantineName: `.vib-delete-gc-${randomUUID()}.json`,
						});
