/**
 * Text rendering for `vib gc` reports. JSON output is produced directly in
...
export function buildGcReportText(report: GcReport): string {
	...
	if (report.operation === "repair_session_index") {
		lines.push("vib gc — session-index repair (other stores are report-only)");
	} else {
		lines.push(report.dry_run ? "vib gc — dry run (no changes made; pass --prune to remove)" : "vib gc — prune");
	}
	lines.push("");
	...
	if (report.session_index) {
		...
		if (index.status === "corrupt")
			lines.push("  Run `vib gc --repair-session-index` to quarantine the corrupt suffix.");
		if (index.status === "unsupported")
			lines.push("  Upgrade Vibrato before attempting a repair; no index data was changed.");
	...
	if (report.empty_delete_receipts) {
		...
		lines.push(`Empty .vib-delete receipts (${empty.records.length})`);
