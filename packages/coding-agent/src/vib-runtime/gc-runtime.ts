/**
 * `vib gc` runtime — a global, liveness-only, dry-run-by-default garbage
 * collector for stale Vibrato session/PID records.
 *
 * Design (see .vib/plans/ralplan/2026-06-13-1347-954f/pending-approval.md):
...
import { exactRemoveDirectoryTree, exactUnlink, snapshotDirectoryTree } from "@vib-rato/natives";
import { getAgentDir, getBlobsDir, getSessionsDir, isEnoent, VERSION } from "@vib-rato/utils";
...
function resolveGcAgentDir(env: NodeJS.ProcessEnv): string {
	return env.VIB_CODING_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim() || getAgentDir();
}
...
async function collectGcSessionScope(cwd: string, agentDir: string): Promise<GcSessionScopeUsage | undefined> {
	try {
		...
		const { getSessionsDir } = await import("@vib-rato/utils");
...
export async function runVibGcCommand(
	argv: string[],
	...
	try {
		...
		return { stdout: "", stderr: `vib gc: ${message}\n`, status: 2 };
	}
	...
	if (parsed.emptyDeleteReceipts) {
		...
		if (emptyDeleteRoots.some(root => root.includes("\0"))) {
			return { stdout: "", stderr: "vib gc: root_invalid\n", status: 2 };
		}
...
export function gcHelpText(): string {
	return [
		"vib gc - garbage-collect stale Vibrato session/PID records",
		"",
		"USAGE",
		"  $ vib gc [--prune|--force] [--disk] [--repair-session-index] [--empty-delete-receipts --root <dir>] [--json]",
		...
		"  --empty-delete-receipts  Report/prune empty .vib-delete-* under --root / --manifest",
		...
		"family. Managed worktrees under ~/.vib/wt are never touched.",
		"",
	].join("\n");
}
...
// =============================================================================
// Disk-retention axis (`vib gc --disk`)
// =============================================================================
//
...
//
// Deliberately out of scope: `~/.vib/wt` managed worktrees. Removing a worktree
...
/** Bound every recursive size walk so a pathological tree cannot stall `vib gc`. */
...
async function collectGcSessionReferences(agentDir: string, env: NodeJS.ProcessEnv): Promise<GcSessionReferences> {
	...
	const localRoots = path.join(env.TMPDIR?.trim() || os.tmpdir(), "vib-local");
	try {
 ...
 * or appended to while `vib gc` was measuring the store can reference a blob the
...
/**
 * Which family of Vibrato-written file a name belongs to, derived from the name's
...
/**
 * The per-session artifact directories Vibrato writes its own tool output into:
		...
		await sessionIndex.withLocked(async () => {
			...
			for (const transcript of survivors) {
				...
				for (const entry of entries) {
					...
					// how a scope cleanup reaches bytes that are not Vibrato's to reclaim.
...
async function runGcDiskBackups(input: {
	...
	// `~/.vib/*.bak` — sibling roots left by update/restore (agent.bak, natives-*.bak).
	try {
		for (const entry of await fsp.readdir(vibRoot, { withFileTypes: true })) {
			if (!entry.name.endsWith(".bak") || entry.isSymbolicLink()) continue;
			candidates.push({ id: entry.name, path: path.join(vibRoot, entry.name) });
		}
	} catch (error) {
		if (!isEnoent(error)) errors.push({ surface: "backups", scope: vibRoot, message: gcDiskErrorText(error) });
	}
...
export async function collectGcDiskReport(input: {
	...
	const vibRoot = path.dirname(path.resolve(agentDir));
	...
	const surfaces: Record<GcDiskSurface, GcDiskSurfaceReport> = {
		...
		natives: emptyGcDiskSurface("natives", path.join(vibRoot, "natives")),
		backups: emptyGcDiskSurface("backups", path.join(vibRoot, "backups")),
	};
	...
	await runGcDiskBackups({ surface: surfaces.backups, vibRoot, policy, now, prune, errors });
...
export function buildGcDiskReportText(disk: GcDiskReport): string {
	...
	lines.push(
		disk.dry_run
			? "vib gc --disk — report only (no bytes reclaimed; pass --prune to reclaim)"
			: "vib gc --disk --prune — reclaim",
	);
	lines.push(
