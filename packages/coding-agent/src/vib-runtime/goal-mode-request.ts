...
import { resolveVibSessionForRead, resolveVibSessionForWrite, writeSessionActivityMarker } from "./session-resolution";
import { removeFileAudited, writeJsonAtomic } from "./state-writer";

export const VIB_SESSION_FILE_ENV = "VIB_SESSION_FILE";
export const VIB_SESSION_ID_ENV = "VIB_SESSION_ID";
export const VIB_SESSION_CWD_ENV = "VIB_SESSION_CWD";

const REQUEST_VERSION = 1;
export const DEFAULT_ULTRAGOAL_OBJECTIVE =
	"Complete the durable ultragoal plan in .vib/ultragoal/goals.json, including later accepted/appended stories, under the original brief constraints; use .vib/ultragoal/ledger.jsonl as the audit trail.";
...
export interface PendingGoalModeRequest {
	...
	/**
	 * Session id that produced this request (from VIB_SESSION_ID). When present,
	 * only the originating session may consume it, so concurrent sessions sharing
	 * the same `.vib` project state never auto-run each other's ultragoal.
	 */
	sessionId?: string;
}
...
export async function readUltragoalVibObjective(
	cwd: string,
	sessionId?: string | null,
): Promise<{ objective: string; goalsPath: string; provenance: Extract<GoalProvenance, { source: "ultragoal" }> }> {
	const session = sessionId?.trim()
		? { vibSessionId: sessionId.trim() }
		: await resolveVibSessionForRead(cwd, { envSessionId: process.env.VIB_SESSION_ID });
	const goalsPath = ultragoalGoalsPath(cwd, session.vibSessionId);
	try {
		const plan = (await Bun.file(goalsPath).json()) as UltragoalPlanShape;
		const objective = typeof plan.vibObjective === "string" ? plan.vibObjective.trim() : "";
		const goalId = typeof plan.goals?.[0]?.id === "string" ? plan.goals[0].id : "aggregate";
		return {
			objective: objective || DEFAULT_ULTRAGOAL_OBJECTIVE,
			goalsPath,
			provenance: { source: "ultragoal", runId: session.vibSessionId, goalId },
		};
	} catch (error) {
		if (isEnoent(error)) {
			return {
				...
				goalsPath,
				provenance: { source: "ultragoal", runId: session.vibSessionId, goalId: "aggregate" },
			};
		}
...
export async function writePendingGoalModeRequest(input: {
	...
	const resolvedSessionId =
		...
		resolveVibSessionForWrite(input.cwd, { envSessionId: process.env.VIB_SESSION_ID }).vibSessionId;
	...
	await writeJsonAtomic(filePath, request, {
		cwd: input.cwd,
		audit: { category: "state", verb: "write", owner: "vib-runtime", sessionId },
	});
...
export async function writeCurrentSessionGoalModeState(input: {
	...
	// The session transcript file lives outside `.vib/` (VIB_SESSION_FILE), so it is not a
...
export async function consumePendingGoalModeRequest(
	...
	const session = currentSessionId?.trim()
		? { vibSessionId: currentSessionId.trim() }
		: await resolveVibSessionForRead(cwd, { envSessionId: process.env.VIB_SESSION_ID });
	const filePath = requestPath(cwd, session.vibSessionId);
	let raw: unknown;
	try {
	...
	if (ownerSessionId && ownerSessionId !== session.vibSessionId) {
		return null;
	}
	await removeFileAudited(filePath, {
		cwd,
		audit: { category: "prune", verb: "remove", owner: "vib-runtime", sessionId: session.vibSessionId },
	}).catch(error => {
...
export function buildVibRuntimeSessionEnv(input: {
	sessionFile?: string | null;
	sessionId?: string | null;
	cwd?: string | null;
}): Record<string, string> {
	const env: Record<string, string> = {};
	if (input.sessionFile) env[VIB_SESSION_FILE_ENV] = input.sessionFile;
	if (input.sessionId) env[VIB_SESSION_ID_ENV] = input.sessionId;
	if (input.cwd) env[VIB_SESSION_CWD_ENV] = input.cwd;
	return env;
}
