...
import { VERSION } from "@vib-rato/utils/dirs";
import { isCompiledBinary } from "@vib-rato/utils/env";
import { safeStderrWrite } from "@vib-rato/utils/safe-stderr";
...
import {
	...
	VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL_ENV,
...
import {
	...
	resolveVibTmuxBinary,
	resolveVibTmuxCommand,
	resolveVibTmuxProviderContext,
	VIB_DEFAULT_TMUX_SESSION,
	VIB_TMUX_ACTIVE_SESSION_ENV,
	VIB_TMUX_COMMAND_ENV,
	VIB_TMUX_MOUSE_ENV,
	VIB_TMUX_PROFILE_ENV,
	VIB_TMUX_SESSION_PREFIX,
	type VibTmuxProfileCommand,
} from "./tmux-common";
import {
...
import { assertVibTmuxStagedMutationAuthoritySync } from "./tmux-provider-context";
import {
	findVibTmuxSessionByName,
	findVibTmuxSessionByScope,
	type ProvenTmuxSessionIdentity,
	proveVibTmuxSessionMutationTarget,
	type VibTmuxSessionStatus,
} from "./tmux-sessions";
import {
...
export const VIB_LAUNCH_POLICY_ENV = "VIB_LAUNCH_POLICY";
export const VIB_TMUX_WINDOW_LABEL_MAX_WIDTH = 48;
...
function explicitTmuxSessionName(env: NodeJS.ProcessEnv): string | undefined {
	return env.VIB_TMUX_SESSION?.trim() || undefined;
}
function hasCurrentVibVersion(session: VibTmuxSessionStatus | undefined): boolean {
	return session?.version === VERSION;
}

function allowsExistingTmuxAttach(parsed: Args, env: NodeJS.ProcessEnv): boolean {
	// `--resume` belongs to the inner Vibrato session resolver. Let it reach main.ts so
...
function findExistingSessionForLaunch(context: {
	...
	if (explicit) return findVibTmuxSessionByName(explicit, context.env)?.name;
	const scoped = findVibTmuxSessionByScope(context.project, context.branch, context.env);
	return hasCurrentVibVersion(scoped) ? scoped?.name : undefined;
}

export interface VibTmuxProfileResult {
	skipped: boolean;
	commands: VibTmuxProfileCommand[];
	failures: Array<{ command: VibTmuxProfileCommand; stderr?: string }>;
}

export interface VibTmuxProfileContext {
	tmuxCommand: string;
	target: string;
	cwd?: string;
	env?: NodeJS.ProcessEnv;
...
function buildPosixTmuxExitMarkerPrefix(markerPath: string): string {
	...
	return [
		`__vib_tmux_exit_marker=${shellQuote(markerPath)}`,
		"__vib_tmux_write_exit_marker() { __vib_tmux_status=$?",
		"__vib_tmux_ended_at=$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date)",
		`mkdir -p ${shellQuote(markerDir)} 2>/dev/null || true`,
		'printf \'{"schema_version":1,"source":"tmux_inner_shell","ended_at":"%s","exit_code":%s}\\n\' "$__vib_tmux_ended_at" "$__vib_tmux_status" > "$__vib_tmux_exit_marker" 2>/dev/null || true',
		"}",
		"trap __vib_tmux_write_exit_marker EXIT",
	].join("; ");
}
...
function parseLaunchPolicy(env: NodeJS.ProcessEnv): LaunchPolicy {
	const raw = env[VIB_LAUNCH_POLICY_ENV]?.trim().toLowerCase();
	if (raw === "direct" || raw === "tmux") return raw;
	if (env.VIB_NO_TMUX === "1" || env.VIB_NO_TMUX === "true") return "direct";
	return "tmux";
}
...
function formatTmuxLaunchDiagnostic(stage: string, stderr?: string): string {
	...
	return `vib --tmux failed after creating tmux session: ${stage}.${suffix}\n`;
}
...
/**
 * Detect a corrupted vib.cmd / vib.bat wrapper at well-known PATH locations.
 * On Windows, `vib.cmd` / `vib.bat` files at the front of PATH that turn out
 ...
 * read.
 */
function detectCorruptedVibWrapper(): string | null {
	if (process.platform !== "win32") return null;
	const pathEnv = process.env.PATH ?? "";
	if (!pathEnv) return null;
	const seen = new Set<string>();
	for (const dir of pathEnv.split(path.delimiter)) {
		for (const name of ["vib.cmd", "vib.bat"]) {
			...
			try {
				...
				if (stat.size > 64 * 1024) {
					return `Detected suspicious vib wrapper at ${full}: ${stat.size} bytes (expected <1KB). The wrapper may be corrupted; cmd.exe will hang reading it as text. Recreate it from the vib-tmux.cmd template.`;
				}
				const head = fs.readFileSync(full);
				if (head.byteLength < 2) continue;
				const view = new Uint8Array(head);
				if (view[0] === 0x4d && view[1] === 0x5a) {
					return `Detected PE-binary vib wrapper at ${full} (MZ header, ${stat.size} bytes). cmd.exe will hang reading it as text. Recreate the wrapper from the vib-tmux.cmd template.`;
				}
			} catch {}
		}
	}
...
function formatTmuxUnavailableDiagnostic(platform: NodeJS.Platform): string {
	if (platform === "win32") {
		return (
			...
			"Vibrato searched for psmux, pmux, and tmux on PATH. " +
			"Install psmux (https://github.com/psmux/psmux) for native Windows tmux support, or use WSL with real tmux. " +
			"You can also point Vibrato at a specific binary via VIB_TMUX_COMMAND.\n"
		);
	}
	return "vib --tmux requested but no tmux executable was found; cannot continue without a tmux-backed session.\n";
}
...
function coordinatorSidecarSigningEnv(env: NodeJS.ProcessEnv): Record<string, string> {
	const marker = env[VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED_ENV]?.trim();
	const bootstrap = coordinatorSidecarSigningBootstrapEnv();
	const key =
		env[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV]?.trim() ?? bootstrap[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV];
	const keyId = env[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]?.trim() ?? bootstrap[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV];
	...
	return {
		...
		[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV]: key,
...
function coordinatorSidecarControlEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
	...
	delete controlEnv[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV];
	return controlEnv;
}
...
export function applyVibTmuxProfile(context: VibTmuxProfileContext): VibTmuxProfileResult {
	const env = context.env ?? process.env;
	const branchSlug = context.branch ? buildVibTmuxSessionSlug(context.branch) : (context.branchSlug ?? null);
	// The psmux UX filter (mouse / set-clipboard / mode-style /
	// set-window-option) now lives in buildVibTmuxProfileCommands so every
	// caller — vib --tmux planning, vib session create —
	...
	const commands = buildVibTmuxProfileCommands(
		...
		env,
		{
			...
			sessionId: context.sessionId ?? env[VIB_COORDINATOR_SESSION_ID_ENV] ?? null,
			sessionStateFile: context.sessionStateFile ?? env[VIB_COORDINATOR_SESSION_STATE_FILE_ENV] ?? null,
	...
	if (context.psmuxIncarnation)
		commands.push({
			...
			args: ["set-option", "-t", context.target, VIB_TMUX_PSMUX_INCARNATION_OPTION, context.psmuxIncarnation],
		});
	...
	const failures: VibTmuxProfileResult["failures"] = [];
...
function resolveCurrentVibCommand(context: CommandResolutionContext): string[] {
	...
	const isVibExecutable = (value: string | undefined): value is string =>
		isRealAbsolutePath(value) && /^vib(?:[._-]|$)/i.test(pathModule.basename(value.trim()));
	...
	if (entrypoint && !isBunVirtualPath(entrypoint) && /\.(?:[cm]?[jt]s)$/i.test(entrypoint)) {
		...
		if (!executable)
			throw new Error(
				"Unable to determine the current Vibrato source runtime for tmux launch; invoke Vibrato through an absolute runtime path.",
			);
	...
	const executable =
		(isVibExecutable(entrypoint) ? entrypoint : undefined) ??
		(isVibExecutable(runtime) ? runtime : undefined) ??
		(isVibExecutable(context.execPath) ? context.execPath.trim() : undefined);
	if (executable) return [executable];
	throw new Error(
		"Unable to determine the current Vibrato executable for tmux launch; Bun virtual paths and PATH fallback are not accepted.",
	);
}
...
function buildInnerCommand(context: CommandResolutionContext, rawArgs: string[]): string {
	if (isWindowsPlatform(context.platform))
		return buildWindowsPowerShellInnerCommand({
			command: resolveCurrentVibCommand(context),
	...
	const invocation = `env ${VIB_TMUX_LAUNCHED_ENV}=1${buildEnvAssignments({ ...context.extraEnv, ...supervisorEnv })}${
...
function stageCoordinatorSidecarBootstrap(
	...
	const key = signingEnv[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV];
	const keyId = signingEnv[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV];
	if (!key || !keyId) return undefined;
	let bootstrapDir = stateDir;
	try {
		fs.mkdirSync(bootstrapDir, { recursive: true, mode: 0o700 });
	} catch {
		bootstrapDir = path.join(os.tmpdir(), "vib-coordinator-bootstrap");
		fs.mkdirSync(bootstrapDir, { recursive: true, mode: 0o700 });
	}
...
function writeCoordinatorSidecarBootstrapFile(plan: TmuxLaunchPlan, env: NodeJS.ProcessEnv): void {
	...
	if (!bootstrap) return;
	const key = coordinatorSidecarSigningEnv(env)[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV];
	...
	const worker = isCompiledBinary()
		? new Worker("./packages/coding-agent/src/vib-runtime/coordinator-sidecar-bootstrap-worker.ts", {
				type: "module",
			})
...
const VIB_TMUX_WINDOW_BRANCH_SEPARATOR = "-";
const VIB_TMUX_WINDOW_TITLE_PREFIX = "Vibrato-";
const VIB_TMUX_TERMINAL_TITLE_PREFIX = "Vibrato: ";
const VIB_TMUX_ROOT_TERMINAL_TITLE_OPTION = "@vib-root-terminal-title";
const VIB_TMUX_ROOT_TERMINAL_TITLE_SESSION_OPTION = "@vib-root-terminal-title-session";
const VIB_TMUX_DYNAMIC_SESSION_TITLE = "Vibrato: #{session_name}";
...
function sanitizeTmuxWindowProjectName(project: string): string {
	...
	if (!trimmed || /^\.+$/.test(trimmed)) return "vib";
...
function buildVibTmuxPrefixedTitle(prefix: string, cwd: string, branch: string | null | undefined): string {
	const project = sanitizeTmuxWindowProjectName(path.basename(path.resolve(cwd)) || "vib");
	const projectTitle = `${prefix}${project}`;
	const trimmedBranch = sanitizeTmuxWindowTitleSegment(branch?.trim() ?? "");
	if (!trimmedBranch) return truncateVisible(projectTitle, VIB_TMUX_WINDOW_LABEL_MAX_WIDTH);

	const separatorWidth = visibleWidth(VIB_TMUX_WINDOW_BRANCH_SEPARATOR);
	const projectWidth = visibleWidth(projectTitle);
	const fullTitle = `${projectTitle}${VIB_TMUX_WINDOW_BRANCH_SEPARATOR}${trimmedBranch}`;
	if (visibleWidth(fullTitle) <= VIB_TMUX_WINDOW_LABEL_MAX_WIDTH) return fullTitle;

	const remainingBranchWidth = VIB_TMUX_WINDOW_LABEL_MAX_WIDTH - projectWidth - separatorWidth;
	if (remainingBranchWidth <= 0) return truncateVisible(projectTitle, VIB_TMUX_WINDOW_LABEL_MAX_WIDTH);

	return `${projectTitle}${VIB_TMUX_WINDOW_BRANCH_SEPARATOR}${truncateVisibleTail(trimmedBranch, remainingBranchWidth)}`;
}

export function buildVibTmuxWindowTitle(cwd: string, branch: string | null | undefined): string {
	return buildVibTmuxPrefixedTitle(VIB_TMUX_WINDOW_TITLE_PREFIX, cwd, branch);
}

function buildVibTmuxRootTerminalTitle(cwd: string, branch: string | null | undefined): string {
	return buildVibTmuxPrefixedTitle(VIB_TMUX_TERMINAL_TITLE_PREFIX, cwd, branch);
}

function sanitizeVibTmuxRootTerminalTitle(title: string): string {
	return title.replace(TERMINAL_TITLE_CONTROL_CHARS, "").trim() || "Vibrato";
}

function buildVibTmuxRootTerminalTitleFormat(sessionName: string): string {
	if (!sessionName.startsWith(VIB_TMUX_SESSION_PREFIX)) return VIB_TMUX_DYNAMIC_SESSION_TITLE;
	return `#{?#{==:#{${VIB_TMUX_ROOT_TERMINAL_TITLE_SESSION_OPTION}},#{session_name}},#{${VIB_TMUX_ROOT_TERMINAL_TITLE_OPTION}},${VIB_TMUX_DYNAMIC_SESSION_TITLE}}`;
}

function buildVibTmuxRootTerminalTitleCommands(
	target: string,
	sessionName: string,
	title: string,
): VibTmuxProfileCommand[] {
	const sanitized = sanitizeVibTmuxRootTerminalTitle(title);
	const format = buildVibTmuxRootTerminalTitleFormat(sessionName);
	return [
		{
			description: "remember tmux client terminal title fallback",
			args: ["set-option", "-t", target, VIB_TMUX_ROOT_TERMINAL_TITLE_OPTION, sanitized],
		},
		{
			description: "remember tmux client terminal title session",
			args: ["set-option", "-t", target, VIB_TMUX_ROOT_TERMINAL_TITLE_SESSION_OPTION, sessionName],
		},
...
function applyVibTmuxRootTerminalTitleProfile(context: {
	tmuxCommand: string;
	target: string;
	sessionName: string;
	...
	for (const command of buildVibTmuxRootTerminalTitleCommands(context.target, context.sessionName, context.title)) {
		context.spawnSync(context.tmuxCommand, command.args, context.options);
	}
}

function shouldSetVibTmuxRootTerminalTitle(parsed: Args, env: NodeJS.ProcessEnv): boolean {
	return !parsed.noTitle && !(env.VIB_NO_TITLE || env.PI_NO_TITLE);
}
...
function renameExistingTmuxWindowIfNeeded(context: TmuxLaunchContext): void {
	const env = context.env ?? process.env;
	if (!env.TMUX || env[VIB_TMUX_LAUNCHED_ENV] === "1") return;
	...
	// vib --tmux runs through PowerShell to a psmux backend.
	...
	const tmuxCommand = resolveVibTmuxCommand(env);
	...
	const title = buildVibTmuxWindowTitle(context.project ?? cwd, branch);
	const spawnSync = context.spawnSync ?? defaultSpawnSync;
...
function createdSessionExactTarget(plan: TmuxLaunchPlan, env: NodeJS.ProcessEnv): string {
	return (
		...
		buildVibTmuxExactSessionTarget(plan.sessionName, {
			env,
...
function createdSessionOptionTarget(plan: TmuxLaunchPlan, env: NodeJS.ProcessEnv): string {
	...
	return buildVibTmuxExactOptionTarget(plan.sessionName, {
		env,
		platform: plan.platform,
		binary: { command: plan.tmuxCommand, isPsmux: plan.isPsmux, viaExplicitOverride: true },
	});
...
function cleanupCreatedTmuxSession(
	...
	if (plan.isPsmux || !isCreatedTmuxSessionIdentityStable(plan, spawnSync, options, probe))
		throw new Error("vib_tmux_exact_cleanup_uncertain");
	...
	// cleanup on those platforms into `vib_tmux_exact_cleanup_uncertain`.
	...
	const guarded = spawnSync(
		...
		[
			...
			`kill-session -t ${nativeSessionId} \\; display-message -p __vib_tmux_guarded_cleanup_ok__`,
			"display-message -p __vib_tmux_guarded_cleanup_refused__",
		],
		options,
	);
	if (guarded.exitCode !== 0 || guarded.stdout?.trim() !== "__vib_tmux_guarded_cleanup_ok__")
		throw new Error("vib_tmux_exact_cleanup_uncertain");
}
...
function readTmuxStatusLineCount(tmuxCommand: string, cwd: string, env: NodeJS.ProcessEnv): number {
	if (resolveVibTmuxBinary({ env }).isPsmux) return 0;
...
export function buildDefaultTmuxLaunchPlan(context: TmuxLaunchContext): TmuxLaunchPlan | undefined {
	...
	if (env.TMUX || env[VIB_TMUX_LAUNCHED_ENV] === "1") return undefined;
	...
	const sessionName = buildVibTmuxSessionName(env, { branch });
	...
	// PATH, so the default `vib --tmux` flow lands on a real multiplexer even
	// without an explicit VIB_TMUX_COMMAND override.
	const resolvedBinary = resolveVibTmuxBinary({ platform, env });
	const tmuxCommand = resolvedBinary.command;
	const sessionId = env[VIB_COORDINATOR_SESSION_ID_ENV]?.trim() || sessionName;
	// The session ROOT is keyed by the active Vibrato session (VIB_SESSION_ID), NOT the
	// coordinator/tmux identity. Fall back to the coordinator id only for standalone
	// tmux launches with no Vibrato session context.
	const vibSessionId = env.VIB_SESSION_ID?.trim() || sessionId;
	const sessionStateFile =
		env[VIB_COORDINATOR_SESSION_STATE_FILE_ENV]?.trim() ||
		tmuxRuntimeSessionPath(cwd, vibSessionId, buildVibTmuxSessionSlug(sessionName));
	...
	delete childSigningEnv[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV];
	const tmuxAvailable = context.tmuxAvailable ?? Bun.which(tmuxCommand) !== null;
	if (!tmuxAvailable) {
	...
	const innerCommand = buildInnerCommand(
		{
			...
			extraEnv: {
				...
				[VIB_COORDINATOR_SESSION_STATE_FILE_ENV]: sessionStateFile,
				// Carry the Vibrato-managed session name into the child so tmux-backed
				...
				[VIB_TMUX_ACTIVE_SESSION_ENV]: sessionName,
			},
...
function prepareManagedOwnerLifecycle(plan: TmuxLaunchPlan, context: TmuxLaunchContext): void {
	...
	const stateDir = path.dirname(plan.sessionStateFile ?? path.join(plan.cwd, ".vib", "runtime"));
	...
	const innerCommand = buildInnerCommand(
		{
			...
			extraEnv: {
				...(plan.coordinatorSidecarBootstrap
					? {
							[VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED_ENV]: "true",
							[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]: plan.coordinatorSidecarBootstrap.keyId,
						}
					: {}),
				[VIB_COORDINATOR_SESSION_ID_ENV]: sessionId,
				[VIB_COORDINATOR_SESSION_STATE_FILE_ENV]: plan.sessionStateFile ?? "",
				...
				[VIB_TMUX_ACTIVE_SESSION_ENV]: plan.sessionName,
				[VIB_TMUX_OWNER_GENERATION_ENV]: generation,
				[VIB_TMUX_OWNER_STATE_DIR_ENV]: stateDir,
				[VIB_TMUX_OWNER_SERVER_KEY_ENV]: plan.tmuxCommand,
				...
				...(replacement
					? {
							...
							[MANAGED_OWNER_TRANSCRIPT_PATH_ENV]:
								context.env?.VIB_SESSION_FILE ?? process.env.VIB_SESSION_FILE ?? "",
						}
					: {}),
			},
			bootstrapSecret: plan.coordinatorSidecarBootstrap
				? {
						...
						urlEnv: VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL_ENV,
						keyIdEnv: VIB_COORDINATOR_SIDECAR_KEY_ID_ENV,
			...
			// in front of it; `buildInnerCommand` therefore execs the Vibrato owner directly.
...
function requiredProfileFailure(profile: VibTmuxProfileResult): VibTmuxProfileResult["failures"][number] | undefined {
	const requiredOptions = new Set([
		"@vib-profile",
		"@vib-session-id",
		"@vib-session-state-file",
		"@vib-owner-generation",
		"@vib-owner-server-key",
...
function emitOptionalProfileDiagnostics(profile: VibTmuxProfileResult, diagnostic: (message: string) => void): void {
...
export function launchDefaultTmuxIfNeeded(context: TmuxLaunchContext): boolean {
	...
	if (!plan && env.TMUX && (context.platform ?? process.platform) === "win32") {
		const ambientProvider = resolveVibTmuxBinary({ platform: "win32", env });
	...
	if (!plan) {
		...
		const platform = context.platform ?? process.platform;
		const tmuxCommand = resolveVibTmuxBinary({ platform, env }).command;
		const tmuxAvailable = context.tmuxAvailable ?? Bun.which(tmuxCommand) !== null;
		if (
	...
	if (plan.isPsmux && plan.platform === "win32") {
		try {
			if (plan.attachSessionName) {
				...
				if (!stateDir || !sessionId || !generation) throw new Error("vib_tmux_provider_authority_unavailable");
				plan.authority = readVibTmuxProviderAuthoritySync({ stateDir, sessionId, generation });
			} else {
				prepareManagedOwnerLifecycle(plan, context);
				if (!plan.sessionId || !plan.ownerGeneration || !plan.sessionStateFile)
					throw new Error("vib_tmux_provider_authority_missing_lifecycle_identity");
				...
				plan.authority = previousAuthority
					? bindVibTmuxProviderAuthority(previousAuthority, {
							stateDir,
							sessionId: plan.sessionId,
			...
			if (!plan.attachSessionName) {
				(context.providerAuthorityPersist ?? persistVibTmuxProviderAuthoritySync)(plan.authority);
				(context.providerAuthorityStagedAssert ?? assertVibTmuxStagedMutationAuthoritySync)(plan.authority);
			}
		} catch (error) {
	...
	const spawnSync: TmuxSpawnSync = (command, args, options) => {
		...
		const assertAuthority = providerAuthorityPublished
			? (context.providerAuthorityAssert ?? assertVibTmuxMutationAuthoritySync)
			: (context.providerAuthorityStagedAssert ?? assertVibTmuxStagedMutationAuthoritySync);
	...
	const spawnEnv = { ...env };
	delete spawnEnv[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV];
	const signingEnv = coordinatorSidecarSigningEnv(env);
	...
	const windowTitle = buildVibTmuxWindowTitle(plan.project ?? plan.cwd, plan.branch);
	const rootTerminalTitle = shouldSetVibTmuxRootTerminalTitle(context.parsed, env)
		? buildVibTmuxRootTerminalTitle(plan.project ?? plan.cwd, plan.branch)
		: undefined;
	const buildProfileInputs = (): VibTmuxProfileContext => ({
	...
	const hasExactRequiredPsmuxMetadata = (): boolean => {
		...
		const required = [
			...buildVibTmuxProfileCommands(createdSessionOptionTarget(plan, env), env, {
				sessionId: plan.sessionId,
	...
	if (plan.attachSessionName) {
		...
		if (plan.platform !== "linux") {
			existingTarget = buildVibTmuxExactSessionTarget(plan.attachSessionName, {
				env,
				platform: plan.platform,
				binary: { command: plan.tmuxCommand, isPsmux: plan.isPsmux, viaExplicitOverride: true },
			});
			...
			try {
				existingProof = proveVibTmuxSessionMutationTarget(plan.attachSessionName, env);
		...
		if (!plan.isPsmux)
			applyVibTmuxRootTerminalTitleProfile({
				tmuxCommand: plan.tmuxCommand,
				target:
					plan.platform === "linux" && existingTarget.startsWith("$")
						? `${existingTarget}:`
						: buildVibTmuxExactOptionTarget(plan.attachSessionName, {
								env,
								platform: plan.platform,
								binary: { command: plan.tmuxCommand, isPsmux: plan.isPsmux, viaExplicitOverride: true },
							}),
		...
		if (plan.platform === "linux") {
			try {
				const proof = proveVibTmuxSessionMutationTarget(plan.attachSessionName, env);
				if (
					!existingProof ||
	...
	if (created.exitCode === 0) {
		...
		const profile = applyVibTmuxProfile(buildProfileInputs());
		// If the @vib-profile ownership write failed, the cause can be
		...
		if (ownershipFailure) {
			...
			const retryProfile = applyVibTmuxProfile(buildProfileInputs());
	...
	const probeWarning = detectCorruptedVibWrapper();
	if (created.exitCode !== 0) {
		...
		// symptom is actually caused by a corrupted vib.cmd / vib.bat on
