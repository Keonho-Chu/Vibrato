...
export interface VibLaunchWorktreeResult extends VibLaunchWorktreePlan {
	created: boolean;
	reused: boolean;
 ...
 * beside the first.
 */
const WORKTREE_BUCKET_ENV = "VIB_WORKTREE_DIR";
/**
...
function ensureRepositoryBucketIgnored(repoRoot: string, bucketPath: string): void {
	...
	const ignoreProbe = path.join(relativeBucket, ".vib-worktree-probe");
	...
	throw new Error(
		[
			...
			"The Vibrato launch worktree bucket is inside the repository but is not ignored by Git.",
...
function formatWorktreeTargetMismatch(plan: VibLaunchWorktreePlan, existing: GitWorktreeEntry): string {
	...
	return [
		...
		`Vibrato launch worktree target is already registered for ${describeWorktreeEntry(existing)}, but this launch expects ${expected}.`,
...
function brokenBucketSymlinkError(bucketPath: string): Error {
	return new Error(
		[
			...
			"The Vibrato launch worktree bucket is a symbolic link whose target cannot be resolved; it may be unmounted or offloaded cold storage.",
			`Path: ${formatBucketPath(bucketPath)}`,
			"Safe remediation: restore or remount the link target, or inspect and remove the dangling link with platform-appropriate filesystem tools, then relaunch. Vibrato did not delete or replace the entry.",
		].join("\n"),
	);
}
...
function bucketNotDirectoryError(bucketPath: string, symlinkTarget = false): Error {
	return new Error(
		[
			...
			symlinkTarget
				? "The Vibrato launch worktree bucket is a symbolic link whose target is not a directory."
				: "The Vibrato launch worktree bucket path exists but is not a directory.",
			`Path: ${formatBucketPath(bucketPath)}`,
			"Safe remediation: inspect the obstructing entry and move or remove it with platform-appropriate filesystem tools, then relaunch. Vibrato did not delete or replace the entry.",
		].join("\n"),
	);
}
	...
	try {
		...
		throw new Error(
			[
				...
				`Vibrato could not inspect the launch worktree bucket${fileSystemErrorCode(error) ? ` (${fileSystemErrorCode(error)})` : ""}.`,
				`Path: ${formatBucketPath(bucketPath)}`,
				"Safe remediation: verify that the bucket parent is accessible, then relaunch. Vibrato did not modify the entry.",
			].join("\n"),
		);
	}
	...
	try {
		...
		throw new Error(
			[
				...
				`Vibrato could not inspect the launch worktree bucket link target${code ? ` (${code})` : ""}.`,
				`Path: ${formatBucketPath(bucketPath)}`,
				"Safe remediation: verify that the link target is accessible, then relaunch. Vibrato did not modify the link.",
			].join("\n"),
		);
	}
	...
	try {
		...
		throw new Error(
			[
				...
				`Vibrato could not create or reuse the launch worktree bucket${code ? ` (${code})` : ""}.`,
				`Path: ${formatBucketPath(bucketPath)}`,
				"Safe remediation: verify parent permissions and bucket accessibility, then relaunch. Vibrato did not delete or replace any entry.",
			].join("\n"),
		);
	}
	if (inspectBucketDir(bucketPath) === "missing") {
		throw new Error(
			[
				...
				"The Vibrato launch worktree bucket disappeared while launch was preparing it.",
				`Path: ${formatBucketPath(bucketPath)}`,
				"Safe remediation: stabilize the bucket mount or parent directory, then relaunch. Vibrato did not delete or replace any entry.",
			].join("\n"),
		);
	}
}
...
export function parseLaunchWorktreeMode(args: string[]): ParsedLaunchWorktreeMode {
	let mode: VibLaunchWorktreeMode = { enabled: false };
...
export function planLaunchWorktree(
	cwd: string,
	mode: VibLaunchWorktreeMode,
): VibLaunchWorktreePlan | { enabled: false } {
...
export function ensureLaunchWorktree(
	plan: VibLaunchWorktreePlan | { enabled: false },
): VibLaunchWorktreeResult | { enabled: false } {
	...
	if (listedAtPath && !existingAtPath) {
		...
		throw new Error(
			[
				...
				"Safe remediation: inspect the worktree lock and remove or repair it with git worktree remove/prune when it is no longer needed, then relaunch. Vibrato did not delete or replace the entry.",
			].join("\n"),
		);
	}
...
export interface PreparedLaunchWorktree {
	cwd: string;
	args: string[];
	worktree: VibLaunchWorktreeResult | { enabled: false };
}
