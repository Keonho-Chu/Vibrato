 ...
 * events (those skills persist through `bash`-backed `vib` CLI calls whose tool
 * `details` carry no structured payload). The durable source of truth is the
 * append-only ledgers:
 *   - ultragoal: `.vib/ultragoal/ledger.jsonl`
 *   - ralplan:   `.vib/plans/ralplan/<run-id>/index.jsonl`
 *
