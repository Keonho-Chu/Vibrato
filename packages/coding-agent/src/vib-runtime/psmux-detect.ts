 ...
 * set-window-option profile values vib emits, which is what unblocks the
 * native Windows vib --tmux path. This module detects that capability so vib
 ...
 *   - VIB_PSMUX_FORCE_DETECT=1 re-probes each call.
 *   - VIB_PSMUX_DETECTION=off skips probing entirely.
 */

export const VIB_PSMUX_COMMAND_ENV = "VIB_PSMUX_COMMAND";
export const VIB_PSMUX_DETECTION_ENV = "VIB_PSMUX_DETECTION";
export const VIB_PSMUX_FORCE_DETECT_ENV = "VIB_PSMUX_FORCE_DETECT";
...
/**
 * VIB_PSMUX_FORCE_DETECT opt-in re-probe switch. Any non-empty value other
...
export function resolveVibTmuxExecutablePath(command: string): string | null {
	return resolveBinaryPath(command);
}

export function resolveVibTmuxExecutableIdentity(executablePath: string): string | null {
...
function classifyWindowsTmuxAlias(command: string, env: NodeJS.ProcessEnv, runner: PsmuxSpawnRunner): boolean {
	...
	try {
		...
		throw new Error("vib_tmux_provider_ambiguous: selected Windows tmux command resolution failed");
	}
	if (normalizedCommandBaseName(command) !== "tmux" && !env[VIB_PSMUX_COMMAND_ENV]?.trim()) return false;
	...
	if (!selectedPath) {
		throw new Error("vib_tmux_provider_ambiguous: selected Windows tmux command could not be resolved");
	}
	const selectedIdentity = activeExecutableIdentityResolver(selectedPath);
	if (!selectedIdentity) {
		throw new Error("vib_tmux_provider_ambiguous: selected Windows tmux executable identity is unavailable");
	}

	const explicitCompanion = env[VIB_PSMUX_COMMAND_ENV]?.trim();
	const companions = [
	...
	for (const companion of companions) {
		...
		try {
			...
			throw new Error(`vib_tmux_provider_ambiguous: Windows ${companion} command resolution failed`);
		}
		if (!companionPath) {
			if (companion === explicitCompanion)
				throw new Error("vib_tmux_provider_ambiguous: VIB_PSMUX_COMMAND could not be resolved");
			continue;
		}
		const companionIdentity = activeExecutableIdentityResolver(companionPath);
		if (!companionIdentity) {
			throw new Error(`vib_tmux_provider_ambiguous: Windows ${companion} executable identity is unavailable`);
		}
		if (companionIdentity === selectedIdentity) matched = true;
		else {
			distinct = true;
			if (companion === explicitCompanion)
				throw new Error("vib_tmux_provider_ambiguous: VIB_PSMUX_COMMAND selects a different executable");
		}
	}
	if (matched && distinct) throw new Error("vib_tmux_provider_ambiguous: Windows psmux companion identities conflict");
	return matched;
}
...
export function detectPsmux(
	...
	const explicit = env[VIB_PSMUX_COMMAND_ENV]?.trim();
	if (explicit) {
		// The override is authoritative on its own — we trust the user's
		// VIB_PSMUX_COMMAND value when they name a psmux-class binary, even
	...
	if (envDisabled(env[VIB_PSMUX_DETECTION_ENV])) return false;
	const force = options.force === true || envForcesProbe(env[VIB_PSMUX_FORCE_DETECT_ENV]);
...
export interface ResolveVibTmuxBinaryOptions {
...
/**
 * Resolve the tmux command Vibrato should invoke. Honors the existing
 * VIB_TMUX_COMMAND override; on Windows when no
 * override is set, psmux (installed as psmux, pmux, or tmux) is picked
 * automatically so the default vib --tmux flow lands on a real multiplexer.
 */
export function resolveVibTmuxBinary(options: ResolveVibTmuxBinaryOptions = {}): ResolvedTmuxBinary {
	const env = options.env ?? process.env;
	const platform = options.platform ?? process.platform;
	const runner = options.runner ?? readSpawnRunner();
	const explicit = env.VIB_TMUX_COMMAND?.trim();
	if (explicit) {
		const isPsmux =
			platform === "win32"
	...
	if (platform === "win32") {
		for (const command of ["psmux", "pmux"] as const) {
			...
			if (!activeExecutableIdentityResolver(executablePath))
				throw new Error(`vib_tmux_provider_ambiguous: Windows ${command} executable identity is unavailable`);
		...
		if (tmuxPath) {
			...
			if (isPsmux && !activeExecutableIdentityResolver(tmuxPath))
				throw new Error("vib_tmux_provider_ambiguous: Windows tmux executable identity is unavailable");
...
export function probePsmux(
	...
	const output = probeVersionOutput(resolved, runner);
	const isPsmux = outputMentionsPsmux(output) || env[VIB_PSMUX_COMMAND_ENV]?.trim() === resolved;
