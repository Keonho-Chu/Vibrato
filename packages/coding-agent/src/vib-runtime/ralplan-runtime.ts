...
import { isRestrictedRoleAgentBash, VIB_RALPLAN_ARTIFACT_ENV } from "./restricted-role-agent-bash";
import { modeStatePath, sessionIdFromDirName, sessionPlansDir, vibRoot } from "./session-layout";
import { resolveVibSessionForWrite, writeSessionActivityMarker } from "./session-resolution";
...
/**
 * Native implementation of `vib ralplan`.
 *
 * Two invocation shapes are handled natively:
 *
 * 1. **Consensus handoff**: `vib ralplan [--interactive] [--deliberate] [--architect <kind>]
 *    [--critic <kind>] [--session-id <id>] "<task>"` validates the documented flag surface,
 *    seeds `.vib/state/ralplan-state.json`, and updates the shared HUD rail via
 ...
 *
 * 2. **Artifact write**: `vib ralplan --write --stage <type> --stage_n <N>
 *    (--artifact <path-or-string> | --artifact-env VIB_RALPLAN_ARTIFACT)
 ...
 *    `.vib/plans/ralplan/<run-id>/`, maintains an `index.jsonl` audit log, copies `final`
...
/** Inclusive upper bound for `vib.ralplan.maxIterations` settings overrides. */
...
/** Inclusive upper bound for `vib.ralplan.maxReviewPassesPerLane` settings overrides. */
 ...
 * Project `.vib/config.yml` and `.vib/settings.json` beat user layers.
 */
export async function resolveRalplanMaxIterations(
	...
	const { value, source } = await resolveStrictRalplanSetting(
		...
		workflowSettingParse(
			...
			`expected vib.ralplan.maxIterations to be an integer between 1 and ${RALPLAN_MAX_ITERATIONS_LIMIT}`,
		),
		RALPLAN_DEFAULT_MAX_ITERATIONS,
		agentDir,
	);
...
function parsePresentRalplanAutoHandoff(value: unknown): WorkflowSettingParseResult<RalplanAutoHandoffTarget> {
	...
	return target === undefined
		? { kind: "invalid", reason: "expected vib.ralplan.autoHandoff to be one of off, ultragoal, autoresearch" }
...
export async function resolveRalplanMaxReviewPassesPerLane(
	...
	const { value, source } = await resolveStrictRalplanSetting(
		...
		workflowSettingParse(
			...
			`expected vib.ralplan.maxReviewPassesPerLane to be an integer between 1 and ${RALPLAN_MAX_REVIEW_PASSES_PER_LANE_LIMIT}`,
		),
		RALPLAN_DEFAULT_MAX_REVIEW_PASSES_PER_LANE,
		agentDir,
	);
...
export async function assertExplicitTargetVibNotSymlinked(root: string): Promise<void> {
	const vibRoot = path.join(root, ".vib");
	let st: fssync.Stats;
	try {
		st = await fs.lstat(vibRoot);
	} catch (error) {
		const err = error as NodeJS.ErrnoException;
		if (err.code === "ENOENT") return;
		throw new RalplanCommandError(2, `failed to inspect ${vibRoot}: ${err.message}`);
	}
	if (st.isSymbolicLink()) {
		throw new RalplanCommandError(2, `ralplan --worktree-root refuses a symlinked .vib root: ${vibRoot}`);
	}
	const realVib = await fs.realpath(vibRoot);
	const realRoot = await fs.realpath(root);
	const relative = path.relative(realRoot, realVib);
	if (relative.startsWith("..") || path.isAbsolute(relative)) {
		throw new RalplanCommandError(2, `ralplan --worktree-root .vib escapes the target worktree: ${vibRoot}`);
	}
}
	...
	return await withWorkflowStateLock(
		...
		async () => {
			...
			await writeWorkflowEnvelopeAtomic(statePath, existing, {
				cwd,
				lockHeld: true,
				receipt: { cwd, skill: "ralplan", owner: "vib-runtime", command: "vib ralplan persist-run-id", sessionId },
				audit: { category: "state", verb: "write", owner: "vib-runtime", skill: "ralplan", sessionId },
			});
		},
		{ cwd },
	);
		...
		async () => {
			...
			await writeWorkflowEnvelopeAtomic(statePath, existing, {
				...
				receipt: {
					...
					skill: "ralplan",
					owner: "vib-runtime",
					command: `vib ralplan ${update.role}-state`,
					sessionId,
				},
				audit: { category: "state", verb: "write", owner: "vib-runtime", skill: "ralplan", sessionId },
			});
	...
	return await withWorkflowStateLock(
		...
		async () => {
			...
			await writeWorkflowEnvelopeAtomic(statePath, existing, {
				cwd,
				lockHeld: true,
				receipt: { cwd, skill: "ralplan", owner: "vib-runtime", command: "vib ralplan lane-verdict", sessionId },
				audit: { category: "state", verb: "write", owner: "vib-runtime", skill: "ralplan", sessionId },
			});
			return true;
		},
	...
	await withWorkflowStateLock(
		...
		async () => {
			...
			await writeWorkflowEnvelopeAtomic(statePath, existing, {
				cwd,
				lockHeld: true,
				receipt: { cwd, skill: "ralplan", owner: "vib-runtime", command: "vib ralplan planning-stuck", sessionId },
				audit: { category: "state", verb: "write", owner: "vib-runtime", skill: "ralplan", sessionId },
			});
		},
		{ cwd },
	...
	await withWorkflowStateLock(
		...
		async () => {
			...
			await writeWorkflowEnvelopeAtomic(statePath, existing, {
				cwd,
				lockHeld: true,
				receipt: { cwd, skill: "ralplan", owner: "vib-runtime", command: "vib ralplan final-admission", sessionId },
				audit: { category: "state", verb: "write", owner: "vib-runtime", skill: "ralplan", sessionId },
			});
		},
		{ cwd },
...
async function findExistingRalplanRunOwners(cwd: string, runId: string): Promise<string[]> {
	...
	try {
		entries = await fs.readdir(vibRoot(cwd), { withFileTypes: true });
	} catch (error) {
		const err = error as NodeJS.ErrnoException;
...
async function resolveArtifactSessionId(args: readonly string[], cwd: string, explicitRunId: string | undefined) {
	...
	const currentSession = resolveVibSessionForWrite(cwd, {
		flagValue: flagSessionId,
		envSessionId: process.env.VIB_SESSION_ID,
	});
	if (!explicitRunId) return currentSession.vibSessionId;

	const owners = await findExistingRalplanRunOwners(cwd, explicitRunId);
	if (owners.length === 0) return currentSession.vibSessionId;
	if (owners.length > 1) {
		throw new RalplanCommandError(
			2,
	...
	if (flagSessionId !== undefined && currentSession.vibSessionId !== ownerSessionId) {
		throw new RalplanCommandError(
			2,
			`ralplan run ${explicitRunId} is owned by session ${ownerSessionId}, not ${currentSession.vibSessionId}`,
		);
	}
...
async function resolveArtifactArgs(
	...
	if (artifactEnvName !== undefined && artifactEnvName !== VIB_RALPLAN_ARTIFACT_ENV) {
		throw new RalplanCommandError(2, `--artifact-env must be ${VIB_RALPLAN_ARTIFACT_ENV}`);
	}
...
async function handleArtifactWrite(
	...
	if (target.explicit) await assertExplicitTargetVibNotSymlinked(persistCwd);
	...
	// `vib ralplan --write` invocation. This is NOT exclusive across processes: no
...
function extractPositionalTask(args: readonly string[]): string {
	...
	for (const arg of args) {
		...
		if (arg.startsWith("-")) {
			throw new RalplanCommandError(2, `unknown flag for vib ralplan: ${arg}`);
		}
		parts.push(arg);
	}
...
function resolveConsensusArgs(args: readonly string[], cwd: string): ConsensusHandoffArgs {
	if (hasFlag(args, "--lane-verdict")) {
		throw new RalplanCommandError(2, "--lane-verdict is only supported with vib ralplan --write.");
	}
	...
	const session = resolveVibSessionForWrite(cwd, {
		flagValue: flagValue(args, "--session-id"),
		envSessionId: process.env.VIB_SESSION_ID,
	});
	const sessionId = session.vibSessionId;
	assertSafePathComponent(sessionId, "session-id");
...
async function seedRalplanState(
	...
	// Reuse an existing run id when present so a re-invocation of `vib ralplan "task"` doesn't
...
async function handleConsensusHandoff(args: readonly string[], cwd: string): Promise<RalplanCommandResult> {
	...
	if (!resolved.task) {
		throw new RalplanCommandError(2, 'vib ralplan requires a task description, e.g. `vib ralplan "<task>"`.');
	}
