/**
 * Boundary session resolution for Vibrato workflow state.
 *
...
import {
	...
	sessionRoot,
	VIB_SESSION_ACTIVITY_FILE,
	type VibSessionContext,
	type VibSessionSource,
	vibRoot,
} from "./session-layout";
...
export function resolveSessionIdFromSources(sources: SessionIdSources): ResolvedFromSources | undefined {
	...
	if (flagValue !== undefined) {
		...
		return { vibSessionId: trimmed, source: "flag" };
	}
	if (typeof payloadSessionId === "string" && payloadSessionId.trim() !== "") {
		const trimmed = payloadSessionId.trim();
		assertSafeResolvedSessionId(trimmed);
		return { vibSessionId: trimmed, source: "payload" };
	}
	if (typeof envSessionId === "string" && envSessionId.trim() !== "") {
		const trimmed = envSessionId.trim();
		assertSafeResolvedSessionId(trimmed);
		return { vibSessionId: trimmed, source: "env" };
	}
	return undefined;
}

/** Resolve session context for a WRITE command. Errors when no explicit id is present. */
export function resolveVibSessionForWrite(cwd: string, sources: SessionIdSources): VibSessionContext {
	const resolved = resolveSessionIdFromSources(sources);
	if (!resolved) {
		throw new SessionResolutionError(
			"a session id is required to write state: pass --session-id, payload session_id, or set VIB_SESSION_ID",
			"missing_for_write",
		);
	}
	return {
		vibSessionId: resolved.vibSessionId,
		sessionRoot: sessionRoot(cwd, resolved.vibSessionId),
 ...
 */
export async function resolveVibSessionForRead(cwd: string, sources: SessionIdSources): Promise<VibSessionContext> {
	const resolved = resolveSessionIdFromSources(sources);
	if (resolved) {
		return {
			vibSessionId: resolved.vibSessionId,
	...
	return { vibSessionId: latest.vibSessionId, sessionRoot: latest.sessionRoot, source: "latest" };
}
...
/**
 * Scan `.vib/_session-*` directories and select the most-recently-active one by
 * its activity marker. Never uses raw directory mtime. Throws on zero candidates
 * or an ambiguous tie.
 */
export async function detectLatestSession(cwd: string): Promise<VibSessionContext> {
	const candidates = await collectActiveSessionCandidates(cwd);
	if (candidates.length === 0) {
		throw new SessionResolutionError(
			"no active Vibrato session found: pass --session-id or set VIB_SESSION_ID",
			"no_session",
		);
	}
	...
	if (second && first.activityMs - second.activityMs <= LATEST_SESSION_TIE_WINDOW_MS) {
		const tied = candidates
			...
			.map(c => c.vibSessionId);
		throw new SessionResolutionError(
			`ambiguous latest session among [${tied.join(", ")}]: pass --session-id or set VIB_SESSION_ID`,
			"ambiguous",
		);
	}
	return { vibSessionId: first.vibSessionId, sessionRoot: first.sessionRoot, source: "latest" };
}

async function collectActiveSessionCandidates(cwd: string): Promise<SessionCandidate[]> {
	const root = vibRoot(cwd);
	...
	for (const entry of entries) {
		...
		const vibSessionId = sessionIdFromDirName(entry.name);
		if (!vibSessionId) continue;
		assertSafeResolvedSessionId(vibSessionId);
		const dir = path.join(root, entry.name);
		const activityMs = await readActivityMs(path.join(dir, VIB_SESSION_ACTIVITY_FILE));
		...
		candidates.push({ vibSessionId, sessionRoot: dir, activityMs });
	}
	return candidates;
}
...
export async function writeSessionActivityMarker(
	...
	const markerPath = path.join(sessionRoot(cwd, vibSessionId), VIB_SESSION_ACTIVITY_FILE);
	await fs.mkdir(path.dirname(markerPath), { recursive: true });
	const payload = {
		session_id: vibSessionId,
