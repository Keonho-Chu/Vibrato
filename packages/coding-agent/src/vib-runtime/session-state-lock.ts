...
function nativeSessionStateLock(): SessionStateLockNativeBindings {
	try {
		...
		if (!loadedNativeBindings) loadedNativeBindings = require("@vib-rato/natives") as SessionStateLockNativeBindings;
...
function lockQuarantineName(existing?: string): string {
	const name =
		...
		`.vib-delete-session-state-lock-${randomUUID()}.json`;
	if (!existing) {
...
export async function removeVerifiedEmptyQuarantine(directory: string, name: string): Promise<void> {
	...
	const result = nativeSessionStateLock().exactUnlinkDirect(target, {
		...
		quarantineName: `.vib-delete-cleanup-${randomUUID()}.json`,
	});
	if (result.ok) return;
	const retained = [
		result.detachedPath,
