...
import { normalizePathForComparison, postmortem } from "@vib-rato/utils";
...
export const VIB_TMUX_OWNER_GENERATION_ENV = "VIB_TMUX_OWNER_GENERATION";
export const VIB_TMUX_OWNER_STATE_DIR_ENV = "VIB_TMUX_OWNER_STATE_DIR";
export const VIB_TMUX_OWNER_SERVER_KEY_ENV = "VIB_TMUX_OWNER_SERVER_KEY";
export const VIB_COORDINATOR_SESSION_STATE_FILE_ENV = "VIB_COORDINATOR_SESSION_STATE_FILE";
export const VIB_COORDINATOR_SESSION_ID_ENV = "VIB_COORDINATOR_SESSION_ID";
export const VIB_COORDINATOR_SESSION_BRANCH_ENV = "VIB_COORDINATOR_SESSION_BRANCH";
export const VIB_COORDINATOR_SESSION_LAUNCH_ID_ENV = "VIB_COORDINATOR_SESSION_LAUNCH_ID";
export const VIB_COORDINATOR_SESSION_READINESS_FILE_ENV = "VIB_COORDINATOR_SESSION_READINESS_FILE";
export const VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV = "VIB_COORDINATOR_SIDECAR_SIGNING_KEY";
export const VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL_ENV = "VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL";
export const VIB_COORDINATOR_SIDECAR_KEY_ID_ENV = "VIB_COORDINATOR_SIDECAR_KEY_ID";
/** Explicitly binds this launch to Coordinator-minted sidecar signing authority. */
export const VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED_ENV = "VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED";
 ...
 */
const coordinatorSidecarSigningBootstrapUrl = process.env[VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL_ENV]?.trim();
const coordinatorSidecarSigningKeyBootstrap = await (async (): Promise<string | undefined> => {
	if (!coordinatorSidecarSigningBootstrapUrl) return process.env[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV];
	try {
...
const coordinatorSidecarSigningKey = (() => {
	...
	delete process.env[VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL_ENV];
...
export function coordinatorSidecarSigningBootstrapEnv(): Record<string, string> {
	const keyId = process.env[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]?.trim();
	return coordinatorSidecarSigningKeyBootstrap && keyId && coordinatorSidecarSigningKey
		? {
				...
				[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV]: coordinatorSidecarSigningKeyBootstrap,
...
const VIB_SESSION_PROMPT_ACCEPTED_JSON_ENV = "VIB_SESSION_PROMPT_ACCEPTED_JSON";
const VIB_SESSION_WORKTREE_BASELINE_DIRTY_ENV = "VIB_SESSION_WORKTREE_BASELINE_DIRTY";
...
function isRuntimeInputReadyMarker(value: unknown): value is RuntimeInputReadyMarker {
	...
	return (
		...
		marker.source === "vib_interactive_runtime" &&
...
export async function persistCoordinatorRuntimeInputReady(): Promise<RuntimeInputReadyMarker | null> {
	const stateFile = process.env[VIB_COORDINATOR_SESSION_STATE_FILE_ENV]?.trim();
	const sessionId = process.env[VIB_COORDINATOR_SESSION_ID_ENV]?.trim();
	const launchId = process.env[VIB_COORDINATOR_SESSION_LAUNCH_ID_ENV]?.trim();
	const readinessFile = process.env[VIB_COORDINATOR_SESSION_READINESS_FILE_ENV]?.trim();
...
function normalizedIdentity(
	...
	const explicitStateFile = process.env[VIB_COORDINATOR_SESSION_STATE_FILE_ENV]?.trim();
	const sessionId = explicitStateFile
		? process.env[VIB_COORDINATOR_SESSION_ID_ENV]?.trim() || context.sessionId.trim()
		: context.sessionId.trim();
	const cwd = context.cwd.trim();
	const platform = context.platform ?? process.platform;
	const pathApi = platform === "win32" ? path.win32 : path;
	const sidecarSignatureRequired = process.env[VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED_ENV]?.trim() === "true";
	const sidecarKeyId = sidecarSignatureRequired
		? process.env[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]?.trim() || null
		: null;
	if (
		!sessionId ||
		!cwd ||
...
function runtimeStateFileForContext(context: RuntimeStateContext): string | null {
	const explicit = process.env[VIB_COORDINATOR_SESSION_STATE_FILE_ENV]?.trim();
...
function branchForContext(context: RuntimeStateContext): string | null {
	return context.branch ?? (process.env[VIB_COORDINATOR_SESSION_BRANCH_ENV]?.trim() || null);
}
...
function promptAcceptedFromEnv(): boolean {
	const promptAcceptedJson = process.env[VIB_SESSION_PROMPT_ACCEPTED_JSON_ENV]?.trim();
...
function worktreeBaselineDirtyFromEnvOrMarker(): boolean | null {
	...
	const envValue = booleanFromUnknown(process.env[VIB_SESSION_WORKTREE_BASELINE_DIRTY_ENV]);
...
function errorMessageForPostmortem(reason: postmortem.Reason): string {
	return publicSafeErrorMessage(`Vibrato process cleanup ran for ${reason}`);
}
						...
						error: {
							...
							message: publicSafeErrorMessage(
								exitedBeforeTerminalState
									? "Vibrato process exited before emitting terminal agent state"
									: `Vibrato process exited with code ${exitCode}`,
							),
							recoverable: true,
						},
						recovery: {
...
async function writeStateFileSync(
	stateFile: string,
	payload: Record<string, unknown>,
	keyId: string | null = process.env[VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED_ENV]?.trim() === "true"
		? process.env[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]?.trim() || null
		: null,
): Promise<void> {
							...
							...(terminalReceipt?.receipt === "missing"
								...
								: state === "errored"
									? {
											error: {
												code: "agent_error",
												message: "Vibrato agent reported an error",
												recoverable: true,
											},
										}
									: {}),
...
export function ownerTerminalContextFromEnvironment(): OwnerTerminalContext | "invalid" | null {
	const generation = process.env[VIB_TMUX_OWNER_GENERATION_ENV];
	const stateDir = process.env[VIB_TMUX_OWNER_STATE_DIR_ENV];
	const socketKey = process.env[VIB_TMUX_OWNER_SERVER_KEY_ENV];
	const supplied = [generation, stateDir, socketKey].some(value => value !== undefined);
	const managedLaunch = process.platform === "linux" && process.env.VIB_TMUX_LAUNCHED === "1";
...
async function persistInvalidOwnerTerminalMetadata(
	...
	await writeStateFileSync(stateFile, {
		...
		error: {
			...
			message: "Vibrato managed tmux owner metadata was unavailable or invalid",
			recoverable: true,
		},
		recovery: {
			action: "recover_or_resume_session",
	...
	try {
			...
			...(expected
				...
				: {
						error: {
							...
							message: "Vibrato owner terminal verdict requires session recovery",
							recoverable: true,
						},
						recovery: {
							action: "recover_or_resume_session",
		...
		await writeStateFileSync(stateFile, {
			...
			error: {
				...
				message: "Vibrato owner terminal verdict was unavailable",
				recoverable: true,
			},
			recovery: {
				action: "recover_or_resume_session",
