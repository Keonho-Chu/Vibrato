...
export type StateGraphSkill = CanonicalVibWorkflowSkill | "all";
...
function skillsFor(skill: StateGraphSkill): CanonicalVibWorkflowSkill[] {
	return skill === "all" ? [...CANONICAL_VIB_WORKFLOW_SKILLS] : [skill];
}
...
function dotId(skill: CanonicalVibWorkflowSkill, state: string): string {
	return `"${skill}:${state}"`;
}

function renderDot(skill: StateGraphSkill): string {
	const lines = ["digraph vib_state {", "  rankdir=LR;"];
	for (const item of skillsFor(skill)) {
		const manifest = getSkillManifest(item);
