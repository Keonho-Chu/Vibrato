...
function canonicalSkillOrThrow(skill: string): CanonicalVibWorkflowSkill {
	const canonical = canonicalWorkflowSkill(skill);
	if (!canonical) throw new Error(`Unsupported Vibrato workflow skill: ${skill}`);
	return canonical;
}
...
function legacyPhaseForSkill(skill: CanonicalVibWorkflowSkill, phase: string): string {
	if (phase === "planning") return initialPhaseForSkill(skill);
	return phase;
}

function normalizePhase(skill: CanonicalVibWorkflowSkill, value: unknown): string {
...
function receiptWithRequiredFields(raw: unknown, skill: CanonicalVibWorkflowSkill): Record<string, unknown> {
	const receipt =
	...
	receipt.skill = skill;
	if (receipt.owner !== "vib-state-cli" && receipt.owner !== "vib-runtime" && receipt.owner !== "vib-hook") {
...
function migrateV1ToV2(state: Record<string, unknown>, skill: CanonicalVibWorkflowSkill): Record<string, unknown> {
 ...
 * the only path allowed to write normalized upgrades back to `.vib/state/**`.
 */
...
export async function migrateAndPersistLegacyState(
	...
	const persistedPath = await writeWorkflowEnvelopeAtomic(args.statePath, state, {
		...
		receipt: {
			...
			owner: "vib-state-cli",
			command: `vib state ${canonicalSkill} migrate`,
