...
export function renderContractMarkdown(skill: CanonicalVibWorkflowSkill, contract: unknown): string {
...
export function renderUltragoalStatusMarkdown(summary: {
	...
	if (!summary.exists)
		return `# ultragoal status\n\n- status: missing\n- No ultragoal plan found at ${summary.paths.goalsPath}. Run \`vib ultragoal create-goals --brief "..."\` first.\n`;
	...
	if (summary.vibObjective) lines.push(`- objective: ${summary.vibObjective}`);
