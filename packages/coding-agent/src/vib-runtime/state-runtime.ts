...
// Subpath import keeps this module native-free for the vib-state-gates shards:
// the package barrel pulls procmgr/ptree → @vib-rato/natives.
...
/**
 * Native implementation of the `vib state read|write|clear` command surface.
 *
 * Simple file-receipt operations against session-scoped state under
 * `.vib/_session-{id}/state/`. This is the sanctioned CLI mediator for
 * mutation-guarded Vibrato state — agents call it instead of editing those files directly.
 */
...
const KNOWN_MODES: readonly string[] = CANONICAL_VIB_WORKFLOW_SKILLS;
...
function assertKnownFlags(classification: StateArgvClassification): void {
	...
	if (unknownFlag) throw new StateCommandError(2, `unknown vib state flag: ${unknownFlag}`);
}

function isKnownMode(mode: string): mode is CanonicalVibWorkflowSkill {
	return KNOWN_MODES.includes(mode);
}

function assertKnownMode(mode: string): asserts mode is CanonicalVibWorkflowSkill {
	if (!isKnownMode(mode)) {
...
interface ResolvedSelectors {
	mode: CanonicalVibWorkflowSkill | undefined;
	vibSessionId: string;
...
async function resolveSelectors(args: readonly string[], cwd: string, action: StateAction): Promise<ResolvedSelectors> {
	...
	const session = WRITE_SESSION_ACTIONS.has(action)
		? resolveVibSessionForWrite(cwd, sessionSources)
		: await resolveVibSessionForRead(cwd, sessionSources);
	...
	return {
		mode: mode as CanonicalVibWorkflowSkill | undefined,
		vibSessionId: session.vibSessionId,
		threadId,
		turnId,
		payload,
	};
...
async function inferModeFromActiveState(
	cwd: string,
	sessionId: string,
): Promise<CanonicalVibWorkflowSkill | undefined> {
...
async function readActivePhaseForSkill(
	cwd: string,
	sessionId: string,
	mode: CanonicalVibWorkflowSkill,
 ...
 * {@link StateCommandResult.stderr} channel, so `vib state` automation still
...
async function collectDoctorSummary(
	cwd: string,
	skill: CanonicalVibWorkflowSkill | undefined,
	sessionId: string,
): Promise<DoctorSummary> {
	const root = sessionStateDir(cwd, sessionId);
	const skills = skill ? [skill] : [...CANONICAL_VIB_WORKFLOW_SKILLS];
		...
		if (raw.error) {
			problems.push(
				doctorProblem(
					...
					`vib state ${currentSkill} migrate`,
					currentSkill,
				),
			);
			...
			if (activeFlag(entry.value) && (!state.exists || !activeFlag(state.value))) {
				problems.push(
					doctorProblem(
						...
						canonical ? `vib state ${canonical} clear` : "vib state prune --hard",
						canonical ?? undefined,
					),
				);
			}
...
async function handleDoctor(
	...
	const session = await resolveVibSessionForRead(cwd, {
		flagValue: flagValue(args, "--session-id"),
		payloadSessionId: payload?.session_id,
		envSessionId: process.env.VIB_SESSION_ID,
	});
	const summary = await collectDoctorSummary(
		cwd,
		rawSkill as CanonicalVibWorkflowSkill | undefined,
		session.vibSessionId,
	);
	return {
...
async function writeJsonAtomic(
	...
	const writeResult = await writeGuardedWorkflowEnvelopeAtomic(filePath, value, {
		...
		audit: {
			...
			category: "state",
			verb,
			owner: options?.owner ?? "vib-state-cli",
			skill: options?.skill,
			mutationId: options?.mutationId,
...
function parseLimitFlag(args: readonly string[], defaultLimit = 50): number {
	...
	if (!Number.isInteger(parsed) || parsed <= 0 || parsed > 500) {
		throw new StateCommandError(2, "vib state --limit requires an integer from 1 to 500");
	}
	return parsed;
}
...
function parseSinceFlag(args: readonly string[]): string | undefined {
	...
	if (Number.isNaN(Date.parse(raw)))
		throw new StateCommandError(2, "vib state --since requires an ISO timestamp or duration like 30m, 6h, 7d");
 ...
 * payload. Unlike `vib state write`, this is a derived repair: callers reconcile from
 ...
 * detection. Receipts carry `owner: "vib-runtime"` and `verb: "reconcile"` so the
...
export async function reconcileWorkflowSkillState(options: {
	...
	const { vibSessionId: sessionId } = resolveVibSessionForWrite(options.cwd, {
...
export async function readWorkflowStateJson(
	...
	return (await readJsonFile(modeStateFile(cwd, skill, session.vibSessionId), onWarning)) ?? {};
}
...
async function handleRead(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	const mode = selectors.mode ?? (await inferModeFromActiveState(cwd, selectors.vibSessionId));
	const fields = parseFieldsFlag(args);
	// Corrupt-state warnings are TUI-safe file-logged inside the readers; the CLI
	// path also surfaces them on the command result so `vib state read`
	...
	if (mode) {
		const filePath = modeStateFile(cwd, mode, selectors.vibSessionId);
		const existing = await readWorkflowStateJson(cwd, mode, selectors.vibSessionId, warning =>
	...
	const filePath = activeStateFile(cwd, selectors.vibSessionId);
...
async function handleStatus(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode) {
		throw new StateCommandError(
			2,
			"vib state status requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
	}
	const filePath = modeStateFile(cwd, mode, selectors.vibSessionId);
	const warnings: string[] = [];
	const existing = await readWorkflowStateJson(cwd, mode, selectors.vibSessionId, warning => warnings.push(warning));
...
async function handleWrite(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	const { vibSessionId: sessionId, threadId, turnId, payload } = selectors;
	if (!payload) throw new StateCommandError(2, "vib state write requires --input '<json>'");
	const mode = selectors.mode ?? (await inferModeFromActiveState(cwd, sessionId));
	if (!mode)
		throw new StateCommandError(
			2,
			"vib state write requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
...
async function handleClear(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	const { vibSessionId: sessionId, threadId, turnId } = selectors;
	const mode = selectors.mode ?? (await inferModeFromActiveState(cwd, sessionId));
	if (!mode)
		throw new StateCommandError(
			2,
			"vib state clear requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
 ...
 *     `vib state <skill> write` are declaring "I am ready to be handed off";
...
async function handleHandoffUnlocked(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!caller) {
		throw new StateCommandError(
			2,
			"vib state handoff requires --mode <caller>, positional <caller>, input.skill, or an active workflow in the current session active state",
		);
	}
	const calleeRaw = flagValue(args, "--to")?.trim();
	if (!calleeRaw) {
		throw new StateCommandError(2, "vib state handoff requires --to <callee>");
	}
	assertSafePathComponent(calleeRaw, "to");
	const callee = calleeRaw;
	const calleeIsWorkflow = isKnownMode(callee);
	if (callee === caller) {
		throw new StateCommandError(2, `vib state handoff: --to must differ from caller (both are "${caller}")`);
	}
	...
	if (callerRead.kind === "absent") {
		throw new StateCommandError(
			2,
			`vib state ${caller} handoff: caller is not active (no mode-state file at ${callerPath})`,
		);
	}
	...
	const callerReceipt = buildWorkflowStateReceipt({
		...
		skill: caller,
		owner: "vib-state-cli",
		command: `vib state ${caller} handoff --to ${callee}`,
		sessionId,
		nowIso: handoffAt,
		mutationId,
	});
	...
	if (!calleePath) {
		throw new StateCommandError(2, `vib state handoff failed to resolve workflow callee path for ${callee}`);
	}
	...
	if (process.env.VIB_STATE_HANDOFF_FAIL_AFTER_CALLER === mutationId) {
...
async function handleHandoff(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	// `process.cwd()`.
	const handoffLock = path.join(sessionStateDir(cwd, selectors.vibSessionId), "handoff");
...
async function handleContract(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode) {
		throw new StateCommandError(2, "vib state contract requires --mode <skill>, positional <skill>, or input.skill");
	}
...
function parseNonNegativeIntegerFlag(args: readonly string[], flag: string): number | undefined {
	...
	if (!Number.isInteger(parsed) || parsed < 0) {
		throw new StateCommandError(2, `vib state ${flag} requires a non-negative integer value`);
	}
	return parsed;
}
...
interface GcSummary {
	skill: CanonicalVibWorkflowSkill | "all";
	dry_run: boolean;
	eligible: string[];
	pruned: string[];
...
async function collectRetentionCandidates(
	cwd: string,
	sessionId: string,
	skills: readonly CanonicalVibWorkflowSkill[],
...
async function buildGcSummary(
	...
	const skills = rawSkill === "all" ? CANONICAL_VIB_WORKFLOW_SKILLS : [rawSkill as CanonicalVibWorkflowSkill];
	const session = await resolveVibSessionForRead(cwd, {
		flagValue: flagValue(args, "--session-id"),
		envSessionId: process.env.VIB_SESSION_ID,
	});
	const eligible = selectRetentionEligible(await collectRetentionCandidates(cwd, session.vibSessionId, skills));
	const counts: Record<string, number> = {};
	...
	if (!dryRun && targets.length > 0) {
		...
		pruned = await hardPrune(targets, context => eligiblePaths.has(path.resolve(context.path)), {
			...
			audit: {
				cwd,
				sessionId: session.vibSessionId,
				skill: rawSkill,
				category: "prune",
				verb: "gc",
				owner: "vib-state-cli",
	...
	return {
		skill: rawSkill as CanonicalVibWorkflowSkill | "all",
		dry_run: dryRun,
		eligible: eligible.map(candidate => candidate.relativePath),
		pruned: pruned.map(filePath => path.relative(sessionStateDir(cwd, session.vibSessionId), filePath)),
		counts,
	};
}
...
async function handleGraph(
	...
	if (hasFlag(args, "--history")) {
		const session = await resolveVibSessionForRead(_cwd, {
			flagValue: flagValue(args, "--session-id"),
			envSessionId: process.env.VIB_SESSION_ID,
		});
		const history = await readAuditWindow(_cwd, args, session.vibSessionId);
		return {
			status: 0,
	...
	return {
		status: 0,
		stdout: renderStateGraph(rawSkill as CanonicalVibWorkflowSkill | "all", format as StateGraphFormat),
	};
}
...
async function handlePrune(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode) {
		throw new StateCommandError(
			2,
			"vib state prune requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
	}
	const filePath = modeStateFile(cwd, mode, selectors.vibSessionId);
	...
	const audit: StateWriterAuditContext = {
		cwd,
		sessionId: selectors.vibSessionId,
		skill: mode,
		category: "prune",
	...
	try {
		...
		if (await matchesSelector(stat, async () => JSON.parse(await fs.readFile(filePath, "utf-8")))) {
			const archivedPath = await softDelete(
				filePath,
				{ skill: mode, reason: "vib state prune", status: status ?? null, older_than_days: olderThanDays ?? null },
				{ cwd, audit },
			);
...
async function handleMigrate(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode) {
		throw new StateCommandError(
			2,
			"vib state migrate requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
	}
	const filePath = modeStateFile(cwd, mode, selectors.vibSessionId);
	const forced = hasFlag(args, "--force");
	const mismatchWarning = await warnAndAuditOutOfBandIfNeeded(cwd, selectors.vibSessionId, filePath, mode, {
		forced,
	});
