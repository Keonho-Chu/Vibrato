/**
 * Canonical zod schemas for Vibrato workflow state (Workstream A, v4).
 *
...
const skillEnum = z.enum([...CANONICAL_VIB_WORKFLOW_SKILLS]);
const ownerEnum = z.enum(["vib-state-cli", "vib-runtime", "vib-hook"]);
 ...
 */
export type ReadVibJsonResult<T> = { ok: true; value: T; raw: unknown } | { ok: false; error: string; raw: unknown };
...
/**
 * Parse + schema-validate a `.vib` JSON file at the read boundary.
 ...
 */
export async function readVibJson<T>(filePath: string, schema: z.ZodType<T>): Promise<ReadVibJsonResult<T> | null> {
	let text: string;
	try {
