...
/**
 * Sole sanctioned project `.vib/**` writer module (gate G1).
 *
 * All native `.vib/**` filesystem mutations must route through these primitives.
 * The primitives validate project `.vib/**` ownership, create parent directories,
...
export interface StateWriterAuditContext {
	...
	verb: string;
	owner: WorkflowStateMutationOwner;
	skill?: CanonicalVibWorkflowSkill | string;
	mutationId?: string;
	fromPhase?: string;
	toPhase?: string;
	forced?: boolean;
...
export interface WorkflowTransactionJournal {
	...
	created_at: string;
	updated_at: string;
	caller?: CanonicalVibWorkflowSkill;
	callee?: CanonicalVibWorkflowSkill;
	paths: string[];
	steps: string[];
}
...
function resolveVibTarget(targetPath: string, cwd = process.cwd()): string {
	if (!targetPath.trim()) throw new Error("targetPath is required");
	const projectRoot = path.resolve(cwd);
	const vibRoot = path.join(projectRoot, ".vib");
	const resolved = path.resolve(projectRoot, targetPath);
	const relative = path.relative(vibRoot, resolved);
	if (relative === "" || relative.startsWith("..") || path.isAbsolute(relative)) {
		throw new Error(`target path must be within project .vib/**: ${targetPath}`);
	}
	return resolved;
}
...
function requireSessionId(sessionScope: string | ActiveSessionScope | undefined, source: string): string {
	...
	if (!normalizedSessionId) throw new Error(`a non-empty Vibrato session id is required (${source})`);
...
export async function writeGuardedJsonAtomic(
	...
	const filePath = resolveVibTarget(targetPath, cwdForOptions(options));
...
async function recordInvalidWorkflowTransition(args: {
	...
	try {
		await appendAuditEntry(cwd, options?.audit?.sessionId ?? "", {
			...
			owner: options?.audit?.owner ?? "vib-runtime",
...
export async function writeWorkflowEnvelopeAtomic(
	...
	const write = async (): Promise<string> => {
		...
		// envelopes directly, bypassing the `vib state` CLI transition gate (`isValidTransition`,
		...
		// transitions silently. Forced writes (`vib state ... --force`, reconcile repairs) carry
		...
		// (`active: false`, e.g. `vib state clear`, which persists the universal `complete`
		...
		if (options?.audit?.forced !== true && parsed.data.active === true) {
			...
			if (toPhase) {
				// Lazy import: workflow-manifest dereferences CANONICAL_VIB_WORKFLOW_SKILLS at
 ...
 * writers of the same `.vib/**` target. Uses the cross-process directory lock
...
/**
 * Active entry files under `.vib/_session-{id}/state/active/<skill>.json` are authoritative. The
...
export async function writeArtifact(
	...
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "artifact", verb: "write", owner: "vib-runtime" },
	});
}
...
export async function writeReport(targetPath: string, content: string, options?: StateWriterOptions): Promise<string> {
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "report", verb: "write", owner: "vib-runtime" },
	});
}
...
export async function writeLogJsonl(targetPath: string, entry: unknown, options?: StateWriterOptions): Promise<string> {
	return appendJsonl(targetPath, entry, {
		...options,
		audit: options?.audit ?? { category: "log", verb: "append", owner: "vib-runtime" },
	});
}
...
export async function softDelete(
	...
	return updateJsonAtomic<Record<string, unknown>>(
		...
		{
			...options,
			audit: options?.audit ?? { category: "prune", verb: "soft-delete", owner: "vib-runtime" },
		},
	);
}
...
export async function hardPrune(
	...
	for (const target of targets) {
		const filePath = resolveVibTarget(target.path, cwd);
		let stat: Stats;
		try {
...
export async function forceOverwrite(
	...
	const auditOptions = {
		...options,
		audit: options?.audit ?? { category: "force", verb: "force-overwrite", owner: "vib-state-cli", forced: true },
	};
	if (options?.raw === true) {
		const filePath = resolveVibTarget(targetPath, cwdForOptions(options));
...
export async function appendAuditEntry(
	...
	if (!sessionId) throw new Error("a non-empty Vibrato session id is required (appendAuditEntry)");
	const entry = typeof sessionIdOrEntry === "string" ? maybeEntry : sessionIdOrEntry;
	if (!entry) throw new Error("audit entry is required");
	const filePath = resolveVibTarget(layoutAuditPath(cwd, sessionId), cwd);
	await fs.mkdir(path.dirname(filePath), { recursive: true });
	await fs.appendFile(filePath, `${JSON.stringify(entry)}\n`, "utf-8");
	return filePath;
}
