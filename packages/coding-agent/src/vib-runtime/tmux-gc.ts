/**
 * GC adapter for vib-tagged tmux sessions. Destructive cleanup is authorized
...
import { hasVibTmuxProviderAuthoritySync, VIB_TMUX_PROFILE_VALUE, VIB_TMUX_SESSION_PREFIX } from "./tmux-common";
import {
	listTmuxSessionsForGc,
	readTmuxSessionTagsForGc,
	removeVibTmuxSession,
	type VibTmuxSessionStatus,
	type VibTmuxSessionsForGc,
} from "./tmux-sessions";
...
function collectedIdentity(session: VibTmuxSessionStatus): CollectedTmuxIdentity | undefined {
	if (
	...
	const providerKind = hasVibTmuxProviderAuthoritySync({
		stateDir,
...
function authorityEnv(identity: CollectedTmuxIdentity, env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
	...
	return {
		...env,
		VIB_TMUX_COMMAND: identity.providerCommand,
		[VIB_COORDINATOR_SESSION_ID_ENV]: identity.sessionId,
		[VIB_TMUX_OWNER_GENERATION_ENV]: identity.ownerGeneration,
		[VIB_TMUX_OWNER_STATE_DIR_ENV]: path.dirname(identity.sessionStateFile),
	};
}
...
function isSessionLive(session: Pick<VibTmuxSessionStatus, "attached" | "panePids">): boolean {
	return session.attached || session.panePids.length > 0;
}

function liveRecord(session: VibTmuxSessionStatus, reason: string): GcRecord {
	return {
		store: STORE,
		id: session.name,
...
function staleRecord(session: VibTmuxSessionStatus, reason: string): GcRecord {
	return {
		store: STORE,
		id: session.name,
		path: session.project,
...
function isOldEnoughForOrphanGc(session: VibTmuxSessionStatus): boolean {
	const createdAt = Date.parse(session.createdAt);
	return Number.isFinite(createdAt) && Date.now() - createdAt >= ORPHAN_MAX_AGE_MS;
}

function isVibOwnedOrphan(session: VibTmuxSessionStatus): boolean {
	return session.name.startsWith(VIB_TMUX_SESSION_PREFIX) || session.name === "vib_rato";
}

async function classifyTaggedSession(session: VibTmuxSessionStatus): Promise<GcRecord> {
	...
	if (!project || !branch) {
		const reason =
			isVibOwnedOrphan(session) && isOldEnoughForOrphanGc(session)
				? "metadata_less_vib_owned_idle_orphan_missing_terminal_marker"
...
function classifyUntaggedSession(session: VibTmuxSessionStatus): GcRecord {
...
async function revalidateRemovable(
	...
	if (tags.profile !== VIB_TMUX_PROFILE_VALUE) return false;
...
export const tmuxSessionsGcAdapter: GcStoreAdapter = {
	...
	async collect(ctx: GcContext): Promise<GcCollectResult> {
		...
		let sessions: VibTmuxSessionsForGc;
		try {
	...
	async prune(record: GcRecord, ctx: GcContext): Promise<GcPruneOutcome> {
		...
		try {
			...
			removeVibTmuxSession(record.id, authorityEnv(identity, ctx.env), identity);
			return { removed: true };
		} catch (error) {
