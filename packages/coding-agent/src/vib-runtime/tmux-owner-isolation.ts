/**
 * Provenance and lifecycle primitives for tmux-resident Vibrato owners.
...
let nativeRecoveryFsRoot: typeof import("@vib-rato/natives")["openRecoveryFsRoot"] | undefined;

function openRecoveryFsRootNative(): typeof import("@vib-rato/natives")["openRecoveryFsRoot"] {
	nativeRecoveryFsRoot ??= (
		require("@vib-rato/natives") as {
			openRecoveryFsRoot: typeof import("@vib-rato/natives")["openRecoveryFsRoot"];
		}
	).openRecoveryFsRoot;
...
export function tmuxOwnerIsolationBootstrapArgv(): string[] {
	...
	return [process.execPath, path.resolve(import.meta.dir, "../../bin/vib.js"), TMUX_OWNER_ISOLATION_CLI_FLAG];
}
...
export function classifyCgroup(input: { platform: NodeJS.Platform; cgroupText?: string | null }): CgroupInfo {
	...
	const scope = paths.find(value => /(?:^|\/)(?:user|session|app|init|vib)[^/]*\.scope(?:\/|$)/.test(value));
...
export function planTmuxOwnerIsolationSync(request: PlanRequest, probe: OwnerIsolationProbeSync): PlanResponse {
	...
	try {
		...
		const expectedScope = `vib-owner-${token}.scope`;
...
export async function bootstrapTmuxOwnerIsolation(
	...
	try {
		...
		if (
			...
			request.expected_scope !== `vib-owner-${request.attempt.token}.scope`
		)
			return {
				schema_version: 1,
				ok: false,
