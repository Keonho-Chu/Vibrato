 ...
 */
export function vibTmuxAuthorityPlatform(): NodeJS.Platform {
...
function requireWindowsAuthorityPlatform(): void {
	if (authorityPlatform() !== "win32") throw new Error("vib_tmux_provider_authority_windows_required");
}
...
function rejectUnsafeToken(value: string, name: string): string {
	...
	if (!trimmed || /[\0\r\n]|\s/.test(trimmed) || /[;&|`$<>]/.test(trimmed))
		throw new Error(`vib_tmux_provider_invalid_${name}`);
	return trimmed;
}
...
function randomNamespace(): string {
	return `vib-${crypto.randomBytes(16).toString("hex")}`;
}

/** Resolve a structured provider. Native tmux remains byte-for-byte argv compatible. */
export function resolveVibTmuxProviderContext(
	options: {
	...
	const platform = options.platform ?? vibTmuxAuthorityPlatform();
	const binary = options.binary ?? resolveVibTmuxBinary(options);
	const selectedCommand = rejectUnsafeToken(binary.command, "command");
	if (binary.isPsmux && platform !== "win32")
		throw new Error("vib_tmux_provider_ambiguous: selected psmux command requires Windows");
	if (!binary.isPsmux) {
	...
	const resolved = resolveVibTmuxExecutablePath(selectedCommand);
	if (!resolved || !(path.win32.isAbsolute(resolved) || path.isAbsolute(resolved)))
		throw new Error("vib_tmux_provider_ambiguous: selected Windows psmux command is not an absolute executable");
	const identity = resolveVibTmuxExecutableIdentity(resolved);
	if (!identity)
		throw new Error("vib_tmux_provider_ambiguous: selected Windows psmux executable identity is unavailable");
...
export function buildTmuxProviderCommand(
	...
	if (!/^[a-z][a-z-]*$/i.test(command) || args.some(arg => typeof arg !== "string" || arg.includes("\0")))
		throw new Error("vib_tmux_provider_invalid_command");
...
function assertCurrentGeneration(root: string, sessionId: string, generation: string): void {
	...
	try {
		...
		throw new Error("vib_tmux_provider_authority_generation_unavailable");
	}
	...
	if (
		...
		throw new Error("vib_tmux_provider_authority_generation_mismatch");
}
/** Returns whether an owner generation has a persisted Windows psmux authority. */
export function hasVibTmuxProviderAuthoritySync(input: {
	stateDir: string;
	sessionId: string;
	generation: string;
}): boolean {
	...
	readVibTmuxProviderAuthoritySync({ stateDir, sessionId, generation });
	return true;
}
...
function parseRecord(data: Uint8Array): ProviderRecord {
	if (data.byteLength === 0 || data.byteLength > MAX_AUTHORITY_BYTES)
		throw new Error("vib_tmux_provider_authority_invalid_record");
	let payload: unknown;
	try {
	...
	if (!payload || typeof payload !== "object") throw new Error("vib_tmux_provider_authority_invalid_record");
	...
	if (
		...
		!/^vib-[a-f0-9]{32}$/.test(value.namespace) ||
...
export function bindVibTmuxProviderAuthority(
	context: ProviderContext,
	input: { stateDir: string; sessionId: string; generation: string },
): ProviderAuthority {
	const stateDir = input.stateDir.trim();
	if (!stateDir || /[\0\r\n]/.test(stateDir)) throw new Error("vib_tmux_provider_invalid_state_dir");
	return Object.freeze({
		...context,
...
function recordFor(authority: ProviderAuthority): ProviderRecord {
	if (!authority.namespace || !authority.executableIdentity)
		throw new Error("vib_tmux_provider_authority_invalid_context");
	return {
		schema_version: 2,
		kind: "windows-psmux",
		platform: "win32",
...
export function persistVibTmuxProviderAuthoritySync(authority: ProviderAuthority): void {
	...
	if (verified.record.session_id !== authority.sessionId || verified.record.owner_generation !== authority.generation)
		throw new Error("vib_tmux_provider_authority_publish_failed");
}

export function readVibTmuxProviderAuthoritySync(input: {
	stateDir: string;
	sessionId: string;
	generation: string;
}): ProviderAuthority {
	...
	if (record.session_id !== sessionId || record.owner_generation !== generation)
		throw new Error("vib_tmux_provider_authority_mismatch");
	const identity = resolveVibTmuxExecutableIdentity(record.executable_path);
	if (!identity || identity !== record.executable_identity)
		throw new Error("vib_tmux_provider_authority_executable_changed");
	return Object.freeze({
		kind: "windows-psmux",
...
export function listVibTmuxProviderAuthoritiesSync(stateDirInput: string): ProviderAuthority[] {
	...
	try {
		...
		throw new Error("vib_tmux_provider_authority_unavailable");
	}
...
export function assertVibTmuxStagedMutationAuthoritySync(authority: ProviderAuthority): void {
	if (authority.kind !== "windows-psmux") return;
	requireWindowsAuthorityPlatform();
	...
	if (
		...
		throw new Error("vib_tmux_provider_authority_mismatch");
	const identity = resolveVibTmuxExecutableIdentity(authority.command);
...
export function assertVibTmuxMutationAuthoritySync(authority: ProviderAuthority): void {
	if (authority.kind !== "windows-psmux") return;
	requireWindowsAuthorityPlatform();
	const root = prepareWindowsAuthorityRoot(rootFor(authority));
