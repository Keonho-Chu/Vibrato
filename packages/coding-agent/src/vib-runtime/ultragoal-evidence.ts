...
const CLI_REPLAY_ENV_BASE: Record<string, string> = { CI: "1", NO_COLOR: "1", VIB_ULTRAGOAL_REPLAY: "1" };
...
export function resolveCliReplayCommand(command: string[], options?: { compiled?: boolean }): string[] {
	if (options?.compiled ?? isCompiledBinary()) {
		throw new Error(
			"CLI replay execution is unavailable in the compiled Vibrato runtime because process.execPath is the Vibrato application, not a Bun CLI executable",
		);
	}
...
function isMeaningfulCliReplayInvariant(invariant: JsonObject, stdout: string, fieldName: string): boolean {
	...
	if (type === "regex") {
		...
		if (expression.test("") || expression.test("vib-replay-random-nonce-7f3a9c")) return false;
...
export async function validateCliReplay(
	...
	const executionCwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-cli-replay-"));
	try {
