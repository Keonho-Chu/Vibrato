...
import { resolveVibSessionForRead, SessionResolutionError } from "./session-resolution";
import {
...
function objectiveMatches(currentGoal: CurrentGoalLike, plan: UltragoalPlan, sessionId?: string | null): boolean {
	...
	if (normalized === plan.vibObjective || normalized === DEFAULT_ULTRAGOAL_OBJECTIVE) return true;
	if (plan.vibObjectiveAliases?.some(alias => alias === normalized)) return true;
...
async function ultragoalReadPaths(
	...
	const explicitSessionId = options.sessionId?.trim() || process.env.VIB_SESSION_ID?.trim();
	if (explicitSessionId) return { paths: getUltragoalPaths(cwd, explicitSessionId), sessionId: explicitSessionId };
	try {
		const session = await resolveVibSessionForRead(cwd, { envSessionId: process.env.VIB_SESSION_ID });
		return { paths: getUltragoalPaths(cwd, session.vibSessionId), sessionId: session.vibSessionId };
	} catch (error) {
...
function activeAskDiagnostic(input: {
	...
	return {
		...
		message: `${input.reason} Use \`vib ultragoal record-review-blockers\` instead of asking the user.`,
	};
}
...
function findReceiptGoal(
	...
	if (
		currentObjective === plan.vibObjective ||
		currentObjective === DEFAULT_ULTRAGOAL_OBJECTIVE ||
		plan.vibObjectiveAliases?.some(alias => alias === currentObjective)
	) {
...
export async function readUltragoalVerificationState(input: {
	...
	if (!plan) {
		if (isKnownUltragoalObjective(currentObjective) || (await hasDurableUltragoalState(input.cwd))) {
			return {
				state: "unreadable_fail_closed",
				message: "Active Ultragoal objective is missing durable .vib/ultragoal/goals.json state.",
			};
		}
	...
	const receiptTarget =
		provenance?.source === "ultragoal"
			? provenance.goalId === "aggregate" || plan.vibGoalMode === "aggregate"
				? (() => {
		...
		if (completedRequired.length > 0 && runState.incompleteGoals.length > 0) {
			return {
				...
				message: `Ultragoal still has incomplete required goals: ${runState.incompleteGoals
					.map(goal => goal.id)
					.join(", ")}. Run \`vib ultragoal complete-goals\` to continue.`,
			};
		}
		return {
			state: "active_missing_final_receipt",
	...
	if (runState.incompleteGoals.length > 0) {
		return {
			state: "active_missing_final_receipt",
			message: `Ultragoal still has incomplete required goals: ${runState.incompleteGoals.map(goal => goal.id).join(", ")}. Run \`vib ultragoal complete-goals\` to continue.`,
...
export async function verifyUltragoalDurableCompletionState(input: {
	...
	if (sessionId === null)
		return { state: "inactive", message: "No active Vibrato session resolved; ultragoal is inactive." };
	try {
		await fs.stat(paths.dir);
	} catch (error) {
		if (isEnoent(error)) return { state: "inactive", message: "No durable .vib/ultragoal state exists." };
		return {
			state: "unreadable_fail_closed",
			message: `Durable .vib/ultragoal state is present but unreadable: ${error instanceof Error ? error.message : String(error)}`,
		};
	}
...
export async function isUltragoalAskBlocked(
	...
	if (sessionId === null) {
		return inactiveAskDiagnostic({
			reason: "No active Vibrato session resolved; ultragoal is inactive.",
			source: "absent",
	...
	try {
		...
		if (isEnoent(error)) {
			return inactiveAskDiagnostic({
				reason: "No durable .vib/ultragoal state exists.",
				source: "absent",
				goalsPath: paths.goalsPath,
				ledgerPath: paths.ledgerPath,
			});
		...
		return activeAskDiagnostic({
			reason: `Durable .vib/ultragoal state is present but unreadable: ${error instanceof Error ? error.message : String(error)}`,
			source: "durable_state_unreadable",
			goalsPath: paths.goalsPath,
			ledgerPath: paths.ledgerPath,
		});
	...
	if (!plan) {
		...
		return activeAskDiagnostic({
			reason: "Durable .vib/ultragoal state exists but goals.json is missing or empty.",
			source: "durable_state_unreadable",
			goalsPath: paths.goalsPath,
			ledgerPath: paths.ledgerPath,
		});
...
export async function assertCanCompleteCurrentGoal(input: {
	...
	throw new Error(
		`${diagnostic.message} Run \`vib ultragoal checkpoint --status complete --quality-gate-json <file>\` first, or record review blockers and rerun verification.`,
	);
}
...
export async function isUltragoalPauseBlocked(cwd: string): Promise<UltragoalPauseBlockDiagnostic> {
	...
	if (terminalCriticCeilingReached(ledger) && !terminalCriticGateOverridden(ledger)) {
		return {
			blocked: true,
			reason:
				"The Ultragoal run hit the terminal-critic ceiling; requires human/leader `vib ultragoal record-critic-gate-override` before further terminal attempts.",
		};
	}
...
export async function assertUltragoalPauseAllowed(cwd: string, agentDir?: string): Promise<void> {
	...
	throw new Error(
		[
			diagnostic.reason,
			"Resolvable blockers must be worked, not paused: investigate, `vib ultragoal steer --kind add_subgoal`, delegate an executor, or `vib ultragoal record-review-blockers`.",
			'If the blocker is genuinely human-only, record `vib ultragoal classify-blocker --classification human_blocked --evidence "<human-only dependency>"`, then record a clean bound `vib ultragoal record-critic-verdict --terminus pause --classification-event-id <eventId> --verdict OKAY --evidence "<critic evidence>"` before pausing.',
		].join("\n"),
	);
}
/**
...
export async function assertUltragoalDropAllowed(input: {
	...
	if (plan.vibGoalMode !== "aggregate") return;
