...
import { sessionUltragoalDir, vibRoot } from "./session-layout";
import {
...
export type UltragoalVibGoalMode = "aggregate" | "per-story";
...
export interface UltragoalPlan {
	version: 1;
	brief: string;
	vibGoalMode: UltragoalVibGoalMode;
	vibObjective: string;
	vibObjectiveAliases?: string[];
	goals: UltragoalGoal[];
...
export function currentUltragoalSessionId(cwd: string): string {
	return resolveVibSessionForWrite(cwd, { envSessionId: process.env.VIB_SESSION_ID }).vibSessionId;
}
...
export function getUltragoalPaths(cwd: string, sessionId?: string | null): UltragoalPaths {
	const explicitSessionId = sessionId?.trim() || process.env.VIB_SESSION_ID?.trim();
	const dir = explicitSessionId ? sessionUltragoalDir(cwd, explicitSessionId) : path.join(vibRoot(cwd), "ultragoal");
	return {
		dir,
...
export async function appendLedger(
	...
	await appendJsonl(paths.ledgerPath, entry, {
		cwd,
		audit: { category: "ledger", verb: "append", owner: "vib-runtime", sessionId: resolvedSessionId },
	});
...
export async function readUltragoalLedger(cwd: string, sessionId?: string | null): Promise<UltragoalLedgerEvent[]> {
	const resolvedSessionId =
		sessionId?.trim() ||
		(await resolveVibSessionForRead(cwd, { envSessionId: process.env.VIB_SESSION_ID })).vibSessionId;
	try {
...
export async function resolveUltragoalNudgeBudget(
	...
	const resolution = await resolveWorkflowSetting(cwd, "vib.ultragoal.nudgeBudget", {
		defaultValue: DEFAULT_ULTRAGOAL_NUDGE_BUDGET,
		parse: value => {
			const parsed = parseNudgeBudgetValue(value);
			return parsed === null
				? { kind: "invalid", reason: "expected vib.ultragoal.nudgeBudget to be a non-negative integer" }
...
export async function writePlan(cwd: string, plan: UltragoalPlan, sessionId?: string | null): Promise<void> {
	...
	await writeArtifact(paths.briefPath, `${plan.brief.trim()}\n`, {
		cwd,
		audit: { category: "artifact", verb: "write", owner: "vib-runtime", sessionId: resolvedSessionId },
	});
	await writeGuardedJsonAtomic(paths.goalsPath, plan, {
		...
		audit: { category: "state", verb: "write", owner: "vib-runtime", sessionId: resolvedSessionId },
	});
...
function chooseReceiptKind(
	...
	if (plan.vibGoalMode === "per-story") return "per-goal";
...
function buildCompletionReceipt(input: {
	...
	return {
		...
		vibGoalMode: input.plan.vibGoalMode,
		vibObjective: input.plan.vibObjective,
...
function parseValidationBatchInput(
	...
	if (vibGoalMode !== "aggregate") throw new Error("validation batches require aggregate ultragoal mode");
...
function normalizePlan(raw: unknown): UltragoalPlan {
	...
	const vibGoalMode = record.vibGoalMode === "per-story" ? "per-story" : "aggregate";
	const vibObjective = nonEmptyString(record.vibObjective) ?? DEFAULT_ULTRAGOAL_OBJECTIVE;
	...
	const aliases = Array.isArray(record.vibObjectiveAliases)
		? record.vibObjectiveAliases.filter(
...
export async function createUltragoalPlan(input: {
	...
	const validationBatches =
		validationBatchInput === undefined
			? []
			: parseValidationBatchInput(validationBatchInput, goalIds, input.vibGoalMode ?? "aggregate");
	...
	const plan: UltragoalPlan = {
		version: 1,
		brief,
		vibGoalMode: input.vibGoalMode ?? "aggregate",
		vibObjective: DEFAULT_ULTRAGOAL_OBJECTIVE,
		goals,
		repositoryBinding,
		createdAt: now,
		updatedAt: now,
...
export function getUltragoalRunCompletionState(
	...
	return {
		...
		needsFinalAggregateReceipt: plan.vibGoalMode === "aggregate" && incompleteGoals.length === 0,
	};
}
...
export async function startNextUltragoalGoal(input: {
	...
	if (!plan) throw new Error("No ultragoal plan found. Run `vib ultragoal create-goals --brief ...` first.");
	...
	if (isFinalAggregate) {
		if (
			...
			found.add(
				"criticReview",
				"terminal_critic_ceiling",
				"checkpoint --status complete blocked: terminal-critic ceiling reached; requires human/leader vib ultragoal record-critic-gate-override before completion",
			);
		}
...
export async function recordUltragoalReviewBlockers(input: {
	...
	if (!prePlan) throw new Error("No ultragoal plan found. Run `vib ultragoal create-goals --brief ...` first.");
...
export async function recordUltragoalCriticVerdict(input: {
	...
	const resolvedSessionId = resolveVibSessionForWrite(input.cwd, {
...
async function readOptionalExecutorQa(cwd: string, value: string | undefined): Promise<JsonObject> {
	if (!value) {
		return {
			...
			e2eCommands: ["vib ultragoal review"],
			redTeamCommands: ["vib ultragoal review"],
			artifactRefs: [],
			contractCoverage: [],
			surfaceEvidence: [],
			adversarialCases: [],
...
function renderUltragoalHelp(args: readonly string[]): string | null {
	...
	if (subject === "checkpoint") {
		return [
			"Run native Vibrato Ultragoal workflow commands",
			"",
			"USAGE",
			"  $ vib ultragoal checkpoint --goal-id <id> --status <status> --evidence <text> [FLAGS]",
			"",
			"FLAGS",
			"      --goal-id=<value>            Durable .vib/ultragoal goal id, e.g. G001",
			...
			"",
			"EXAMPLES",
			'  $ vib ultragoal checkpoint --goal-id G001 --status blocked --evidence "waiting on review"',
			'  $ vib ultragoal checkpoint --goal-id G001 --status complete --evidence "tests passed" --quality-gate-json ./quality-gate.json --json',
			"",
		].join("\n");
	}
	if (subject === "review") {
		return [
			...
			"",
			"USAGE",
			"  $ vib ultragoal review [--pr <n> | --branch <ref>] [--spec <path>] [--executor-qa-json <json-or-path>] [FLAGS]",
			"",
			"FLAGS",
	...
	if (subject === "classify-blocker") {
		return [
			...
			"",
			"USAGE",
			"  $ vib ultragoal classify-blocker --classification <human_blocked|resolvable> --evidence <text> [FLAGS]",
			"",
			"FLAGS",
			...
			"      --goal-id=<value>            Optional durable .vib/ultragoal goal id, e.g. G001",
			"      --json                       Output a machine-readable receipt",
			"",
			"EXAMPLES",
			'  $ vib ultragoal classify-blocker --classification resolvable --evidence "failing test can be fixed autonomously"',
			'  $ vib ultragoal classify-blocker --classification human_blocked --evidence "user must provide production API credentials" --goal-id G001',
			"",
		].join("\n");
	}
	if (subject === "record-critic-verdict") {
		return [
			...
			"",
			"USAGE",
			"  $ vib ultragoal record-critic-verdict --terminus <completion|pause> --verdict <OKAY|ITERATE|REJECT> --evidence <text> [--blockers-json <json>] [--goal-id <id>] [--classification-event-id <id>]",
			"",
			"FLAGS",
			...
			"",
			"EXAMPLES",
			'  $ vib ultragoal record-critic-verdict --terminus completion --verdict OKAY --evidence "all final-aggregate checkpoint evidence is current"',
			"",
		].join("\n");
	}
	if (subject === "record-critic-gate-override") {
		return [
			...
			"",
			"USAGE",
			"  $ vib ultragoal record-critic-gate-override --evidence <text> [--json]",
			"",
			"FLAGS",
			...
			"",
			"EXAMPLES",
			'  $ vib ultragoal record-critic-gate-override --evidence "leader approved another terminal attempt after reviewing all five findings"',
			"",
		].join("\n");
	}
	...
	if (subject === "quality-gate") {
		return [
			...
			"",
			"USAGE",
			"  $ vib ultragoal quality-gate init [--surface <name> ...] --out <path>",
			"  $ vib ultragoal quality-gate source-hash [--json]",
			"  $ vib ultragoal quality-gate validate --quality-gate-json <json-or-path> [--goal-id <id>] [--json]",
			"",
			"FLAGS",
			...
			"",
			"EXAMPLES",
			"  $ vib ultragoal quality-gate init --surface web --surface api --out ./quality-gate.json",
			"  $ vib ultragoal quality-gate source-hash --json",
			"  $ vib ultragoal quality-gate validate --quality-gate-json ./quality-gate.json --json",
			"",
		].join("\n");
	}
	...
	return [
		...
		"",
		"Run `vib ultragoal checkpoint --help`, `vib ultragoal review --help`, `vib ultragoal classify-blocker --help`, `vib ultragoal record-critic-verdict --help`, or `vib ultragoal record-critic-gate-override --help`, or `vib ultragoal quality-gate --help` for command-specific requirements.",
		"",
	].join("\n");
}
...
function renderCompleteHandoff(
	...
	if (json) {
		const receipt: Record<string, unknown> = {
			...
			vib_objective: result.plan.vibObjective,
		...
		if (nextAction.kind === "resolve-blockers" && nextAction.blockedGoals) {
			...
			receipt.recovery_hints = [
				"vib ultragoal classify-blocker --help",
				"vib ultragoal record-review-blockers --help",
				"vib ultragoal steer --kind add_subgoal --help",
				"vib ultragoal steer --kind mark_blocked_superseded --help",
			];
		}
		if (nextAction.kind === "retry-failed" && nextAction.failedGoals) {
			receipt.failed_goal_ids = nextAction.failedGoals.map(goal => goal.id);
			receipt.recovery_hints = ["vib ultragoal complete-goals --retry-failed"];
		}
	...
	if (nextAction.kind === "execute-goal" && nextAction.goal) {
		return [
			...
			`vib-objective=${result.plan.vibObjective}`,
	...
	if (nextAction.kind === "retry-failed" && nextAction.failedGoals && nextAction.failedGoals.length > 0) {
		...
		return [
			...
			`failed-goal-ids=${ids}`,
			"hint=run `vib ultragoal complete-goals --retry-failed` after the failure is addressed",
			"",
		].join("\n");
	}
...
function renderCheckpointContinuation(
	...
	if (status === "complete") {
		if (result.allComplete) {
			...
			lines.push(`Vibrato objective: ${result.plan.vibObjective}`);
			lines.push(
				result.startedNext
					? "The next ultragoal goal is active; continue the current aggregate Vibrato goal and checkpoint this story when verified."
					: "Run `vib ultragoal complete-goals` to activate the next ultragoal story.",
			);
		}
	} else if (status === "failed") {
		lines.push("Resume failed goals with `vib ultragoal complete-goals --retry-failed` after the blocker is fixed.");
...
async function dispatchUltragoalCommand(
	...
	try {
		...
		switch (command) {
			...
			case "create":
			case "create-goals": {
				const mode = flagValue(args, "--vib-goal-mode") === "per-story" ? "per-story" : "aggregate";
			...
			case "quality-gate": {
				...
				if (subcommand !== "validate") {
					return {
						status: 1,
						stderr: `Unknown vib ultragoal quality-gate subcommand: ${subcommand ?? "(missing)"}; supported: init, source-hash, lane-selection, validate\n`,
					};
				}
			...
			default:
				return { status: 1, stderr: `Unknown vib ultragoal command: ${command}\n` };
		}
	} catch (error) {
 ...
 * ultragoal mode-state + active-state/HUD so `vib state ultragoal read`, the
 * skill-tool chain guard, and the HUD chip mirror the plan/ledger. Session scope
 * follows `vib state` (`VIB_SESSION_ID`). This is a derived repair: it never changes
...
async function reconcileUltragoalState(cwd: string, agentDir?: string): Promise<void> {
	...
	try {
		...
		if (summary.vibObjective) payload.vib_objective = summary.vibObjective;
