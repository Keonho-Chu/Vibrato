...
export const VIB_TMUX_LAUNCHED_ENV = "VIB_TMUX_LAUNCHED";
...
function buildPowerShellTmuxExitMarkerFinally(markerPath: string): string {
	...
	return [
		...
		"\t\t$__vibTmuxExitMarker = @{ schema_version = 1; source = 'tmux_inner_shell'; ended_at = (Get-Date).ToUniversalTime().ToString('o'); exit_code = $__vibTmuxExitCode } | ConvertTo-Json -Compress",
		`\t\tSet-Content -LiteralPath ${powershellQuote(markerPath)} -Value $__vibTmuxExitMarker -Encoding UTF8 -ErrorAction Stop`,
		"\t} catch {",
		"\t}",
		"}",
	].join("\n");
...
export function buildWindowsPowerShellInnerCommand({
	...
	const envLines = Object.entries({ [VIB_TMUX_LAUNCHED_ENV]: "1", ...(environment ?? {}) }).map(
	...
	const script = tmuxExitMarkerPath
		? [
				...
				"try {",
				`\t${invocation}`,
				"\tif ($null -ne $LASTEXITCODE) { $__vibTmuxExitCode = $LASTEXITCODE } else { $__vibTmuxExitCode = 1 }",
				buildPowerShellTmuxExitMarkerFinally(tmuxExitMarkerPath),
				"exit $__vibTmuxExitCode",
			].join("\n")
