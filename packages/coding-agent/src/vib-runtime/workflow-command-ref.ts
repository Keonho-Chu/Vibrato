import { CANONICAL_VIB_WORKFLOW_SKILLS, type CanonicalVibWorkflowSkill } from "../skill-state/canonical-skills";
...
const skillPath = (skill: CanonicalVibWorkflowSkill): string =>
	`packages/coding-agent/src/defaults/vib/skills/${skill}/SKILL.md`;

const stateWrite = (skill: CanonicalVibWorkflowSkill): CommandRefCommand => ({
	tokens: ["vib", "state", skill, "write", "--input", `'{"current_phase":"handoff"}'`, "--json"],
	rendered: `vib state ${skill} write --input '{"current_phase":"handoff"}' --json`,
	visibility: "public",
...
const stateHandoff = (
	skill: CanonicalVibWorkflowSkill,
	targets: readonly CanonicalVibWorkflowSkill[],
): CommandRefCommand => ({
	tokens: ["vib", "state", skill, "handoff", "--to", `<${targets.join("|")}>`, "--json"],
	rendered: `vib state ${skill} handoff --to <${targets.join("|")}> --json`,
	visibility: "public",
	includeWhen: "implemented-only",
...
export const WORKFLOW_COMMAND_REF_BLOCKS: readonly CommandRefBlock[] = [
	{
		...
		renderOrder: 10,
		markers: {
			start: "<!-- vib:cmdref:start state -->",
			end: "<!-- vib:cmdref:end state -->",
		},
		commands: [
			stateWrite("deep-interview"),
			{
				...
				rendered:
					"vib deep-interview --write --stage final --slug {slug} --spec <markdown-or-path> --deliberate --json",
				visibility: "public",
				includeWhen: "implemented-only",
		...
		examples: [
			{
				...
				bytes: '```\nvib state deep-interview write --input \'{"current_phase":"handoff"}\' --json\n```',
			},
			{
	...
	},
	{
		...
		renderOrder: 10,
		markers: { start: "<!-- vib:cmdref:start state -->", end: "<!-- vib:cmdref:end state -->" },
		...
		aliasesAndBridges: [
			{
				...
				rendered: "vib state ralplan handoff --to <autoresearch|ultragoal> --json",
			},
		],
		notes: [
		...
		aliasesAndBridges: [
			{
				...
				rendered: "vib state ultragoal handoff --to <ralplan|deep-interview> --json",
			},
		],
		notes: [
	...
	},
	{
		...
		examples: [
			{
				label: "handoff state write",
				bytes: '```\nvib state autoresearch write --input \'{"current_phase":"handoff"}\' --json\n```',
			},
		],
		aliasesAndBridges: [
			{
				...
				rendered: "vib state autoresearch handoff --to <ralplan|deep-interview|ultragoal> --json",
			},
		],
		notes: [
...
export function listCommandRefBlocks(skill?: CanonicalVibWorkflowSkill): CommandRefBlock[] {
	const blocks =
		skill === undefined
...
export function renderCommandRefBlock(skill: CanonicalVibWorkflowSkill, blockId = "state"): RenderedCommandRefBlock {
...
export function isCanonicalVibWorkflowSkill(value: string): value is CanonicalVibWorkflowSkill {
