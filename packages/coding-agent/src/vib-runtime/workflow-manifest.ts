/**
 * TypeScript is the authoritative source of truth for Vibrato workflow manifests.
...
const COMMON_TYPED_ARGS: TypedArgSpec[] = [
	...
	{ name: "mode", type: "enum", enumValues: [...CANONICAL_VIB_WORKFLOW_SKILLS], appliesToVerbs: [...STATE_VERBS] },
	{
		name: "session-id",
		type: "string",
		appliesToVerbs: [
	...
	{ name: "skill", type: "enum", enumValues: [...CANONICAL_VIB_WORKFLOW_SKILLS], appliesToVerbs: ["doctor"] },
...
export const WORKFLOW_MANIFEST: Record<CanonicalVibWorkflowSkill, SkillManifest> = {
...
export function getSkillManifest(skill: CanonicalVibWorkflowSkill): SkillManifest {
	return WORKFLOW_MANIFEST[skill];
}

export function isKnownWorkflowState(skill: CanonicalVibWorkflowSkill, state: string): boolean {
	return WORKFLOW_MANIFEST[skill].states.some(entry => entry.id === state);
}

export function isValidTransition(skill: CanonicalVibWorkflowSkill, from: string, to: string): boolean {
	if (from === to) return true;
	return WORKFLOW_MANIFEST[skill].transitions.some(transition => transition.from === from && transition.to === to);
}

export function listVerbs(skill: CanonicalVibWorkflowSkill): string[] {
	return WORKFLOW_MANIFEST[skill].verbs.map(verb => verb.name);
}

export function typedArgsFor(skill: CanonicalVibWorkflowSkill, verb: string): TypedArgSpec[] {
