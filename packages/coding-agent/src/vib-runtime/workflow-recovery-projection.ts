...
export async function projectUltragoalRun(input: {
	...
	const scope: WorkflowRecoveryScopeItem[] = [
		{
			kind: "accepted",
			text: boundText(plan.vibObjective, MAX_OBJECTIVE_CHARS) ?? "Ultragoal aggregate run",
		},
	];
	...
	const projection: Omit<WorkflowRecoveryProjection, "zeroProgress"> = {
		...
		objective: boundText(plan.vibObjective, MAX_OBJECTIVE_CHARS) ?? "Ultragoal aggregate run",
		scope,
		acceptanceCriteria: acceptance,
