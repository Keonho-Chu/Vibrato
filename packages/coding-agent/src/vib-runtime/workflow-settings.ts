 ...
 *   2. user `<agentDir>/config.yml` (default `~/.vib/agent/config.yml`)
 *   3. built-in default
 *
 ...
 * (a later `vib config unset` sticks), so an owned key is never resurrected;
 * the project legacy sits above the agent config.yml, the config-root legacy
 * below it. Settings therefore writes exactly what the runtimes read: `vib
 * config set vib.ralplan.maxIterations 7` is honored by ralplan.
 *
 ...
 * pure `vibRoot`/`dirs` utilities, never `Settings`, discovery/capability
...
import { getAgentDir, getConfigRootDir, isEnoent, resolveEquivalentPath, standardizeMacOSPath } from "@vib-rato/utils";
import { YAML } from "bun";
import { vibRoot } from "./session-layout";
...
const LAYER_CANDIDATES: ReadonlyArray<ResolverCandidate> = [
	{ layer: "project-config", format: "yaml", buildPath: cwd => path.resolve(vibRoot(cwd), "config.yml") },
];

/**
 * The project migration's per-key ownership marker (`.vib/state/
...
async function projectKeyMigrated(cwd: string, key: WorkflowSettingKey): Promise<boolean> {
	let raw: string;
	try {
		raw = await Bun.file(path.join(vibRoot(cwd), "state", "settings.json.migrated-keys")).text();
	} catch (error) {
 ...
 * published to (e.g. an unwritable project `.vib` or config root). The per-key
 * ownership marker consulted per candidate suppresses a key the migration DID
 * complete for, so a deliberate `vib config unset` still sticks.
 */
 ...
 * effective override. Flat keys are checked before the nested `vib: { ... }`
...
export function extractWorkflowSetting(
	...
	if (segments.length < 2 || segments[0] !== "vib") return { present: false, value: undefined };
	// A PRESENT but non-mapping `vib` (or any intermediate segment) is a
	...
	const hasVib = Object.hasOwn(settings, "vib");
	const vib = settings.vib;
	if (vib === null || typeof vib !== "object" || Array.isArray(vib)) {
		return hasVib
			? { present: false, value: undefined, malformedParent: true }
			: { present: false, value: undefined };
	}
	let current: unknown = vib;
 ...
 * `.vib/config.yml`, then user `<agentDir>/config.yml`, then the built-in
...
export async function resolveWorkflowSetting<T>(
	...
	// `.vib` or config root rejected the write, so the source was retained). A
	// completed migration owns the value instead: the project per-key
	// migrated-keys marker suppresses an owned key (a later `vib config unset`
	...
	const projectSettingsJson = path.resolve(vibRoot(cwd), "settings.json");
	...
	// When the config-root source ALIASES the project source (Vibrato run from the
	// config root itself, or a symlink/hardlink to it), a project-owned key must
	// not resurrect through the agent slot either after `vib config unset`
	...
	for (const candidate of candidates) {
		...
		if (candidate.layer === "agent-config" && invalidPolicy === "throw" && !(await projectKeyMigrated(cwd, key))) {
			const retained = await readRetainedStrictEvidence(
				path.join(vibRoot(cwd), "state", STRICT_INVALID_EVIDENCE_FILENAME),
				key,
			);
			if (retained) {
				throw new WorkflowSettingError({
					...
					reason: retained.malformed
						? `retained malformed project .vib/settings.json; repair or remove the source and reload`
						: `retained invalid ${key} value from the retired project .vib/settings.json; repair or remove the source and reload`,
				});
			}
		}
		...
		if (extracted.malformedParent) {
			// A present non-mapping parent (e.g. `vib: "invalid"` or
			// `vib: { ralplan: [] }`) is a malformed explicit layer: strict
		...
		// `vib.ralplan.maxIterations: "7"`) is coerced to a number, exactly as
	...
	// retained an invalid STRICT ralplan legacy value (so `vib ralplan` can
