...
import { Args, Command, type CommandEntry, Flags } from "@vib-rato/utils/cli";
...
async function makeTempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-completion-test-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("Vibrato inshellisense completion spec", () => {
	it("maps command aliases, root flags, option args, and variadic args into Fig shape", async () => {
		const spec = await buildVibFigSpec(fakeEntries());

		expect(spec.name).toBe("vib");
	...
	it("keeps command-name completion available when optional command metadata cannot load", async () => {
		const spec = await buildVibFigSpec([
	...
	it("exports the real Vibrato command surface, aliases, and root launch flags", async () => {
		...
		// the fast-help/completion mirror this spec builds from, so `vib --help` and shell
	...
	it("installs a Vibrato-only index and spec into an empty target directory", async () => {
		const dir = await makeTempDir();
		const spec = await buildVibFigSpec(fakeEntries());

		const result = await installVibInshellisenseSpec(spec, { dir });

		expect(result).toMatchObject({ directory: dir, indexStatus: "created" });
		expect(await Bun.file(path.join(dir, "index.js")).text()).toContain("Generated by vib completion inshellisense");
		expect(await Bun.file(path.join(dir, "vib.js")).text()).toContain("web-search");
	});
	...
	it("refuses to clobber an unrelated inshellisense index without force", async () => {
		...
		await expect(installVibInshellisenseSpec(await buildVibFigSpec(fakeEntries()), { dir })).rejects.toThrow(
			/does not list vib/,
		);
	...
	it("overwrites an unrelated index only when force is explicit", async () => {
		...
		const result = await installVibInshellisenseSpec(await buildVibFigSpec(fakeEntries()), { dir, force: true });

		expect(result.indexStatus).toBe("overwritten");
		expect(await Bun.file(path.join(dir, "index.js")).text()).toContain('export default ["vib"]');
	});
});
