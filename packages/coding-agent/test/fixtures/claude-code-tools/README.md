...
validation test. Vibrato's inline `Monitor` tool must mirror the `monitor` surface
exactly; any divergence there is a bug.

> **Cron divergence:** Vibrato intentionally consolidates upstream's `CronCreate` /
...
> validates those frozen JSON records, not Vibrato's live `cron` tool shape.
...
- No fallback or Vibrato-original tool surface may ship.
