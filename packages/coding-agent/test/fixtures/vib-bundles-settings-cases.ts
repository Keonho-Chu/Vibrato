import type {
	...
	VibRuntimeSnapshotState,
	VibUpdatePreview,
} from "../../src/extensibility/vib-plugins/types";

export interface VibBundleSettingsFixture {
	bundles: VibBundleSummary[];
	updatePreview: VibUpdatePreview | null;
	runtime: VibRuntimeSnapshotState;
}

interface VibBundleSettingsState {
	id: string;
	description: string;
	fixture: VibBundleSettingsFixture;
}

interface VibBundleSettingsViewport {
	id: string;
	cols: number;
	rows: number;
}

interface VibBundleSettingsVariant {
	id: string;
	stateId: string;
	viewportId: string;
...
interface VibBundleSettingsEntry {
	entryId: string;
	stateId: string;
	viewportId: string;
	renderMode: "ascii-no-color" | "unicode-color";
...
const EMPTY_RUNTIME: VibRuntimeSnapshotState = { status: "unavailable" };

function surface(extensionId: string, name: string, enabled = true, quarantined = false): VibBundleSurfaceSummary {
	return {
		extensionId,
		kind: "tool",
		name,
...
function bundle(
	...
	return {
		identity: { kind: "vib-bundle", scope, name },
		version: "1.2.0",
...
const UPDATE_PREVIEW: VibUpdatePreview = {
...
const CURRENT_RUNTIME: VibRuntimeSnapshotState = {
	status: "current",
	snapshot: {
		generation: 7,
		findings: [
...
function fixture(
	bundles: VibBundleSummary[] = [STANDARD_BUNDLE],
	updatePreview: VibUpdatePreview | null = null,
	runtime: VibRuntimeSnapshotState = EMPTY_RUNTIME,
): VibBundleSettingsFixture {
...
export const VIB_BUNDLE_SETTINGS_STATES = Object.freeze([
...
] satisfies VibBundleSettingsState[]);

export const VIB_BUNDLE_SETTINGS_VIEWPORTS = Object.freeze([
	{ id: "80x24", cols: 80, rows: 24 },
	{ id: "120x36", cols: 120, rows: 36 },
	{ id: "160x48", cols: 160, rows: 48 },
] satisfies VibBundleSettingsViewport[]);

export const VIB_BUNDLE_SETTINGS_VARIANTS = Object.freeze([
...
] satisfies VibBundleSettingsVariant[]);

export const VIB_BUNDLE_SETTINGS_ENTRIES = Object.freeze([
	...VIB_BUNDLE_SETTINGS_STATES.flatMap(state =>
		VIB_BUNDLE_SETTINGS_VIEWPORTS.map(viewport => ({
	...
	...VIB_BUNDLE_SETTINGS_VARIANTS.map(variant => ({
...
] satisfies VibBundleSettingsEntry[]);
