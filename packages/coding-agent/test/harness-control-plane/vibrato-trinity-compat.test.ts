/**
 * Vibrato Trinity compatibility tests.
 *
 * The vibrato receipt runtime (separate repo) consumes ReceiptEnvelope JSON
 ...
 * golden fixtures consumed by vibrato's connector tests must always validate
...
async function loadGoldenReceipt(): Promise<ReceiptEnvelope<Record<string, unknown>>> {
	return (await Bun.file(
		path.join(FIXTURES_DIR, "vibrato-trinity-completion-receipt.json"),
	).json()) as ReceiptEnvelope<Record<string, unknown>>;
}

describe("vibrato trinity wire contract", () => {
	...
	it("golden envelope carries every field the vibrato whitelist mapper requires", async () => {
		const golden = await loadGoldenReceipt();
	...
	it("the supported-harness seam is unchanged", () => {
		expect(SUPPORTED_HARNESSES).toEqual(["vib-rato"]);
	});
