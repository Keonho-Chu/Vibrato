...
const TELEGRAM_ORCHESTRATION_ENV_KEYS = [
	...
	"VIB_COORDINATOR_SESSION_STATE_FILE",
...
export function withTelegramOrchestrationProvenance<T>(run: () => T): T {
	const previousCoordinatorSessionId = process.env.VIB_COORDINATOR_SESSION_ID;
	const previousSessionId = process.env.VIB_SESSION_ID;
	process.env.VIB_COORDINATOR_SESSION_ID = "test-telegram-orchestration";
