...
async function freshRoot(): Promise<string> {
	tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vib-hermes-agent-dir-"));
	return tempRoot;
}

describe("vib setup hermes --coding-agent-dir", () => {
	afterEach(cleanup);
	...
	it("renders VIB_CODING_AGENT_DIR next to VIB_COORDINATOR_MCP_STATE_ROOT when set", async () => {
		...
		expect(preview).toContain(`VIB_COORDINATOR_MCP_STATE_ROOT: ${stateRoot}`);
		expect(preview).toContain(`VIB_CODING_AGENT_DIR: ${agentDir}`);
	});

	it("omits VIB_CODING_AGENT_DIR when the flag is unset", async () => {
		const root = await freshRoot();
		const result = await runHermesSetup({ json: true, root: [root] });

		const preview = result.previews.find(entry => entry.path.endsWith(".yaml"))?.content ?? "";
		expect(preview).not.toContain("VIB_CODING_AGENT_DIR");
		expect(preview).not.toContain("VIB_COORDINATOR_MCP_STATE_ROOT");
	});
	...
	it("keeps the two variables independent: setting one never renders the other", async () => {
		...
		expect(agentOnlyPreview).toContain("VIB_CODING_AGENT_DIR");
		expect(agentOnlyPreview).not.toContain("VIB_COORDINATOR_MCP_STATE_ROOT");
		...
		expect(stateOnlyPreview).toContain("VIB_COORDINATOR_MCP_STATE_ROOT");
		expect(stateOnlyPreview).not.toContain("VIB_CODING_AGENT_DIR");
	});
	...
	it("preserves an existing VIB_CODING_AGENT_DIR on install when the flag is omitted", async () => {
		const root = await freshRoot();
		const profileDir = path.join(root, "profile");
		...
		expect(preserved.VIB_CODING_AGENT_DIR).toBe(operatorDir);

		// Idempotent: a third install without the flag still keeps it.
		await runHermesSetup({ install: true, root: [root], profileDir });
		expect((await readServerEnv(profileDir)).VIB_CODING_AGENT_DIR).toBe(operatorDir);
	});
	...
	it("overrides the managed value when --coding-agent-dir is provided", async () => {
		...
		expect((await readServerEnv(profileDir)).VIB_CODING_AGENT_DIR).toBe(agentNew);
		// The overridden value itself survives a later flag-less re-install.
		await runHermesSetup({ install: true, root: [root], profileDir });
		expect((await readServerEnv(profileDir)).VIB_CODING_AGENT_DIR).toBe(agentNew);
	});
	it("does not preserve a foreign VIB_CODING_AGENT_DIR from an unmanaged block under --force", async () => {
		const root = await freshRoot();
		...
		await Bun.write(
			...
			YAML.stringify({
				mcp_servers: {
					vib_coordinator: {
						command: "custom",
						env: { VIB_CODING_AGENT_DIR: path.join(root, "foreign-agent") },
					},
				},
			}),
		);
		...
		// Preserve is scoped to blocks Vibrato signed itself; --force re-renders from
		// flags alone instead of adopting an untrusted env value.
		expect(parsed.mcp_servers.vib_coordinator?.env?.VIB_CODING_AGENT_DIR).toBeUndefined();
	});
	...
	it("refuses a tampered managed block instead of preserving the edited value", async () => {
		...
		const tamperedConfig = (await Bun.file(configPath).text()).replace(
			`VIB_CODING_AGENT_DIR: ${path.join(root, "agent-a")}`,
			"VIB_CODING_AGENT_DIR: relative-injection",
		);
	...
	it("normalizes the rendered value to an absolute resolved path", () => {
		const root = "/tmp/vib-hermes-abs";
		const spec = buildHermesSetupSpec({
			root: ["/tmp/vib-hermes-roots"],
			codingAgentDir: path.join(root, "nested", ".."),
		});
		expect(spec.codingAgentDir).toBe("/tmp/vib-hermes-abs");
	});

	it("rejects relative paths", () => {
		const root = "/tmp/vib-hermes-roots";
		expect(() => buildHermesSetupSpec({ root: [root], codingAgentDir: "relative/agent" })).toThrow(
			/--coding-agent-dir must be an absolute path/,
		);
		expect(() => buildHermesSetupSpec({ root: [root], codingAgentDir: "~/vib-agent" })).toThrow(
			/--coding-agent-dir must be an absolute path/,
		);
	});
	...
	it("accepts Windows drive and UNC spellings as absolute on win32 path rules", () => {
		...
		const winDrive = "C:\\vib\\agent";
		const winUnc = "\\\\server\\share\\vib-agent";
	...
	async function readServerEnv(profileDir: string): Promise<Record<string, string>> {
		...
		return parsed.mcp_servers.vib_coordinator?.env ?? {};
	}
});
