...
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the vib path that
...
// does not follow filesystem links, so it misclassified Bun-installed vib
// as "binary" and tried to swap vib.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	it("classifies vib reached through a symlinked bin dir as bun-managed", () => {
	...
	it("classifies vib at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const ompAtReal = path.join(realBinDir, "vib");
