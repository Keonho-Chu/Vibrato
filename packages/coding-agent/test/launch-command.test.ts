...
import { persistCoordinatorLaunchFailure } from "@vib-rato/coding-agent/commands/launch";
import {
...
describe("persistCoordinatorLaunchFailure", () => {
	it("persists the exact managed owner generation without normalizing it", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-launch-error-"));
		try {
			...
			await persistCoordinatorLaunchFailure(new Error("launch_failed: detail"), root, {
				...
				[VIB_COORDINATOR_SESSION_ID_ENV]: "coordinator-123",
