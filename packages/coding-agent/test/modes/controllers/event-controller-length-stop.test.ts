...
describe("EventController textless length stop", () => {
	...
	it("uses output-limit wording instead of a successful completion notification", async () => {
		...
		expect(options.env?.VIB_NOTIFICATION_TITLE).toBe("length-stop-test: Output limit reached");
		expect(options.env?.VIB_NOTIFICATION_BODY).toBe(TEXTLESS_LENGTH_WARNING);
	});
});
