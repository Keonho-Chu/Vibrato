...
import { initializeExtensions } from "@vib-rato/coding-agent/modes/runtime-init";
...
describe("custom tool lifecycle error boundaries", () => {
	...
	test("hostile custom-tool onSession errors remain non-fatal", async () => {
		const root = await mkdtemp(path.join(os.tmpdir(), "vibrato-sdk-hostile-on-session-"));
	...
	test("denied owned-completion deliveries never allocate an async artifact", async () => {
		...
		const root = await mkdtemp(path.join(os.tmpdir(), "vibrato-sdk-owned-artifact-"));
		temporaryDirectories.push(root);
		const cwd = path.join(root, "project");
		const agentDir = path.join(root, "agent");
		await Promise.all([mkdir(cwd), mkdir(agentDir)]);
