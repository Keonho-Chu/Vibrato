 ...
 * into the operator's live `~/.vib/agent`. The decision is unit-tested here
...
const DEFAULT_AGENT_DIR = path.join(HOME, ".vib", "agent");
...
describe("test agent-dir isolation decision", () => {
	...
	test("isolates an ambient override that only restates the default agent dir", () => {
		// A vib parent process exports this into every child it spawns, so
		// equality with the default carries no test intent.
		expect(
			decideAgentDirIsolation({
				home: HOME,
				env: { VIB_CODING_AGENT_DIR: DEFAULT_AGENT_DIR },
				projectEnv: {},
				realpath: noRealpath,
			}),
	...
	test("isolates a default restated under a custom config dir name", () => {
		expect(
			decideAgentDirIsolation({
				home: HOME,
				env: { VIB_CONFIG_DIR: ".qa-vib", VIB_CODING_AGENT_DIR: path.join(HOME, ".qa-vib", "agent") },
				projectEnv: {},
				realpath: noRealpath,
			}),
		).toEqual({ action: "isolate", reason: "default" });
	...
	test("isolates a symlinked spelling of the default agent dir", () => {
		...
		expect(
			decideAgentDirIsolation({
				home: HOME,
				env: { VIB_CODING_AGENT_DIR: "/link/to/agent" },
				projectEnv: {},
				realpath: () => canonical,
			}),
		).toEqual({ action: "isolate", reason: "default" });
	...
	test("isolates an override planted by the project .env even when non-default", () => {
		...
		expect(
			decideAgentDirIsolation({
				home: HOME,
				env: { VIB_CODING_AGENT_DIR: planted },
				projectEnv: { VIB_CODING_AGENT_DIR: planted },
				realpath: noRealpath,
			}),
	...
	test("honors an explicit trusted non-default pin", () => {
		expect(
			decideAgentDirIsolation({
				home: HOME,
				env: { VIB_CODING_AGENT_DIR: "/tmp/pinned-agent" },
				projectEnv: {},
				realpath: noRealpath,
			}),
	...
	test("a project-.env config dir name does not move the computed default", () => {
		// The name is distrusted, so the default stays under `.vib` and an ambient
		// `.vib/agent` override is still recognized as the default.
		expect(defaultAgentDirFor(HOME, { VIB_CONFIG_DIR: ".planted" }, { VIB_CONFIG_DIR: ".planted" })).toBe(
			DEFAULT_AGENT_DIR,
		);
	});

	test("an escaping config dir name falls back to the default name", () => {
		expect(defaultAgentDirFor(HOME, { VIB_CONFIG_DIR: "../escape" }, {})).toBe(DEFAULT_AGENT_DIR);
	});
});

describe("project .env reader", () => {
	test("parses assignments, strips quotes, and ignores comments", async () => {
		const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "vib-envread-"));
		try {
			await fs.promises.writeFile(
				path.join(dir, ".env"),
				[
					"# comment",
					'export VIB_CODING_AGENT_DIR="/quoted/dir" # inline comment',
			...
			expect(readProjectEnvFile(dir)).toEqual({
				VIB_CODING_AGENT_DIR: "/quoted/dir",
				PI_CONFIG_DIR: ".plain",
			});
		} finally {
	...
	test("a missing .env is an empty record, never a throw", async () => {
		const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "vib-envread-missing-"));
		try {
			expect(readProjectEnvFile(dir)).toEqual({});
		} finally {
			await fs.promises.rm(dir, { recursive: true, force: true });
...
describe("preload fail-closed behavior (real preload path)", () => {
	...
	test("throws and never falls back to the live agent dir when the temp dir cannot be created", async () => {
		...
		// a suite against the operator's live ~/.vib/agent.
		const blocker = await fs.promises.mkdtemp(path.join(os.tmpdir(), "vib-preload-failclosed-"));
		const notADir = path.join(blocker, "not-a-directory");
		await fs.promises.writeFile(notADir, "");
		try {
			const probe = Bun.spawnSync({
				cmd: [process.execPath, "--preload", preload, "-e", "console.log(process.env.VIB_CODING_AGENT_DIR)"],
				env: {
					...process.env,
					TMPDIR: notADir,
					TMP: notADir,
	...
	test("an explicit trusted non-default pin survives the real preload", async () => {
		const pinned = await fs.promises.mkdtemp(path.join(os.tmpdir(), "vib-preload-pinned-"));
		try {
			const probe = Bun.spawnSync({
				cmd: [process.execPath, "--preload", preload, "-e", "console.log(process.env.VIB_CODING_AGENT_DIR)"],
				env: { ...process.env, VIB_CODING_AGENT_DIR: pinned, PI_CODING_AGENT_DIR: "" },
				stdout: "pipe",
				stderr: "pipe",
			});
	...
	test("an ambient default agent dir is replaced by a fresh isolated dir in the real preload", async () => {
		...
		const probe = Bun.spawnSync({
			cmd: [process.execPath, "--preload", preload, "-e", "console.log(process.env.VIB_CODING_AGENT_DIR)"],
			env: { ...process.env, VIB_CODING_AGENT_DIR: defaultAgentDir, PI_CODING_AGENT_DIR: "" },
			stdout: "pipe",
			stderr: "pipe",
		});
		...
		expect(path.basename(adopted).startsWith("vib-test-agent-")).toBe(true);
