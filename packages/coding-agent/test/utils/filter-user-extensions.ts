...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@vib-rato/utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.vib/`), plus
//    the legacy non-XDG `~/.vib/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.vib/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/vib/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.vib` when
...
// symlinks `~/.vib/agent/extensions/foo.ts` to a dotfiles repo would see the
