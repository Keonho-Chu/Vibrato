...
import {
	renderVibBundleSettingsEntry,
	VIB_BUNDLE_SETTINGS_CAPTURE_FILES,
	vibBundleSettingsCapturePlan,
} from "../scripts/capture-vib-bundle-settings";
...
} from "./fixtures/vib-bundles-settings-cases";
...
function sameIdentity(a: VibBundleIdentity, b: VibBundleIdentity): boolean {
...
class InMemoryLifecyclePort implements VibBundleLifecyclePort {
	readonly bundleToggleCalls: Array<{ identity: VibBundleIdentity; enabled: boolean }> = [];
	readonly surfaceToggleCalls: Array<{ identity: VibBundleIdentity; surfaceId: string; enabled: boolean }> = [];
	readonly previewCalls: VibBundleIdentity[] = [];
	bundleToggleGate: Deferred<VibLifecycleResult<VibToggleResult>> | null = null;
	listGate: Deferred<VibBundleSummary[]> | null = null;
	previewGate: Deferred<VibLifecycleResult<VibUpdatePreview>> | null = null;

	constructor(
		private bundles: VibBundleSummary[],
		private updatePreview: VibUpdatePreview | null = null,
	) {}
	...
	toggleBundle(identity: VibBundleIdentity, enabled: boolean): VibLifecycleResult<VibToggleResult> {
		const bundle = this.bundles.find(candidate => sameIdentity(candidate.identity, identity));
		if (!bundle) return { ok: false, error: { code: "not_installed", message: "Not installed." } };
...
function componentFor(
	fixture: VibBundleSettingsFixture,
	options: {
		lifecycle?: InMemoryLifecyclePort;
		runtime?: VibRuntimeSnapshotProvider;
		onClose?: () => void;
		onRenderRequested?: () => void;
	} = {},
): VibBundleSettingsComponent {
	return new VibBundleSettingsComponent(
		"/safe/project",
...
async function ready(component: VibBundleSettingsComponent): Promise<void> {
	await settle();
	expect(component.stateId).not.toBe("loading");
}

function rendered(component: VibBundleSettingsComponent, width = 120): string {
	return Bun.stripANSI(component.render(width).join("\n"));
}

function select(component: VibBundleSettingsComponent, down = 0): void {
	for (let index = 0; index < down; index += 1) component.handleInput("\x1b[B");
	component.handleInput("\n");
}

describe("Vibrato bundle Settings component", () => {
	test("requests repaint after async bundle and plugin list loads", async () => {
		const fixture = VIB_BUNDLE_SETTINGS_STATES.find(state => state.id === "list")!.fixture;
		const lifecycle = new InMemoryLifecyclePort(fixture.bundles.map(cloneSummary));
		lifecycle.listGate = deferred<VibBundleSummary[]>();
		let bundleRenders = 0;
	...
	test("keeps same-name bundles separated by scope when one is toggled", async () => {
		const fixture = VIB_BUNDLE_SETTINGS_STATES.find(state => state.id === "dual-scope-same-name")!.fixture;
		const lifecycle = new InMemoryLifecyclePort(fixture.bundles.map(cloneSummary));
		...
		expect(lifecycle.bundleToggleCalls).toEqual([
			{ identity: { kind: "vib-bundle", scope: "user", name: "shared-name" }, enabled: false },
		]);
	...
	test("renders mismatched runtime generations as unavailable rather than clear", async () => {
		const fixture = VIB_BUNDLE_SETTINGS_STATES.find(state => state.id === "runtime-current")!.fixture;
	...
	test("blocks quarantined enablement but permits disablement", async () => {
		const blockedFixture = VIB_BUNDLE_SETTINGS_STATES.find(state => state.id === "quarantined-blocked")!.fixture;
		const disabledQuarantined = cloneSummary(blockedFixture.bundles[0]!);
		disabledQuarantined.enabled = false;
		const disabledFixture: VibBundleSettingsFixture = { ...blockedFixture, bundles: [disabledQuarantined] };
		...
		const enabledFixture: VibBundleSettingsFixture = { ...disabledFixture, bundles: [enabledQuarantined] };
	...
	test("locks navigation and suppresses duplicate submissions during a mutation", async () => {
		const fixture = VIB_BUNDLE_SETTINGS_STATES.find(state => state.id === "list")!.fixture;
		const lifecycle = new InMemoryLifecyclePort(fixture.bundles.map(cloneSummary));
		lifecycle.bundleToggleGate = deferred<VibLifecycleResult<VibToggleResult>>();
		let closes = 0;
		const component = componentFor(fixture, {
			lifecycle,
			onClose: () => {
	...
	test("discards a late update preview after focus moves to another identity", async () => {
		const fixture = VIB_BUNDLE_SETTINGS_STATES.find(state => state.id === "dual-scope-same-name")!.fixture;
		const lifecycle = new InMemoryLifecyclePort(fixture.bundles.map(cloneSummary));
		lifecycle.previewGate = deferred<VibLifecycleResult<VibUpdatePreview>>();
		const component = componentFor(fixture, { lifecycle });
		await ready(component);
		select(component);
		select(component, 1);
	...
	test("plans exactly four deterministic artifacts for every expanded showcase entry", async () => {
		const plan = vibBundleSettingsCapturePlan(VIB_BUNDLE_SETTINGS_ENTRIES);
		expect(plan).toHaveLength(77 * 4);
		expect(VIB_BUNDLE_SETTINGS_ENTRIES).toHaveLength(77);
		for (const entry of VIB_BUNDLE_SETTINGS_ENTRIES) {
			expect(
				plan
					...
					.sort(),
			).toEqual([...VIB_BUNDLE_SETTINGS_CAPTURE_FILES].sort());
		}
		const first = await renderVibBundleSettingsEntry(VIB_BUNDLE_SETTINGS_ENTRIES[0]!);
		const second = await renderVibBundleSettingsEntry(VIB_BUNDLE_SETTINGS_ENTRIES[0]!);
