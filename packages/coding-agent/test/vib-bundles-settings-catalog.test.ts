...
describe("Vibrato bundle Settings catalog", () => {
	test("has the fixed state, viewport, and variant counts", () => {
		expect(VIB_BUNDLE_SETTINGS_STATES).toHaveLength(23);
		expect(VIB_BUNDLE_SETTINGS_VIEWPORTS).toHaveLength(3);
		expect(VIB_BUNDLE_SETTINGS_VARIANTS).toHaveLength(8);
		expect(VIB_BUNDLE_SETTINGS_VARIANTS.filter(variant => variant.renderMode === "ascii-no-color")).toHaveLength(6);
		const narrowVariants = VIB_BUNDLE_SETTINGS_VARIANTS.filter(variant => variant.viewportId === "48x36");
	...
	test("expands to the fixed 77-entry matrix with stable unique identifiers", () => {
		...
		const entryIds = VIB_BUNDLE_SETTINGS_ENTRIES.map(entry => entry.entryId);
	...
	test("references known states and contains only inert safe fixture strings", () => {
		const stateIds = new Set(VIB_BUNDLE_SETTINGS_STATES.map(state => state.id));
		for (const variant of VIB_BUNDLE_SETTINGS_VARIANTS) expect(stateIds.has(variant.stateId)).toBe(true);

		const forbidden = /:\/\/user:|@[^\s/]+|[?#]|token|\/Users\/|\/home\//i;
		for (const value of stringsIn(VIB_BUNDLE_SETTINGS_STATES)) expect(value).not.toMatch(forbidden);
	});
});
