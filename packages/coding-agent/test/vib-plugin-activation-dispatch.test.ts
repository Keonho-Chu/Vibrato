...
async function tempProjectWithFixture(fixtureName: string): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-plugin-activation-"));
	tempRoots.push(cwd);
...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-plugin-activation-agent-"));
...
describe("Vibrato sub-skill activation dispatch", () => {
	...
	test("rejects duplicate activation args across plugin roots instead of choosing a winner", async () => {
		...
		await expect(
			...
			constructor: VibPluginLoadError,
			code: "duplicate_arg",
		});
	});
