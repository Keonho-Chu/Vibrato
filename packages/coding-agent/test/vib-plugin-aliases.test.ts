 ...
 *
 * Acceptance 1: canonical Vibrato vocabulary and accepted compatibility aliases
...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-alias-agent-"));
...
async function writeManifestBundle(root: string, manifest: Record<string, unknown>): Promise<string> {
	await fs.writeFile(path.join(root, "vibrato-plugin.json"), JSON.stringify(manifest, null, 2));
	return root;
}

function expectAliasError(
	fn: () => unknown,
	code: VibPluginLoadError["code"],
	...
	try {
		...
		const err = error as VibPluginLoadError;
...
describe("issue #4287 acceptance 1: aliases compile to byte-equivalent normalized surfaces", () => {
	test("canonical mcps and mcpServers alias parse to identical manifest entries", () => {
		const canonical = parseManifest(canonicalManifest, "/plugin/canonical/vibrato-plugin.json");
		const aliased = parseManifest(aliasManifest, "/plugin/aliased/vibrato-plugin.json");
	...
	test("canonical mcps and mcpServers alias compile to byte-equivalent surfaces", async () => {
		const canonicalRoot = await mkTemp("vib-alias-canonical-");
		const aliasRoot = await mkTemp("vib-alias-alias-");
		...
		const [canonical, aliased] = await Promise.all([
			compileVibPluginBundle(canonicalRoot),
			compileVibPluginBundle(aliasRoot),
		]);
	...
	test("transport inference is deterministic: command -> stdio, url -> http, explicit type wins", () => {
		const manifest = parseManifest(
			...
			"/plugin/infer/vibrato-plugin.json",
		);
	...
	test("map insertion order does not change normalized surfaces or config hashes", async () => {
		const firstRoot = await mkTemp("vib-alias-order-a-");
		const secondRoot = await mkTemp("vib-alias-order-b-");
		...
		const [first, second] = await Promise.all([
			compileVibPluginBundle(firstRoot),
			compileVibPluginBundle(secondRoot),
		]);
	...
	test("alias determinism does not rewrite the established canonical config hash", async () => {
		const root = await mkTemp("vib-canonical-hash-compat-");
		...
		const bundle = await compileVibPluginBundle(root);
	...
	test("prototype-like server names remain ordinary deterministic entries", () => {
		const manifest = parseManifest(
			...
			"/plugin/prototype-names/vibrato-plugin.json",
		);
...
describe("issue #4287 acceptance 2: ambiguous/unsafe aliases fail with actionable canonical form", () => {
	test("singular mcp alias names both canonical alternatives", () => {
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", mcp: [] },
					"/plugin/x/vibrato-plugin.json",
				),
			"unsupported_surface",
			['"mcp"', '"mcps"', '"mcpServers"'],
		);
	...
	test("mcpServers rejects fields incompatible with the selected transport", () => {
		for (const entry of [
			...
			expectAliasError(
				() =>
					parseManifest(
						{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", mcpServers: { db: entry } },
						"/plugin/x/vibrato-plugin.json",
					),
				"unsupported_surface",
				["cannot preserve", ".vib/mcp.json"],
			);
		}
	});
	...
	test("top-level skills declaration names the loose skill surface and subskills", () => {
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", skills: ["SKILL.md"] },
					"/plugin/x/vibrato-plugin.json",
				),
			"forbidden_surface",
			['"skills"', '"subskills"', ".vib/skills/"],
		);
	});
	...
	test("Claude Code agents/commands/slash-commands keys name their safe alternative", () => {
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", agents: [{ name: "a" }] },
					"/plugin/x/vibrato-plugin.json",
				),
			"forbidden_surface",
		...
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", commands: [{ name: "c" }] },
					"/plugin/x/vibrato-plugin.json",
				),
			"forbidden_surface",
			['"commands"', ".vib/commands/"],
		);
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", "slash-commands": [] },
					"/plugin/x/vibrato-plugin.json",
				),
			"forbidden_surface",
			['"slash-commands"', ".vib/commands/"],
		);
	});
	...
	test("mcps and mcpServers together is a targeted conflict, not a guess", () => {
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", mcps: [], mcpServers: {} },
					"/plugin/x/vibrato-plugin.json",
				),
			"invalid_manifest",
			['"mcps"', '"mcpServers"'],
...
async function compileAliasWithMcpServers(manifest: Record<string, unknown>): Promise<{
	entries: VibPluginMcpManifestEntry[];
	registryEntry: VibPluginRegistryEntry;
	bundle: NormalizedVibPluginBundle;
}> {
	const root = await mkTemp("vib-alias-sec-");
	await writeManifestBundle(root, manifest);
	const bundle = await compileVibPluginBundle(root);
	const registryEntry: VibPluginRegistryEntry = {
		name: bundle.name,
...
describe("issue #4287 acceptance 5: aliases never bypass security or collision authority", () => {
	test("alias MCP entries still pass through the install MCP security policy", async () => {
		...
		expect(() => assertMcpInstallPolicy(entries[0] as VibPluginMcpManifestEntry, { pluginRoot: "/tmp" })).toThrow(
			VibPluginLoadError,
		);
	});
	...
	test("alias MCP names collide with canonical names in the registry authority", async () => {
		...
		const canonicalRoot = await mkTemp("vib-alias-canonical-sec-");
		...
		const canonical = await compileVibPluginBundle(canonicalRoot);
		const canonicalEntry: VibPluginRegistryEntry = {
			name: canonical.name,
		...
		expect(() => validateInstallPlan(aliasedBundle, [canonicalEntry])).toThrow(VibPluginLoadError);
		expect(() => validateInstallPlan(canonical, [aliasedEntry])).toThrow(VibPluginLoadError);
	});

	test("alias env is rejected at parse time, never silently dropped into the registry", async () => {
		const root = await mkTemp("vib-alias-env-");
		await writeManifestBundle(root, {
			kind: "vib-rato-plugin",
			name: "alias-env",
			version: "1.0.0",
		...
		await expect(compileVibPluginBundle(root)).rejects.toThrow(VibPluginLoadError);
	});

	test("alias MCP policy failure leaves the target scope untouched", async () => {
		const cwd = await mkProjectCwd();
		const root = await mkTemp("vib-alias-policy-atomic-");
		await writeManifestBundle(root, {
			kind: "vib-rato-plugin",
		...
		const scopeRoot = path.join(cwd, ".vib", "vib-plugins");
		await expect(fs.stat(scopeRoot)).rejects.toThrow();

		await expect(installVibBundle({ cwd }, "project", root)).rejects.toMatchObject({ code: "security_policy" });
		await expect(fs.stat(scopeRoot)).rejects.toThrow();
	});
});
...
describe("issue #4287 adversarial: shape attacks, credentials, traversal, determinism", () => {
	test("mcpServers as an array is a shape attack and is rejected", () => {
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", mcpServers: [{ command: "bun" }] },
					"/plugin/x/vibrato-plugin.json",
				),
			"invalid_manifest",
			['"mcpServers"', "map"],
	...
	test("canonical mcps as a map is still rejected (shape contract unchanged)", () => {
		expectAliasError(
			() =>
				parseManifest(
					{ kind: "vib-rato-plugin", name: "x", version: "1.0.0", mcps: { docs: { transport: "stdio" } } },
					"/plugin/x/vibrato-plugin.json",
				),
			"invalid_manifest",
	...
	test("alias stdio args cannot traverse out of the plugin root", async () => {
		const root = await mkTemp("vib-alias-traverse-");
		await writeManifestBundle(root, {
			kind: "vib-rato-plugin",
			name: "alias-traverse",
			version: "1.0.0",
	...
	test("compiling the same alias bundle twice is byte-deterministic", async () => {
		const root = await mkTemp("vib-alias-det-");
		await writeManifestBundle(root, aliasManifest);
		await fs.mkdir(path.join(root, "mcp"), { recursive: true });
		await fs.writeFile(path.join(root, "mcp", "server.ts"), "export default {};\n");
		const [a, b] = [await compileVibPluginBundle(root), await compileVibPluginBundle(root)];
	...
	test("force update: alias bundle upgrades through preview/apply with surface deltas", async () => {
		const cwd = await mkProjectCwd();
		const source = await mkTemp("vib-alias-update-src-");
		...
		const installed = await installVibBundle({ cwd }, "project", source);
		...
		const refused = await installVibBundle({ cwd }, "project", source);
...
async function mkProjectCwd(): Promise<string> {
	const cwd = await mkTemp("vib-alias-cwd-");
	await fs.mkdir(path.join(cwd, ".vib"), { recursive: true });
	return cwd;
}

async function installAliasBundle(cwd: string): Promise<string> {
	const root = await mkTemp("vib-alias-install-");
	await writeManifestBundle(root, aliasManifest);
	await fs.mkdir(path.join(root, "mcp"), { recursive: true });
	await fs.writeFile(path.join(root, "mcp", "server.ts"), "export default {};\n");
	const result = await installVibBundle({ cwd }, "project", root);
...
describe("issue #4287 acceptance 4: install lifecycle remains covered for alias bundles", () => {
	test("alias bundle installs, normalizes in the registry, toggles, and uninstalls", async () => {
		...
		const identity = { kind: "vib-bundle", scope: "project", name: "alias-equivalence" } as const;
		const disabled = await setVibBundleSurfaceEnabled({ cwd }, identity, "mcp:docs", false);
		...
		const uninstalled = await uninstallVibBundle({ cwd }, identity);
	...
	test("prototype-like alias names remain own keys in the runtime MCP config map", async () => {
		const cwd = await mkProjectCwd();
		const root = await mkTemp("vib-alias-prototype-runtime-");
		await writeManifestBundle(root, {
			kind: "vib-rato-plugin",
		...
		await fs.mkdir(path.join(root, "mcp"), { recursive: true });
		await fs.writeFile(path.join(root, "mcp", "server.ts"), "export default {};\n");
		const installed = await installVibBundle({ cwd }, "project", root);
		expect(installed.ok).toBe(true);
