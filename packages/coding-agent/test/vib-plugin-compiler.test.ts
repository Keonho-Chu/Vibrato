...
describe("Vibrato plugin compiler", () => {
	test("compiles a valid six-surface bundle with extension ids and digests", async () => {
		const bundle = await compileVibPluginBundle(sixSurface);
		...
		expect(bundle.files.some(f => f.relativePath === "vibrato-plugin.json")).toBe(true);
	...
	test("never imports plugin tool/hook code during compile", async () => {
		const sentinel = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "vib-sentinel-")), "sentinel.txt");
		...
		try {
			await compileVibPluginBundle(sixSurface);
		} finally {
	...
	test("rejects a path that escapes the plugin root", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-escape-"));
		tempDirs.push(dir);
		await fs.writeFile(
	...
	test("rejects a declared sha256 that does not match file content", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-hash-"));
		tempDirs.push(dir);
	...
	test("rejects appendix with both path and content", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-appendix-"));
		tempDirs.push(dir);
		await fs.mkdir(path.join(dir, "prompts"), { recursive: true });
