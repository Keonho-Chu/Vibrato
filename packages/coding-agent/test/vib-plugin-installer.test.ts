...
async function mkProjectCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-install-"));
	tempDirs.push(cwd);
	return cwd;
}
...
async function mkGitDaemonRepo(manifest: object): Promise<{ url: string; repoDir: string; stop: () => Promise<void> }> {
	const base = await fs.mkdtemp(path.join(os.tmpdir(), "vib-git-src-"));
	tempDirs.push(base);
	const repoDir = path.join(base, "plugin-repo");
	await fs.mkdir(repoDir, { recursive: true });
	await fs.writeFile(path.join(repoDir, "vibrato-plugin.json"), JSON.stringify(manifest));
...
describe("Vibrato plugin installer", () => {
	test("installs a local-path bundle into the project scope", async () => {
		...
		const installedDir = path.join(cwd, ".vib", "vib-plugins", "valid-six-surface-bundle");
		expect(await exists(path.join(installedDir, "vibrato-plugin.json"))).toBe(true);
	...
	test("reinstalling identical content requires upgrade and an upgrade is unchanged", async () => {
		...
		const first = await installVibBundle(ctx, "project", sixSurface);
		expect(first.ok).toBe(true);

		const before = await getVibBundle(ctx, identity);
		expect(before.ok).toBe(true);
		if (!before.ok) throw new Error(before.error.code);
		const second = await installVibBundle(ctx, "project", sixSurface);
		expect(second).toMatchObject({ ok: false, error: { code: "already_installed_use_upgrade" } });
		const after = await getVibBundle(ctx, identity);
	...
	test("reinstalling different content requires upgrade and preview/apply updates", async () => {
		const cwd = await mkProjectCwd();
		const ctx = { cwd };
		const source = await fs.mkdtemp(path.join(os.tmpdir(), "vib-modsrc-"));
		tempDirs.push(source);
		await fs.cp(sixSurface, source, { recursive: true });
		const identity = bundleIdentity("project", "valid-six-surface-bundle");
		const first = await installVibBundle(ctx, "project", source);
		expect(first.ok).toBe(true);

		await fs.appendFile(path.join(source, "prompts", "system-appendix.md"), "\nExtra policy line.\n");
		const reinstall = await installVibBundle(ctx, "project", source);
		...
		const summary = await getVibBundle(ctx, identity);
	...
	test("a bad bundle leaves no files and no registry entry", async () => {
		const cwd = await mkProjectCwd();
		const bad = await fs.mkdtemp(path.join(os.tmpdir(), "vib-bad-"));
		tempDirs.push(bad);
		await fs.writeFile(
			path.join(bad, "vibrato-plugin.json"),
			JSON.stringify({ kind: "vib-rato-plugin", name: "bad-bundle", version: "1.0.0", agents: [] }),
		);
		await expect(installVibBundle({ cwd }, "project", bad)).rejects.toBeInstanceOf(VibPluginLoadError);

		expect(await exists(path.join(cwd, ".vib", "vib-plugins", "bad-bundle"))).toBe(false);
		const registry = await readRegistry("project", cwd);
	...
	test("install never imports plugin code", async () => {
		const cwd = await mkProjectCwd();
		const sentinelDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-install-sentinel-"));
		tempDirs.push(sentinelDir);
		const sentinel = path.join(sentinelDir, "sentinel.txt");
		const prev = process.env.VIB_TEST_IMPORT_SENTINEL;
		process.env.VIB_TEST_IMPORT_SENTINEL = sentinel;
		try {
			await installVibBundle({ cwd }, "project", sixSurface);
		} finally {
			if (prev === undefined) delete process.env.VIB_TEST_IMPORT_SENTINEL;
			else process.env.VIB_TEST_IMPORT_SENTINEL = prev;
		}
	...
	test("installs from a tarball through the same validate step", async () => {
		const cwd = await mkProjectCwd();
		const tarDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tar-"));
		...
		const result = await installVibBundle({ cwd }, "project", tarball);
		expect(result).toMatchObject({ ok: true, value: { status: "installed" } });
		expect(await isVibPluginBundleSource(tarball)).toBe(true);
		const registry = await readRegistry("project", cwd);
	...
	test("installs a git source bundle via a local git daemon", async () => {
		...
		const cwd = await mkProjectCwd();
		try {
			const result = await installVibBundle({ cwd }, "project", served.url);
			expect(result.ok).toBe(true);
			if (!result.ok) throw new Error(result.error.code);
			expect(result.value.status).toBe("installed");
			expect(result.value.summary.source.kind).toBe("git");

			const installedDir = path.join(cwd, ".vib", "vib-plugins", "git-source-bundle");
			expect(await exists(path.join(installedDir, "vibrato-plugin.json"))).toBe(true);
	...
	test("preview and apply agree on unavailable versus reachable malformed tar and git sources", async () => {
		...
		const validTarDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-classify-tar-"));
		...
		expect((await installVibBundle({ cwd: unavailableTarCwd }, "project", validTar)).ok).toBe(true);
		await fs.rm(validTar);
		const tarIdentity = bundleIdentity("project", "valid-six-surface-bundle");
		expect(await previewVibBundleUpdate({ cwd: unavailableTarCwd }, tarIdentity)).toMatchObject({
			ok: false,
			error: { code: "source_unavailable" },
		});
		expect(await applyVibBundleUpdate({ cwd: unavailableTarCwd }, applyToken(tarIdentity))).toMatchObject({
			ok: false,
			error: { code: "source_unavailable" },
		});

		const malformedTarDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-classify-malformed-tar-"));
		...
		const malformedSource = await fs.mkdtemp(path.join(os.tmpdir(), "vib-classify-malformed-source-"));
		...
		expect((await installVibBundle({ cwd: malformedTarCwd }, "project", validTar2)).ok).toBe(true);
		await fs.copyFile(malformedTar, validTar2);
		expect(await previewVibBundleUpdate({ cwd: malformedTarCwd }, tarIdentity)).toMatchObject({
			ok: false,
			error: { code: "invalid_target" },
		});
		expect(await applyVibBundleUpdate({ cwd: malformedTarCwd }, applyToken(tarIdentity))).toMatchObject({
			ok: false,
			error: { code: "invalid_target" },
		});
		...
		expect((await installVibBundle({ cwd: unavailableGitCwd }, "project", unavailableGit.url)).ok).toBe(true);
		await unavailableGit.stop();
		const gitIdentity = bundleIdentity("project", "git-classify");
		expect(await previewVibBundleUpdate({ cwd: unavailableGitCwd }, gitIdentity)).toMatchObject({
			ok: false,
			error: { code: "source_unavailable" },
		});
		expect(await applyVibBundleUpdate({ cwd: unavailableGitCwd }, applyToken(gitIdentity))).toMatchObject({
			ok: false,
			error: { code: "source_unavailable" },
		});
		...
		expect((await installVibBundle({ cwd: malformedGitCwd }, "project", malformedGit.url)).ok).toBe(true);
		await fs.rm(path.join(malformedGit.repoDir, "vibrato-plugin.json"));
		...
		expect(await previewVibBundleUpdate({ cwd: malformedGitCwd }, malformedGitIdentity)).toMatchObject({
			ok: false,
			error: { code: "invalid_target" },
		});
		expect(await applyVibBundleUpdate({ cwd: malformedGitCwd }, applyToken(malformedGitIdentity))).toMatchObject({
			ok: false,
			error: { code: "invalid_target" },
		});
	...
	test("an unavailable git source maps to a typed missing_file install error", async () => {
		...
		try {
			await expect(installVibBundle({ cwd }, "project", rejectingServer.url)).rejects.toMatchObject({
				code: "missing_file",
				name: "VibPluginLoadError",
			});
		} finally {
		...
		const registry = await readRegistry("project", cwd);
		expect(registry.plugins).toEqual([]);
		expect(await exists(path.join(cwd, ".vib", "vib-plugins", "no-such-repo"))).toBe(false);
	});
});
