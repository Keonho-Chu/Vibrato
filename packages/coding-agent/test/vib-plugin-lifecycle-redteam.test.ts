...
import {
	...
	type VibReviewedUpdateToken,
} from "../src/extensibility/vib-plugins";
import { runVibBundleTransaction } from "../src/extensibility/vib-plugins/installer";
import { writeRegistry } from "../src/extensibility/vib-plugins/registry";
...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-lifecycle-redteam-agent-"));
...
async function mkProjectCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-lifecycle-redteam-project-"));
	tempDirs.push(cwd);
	return cwd;
}

async function mkSource(): Promise<string> {
	const source = await fs.mkdtemp(path.join(os.tmpdir(), "vib-lifecycle-redteam-source-"));
...
async function writeToolsOnlyManifest(source: string, version: string, tools: string): Promise<void> {
	...
	await fs.writeFile(
		path.join(source, "vibrato-plugin.json"),
		`{
...
async function install(cwd: string, scope: "project" | "user", source = sixSurface): Promise<VibBundleIdentity> {
...
async function injectQuarantine(
	...
	entries: VibPluginRegistryEntry["quarantine"],
): Promise<void> {
...
function changedToken(token: VibReviewedUpdateToken, changes: Partial<VibReviewedUpdateToken>): VibReviewedUpdateToken {
	return { ...token, ...changes };
}

describe("Vibrato bundle lifecycle adversarial invariants", () => {
	test("keeps same-name user and project bundles byte-isolated through update, toggle, and quarantine", async () => {
		...
		const preview = await previewVibBundleUpdate({ cwd }, project);
		...
		expect(await setVibBundleSurfaceEnabled({ cwd }, project, projectSurface, false)).toMatchObject({ ok: true });
		expect(JSON.stringify(await summary(cwd, user))).toBe(userBefore);
	...
	test("refuses scope/name-swapped, fingerprint-forged, and replayed update tokens without mutation", async () => {
		...
		const userBefore = JSON.stringify(await summary(cwd, user));
		const wrongScope = await applyVibBundleUpdate({ cwd }, changedToken(preview.value.token, { identity: user }));
		...
		const wrongName = await applyVibBundleUpdate(
			{ cwd },
		...
		const forged = await applyVibBundleUpdate(
			{ cwd },
		...
		const replay = await applyVibBundleUpdate({ cwd }, preview.value.token);
	...
	test("the transaction primitive is not publicly reachable, so create-only cannot be bypassed", async () => {
		...
		const publicInstall = await installVibBundle({ cwd }, "project", source);
		...
		const bypass = await runVibBundleTransaction(source, {
			scope: "project",
			cwd,
		...
		// vib-plugin-public-boundary.test.ts. Assert the barrel here too, so this
		// test fails if the primitive is ever re-exported.
		const barrel: Record<string, unknown> = await import("../src/extensibility/vib-plugins");
		for (const forbidden of ["runVibBundleTransaction", "resolveVibBundleCandidate", "candidateRegistryEntry"]) {
	...
	test("reconciles disjoint and re-added surfaces while deduplicating quarantine", async () => {
		...
		expect(await setVibBundleSurfaceEnabled({ cwd }, identity, removedId, false)).toMatchObject({ ok: true });
		await fs.writeFile(path.join(source, "tools", "additional.ts"), "export const additional = true;\n");
		await writeToolsOnlyManifest(source, "2.0.0", '[{ "name": "additional", "path": "tools/additional.ts" }]');
		let preview = await previewVibBundleUpdate({ cwd }, identity);
		expect(preview.ok).toBe(true);
		if (!preview.ok) throw new Error(preview.error.code);
		expect(await applyVibBundleUpdate({ cwd }, preview.value.token)).toMatchObject({ ok: true });
		...
		preview = await previewVibBundleUpdate({ cwd }, identity);
		expect(preview.ok).toBe(true);
		if (!preview.ok) throw new Error(preview.error.code);
		expect(await applyVibBundleUpdate({ cwd }, preview.value.token)).toMatchObject({ ok: true });
	...
	test("blocks quarantined enables but always permits de-escalating disables", async () => {
		...
		expect(await setVibBundleEnabled({ cwd }, identity, true)).toMatchObject({
			ok: false,
	...
	test("serializes concurrent applies and toggles into a parseable legal state", async () => {
		...
		const applied = await Promise.all([
			applyVibBundleUpdate({ cwd }, preview.value.token),
			applyVibBundleUpdate({ cwd }, preview.value.token),
		]);
		...
		const toggles = await Promise.all([
			setVibBundleEnabled({ cwd }, identity, false),
			setVibBundleEnabled({ cwd }, identity, true),
		]);
		...
		expect(
			(await listVibBundles({ cwd })).find(bundle => bundle.identity.scope === "project")?.targetFingerprint,
