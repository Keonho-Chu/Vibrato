...
import * as vibPluginBarrel from "../src/extensibility/vib-plugins";
import {
	discoverVibPluginRoots,
	VibPluginLoadError,
	type VibPluginLoadErrorCode,
} from "../src/extensibility/vib-plugins";
import { loadVibPlugin, loadVibPlugins } from "../src/extensibility/vib-plugins/loader";
...
async function copyFixtureToProject(fixtureName: string): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-plugin-loader-"));
	tempRoots.push(cwd);
	const pluginsDir = path.join(cwd, ".vib", "vib-plugins");
	await fs.mkdir(pluginsDir, { recursive: true });
	await fs.cp(path.join(fixturesRoot, fixtureName), path.join(pluginsDir, fixtureName), { recursive: true });
	return cwd;
}

async function expectLoadError(root: string, code: VibPluginLoadErrorCode): Promise<void> {
	try {
		await loadVibPlugin(root);
	} catch (error) {
...
describe("Vibrato plugin loader", () => {
	test("does not expose the legacy executable loader through the public barrel", () => {
		expect(Object.hasOwn(vibPluginBarrel, "loadVibPlugin")).toBe(false);
		expect(Object.hasOwn(vibPluginBarrel, "loadVibPlugins")).toBe(false);
	});
	test("loads valid skill and agent plugin fixtures", async () => {
		const skill = await loadVibPlugin(path.join(fixturesRoot, "valid-skill-plugin"));
		...
		const agent = await loadVibPlugin(path.join(fixturesRoot, "valid-agent-plugin"));
		expect(agent.bindings[0]).toMatchObject({ parent: "executor", phase: "prompt", activationArg: "domain" });

		const both = await loadVibPlugins([
	...
	test("discovers direct and nested project Vibrato plugin roots", async () => {
		const directCwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-plugin-direct-"));
		tempRoots.push(directCwd);
		await fs.cp(path.join(fixturesRoot, "valid-skill-plugin"), path.join(directCwd, ".vib", "vib-plugins"), {
			recursive: true,
		});
		const directRoots = await discoverVibPluginRoots({ cwd: directCwd });
		expect(directRoots).toContain(path.join(directCwd, ".vib", "vib-plugins"));

		const nestedCwd = await copyFixtureToProject("valid-agent-plugin");
		const nestedRoots = await discoverVibPluginRoots({ cwd: nestedCwd });
		expect(nestedRoots).toContain(path.join(nestedCwd, ".vib", "vib-plugins", "valid-agent-plugin"));
	});
});
