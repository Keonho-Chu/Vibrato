...
const fixtureRoot = path.join(import.meta.dir, "fixtures", "vib-plugins", "valid-six-surface-bundle");
const fixtureFiles = [
	"vibrato-plugin.json",
...
async function mkProjectCwd(): Promise<string> {
	return await mkTempDir("vib-m2-redteam-project-");
}
...
async function writeTarball(entries: TarEntry[], options: { gzip?: boolean } = {}): Promise<string> {
	const dir = await mkTempDir("vib-m2-redteam-tar-");
...
async function makeBundleCopy(name: string, extra?: (dir: string) => Promise<void>): Promise<string> {
	const dir = await mkTempDir("vib-m2-redteam-bundle-");
	await fs.cp(fixtureRoot, dir, { recursive: true });
	const manifestPath = path.join(dir, "vibrato-plugin.json");
...
describe("Vibrato plugin installer M2 red-team", () => {
	test("rejects a tarball traversal entry and writes nothing outside extraction root", async () => {
		...
		await expect(installVibBundle({ cwd }, "project", tarball)).rejects.toMatchObject({
	...
	test("rejects a tarball whose root and first-level directories lack vibrato-plugin.json", async () => {
		const cwd = await mkProjectCwd();
	...
	test("installs a valid gzipped tarball whose bundle root is a subdirectory", async () => {
		...
		expect(
			await exists(path.join(cwd, ".vib", "vib-plugins", "valid-six-surface-bundle", "vibrato-plugin.json")),
		).toBe(true);
	});

	test("install of a forbidden-surface bundle leaves no scope files and no registry entry", async () => {
		const cwd = await mkProjectCwd();
		const bad = await mkTempDir("vib-m2-redteam-bad-");
		await fs.writeFile(
			path.join(bad, "vibrato-plugin.json"),
			JSON.stringify({ kind: "vib-rato-plugin", name: "forbidden-bundle", version: "1.0.0", commands: [] }),
		);

		await expect(installVibBundle({ cwd }, "project", bad)).rejects.toMatchObject({
			code: "forbidden_surface",
		});
		expect(await listEntries(path.join(cwd, ".vib", "vib-plugins"))).toEqual([]);
		expect(await readRegistry("project", cwd)).toMatchObject({ plugins: [] });
	});
	...
	test("reinstall identical content requires upgrade and preview/apply updates", async () => {
		...
		expect(await installVibBundle(ctx, "project", original)).toMatchObject({
			ok: true,
		...
		expect(await applyVibBundleUpdate(ctx, preview.value.token)).toMatchObject({
			ok: true,
			value: { status: "updated" },
		});
		const installedPrompt = await fs.readFile(
			path.join(cwd, ".vib", "vib-plugins", "m2-reinstall-bundle", "prompts", "system-appendix.md"),
			"utf8",
		);
	...
	test("concurrent installs of the same bundle leave exactly one registry entry", async () => {
		...
		const results = await Promise.all([
			installVibBundle({ cwd }, "project", bundle),
			installVibBundle({ cwd }, "project", bundle),
		]);
		...
		expect(await exists(path.join(cwd, ".vib", "vib-plugins", "m2-concurrent-bundle", "vibrato-plugin.json"))).toBe(
			true,
		);
	});
});
