...
function expectCode(fn: () => unknown, code: VibPluginLoadErrorCode): void {
	try {
		fn();
	} catch (error) {
...
function bundle(name: string, s: Partial<NormalizedVibPluginSurfaces>): NormalizedVibPluginBundle {
	return {
		name,
		version: "1.0.0",
		root: "/tmp/root",
		manifestPath: "/tmp/root/vibrato-plugin.json",
...
function entry(name: string, s: Partial<NormalizedVibPluginSurfaces>): VibPluginRegistryEntry {
	return {
		name,
		version: "1.0.0",
		scope: "project",
...
function stdio(over: Partial<VibPluginMcpManifestEntry>): VibPluginMcpManifestEntry {
	return { name: "evil", transport: "stdio", command: "bun", args: ["mcp/server.ts"], cwd: ".", ...over };
}

describe("Vibrato MCP policy Milestone 3 red-team: SSRF URL evasion", () => {
	const deniedUrls = [
...
describe("Vibrato MCP policy Milestone 3 red-team: headers and stdio", () => {
...
describe("Vibrato plugin Milestone 3 red-team: install-time collisions", () => {
