...
describe("plugin MCP live connection", () => {
	test("installs and connects a bundled stdio MCP server, exposing its tools", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-mcp-connect-"));
		tempDirs.push(cwd);
		...
		const sources = {
			domain_docs: { provider: "vib-plugins", providerName: "Vibrato plugin bundle", level: "project" as const },
		};
	...
	test("plugin stdio configs request no-env isolation and the child cannot read host secrets", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-mcp-noenv-"));
		tempDirs.push(cwd);
		const r = await installVibBundle({ cwd }, "project", mcpBundle);
		expect(r.ok).toBe(true);
		...
		const secretKey = "VIB_PLUGIN_TEST_SECRET";
