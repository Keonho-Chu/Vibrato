...
import { buildPluginMcpConfigs } from "../src/extensibility/vib-plugins/runtime-adapters";
...
beforeEach(() => {
	agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "vib-mcp-session-agent-"));
...
describe("always-on plugin-bundle MCP in a live session", () => {
	test("connects an installed bundle MCP server and surfaces its tools as always-on", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "vib-mcp-session-"));
		tempDirs.push(cwd);
		const r = await installVibBundle({ cwd }, "project", mcpBundle);
	...
	test("keeps always-on plugin MCP tools active across newSession and switchSession resume", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "vib-mcp-session-resume-"));
		tempDirs.push(cwd);
		const r = await installVibBundle({ cwd }, "project", mcpBundle);
		expect(r.ok).toBe(true);
	...
	test("filters an explicitly requested mandatory plugin tool from persisted selection authority", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "vib-mcp-session-explicit-"));
		tempDirs.push(cwd);
		const r = await installVibBundle({ cwd }, "project", mcpBundle);
		expect(r.ok).toBe(true);
		const sessionManager = SessionManager.inMemory(cwd);
	...
	test("does not connect any MCP server when no plugin bundle is installed", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "vib-mcp-session-empty-"));
		tempDirs.push(cwd);
	...
	test("subagent inherits the parent's always-on MCP tools and never tears down the parent manager", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "vib-mcp-session-sub-"));
		tempDirs.push(cwd);
		const r = await installVibBundle({ cwd }, "project", mcpBundle);
		expect(r.ok).toBe(true);
	...
	test.each([
		...
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "vib-mcp-session-forged-"));
		tempDirs.push(cwd);
		const r = await installVibBundle({ cwd }, "project", mcpBundle);
		expect(r.ok).toBe(true);
