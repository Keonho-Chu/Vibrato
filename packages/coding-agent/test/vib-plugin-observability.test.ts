...
import {
	...
	summarizeVibPluginObservability,
...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-observability-agent-"));
...
async function mkCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-obs-"));
	tempDirs.push(cwd);
	return cwd;
}
...
describe("plugin observability summary", () => {
	test("lists every surface with stable extension ids and enabled status", async () => {
		...
		const summary = await summarizeVibPluginObservability(cwd);
...
describe("observability quarantine keying", () => {
	...
	function entry(name: string): VibPluginRegistryEntry {
		return {
			name,
			version: "1.0.0",
			scope: "project",
