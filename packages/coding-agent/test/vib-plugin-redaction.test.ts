...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-redaction-agent-"));
...
function registryEntry(source: VibPluginRegistrySource): VibPluginRegistryEntry {
	return {
		...
		manifestPath: "/private/plugin-root/vibrato-plugin.json",
...
describe("Vibrato bundle redaction", () => {
	test("redacts hostile locator forms to host and safe path components", () => {
		...
		for (const { uri, expected } of hostileLocators) {
			const source: VibPluginRegistrySource = {
				kind: "git",
				uri,
