...
import { compileVibPluginBundle } from "../src/extensibility/vib-plugins/compiler";
import { isVibPluginSourceShape } from "../src/extensibility/vib-plugins/installer";
import { isLocalDirectorySourceForTest, storedSourceLocatorForTest } from "../src/extensibility/vib-plugins/lifecycle";
...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-refusal-agent-"));
...
async function mkProjectCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-refusal-"));
	tempDirs.push(cwd);
	return cwd;
}
...
describe("Vibrato bundle refusal purity", () => {
	test("a refused install does not create the scope root", async () => {
		...
		const first = await installVibBundle({ cwd }, "project", sixSurface);
	...
	test("a refused install into an untouched scope leaves no directory behind", async () => {
		...
		expect((await installVibBundle({ cwd }, "project", sixSurface)).ok).toBe(true);
		const userRoot = path.join(agentDir, "vib-plugins");
	...
	test("a refused install does not depend on the source being resolvable", async () => {
		...
		const broken = await fs.mkdtemp(path.join(os.tmpdir(), "vib-refusal-broken-"));
		tempDirs.push(broken);
		await fs.writeFile(
			path.join(broken, "vibrato-plugin.json"),
			JSON.stringify({ name: "valid-six-surface-bundle", version: "9.9.9" }),
		);

		const refused = await installVibBundle({ cwd }, "project", broken);
		expect(refused).toMatchObject({ ok: false, error: { code: "already_installed_use_upgrade" } });
	...
	test("a vanished source cannot be identified, so it resolves rather than refusing", async () => {
		...
		const copy = await fs.mkdtemp(path.join(os.tmpdir(), "vib-refusal-gone-"));
		tempDirs.push(copy);
		await fs.cp(sixSurface, copy, { recursive: true });
		expect((await installVibBundle({ cwd }, "project", copy)).ok).toBe(true);
		const scopeRoot = path.join(cwd, ".vib", "vib-plugins");
		const before = await treeOf(scopeRoot);
		...
		await expect(installVibBundle({ cwd }, "project", copy)).rejects.toMatchObject({
			code: "missing_file",
		});
	...
	test("an unreachable remote locator matching no installed entry is not silently refused", async () => {
		...
		// It must reach resolution and fail there, as a VibPluginLoadError, rather
		// than being absorbed by the preflight as an already-installed refusal.
		const attempt = installVibBundle({ cwd }, "project", "https://example.invalid/nobody/nothing.git");
		await expect(attempt).rejects.toBeInstanceOf(VibPluginLoadError);
	});

	test("a locator shared with another scope never blocks an install", async () => {
		const cwd = await mkProjectCwd();
		const copy = await fs.mkdtemp(path.join(os.tmpdir(), "vib-refusal-reuse-"));
		tempDirs.push(copy);
		await fs.cp(sixSurface, copy, { recursive: true });
		expect((await installVibBundle({ cwd }, "project", copy)).ok).toBe(true);
		...
		const userInstall = await installVibBundle({ cwd }, "user", copy);
	...
	test("a vanished stored source yields typed source_unavailable, never a throw", async () => {
		...
		// uncaught VibPluginLoadError whose message embedded the absolute source
		// path. It must be a typed refusal with no locator in the message.
		const cwd = await mkProjectCwd();
		const copy = await fs.mkdtemp(path.join(os.tmpdir(), "vib-b2-src-"));
		tempDirs.push(copy);
		await fs.cp(sixSurface, copy, { recursive: true });
		expect((await installVibBundle({ cwd }, "project", copy)).ok).toBe(true);
		...
		const applied = await applyVibBundleUpdate(
			{ cwd },
			{
				identity,
	...
	test("a reachable malformed stored source yields invalid_target for preview and apply", async () => {
		const cwd = await mkProjectCwd();
		const copy = await fs.mkdtemp(path.join(os.tmpdir(), "vib-malformed-source-"));
		tempDirs.push(copy);
		await fs.cp(sixSurface, copy, { recursive: true });
		expect((await installVibBundle({ cwd }, "project", copy)).ok).toBe(true);
		const identity = bundleIdentity("project", "valid-six-surface-bundle");
		await fs.rm(path.join(copy, "vibrato-plugin.json"));
	...
	test("source-shape routing never steals npm or marketplace specs", () => {
		// The CLI routes on source SHAPE before resolving, so a deleted Vibrato source
		...
		for (const npmSpec of [
			...
			expect(isVibPluginSourceShape(npmSpec)).toBe(false);
		}
		for (const vibSpec of [
			"./local-bundle",
			"/abs/bundle",
			"../rel",
			"~/home-bundle",
			...
			expect(isVibPluginSourceShape(vibSpec)).toBe(true);
		}
	});
	...
	test("hostile manifest bundle names are rejected before they can be stored or printed", async () => {
		...
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-hostile-name-"));
		tempDirs.push(dir);
		for (const hostile of [
			...
			await fs.writeFile(
				path.join(dir, "vibrato-plugin.json"),
				JSON.stringify({ kind: "vib-rato-plugin", name: hostile, version: "1.0.0" }),
			);
			await expect(compileVibPluginBundle(dir)).rejects.toBeInstanceOf(VibPluginLoadError);
		}
		...
		await fs.writeFile(
			path.join(dir, "vibrato-plugin.json"),
			JSON.stringify({ kind: "vib-rato-plugin", name: "ok-bundle", version: hostileText }),
		);
		await expect(compileVibPluginBundle(dir)).rejects.toBeInstanceOf(VibPluginLoadError);
		...
		for (const hook of [
			...
			await fs.writeFile(
				path.join(dir, "vibrato-plugin.json"),
				JSON.stringify({ kind: "vib-rato-plugin", name: "ok-bundle", version: "1.0.0", hooks: [hook] }),
			);
			await expect(compileVibPluginBundle(dir)).rejects.toBeInstanceOf(VibPluginLoadError);
		}
		await fs.writeFile(
		...
		await expect(compileVibPluginBundle(dir)).resolves.toMatchObject({
			name: "ordinary-bundle_1.0",
			version: "1.0.0-beta.1",
		});
	});
	...
	test("a git spawn failure is a typed, sanitized source failure", async () => {
		...
		try {
			const attempt = installVibBundle({ cwd }, "project", "https://example.invalid/owner/repo.git");
			await expect(attempt).rejects.toBeInstanceOf(VibPluginLoadError);
	...
	test("C1 controls are rejected in prose and stripped from rendered appendices", async () => {
		...
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-c1-"));
		tempDirs.push(dir);
		...
		await expect(compileVibPluginBundle(dir)).resolves.toMatchObject({ name: "ok-bundle" });
	});
});
