...
async function makeCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-registry-v2-"));
	tempRoots.push(cwd);
	return cwd;
}
...
async function writeLegacyEntry(cwd: string, root: string): Promise<void> {
	...
	const entry: VibPluginRegistryEntry = {
		name: bundle.name,
...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-registry-v2-agent-"));
...
describe("Vibrato plugin registry v2 cutover", () => {
	test("auto-migrates a v1 entry before activation without importing implementation code", async () => {
		...
		try {
			...
			delete process.env.VIB_TEST_IMPORT_SENTINEL;
		}
	});
	...
	test("migration failure quarantines the plugin and reports plugin/surface/cause", async () => {
		...
		const root = path.join(cwd, "plugin");
		await fs.cp(fixture, root, { recursive: true });
		const manifestPath = path.join(root, "vibrato-plugin.json");
		...
		const bundle = await compileVibPluginBundle(fixture);
		...
		const entry: VibPluginRegistryEntry = {
			...
			copiedFiles: [
				{
					relativePath: "vibrato-plugin.json",
		...
		const status = (await getVibPluginMigrationStatuses(cwd))[0];
	...
	test("serves canonical schemas without importing implementations", async () => {
		...
		process.env.VIB_TEST_IMPORT_SENTINEL = sentinel;
		try {
			const schemas = await serveVibPluginSchemas(cwd);
	...
	test("implementation hash mismatch refuses the single v2 import path", async () => {
		...
		const registryPath = path.join(cwd, ".vib", "vib-plugins", "registry.json");
		...
		).rejects.toBeInstanceOf(VibPluginLoadError);
	});
});
