...
import { installVibBundle, loadAlwaysOnPluginTools, renderSkillAdvertisement } from "../src/extensibility/vib-plugins";
...
beforeEach(async () => {
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-runtime-adapters-agent-"));
...
async function mkCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-rt-"));
	tempDirs.push(cwd);
	return cwd;
}
...
describe("always-on plugin tool runtime activation", () => {
	...
	test("quarantines on installed-file hash drift", async () => {
		...
		const r = await installVibBundle({ cwd }, "project", sixSurface);
		expect(r.ok).toBe(true);
		const installed = path.join(cwd, ".vib", "vib-plugins", "valid-six-surface-bundle", "tools", "domain-note.ts");
	...
	test("rechecks each always-on tool immediately before its import", async () => {
		const cwd = await mkCwd();
		const source = await fs.mkdtemp(path.join(os.tmpdir(), "vib-runtime-race-source-"));
...
if (process.env.VIB_LATE_IMPORT_SENTINEL) fs.writeFileSync(process.env.VIB_LATE_IMPORT_SENTINEL, "imported");
...
`,
		...
		process.env.VIB_LATE_IMPORT_SENTINEL = lateSentinel;
		try {
			let mutated = false;
			...
			delete process.env.VIB_LATE_IMPORT_SENTINEL;
		}
	});

	test("quarantines runtime_mismatch when factory name != declared name", async () => {
		const cwd = await mkCwd();
		const src = await fs.mkdtemp(path.join(os.tmpdir(), "vib-mismatch-"));
		tempDirs.push(src);
		...
		await fs.writeFile(
			path.join(src, "vibrato-plugin.json"),
			JSON.stringify({
		...
		const r = await installVibBundle({ cwd }, "project", src);
		expect(r.ok).toBe(true);
		const res = await loadAlwaysOnPluginTools({ cwd, reservedToolNames: [] });
