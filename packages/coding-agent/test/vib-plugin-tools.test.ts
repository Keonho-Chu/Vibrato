...
import { loadActiveSubskillTools } from "../src/extensibility/vib-plugins/tools";
import { syncSkillActiveState } from "../src/skill-state/active-state";

const tempRoots: string[] = [];

async function makeTempRoot(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "vib-plugin-tools-"));
	tempRoots.push(cwd);
	return cwd;
}
...
const TEST_SESSION_ID = "vib-plugin-tools-test";
...
describe("Vibrato plugin sub-skill tools", () => {
	test("rechecks the subskill tool digest immediately before import", async () => {
		const cwd = await makeTempRoot();
		const fixture = path.join(import.meta.dir, "fixtures", "vib-plugins", "valid-skill-plugin");
		const installed = await installVibBundle({ cwd }, "project", fixture);
		...
		const toolPath = path.join(cwd, ".vib", "vib-plugins", "valid-skill-plugin", "tools", "domain-note.ts");
		let mutated = false;
