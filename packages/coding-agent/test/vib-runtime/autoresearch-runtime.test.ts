...
} from "@vib-rato/coding-agent/vib-runtime/autoresearch-runtime";
import {
	activeSnapshotPath,
...
const runtimeSourcePath = path.join(import.meta.dir, "..", "..", "src", "vib-runtime", "autoresearch-runtime.ts");
...
async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-autoresearch-runtime-"));
	tempRoots.push(dir);
	return dir;
}
...
describe("autoresearch session layout", () => {
	it("resolves every autoresearch path under .vib/_session-{id}/", () => {
		const root = "/repo";
		const sessionDir = sessionAutoresearchDir(root, TEST_SESSION_ID);
		expect(sessionDir).toBe(path.join(root, ".vib", `_session-${TEST_SESSION_ID}`, "autoresearch"));
		...
		for (const candidate of [paths.dir, paths.missionPath, paths.ledgerPath]) {
			const relative = path.relative(path.join(root, ".vib", `_session-${TEST_SESSION_ID}`), candidate);
	...
	it("never writes the legacy global autoresearch store (grep-style)", async () => {
		...
		expect(source).not.toContain("~/.vib/autoresearch");
		expect(source).not.toContain("getAutoresearchDbPath");
		expect(source).not.toContain("getAutoresearchProjectDir");
		expect(source).not.toContain(".vib/autoresearch");
		// All persisted targets route through the sanctioned .vib/** writers.
...
describe("autoresearch mission write boundary (AC-16)", () => {
	...
	it("rejects a missing mode even when a data file is present (mode is never inferred)", async () => {
		...
		await fs.mkdir(path.join(root, ".vib", `_session-${TEST_SESSION_ID}`, "autoresearch"), {
			recursive: true,
		});
		await fs.writeFile(
			path.join(root, ".vib", `_session-${TEST_SESSION_ID}`, "autoresearch", "DATA.md"),
			"# dataset\n",
			"utf-8",
		);
		const result = await autoresearchWrite({
...
describe("autoresearch intake (AC-14..AC-15)", () => {
	...
	it("rejects the ambiguous handoff verb with both disambiguation hints", async () => {
		...
		for (const args of [["handoff"], ["handoff", "--spec", "x.md"], ["handoff", "--json"]]) {
			...
			expect(result.stderr).toContain("vib autoresearch intake --spec");
			expect(result.stderr).toContain("vib state autoresearch handoff --to");
		}
