...
import { standardizeMacOSPath } from "@vib-rato/utils";
import { YAML } from "bun";
import { safeRmSync } from "../../../../scripts/safe-cleanup";

/**
 * `VIB_CONFIG_DIR` is documented as "Config root dirname under home", and
 ...
 * (`VIB_CONFIG_DIR?.trim() || path.join(os.homedir(), ".vib")`), so a user who
...
function scenario(
	...
	const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "vib-config-root-")));
	roots.push(root);
...
describe("config root is resolved under home", () => {
	it("reads ralplan settings from the user config.yml under <home>/<VIB_CONFIG_DIR> and ignores settings.json", async () => {
		const { home, repo } = scenario({ "vib.ralplan.maxIterations": 99 }, { vib: { ralplan: { maxIterations: 9 } } });
	...
	it("reads ultragoal settings from the user config.yml under <home>/<VIB_CONFIG_DIR> and ignores settings.json", async () => {
		const { home, repo } = scenario({ "vib.ultragoal.nudgeBudget": 77 }, { vib: { ultragoal: { nudgeBudget: 7 } } });
	...
	it("keeps using the default config dir name when unset and ignores settings.json", async () => {
		const { home, repo } = scenario(
			{ "vib.ralplan.maxIterations": 44 },
			{ vib: { ralplan: { maxIterations: 4 } } },
			".vib",
		);
		...
		expect(result.source).toBe(standardizeMacOSPath(path.join(home, ".vib", "agent", "config.yml")));
	});
});
