...
} from "@vib-rato/coding-agent/vib-runtime/deep-interview-state";
...
const originalSessionId = process.env.VIB_SESSION_ID;
...
afterAll(() => {
	if (originalSessionId !== undefined) process.env.VIB_SESSION_ID = originalSessionId;
	else delete process.env.VIB_SESSION_ID;
});

describe("native vib deep-interview runtime", () => {
	...
	it("fails closed on corrupt deep-interview state unless --force is supplied", async () => {
		...
		expect(forcedState.receipt).toMatchObject({ skill: "deep-interview", owner: "vib-runtime" });
	...
	it("persists a final spec under .vib/specs through the native CLI/API", async () => {
		const root = await tempDir();
	...
	it("runs an optional bounded trace pre-step before deep-interview questions", async () => {
		const root = await tempDir();
		await fs.mkdir(path.join(root, "packages/coding-agent/src/vib-runtime"), { recursive: true });
		await fs.writeFile(
			path.join(root, "package.json"),
			JSON.stringify({ name: "trace-fixture", scripts: { test: "bun test", check: "bun check" } }),
		);
		await fs.writeFile(
			path.join(root, "packages/coding-agent/src/vib-runtime/deep-interview-runtime.ts"),
	...
	it("honors vib.deepInterview.ambiguityThreshold in project .vib/config.yml", async () => {
		const root = await tempDir();
		await fs.mkdir(path.join(root, ".vib"), { recursive: true });
		await fs.writeFile(
			path.join(root, ".vib", "config.yml"),
			YAML.stringify({ vib: { deepInterview: { ambiguityThreshold: 0.08 } } }),
		);
		const result = await runNativeDeepInterviewCommand(["--standard", "--json", "idea"], root);
		expect(result.status).toBe(0);
		const payload = JSON.parse(result.stdout ?? "{}");
		expect(payload.threshold).toBeCloseTo(0.08);
		expect(payload.threshold_source).toBe(path.join(root, ".vib", "config.yml"));
	});
	...
	it("prefers project config.yml over user config.yml (project beats user)", async () => {
		...
		await fs.writeFile(
			...
			YAML.stringify({ vib: { deepInterview: { ambiguityThreshold: 0.2 } } }),
		);
		await fs.mkdir(path.join(root, ".vib"), { recursive: true });
		await fs.writeFile(
			path.join(root, ".vib", "config.yml"),
