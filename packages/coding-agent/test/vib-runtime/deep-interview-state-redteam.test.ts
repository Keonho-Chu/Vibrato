...
import { deriveDeepInterviewHud } from "@vib-rato/coding-agent/skill-state/workflow-hud";
import {
	appendOrMergeDeepInterviewRound,
	enrichDeepInterviewRoundScoring,
} from "@vib-rato/coding-agent/vib-runtime/deep-interview-recorder";
import {
	mergeDeepInterviewEnvelope,
	mergeDeepInterviewRounds,
	normalizeDeepInterviewEnvelope,
} from "@vib-rato/coding-agent/vib-runtime/deep-interview-state";
import { activeSnapshotPath, modeStatePath } from "@vib-rato/coding-agent/vib-runtime/session-layout";
import { reconcileWorkflowSkillState, runNativeStateCommand } from "@vib-rato/coding-agent/vib-runtime/state-runtime";
...
async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-di-redteam-"));
	tempRoots.push(dir);
	return dir;
}
...
describe("deep-interview redteam: idempotency and lossless shape normalization", () => {
	...
	it("strips leaked envelope-reserved keys from nested state and self-heals recursive nesting", () => {
		...
		const corrupt = {
			...
			receipt: { owner: "vib-state-cli", command: "vib state deep-interview write" },
			state_revision: 3,
			state: {
				rounds: [
		...
		expect(normalized.receipt).toEqual({ owner: "vib-state-cli", command: "vib state deep-interview write" });
...
describe("deep-interview redteam: reconcile and state write HUD parity", () => {
	...
	it("produces identical active-state HUD chips through reconcile and vib state write", async () => {
