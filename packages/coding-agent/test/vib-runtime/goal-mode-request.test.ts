...
} from "@vib-rato/coding-agent/vib-runtime/goal-mode-request";
import { sessionStateDir, sessionUltragoalDir } from "@vib-rato/coding-agent/vib-runtime/session-layout";
...
describe("Vibrato ultragoal goal mode request", () => {
	...
	it("reads vibObjective from the generated ultragoal plan", async () => {
		...
		await Bun.write(goalsPath, JSON.stringify({ vibObjective: "Complete .vib/ultragoal/goals.json" }));

		const result = await readUltragoalVibObjective(root);

		expect(result.objective).toBe("Complete .vib/ultragoal/goals.json");
	...
	it("queues a pending activation request even when the session file already has an active goal", async () => {
		...
		const result = Bun.spawnSync(["bun", cliPath, "ultragoal", "create-goals", "--brief", "Ship native goal"], {
			cwd: root,
			env: { ...process.env, [VIB_SESSION_FILE_ENV]: sessionFile, [VIB_SESSION_ID_ENV]: "session-owner" },
			stdout: "pipe",
			stderr: "pipe",
		});
		...
		expect(pending?.objective).toContain(".vib/ultragoal/goals.json");
	...
	it("surfaces corrupt ultragoal goals json", async () => {
		...
		await expect(readUltragoalVibObjective(root)).rejects.toThrow(SyntaxError);
	});
});
