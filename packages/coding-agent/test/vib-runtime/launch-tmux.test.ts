...
import { VERSION } from "@vib-rato/coding-agent";
import type { Args } from "@vib-rato/coding-agent/cli/args";
import {
	applyVibTmuxProfile,
	buildDefaultTmuxLaunchPlan,
	buildVibTmuxProfileCommands,
	buildVibTmuxWindowTitle,
	launchDefaultTmuxIfNeeded as launchDefaultTmuxIfNeededRaw,
	type TmuxLaunchContext,
	type TmuxSpawnOptions,
	VIB_TMUX_LAUNCHED_ENV,
	VIB_TMUX_SESSION_PREFIX,
} from "@vib-rato/coding-agent/vib-runtime/launch-tmux";
import {
	__setBinaryResolverForTests,
	__setExecutableIdentityResolverForTests,
} from "@vib-rato/coding-agent/vib-runtime/psmux-detect";
import { sessionRuntimeDir } from "@vib-rato/coding-agent/vib-runtime/session-layout";
import {
...
} from "@vib-rato/coding-agent/vib-runtime/tmux-owner-isolation";
import {
	__setCreateOwnerIsolationForTests,
	__setMutationServerProofForTests,
	createVibTmuxSession,
	removeVibTmuxSession,
} from "@vib-rato/coding-agent/vib-runtime/tmux-sessions";
import { postmortem } from "@vib-rato/utils";
import { SessionStateLockTestHooks } from "../../src/vib-runtime/session-state-lock";
...
const launchTestRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vib-launch-tests-"));
...
function safeAbsentOwnerIsolationProbe(
	...
	return {
		readCallerCgroup: () =>
			platform === "linux" ? "0::/user.slice/user-1000.slice/user@1000.service/app.slice/vib.scope\n" : null,
		probeServer: () => {
			if (!serverCreated) {
				serverCreated = true;
				return { state: "absent" };
...
function launchContext(context: TmuxLaunchContext): TmuxLaunchContext {
	return {
		...
		...context,
		env: {
			VIB_COORDINATOR_SESSION_STATE_FILE: path.join(
				launchTestRoot,
				context.platform === "darwin" ? "darwin" : "runtime",
				`${launchStateSequence++}.json`,
			),
			VIB_TMUX_COMMAND: "tmux",
			VIB_PSMUX_DETECTION: "off",
			...(context.env ?? {}),
		},
	};
}
...
function launchDefaultTmuxIfNeeded(context: TmuxLaunchContext): boolean {
	let createdSessionName = context.env?.VIB_TMUX_SESSION;
		...
		launchContext({
			...
			providerAuthorityResolver:
				...
				(input => ({
					...
					commandPrefix: ["-L", "vib-test-000000000000000000000000000000000000"],
					namespace: "vib-test-000000000000000000000000000000000000",
			...
			spawnSync: suppliedSpawnSync
				? (command, spawnArgs, options) => {
						const commandArgs =
							spawnArgs[0] === "-L" && spawnArgs[1]?.startsWith("vib-test-") ? spawnArgs.slice(2) : spawnArgs;
						...
						if (commandArgs[0] === "set-option") {
							...
							if (option?.startsWith("@vib-") && value) psmuxMetadata.set(option, value);
						}
						if (
							...
							return { ...result, stdout: createdSessionName ?? "vib_rato" };
						if (
							commandArgs[0] === "display-message" &&
							commandArgs.at(-1)?.startsWith("#{@vib-") &&
							result.exitCode === 0 &&
							!result.stdout?.trim()
						) {
						...
						if (
							...
							return { ...result, stdout: `${nativeSessionId}\t${createdSessionName ?? "vib_rato"}` };
						if (
							commandArgs[0] === "if-shell" &&
							result.exitCode === 0 &&
							result.stdout?.trim() !== "__vib_tmux_guarded_cleanup_refused__"
						)
							return { ...result, stdout: "__vib_tmux_guarded_cleanup_ok__" };
						return result;
					}
				: undefined,
		}),
...
beforeAll(() => {
	...
	previousCoordinatorSessionId = process.env.VIB_COORDINATOR_SESSION_ID;
	previousCoordinatorStateFile = process.env.VIB_COORDINATOR_SESSION_STATE_FILE;
	delete process.env.VIB_COORDINATOR_SESSION_ID;
	delete process.env.VIB_COORDINATOR_SESSION_STATE_FILE;
	__setBinaryResolverForTests(candidate => `C:\\vib-test\\${candidate}.exe`);
	__setExecutableIdentityResolverForTests(() => "vib-test-psmux");
});
...
afterAll(() => {
	...
	if (previousCoordinatorSessionId === undefined) delete process.env.VIB_COORDINATOR_SESSION_ID;
	else process.env.VIB_COORDINATOR_SESSION_ID = previousCoordinatorSessionId;
	if (previousCoordinatorStateFile === undefined) delete process.env.VIB_COORDINATOR_SESSION_STATE_FILE;
	else process.env.VIB_COORDINATOR_SESSION_STATE_FILE = previousCoordinatorStateFile;
...
describe("default Vibrato tmux launch", () => {
	afterEach(() => {
	...
	it("builds sanitized project and branch tmux window titles", () => {
		expect(buildVibTmuxWindowTitle("/repo", "feature/demo")).toBe("Vibrato-repo-feature/demo");
		expect(buildVibTmuxWindowTitle("/repo", "main")).toBe("Vibrato-repo-main");
		expect(buildVibTmuxWindowTitle("/repo", null)).toBe("Vibrato-repo");
		expect(buildVibTmuxWindowTitle("/repo", "")).toBe("Vibrato-repo");
	});

	it("replaces colon-bearing tmux window title segments", () => {
		expect(buildVibTmuxWindowTitle("/repo:backend", "main")).toBe("Vibrato-repo-backend-main");
		expect(buildVibTmuxWindowTitle("/repo", "release:main")).toBe("Vibrato-repo-release-main");
		expect(buildVibTmuxWindowTitle("/repo", "feature:::demo")).toBe("Vibrato-repo-feature-demo");
	});

	it("truncates long tmux window titles to 48 visible columns while preserving the project and branch tail", () => {
		const title = buildVibTmuxWindowTitle("/repo", `feature/${"a".repeat(80)}tail`);

		expect(Bun.stringWidth(title)).toBeLessThanOrEqual(48);
		expect(title.startsWith("Vibrato-repo-…")).toBe(true);
	...
	it("truncates wide-character tmux window titles by visible width while preserving the branch tail", () => {
		const title = buildVibTmuxWindowTitle("/저장소", `feature/${"界".repeat(80)}끝`);

		expect(Bun.stringWidth(title)).toBeLessThanOrEqual(48);
		expect(title.startsWith("Vibrato-저장소-…")).toBe(true);
		expect(title.endsWith("끝")).toBe(true);
	});

	it("sanitizes dot-prefixed cwd basenames for tmux window titles", () => {
		expect(buildVibTmuxWindowTitle("/tmp/.claude", null)).toBe("Vibrato-dot-claude");
		expect(buildVibTmuxWindowTitle("/tmp/.claude", "feature/demo")).toBe("Vibrato-dot-claude-feature/demo");
		expect(buildVibTmuxWindowTitle("/tmp/.claude", "repo:main")).toBe("Vibrato-dot-claude-repo-main");
		expect(buildVibTmuxWindowTitle("/tmp/...", null)).toBe("Vibrato-vib");
		expect(buildVibTmuxWindowTitle("/tmp/...", "feature/demo")).toBe("Vibrato-vib-feature/demo");
	});
	...
	it("configures the tmux client terminal title before managed attach", () => {
		...
		expect(calls[titleIndex]?.args).toEqual([
			...
			"$0:",
			"set-titles-string",
			"#{?#{==:#{@vib-root-terminal-title-session},#{session_name}},#{@vib-root-terminal-title},Vibrato: #{session_name}}",
		]);
		expect(
			calls.some(
				call => call.args[3] === "@vib-root-terminal-title" && call.args[4] === "Vibrato: repo-feature/demo",
			),
		).toBe(true);
		expect(
			calls.some(
				call => call.args[3] === "@vib-root-terminal-title-session" && /^vib_rato_/.test(call.args[4] ?? ""),
			),
		).toBe(true);
	...
	it("uses the live tmux session name for already renamed managed sessions", () => {
		...
		expect(handled).toBe(true);
		expect(calls.find(call => call.args[3] === "set-titles-string")?.args.at(-1)).toBe("Vibrato: #{session_name}");
	...
	it("stores literal fallback titles outside the tmux title format", () => {
		...
		expect(
			calls.some(
				call => call.args[3] === "@vib-root-terminal-title" && call.args[4] === "Vibrato: repo-feature/#S/demo",
			),
		).toBe(true);
	...
	it("quotes identity-guarded tmux window titles as one command argument", () => {
		...
		expect(calls[1]?.args).toEqual([
			...
			"rename-window -t @4 -- 'Vibrato--repo-feature/demo'\\'';kill-window'",
		]);
	});
	...
	it("plans an interactive --tmux root launch inside a new Vibrato tmux session", () => {
		const plan = buildDefaultTmuxLaunchPlan({
			parsed: args({ messages: ["hello world"], tmux: true }),
			rawArgs: ["--tmux", "hello world"],
			cwd: "/repo",
			env: { VIB_PSMUX_DETECTION: "off" },
			argv: ["bun", "packages/coding-agent/src/cli.ts"],
			execPath: "/bin/bun",
			platform: "darwin",
			tty: interactiveTty,
		...
		expect(plan.sessionName.startsWith(VIB_TMUX_SESSION_PREFIX)).toBe(true);
		...
		expect(plan.innerCommand).toContain("VIB_COORDINATOR_SESSION_ID=");
		expect(plan.innerCommand).toContain("VIB_COORDINATOR_SESSION_STATE_FILE=");
		expect(plan.innerCommand).toContain("tmux-exit.json");
		expect(plan.innerCommand).toContain("trap __vib_tmux_write_exit_marker EXIT");
	...
	it("keeps the sidecar PKCS#8 private key out of tmux argv projections", () => {
		...
		const plan = buildDefaultTmuxLaunchPlan({
			...
			env: {
				...
				[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV]: privateKey,
				[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]: "a".repeat(64),
			},
			argv: ["bun", "packages/coding-agent/src/cli.ts"],
			execPath: "/bin/bun",
			platform: "darwin",
	...
	function coordinatorSidecarLaunchEnv(root: string, sessionId: string): Record<string, string> {
		return {
			VIB_TMUX_COMMAND: "tmux",
			VIB_PSMUX_DETECTION: "off",
			VIB_COORDINATOR_SESSION_ID: sessionId,
			VIB_COORDINATOR_SESSION_STATE_FILE: path.join(root, "runtime-state.json"),
			[VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED_ENV]: "true",
			[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV]: "pkcs8-private-key-material",
			[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]: "a".repeat(64),
		};
	}
	...
	it("never writes the coordinator sidecar bootstrap key when tmux is unavailable", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-sidecar-unavailable-"));
		try {
			...
			expect(diagnostics).toEqual([
				"vib --tmux requested but no tmux executable was found; cannot continue without a tmux-backed session.\n",
			]);
			expect(writeSpy).not.toHaveBeenCalled();
	...
	it("never writes the coordinator sidecar bootstrap key when attaching an existing session", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-sidecar-attach-"));
		try {
			const calls: string[][] = [];
	...
	it("scrubs the coordinator sidecar bootstrap key when the second attach proof fails", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-sidecar-second-proof-"));
		try {
			const calls: string[][] = [];
			let proofCount = 0;
			__setMutationServerProofForTests(() => {
	...
	it("uses a one-shot coordinator sidecar bootstrap endpoint only for the launching managed owner", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-sidecar-launch-"));
		try {
			...
			const handled = launchDefaultTmuxIfNeeded({
				...
				spawnSync: (_command, spawnArgs) => {
					if (spawnArgs[0] === "new-session") {
						...
						commandBootstrapPath = /VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL='([^']+)'/.exec(inner)?.[1];
					}
					return { exitCode: 0, stdout: NATIVE_SESSION_ID };
				},
			});
	...
	it("POSIX tmux inner wrapper writes a public-safe exit marker and preserves exit status", () => {
		...
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "vib-tmux-exit-marker-"));
		...
		try {
			expect(
				launchDefaultTmuxIfNeeded({
					...
					rawArgs: ["--tmux", "hello world"],
					cwd: "/repo",
					env: { VIB_TMUX_COMMAND: "psmux", VIB_PSMUX_COMMAND: "psmux" },
					argv: ["bun", "packages/coding-agent/src/cli.ts"],
					execPath: "/bin/bun",
					platform: "win32",
					tty: { stdin: true, stdout: true, columns: 178, rows: 35 },
			...
			expect(calls.filter(call => call[0] === "display-message" && call.at(-1)?.startsWith("#{@vib-"))).toHaveLength(
				12,
			);
		} finally {
		}
	...
	it.each([
		...
		launchDefaultTmuxIfNeeded({
			...
			spawnSync: (_command, spawnArgs) => {
				calls.push(spawnArgs);
				if (spawnArgs[0] === "display-message" && spawnArgs.at(-1)?.startsWith("#{@vib-"))
	...
	it.each([
		"VIB_COORDINATOR_SESSION_BRANCH",
		"VIB_COORDINATOR_SESSION_LAUNCH_ID",
		"VIB_COORDINATOR_SESSION_READINESS_FILE",
		...
		try {
			expect(
				launchDefaultTmuxIfNeeded({
					...
					cwd: "/repo",
					env: {
						VIB_TMUX_COMMAND: "psmux",
						VIB_PSMUX_COMMAND: "psmux",
						[markerName]: " marker ",
					},
					argv: ["bun", "cli.ts"],
					execPath: "/bin/bun",
	...
	it("refuses an explicit psmux continuation without durable managed authority", () => {
		...
		try {
			launchDefaultTmuxIfNeeded({
				...
				rawArgs: ["--tmux", "hello"],
				cwd: "/repo",
				env: { VIB_TMUX_COMMAND: "psmux", VIB_PSMUX_COMMAND: "psmux", VIB_TMUX_SESSION: "continued" },
				argv: ["bun", "cli.ts"],
				execPath: "/bin/bun",
				platform: "win32",
				tty: interactiveTty,
	...
	it.each([
		...
		["direct policy", args({ tmux: true }), ["--tmux", "hello"], { VIB_LAUNCH_POLICY: "direct" }, false],
		["already launched", args({ tmux: true }), ["--tmux", "hello"], { [VIB_TMUX_LAUNCHED_ENV]: "1" }, false],
		...
		try {
			expect(
				launchDefaultTmuxIfNeeded({
					...
					rawArgs,
					cwd: "/repo",
					env: { VIB_TMUX_COMMAND: "psmux", VIB_PSMUX_COMMAND: "psmux", ...extraEnv },
					argv: ["bun", "cli.ts"],
					execPath: "/bin/bun",
					platform: "win32",
					tty: interactiveTty,
	...
	it("plans native Windows --tmux launches when tmux is available", () => {
		...
		// buildDefaultTmuxLaunchPlan returns a plan that bootstraps vib through
		...
		const plan = buildDefaultTmuxLaunchPlan({
			...
			cwd: "C:\\repo",
			env: {},
			argv: ["C:\\Program Files\\Vibrato\\vib.exe"],
			execPath: "C:\\Program Files\\Vibrato\\vib.exe",
			platform: "win32",
			tty: interactiveTty,
			tmuxAvailable: true,
			currentBranch: "",
	...
	it("uses a host command for compiled Bun virtual entrypoints", () => {
		const plan = buildDefaultTmuxLaunchPlan({
			...
			cwd: "/repo",
			env: {},
			argv: ["vib", "/$bunfs/root/vib-linux-x64"],
			execPath: "/home/me/.local/bin/vib",
			platform: "darwin",
			tty: interactiveTty,
			tmuxAvailable: true,
			currentBranch: "",
		...
		expect(plan.innerCommand).toContain(`${VIB_TMUX_LAUNCHED_ENV}=1`);
		expect(plan.innerCommand).toContain("'/home/me/.local/bin/vib' 'hello world'");
	});
	...
	it("fails closed when a compiled Bun virtual entrypoint has no same-artifact host path", () => {
		expect(() =>
			buildDefaultTmuxLaunchPlan({
				...
				env: {},
				argv: ["vib", "/$bunfs/root/vib-linux-x64"],
				execPath: "/$bunfs/root/vib-linux-x64",
				platform: "darwin",
				tty: interactiveTty,
				tmuxAvailable: true,
				currentBranch: "",
	...
	it("fails closed for Windows Bun virtual executable paths", () => {
		expect(() =>
			buildDefaultTmuxLaunchPlan({
				...
				cwd: "C:\\repo",
				env: {},
				argv: ["B:\\~BUN\\bun.exe", "B:\\~BUN\\root\\vib-windows-x64.exe"],
				execPath: "B:\\~BUN\\root\\vib-windows-x64.exe",
				platform: "win32",
				tty: interactiveTty,
				tmuxAvailable: true,
				currentBranch: "",
	...
	it("does not implicitly attach existing tagged session for plain worktree branch launch", () => {
		...
		const handled = launchDefaultTmuxIfNeeded({
			...
			tmuxAvailable: true,
			worktreeBranch: "feature/demo",
			existingBranchSessionName: "vib_rato_feature",
			spawnSync: (command, spawnArgs, options) => {
				calls.push({ command, args: spawnArgs, options });
				return { exitCode: 0, stdout: NATIVE_SESSION_ID };
			},
		...
		expect(handled).toBe(true);
		expect(calls.some(call => call.args[0] === "new-session")).toBe(true);
		expect(calls.some(call => call.args[0] === "attach-session" && call.args[2] === "=vib_rato_feature")).toBe(false);
	});
	...
	it("explicit continue attaches existing tagged session for matching worktree branch", () => {
		...
		expect(calls.at(-1)?.args).toEqual(["attach-session", "-t", "=vib_rato_feature"]);
	});
	...
	it("falls through to a fresh session when existing tagged session attach fails", () => {
		...
		const handled = launchDefaultTmuxIfNeeded({
			...
			spawnSync: (command, spawnArgs, options) => {
				calls.push({ command, args: spawnArgs, options });
				if (spawnArgs[0] === "attach-session" && spawnArgs[2] === "=vib_rato_feature") return { exitCode: 1 };
				return { exitCode: 0, stdout: NATIVE_SESSION_ID };
			},
		});
		...
		expect(calls.some(call => call.args[0] === "new-session")).toBe(true);
		expect(calls.some(call => call.args[0] === "attach-session" && call.args[2] !== "=vib_rato_feature")).toBe(true);
	});
	...
	it("honors an explicit VIB_TMUX_SESSION override", () => {
		...
		spyOn(Bun, "spawnSync").mockReturnValue(
			spawnResult(0, "custom-vib\t1\t0\t1770000000\t1\troot\t1\t12345\tfeature/demo\tfeature-demo\t/repo"),
		);
		const plan = buildDefaultTmuxLaunchPlan({
			parsed: args({ messages: ["hello world"], tmux: true }),
			rawArgs: ["--tmux", "hello world"],
			cwd: "/repo",
			env: { VIB_TMUX_SESSION: "custom-vib" },
			argv: ["bun", "packages/coding-agent/src/cli.ts"],
			execPath: "/bin/bun",
			platform: "darwin",
			tty: interactiveTty,
		...
		expect(plan?.sessionName).toBe("custom-vib");
		expect(plan?.attachSessionName).toBe("custom-vib");
		expect(plan?.newSessionArgs.slice(0, 6)).toEqual(["new-session", "-d", "-s", "custom-vib", "-c", "/repo"]);
	});

	it("honors explicit VIB_TMUX_COMMAND on native Windows without direct-launch fallback", () => {
		// Once psmux is a supported Windows multiplexer, an explicit
		// VIB_TMUX_COMMAND override must always produce a tmux plan. The
	...
	it("does not auto-reuse scoped sessions from another Vibrato version", () => {
		spyOn(Bun, "spawnSync").mockReturnValue(
			spawnResult(
				0,
				"old-vib\t1\t0\t1770000000\t1\troot\t1\t12345\tfeature/demo\tfeature-demo\t/repo\told-session\t/state\t0.0.0",
			),
		);
		const plan = buildDefaultTmuxLaunchPlan({
			parsed: args({ messages: ["hello world"], tmux: true }),
	...
	it("does not auto-reuse scoped sessions from the current Vibrato version without explicit resume", () => {
		spyOn(Bun, "spawnSync").mockReturnValue(
			spawnResult(
				0,
				`current-vib\t1\t0\t1770000000\t1\troot\t1\t12345\tfeature/demo\tfeature-demo\t/repo\tcurrent-session\t/state\t\t${VERSION}`,
			),
		);
		const plan = buildDefaultTmuxLaunchPlan({
			parsed: args({ messages: ["hello world"], tmux: true }),
	...
	it("auto-reuses scoped sessions from the current Vibrato version for explicit continue", () => {
		__setBinaryResolverForTests(candidate => (candidate === "tmux" ? "C:\\native\\tmux.exe" : null));
		__setExecutableIdentityResolverForTests(() => "native-tmux");
		spyOn(Bun, "spawnSync").mockReturnValue(
			spawnResult(
		...
		expect(plan?.attachSessionName).toBe("current-vib");
	});
	...
	it("cleans up a newly created managed session when attach fails", () => {
		...
		try {
			...
			expect(writeSpy).not.toHaveBeenCalled();
			expect(diagnostics[0]).toStartWith("vib --tmux failed after creating tmux session: attach failed.");
		} finally {
	...
	it("builds a session-scoped tmux profile without global tmux mutation", () => {
		const commands = buildVibTmuxProfileCommands("vib-session:0", {});
		const args = commands.map(command => command.args);

		expect(args).toContainEqual(["set-option", "-t", "vib-session:0", "mouse", "on"]);
		expect(args).toContainEqual(["set-option", "-t", "vib-session:0", "@vib-profile", "1"]);
		expect(args).toContainEqual(["set-option", "-t", "vib-session:0", "set-clipboard", "on"]);
		...
		expect(
			buildVibTmuxProfileCommands("vib-session:0", { VIB_TMUX_PROFILE: "false" }).map(command => command.args),
		).toEqual([["set-option", "-t", "vib-session:0", "@vib-profile", "1"]]);
		expect(
			buildVibTmuxProfileCommands("vib-session:0", { VIB_MOUSE: "off" }).flatMap(command => command.args),
		).not.toContain("mouse");
	});
	...
	it.each([
		...
		const commands = buildVibTmuxProfileCommands(
			"vib-session:0",
			typeof force === "string" ? { VIB_PSMUX_PROFILE_FORCE: force } : {},
			{},
			{ tmuxCommand: "psmux" },
		);
		...
		expect(keys).toContain("@vib-profile");
	});
	...
	it("records session identity markers in the required tmux profile", () => {
		const commands = buildVibTmuxProfileCommands(
			...
			{},
			{
				...
				sessionStateFile: "/tmp/vib-state/session.json",
				version: VERSION,
			},
		);
		const args = commands.map(command => command.args);

		expect(args).toContainEqual(["set-option", "-t", "vib-session:0", "@vib-session-id", "session-123"]);
		expect(args).toContainEqual([
			"set-option",
			"-t",
			"vib-session:0",
			"@vib-session-state-file",
			"/tmp/vib-state/session.json",
		]);
		expect(args).toContainEqual(["set-option", "-t", "vib-session:0", "@vib-version", VERSION]);
	});
	...
	it("plans matching tmux marker tags and inner process marker env", () => {
		const plan = buildDefaultTmuxLaunchPlan({
			...
			rawArgs: ["--tmux", "hello world"],
			cwd: "/repo",
			env: { VIB_SESSION_ID: TEST_SESSION_ID },
			argv: ["bun", "packages/coding-agent/src/cli.ts"],
			execPath: "/bin/bun",
			platform: "darwin",
			tty: interactiveTty,
		...
		// The runtime state path is rooted on the Vibrato session (VIB_SESSION_ID), not the
		...
		expect(plan.innerCommand).toContain(`VIB_COORDINATOR_SESSION_ID='${plan.sessionId}'`);
		expect(plan.innerCommand).toContain(`VIB_COORDINATOR_SESSION_STATE_FILE='${plan.sessionStateFile}'`);
	});

	it("roots runtime state on VIB_SESSION_ID even when VIB_COORDINATOR_SESSION_ID differs", () => {
		const plan = buildDefaultTmuxLaunchPlan({
			parsed: args({ messages: ["hello world"], tmux: true }),
			rawArgs: ["--tmux", "hello world"],
			cwd: "/repo",
			env: { VIB_SESSION_ID: "vib-sess", VIB_COORDINATOR_SESSION_ID: "coord-sess" },
			argv: ["bun", "packages/coding-agent/src/cli.ts"],
			execPath: "/bin/bun",
			platform: "darwin",
			tty: interactiveTty,
		...
		// Coordinator identity is the coordinator id; the state-file root is the Vibrato session.
		expect(plan.sessionId).toBe("coord-sess");
		expect(path.dirname(plan.sessionStateFile)).toBe(
			path.join(sessionRuntimeDir("/repo", "vib-sess"), "tmux-sessions"),
		);
	});

	it("applies the tmux profile only to the requested target", () => {
		const calls: { command: string; args: string[] }[] = [];
		const result = applyVibTmuxProfile({
			tmuxCommand: "tmux",
			target: "%7",
			cwd: "/repo",
			env: {},
	...
	it("renames the originating tmux window through an identity and index guard", () => {
		...
		expect(calls[1]).toMatchObject({
			...
			args: [
				...
				"rename-window -t @3 -- 'Vibrato-repo-feature/demo'",
			],
		});
	});
	...
	it.skipIf(!nativeTmux)("keeps real tmux renames on the originating window and refuses index drift", () => {
		...
		const socket = `vib-rename-${process.pid}-${Date.now()}`;
		...
		try {
			...
			const invokeDirectLaunch = (branch: string): void => {
				const handled = launchDefaultTmuxIfNeeded({
					...
					rawArgs: ["hello world"],
					cwd: "/repo",
					env: { TMUX: tmuxEnv, TMUX_PANE: paneId, VIB_TMUX_COMMAND: tmuxCommand },
					argv: ["/usr/local/bin/vib"],
					execPath: "/bin/bun",
					platform: "darwin",
					tty: interactiveTty,
			...
			).toBe("Vibrato-repo-feature/demo';kill-window");
			expect(
	...
	it("does not rename direct launches already inside a Vibrato-launched tmux wrapper", () => {
		const calls: Array<{ command: string; args: string[] }> = [];
		const handled = launchDefaultTmuxIfNeeded({
			parsed: args({ messages: ["hello world"] }),
			rawArgs: ["hello world"],
	...
	it("skips direct tmux rename when guard conditions are not met", () => {
		const cases = [
			...
			},
			{
				...
				parsed: args({ messages: ["hello world"] }),
				env: { TMUX: "/tmp/tmux", VIB_LAUNCH_POLICY: "direct" },
				tmuxAvailable: true,
			},
		];
	...
	it("renames managed tmux windows after creating the session", () => {
		...
		expect(calls[renameIndex]?.args).toEqual(["rename-window", "-t", "$0", "--", "Vibrato-repo-feature/demo"]);
	});
	...
	it("handles and reports partial launch when required profile tagging fails", () => {
		...
		const handled = launchDefaultTmuxIfNeeded({
			...
			spawnSync: (command, spawnArgs, options) => {
				calls.push({ command, args: spawnArgs, options });
				if (spawnArgs.includes("@vib-profile")) return { exitCode: 1, stderr: "no server running on /tmp/tmux" };
				return { exitCode: 0, stdout: NATIVE_SESSION_ID };
			},
		});
		...
		expect(diagnostics).toHaveLength(1);
		expect(diagnostics[0]).toStartWith("vib --tmux failed after creating tmux session: profile tagging failed.");
	...
	it("continues root launch when non-ownership metadata tagging fails", () => {
		...
		const handled = launchDefaultTmuxIfNeeded({
			...
			spawnSync: (command, spawnArgs, options) => {
				calls.push({ command, args: spawnArgs, options });
				if (spawnArgs.includes("@vib-branch")) return { exitCode: 1, stderr: "psmux: connection timed out" };
	...
	it("preserves a newly created managed session when attach reports SSH disconnect EIO", () => {
		...
		expect(calls.some(call => call.args[0] === "kill-session")).toBe(false);
		expect(diagnostics).toHaveLength(1);
		expect(diagnostics[0]).toStartWith("vib --tmux failed after creating tmux session: attach disconnected.");
	});
	...
	it("explains the psmux install path when no tmux binary is found on native Windows", () => {
		...
		expect(diagnostics[0]).toContain("VIB_TMUX_COMMAND");
	});
	...
	it("applies session-scoped mouse scrolling when launching tmux on WSL/Linux", () => {
		...
		expect(sessionName.startsWith(VIB_TMUX_SESSION_PREFIX)).toBe(true);
		// The Vibrato-launched tmux/profile path must not bypass mouse scrolling on WSL.
		expect(calls.some(call => call.command === "tmux")).toBe(true);
		expect(calls.map(call => call.args)).toContainEqual(["set-option", "-t", "$0:", "mouse", "on"]);
		expect(calls.map(call => call.args)).toContainEqual(["set-option", "-t", "$0:", "@vib-version", VERSION]);
		// All profile mutations stay scoped to the Vibrato session, never global tmux state.
		expect(calls.flatMap(call => call.args)).not.toContain("-g");
	});

	it("honors VIB_MOUSE=off on WSL/Linux without disabling the rest of the profile", () => {
		const calls: { command: string; args: string[]; options: TmuxSpawnOptions }[] = [];
		const handled = launchDefaultTmuxIfNeeded({
			parsed: args({ messages: ["hello world"], tmux: true }),
			rawArgs: ["--tmux", "hello world"],
			cwd: "/repo",
			env: { WSL_DISTRO_NAME: "Ubuntu", VIB_MOUSE: "off" },
			argv: ["bun", "packages/coding-agent/src/cli.ts"],
			execPath: "/bin/bun",
			platform: "linux",
			tty: interactiveTty,
		...
		expect(calls.map(call => call.args)).toContainEqual(["set-option", "-t", "$0:", "@vib-profile", "1"]);
		expect(calls.map(call => call.args)).toContainEqual(["set-option", "-t", "$0:", "@vib-version", VERSION]);
	});
});

it("emits a BOM-less UTF-16LE encoded command and a direct `&` invocation for native Windows --tmux plans", () => {
	// Regression: vib --tmux on native Windows + psmux previously failed with
	// the literal text "﻿$env:VIB_TMUX_LAUNCHED : The term '﻿$env:...' is not
	...
	// of the actual PowerShell command (`$` from `$env:VIB_TMUX_LAUNCHED`).
...
it("captures psmux stderr in the attach-failed diagnostic", () => {
	...
	// into an unisolated root Vibrato process; diagnostics retain the rejection.
	expect(handled).toBe(true);
	expect(diagnostics.length).toBeGreaterThan(0);
...
it("surfaces a wrapper-corruption warning in the new-session diagnostic on Windows", () => {
	// Regression: when vib.cmd / vib.bat on PATH has been overwritten with
	...
	// script.
	if (process.platform !== "win32") return;
	const dir = fs.mkdtempSync(path.join(require("os").tmpdir(), "vib-wrapper-probe-"));
	const wrapperPath = path.join(dir, "vib.cmd");
...
it("pipes default control-command stderr while preserving interactive attach stderr", () => {
	...
	const handled = launchDefaultTmuxIfNeededRaw({
		...
		env: {
			VIB_TMUX_COMMAND: "tmux",
			VIB_COORDINATOR_SESSION_STATE_FILE: path.join(launchTestRoot, "default-spawn-state.json"),
		},
		argv: ["bun", "cli.ts"],
		execPath: "/bin/bun",
		platform: "linux",
...
it("preserves a native Linux profile failure without retrying or cleaning up", () => {
	...
	const result = launchDefaultTmuxIfNeeded({
		...
		spawnSync: (_command, spawnArgs) => {
			calls.push({ command: spawnArgs[0], args: spawnArgs });
			if (spawnArgs[0] === "new-session") return { exitCode: 0, stdout: "$0" };
			if (spawnArgs.includes("@vib-profile")) return { exitCode: 1, stderr: "native profile failed" };
			return { exitCode: 0, stdout: NATIVE_SESSION_ID };
		},
	});
...
it("provisions psmux authority before generation publication and verifies later namespaced commands", () => {
	const events: string[] = [];
	const calls: string[][] = [];
	const namespace = "vib-test-000000000000000000000000000000000000";
	...
	expect(metadata.get("@vib-psmux-incarnation")).toMatch(/^[0-9a-f-]{36}$/);
});
...
it("does not retry attach recovery when published psmux metadata no longer proves ownership", () => {
	...
	const handled = launchDefaultTmuxIfNeeded({
		...
		spawnSync: (_command, spawnArgs) => {
			...
			if (attachAttempted && spawnArgs.at(-1) === "#{@vib-profile}") return { exitCode: 0, stdout: "wrong-profile" };
			return { exitCode: 0, stdout: "" };
		},
	});
	...
	const context = (): TmuxLaunchContext =>
		launchContext({
			...
			env: {
				VIB_TMUX_COMMAND: "psmux",
				VIB_PSMUX_COMMAND: "psmux",
				VIB_COORDINATOR_SESSION_ID: TEST_SESSION_ID,
				VIB_COORDINATOR_SESSION_STATE_FILE: stateFile,
			},
			argv: ["bun", "cli.ts"],
			execPath: "/bin/bun",
			platform: "win32",
...
it("uses the captured native session ID for every post-create target", () => {
	...
	expect(targetFor("set-option", "@vib-profile")).toEqual(["$0:"]);
...
it.each([
	...
	launchDefaultTmuxIfNeeded({
		...
		existingBranchSessionName: null,
		ownerIsolationProbe: {
			readCallerCgroup: () => "0::/user.slice/user-1000.slice/user@1000.service/app.slice/vib.scope\n",
			probeServer: () => {
				probeCount++;
				return probeCount === 1
...
it.each([
	...
	expect(diagnostics).toEqual([
		"vib --tmux failed after creating tmux session: native session identity was unavailable; preserving session for recovery.\n",
	]);
});
...
describe("tmux owner isolation launch gate", () => {
	...
	it("fails closed when a simulated Linux server probe cannot establish host identity", () => {
		...
		const handled = launchDefaultTmuxIfNeededRaw({
			...
			cwd: launchTestRoot,
			env: {
				VIB_COORDINATOR_SESSION_STATE_FILE: path.join(launchTestRoot, "absent-server-state.json"),
			},
			argv: ["bun", "cli.ts"],
			execPath: "/bin/bun",
			platform: "linux",
	...
	it("persists scoped launch capabilities exclusively at mode 0600 and fsyncs the file and parent directory", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-tmux-attempt-"));
		...
		try {
			...
			launchDefaultTmuxIfNeededRaw({
				...
				cwd: root,
				env: {
					VIB_COORDINATOR_SESSION_ID: "persisted-attempt",
					VIB_COORDINATOR_SESSION_STATE_FILE: path.join(root, "runtime-state.json"),
				},
				argv: ["bun", "cli.ts"],
				execPath: "/bin/bun",
	...
	it("fails closed rather than overwriting an existing scoped launch capability", () => {
		...
		try {
			...
			const handled = launchDefaultTmuxIfNeededRaw({
				...
				rawArgs: ["--tmux", "hello"],
				cwd: root,
				env: { VIB_COORDINATOR_SESSION_STATE_FILE: path.join(root, "runtime-state.json") },
				argv: ["bun", "cli.ts"],
				execPath: "/bin/bun",
				platform: "linux",
				tty: interactiveTty,
	...
	it("preserves a replacement server when native create proof changes before profile mutation", () => {
		const calls: string[][] = [];
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-create-proof-change-"));
		__setCreateOwnerIsolationForTests({
			probe: {
				readCallerCgroup: () => "0::/\n",
		...
		const env = {
			VIB_TMUX_COMMAND: "tmux",
			VIB_TMUX_SESSION: "managed",
			VIB_COORDINATOR_SESSION_ID: "managed",
			VIB_COORDINATOR_SESSION_STATE_FILE: path.join(root, "runtime-state.json"),
		};
		expect(() => createVibTmuxSession(env)).toThrow("vib_tmux_owner_changed_after_create");
	...
	it("preserves a replacement server when native create status proof changes", () => {
		const calls: string[][] = [];
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-create-status-change-"));
		let proofCount = 0;
		__setCreateOwnerIsolationForTests({
			probe: {
				readCallerCgroup: () => "0::/\n",
		...
		expect(() => createVibTmuxSession(env)).toThrow("vib_tmux_precommit_failed_cleanup_failed");
	...
	it("does not kill a native ID reused by a same-name replacement during removal", () => {
		...
		expect(() => removeVibTmuxSession("managed", { VIB_TMUX_COMMAND: "tmux" })).toThrow(
			"vib_tmux_owner_changed:managed",
		);
		expect(calls.filter(call => call[1] === "kill-session")).toEqual([]);
	});
	...
	it("refuses a server swap after new-session before profile or cleanup mutation", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-tmux-server-swap-"));
		try {
			const calls: string[][] = [];
			const diagnostics: string[] = [];
			let probeCount = 0;
	...
	it("publishes one generation and propagates its lifecycle metadata to the managed child", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-tmux-owner-generation-"));
		try {
			...
			const handled = launchDefaultTmuxIfNeeded({
				...
				env: {
					...
					VIB_COORDINATOR_SESSION_STATE_FILE: stateFile,
					[VIB_COORDINATOR_SIDECAR_SIGNATURE_REQUIRED_ENV]: "true",
					[VIB_COORDINATOR_SIDECAR_SIGNING_KEY_ENV]: "managed-private-pkcs8-der-base64",
					[VIB_COORDINATOR_SIDECAR_KEY_ID_ENV]: "a".repeat(64),
				},
				argv: ["bun", "cli.ts"],
				execPath: "/bin/bun",
			...
			expect(innerCommand).toContain(`VIB_TMUX_OWNER_GENERATION='${generation.generation}'`);
			expect(innerCommand).toContain(`VIB_TMUX_OWNER_STATE_DIR='${root}'`);
			expect(innerCommand).toContain("VIB_TMUX_OWNER_SERVER_KEY='tmux'");
			expect(innerCommand).not.toContain("managed-private-pkcs8-der-base64");
			expect(innerCommand).toStartWith("exec env VIB_TMUX_LAUNCHED=1");
			expect(innerCommand).toMatch(
				/VIB_COORDINATOR_SIDECAR_BOOTSTRAP_URL='http:\/\/127\.0\.0\.1:\d+\/bootstrap\/[0-9a-f-]{36}'/i,
			);
			expect(innerCommand).not.toContain("__vib_sidecar_key");
			expect(innerCommand).toMatch(/VIB_MANAGED_OWNER_RUN_ID='[0-9a-f-]{36}'/i);
			expect(innerCommand).toMatch(/VIB_MANAGED_OWNER_INCARNATION='[0-9a-f-]{36}'/i);
			expect(innerCommand).not.toContain("VIB_MANAGED_OWNER_PREDECESSOR_TOKEN");
			expect(innerCommand).not.toContain("tmux-exit.json");
			expect(
				calls.some(call => call.includes("@vib-owner-generation") && call.at(-1) === generation.generation),
			).toBe(true);
			expect(calls.some(call => call.includes("@vib-owner-server-key") && call.at(-1) === "tmux")).toBe(true);
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
		}
	});
	...
	it("propagates only an exact durable SIGABRT predecessor token into a replacement launch", () => {
		...
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-tmux-replacement-"));
		try {
			...
			const handled = launchDefaultTmuxIfNeeded({
				...
				env: {
					VIB_COORDINATOR_SESSION_ID: sessionId,
					VIB_COORDINATOR_SESSION_STATE_FILE: path.join(root, "runtime-state.json"),
					VIB_TMUX_OWNER_STATE_DIR: root,
					VIB_TMUX_OWNER_GENERATION: generation,
					VIB_MANAGED_OWNER_RUN_ID: runId,
					VIB_MANAGED_OWNER_INCARNATION: incarnation,
					VIB_MANAGED_OWNER_PREDECESSOR_TOKEN: predecessorToken,
				},
				argv: ["bun", "cli.ts"],
				execPath: "/bin/bun",
				platform: "linux",
			...
			expect(handled).toBe(true);
			const innerCommand = calls.find(call => call[0] === "new-session")?.at(-1);
			expect(innerCommand).toContain(`VIB_MANAGED_OWNER_PREDECESSOR_TOKEN='${predecessorToken}'`);
			expect(innerCommand).toMatch(/VIB_TMUX_OWNER_GENERATION='[0-9a-f-]{36}'/i);
			expect(innerCommand).toMatch(/VIB_MANAGED_OWNER_RUN_ID='[0-9a-f-]{36}'/i);
			expect(innerCommand).toMatch(/VIB_MANAGED_OWNER_INCARNATION='[0-9a-f-]{36}'/i);
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
	...
	it("exact-rolls back a spawned owner when generation publication loses its baseline", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-tmux-owner-generation-race-"));
		try {
			...
			expect(cleanup).toEqual(
				expect.arrayContaining([
					...
					`kill-session -t ${NATIVE_SESSION_ID} \\; display-message -p __vib_tmux_guarded_cleanup_ok__`,
					"display-message -p __vib_tmux_guarded_cleanup_refused__",
				]),
			);
			expect(calls.some(call => call[0] === "attach-session")).toBe(false);
	...
	it("persists a fail-closed portable owner terminal verdict on Darwin", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-darwin-owner-finalization-"));
...
describe("tmux owner-isolation scoped bootstrap receipt", () => {
	...
	it("accepts only a bounded single-line receipt carrying an immutable native ID", () => {
		expect(
			isExactScopedBootstrapSuccessReceipt(
				'{"schema_version":1,"ok":true,"code":"bootstrapped","native_session_id":"$42","server_pid":7,"server_start_time":"77","session_name":"vib_rato"}',
			),
		).toBe(true);
	});
});
