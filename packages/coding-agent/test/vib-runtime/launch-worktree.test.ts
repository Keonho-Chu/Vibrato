...
import { buildDefaultTmuxLaunchPlan } from "@vib-rato/coding-agent/vib-runtime/launch-tmux";
import {
...
} from "@vib-rato/coding-agent/vib-runtime/launch-worktree";
...
function withWorktreeBucketDir<T>(value: string, body: () => T): T {
	const previous = process.env.VIB_WORKTREE_DIR;
	process.env.VIB_WORKTREE_DIR = value;
	try {
		return body();
	} finally {
		if (previous === undefined) delete process.env.VIB_WORKTREE_DIR;
		else process.env.VIB_WORKTREE_DIR = previous;
	}
}
...
describe("default launch worktrees", () => {
	...
	it("creates and reuses a detached launch worktree inside the source repo", async () => {
		const repo = await createRepo("vib-launch-worktree-");
	...
	for (const manager of ["bun", "npm", "pnpm"] as const) {
		it(`installs ${manager} workspace dependencies locally instead of linking the source tree`, async () => {
			const repo = await createRepo(`vib-launch-worktree-${manager}-workspace-`);
	...
	it("refuses an unowned external node_modules link instead of installing through it", async () => {
		const repo = await createRepo("vib-launch-worktree-external-modules-");
		await Bun.write(
		...
		const worktree = ensureLaunchWorktree(plan);
		if (!worktree.enabled) throw new Error("expected enabled worktree");
		const externalModules = await fs.mkdtemp(path.join(os.tmpdir(), "vib-external-node-modules-"));
	...
	it("creates launch worktrees under the canonical source repo when launched from an existing worktree", async () => {
		const repo = await fs.realpath(await createRepo("vib-launch-nested-source-worktree-"));
	...
	it("reports actionable diagnostics when the deterministic detached target is a different branch", async () => {
		const repo = await createRepo("vib-launch-target-mismatch-");
		const first = prepareLaunchWorktree(repo, ["--worktree"]);
		expect(first.worktree.enabled && first.worktree.created).toBe(true);
	...
	it("updates a clean reused detached launch worktree when source HEAD advances", async () => {
		const repo = await createRepo("vib-launch-advance-worktree-");
		const first = prepareLaunchWorktree(repo, ["--worktree"]);
		expect(first.worktree.enabled && first.worktree.created).toBe(true);
	...
	it("rejects dirty detached launch worktrees when source HEAD advances", async () => {
		const repo = await createRepo("vib-launch-dirty-worktree-");
		const first = prepareLaunchWorktree(repo, ["--worktree"]);
		expect(first.worktree.enabled && first.worktree.created).toBe(true);
	...
	it("creates named worktrees without reusing a dirty detached source-branch worktree", async () => {
		const repo = await createRepo("vib-launch-dirty-detached-named-worktree-");
	...
	it("reports a private, platform-neutral error for a broken bucket symlink without deleting it", async () => {
		const repo = await createRepo("vib launch 'broken-bucket-symlink-");
		...
		expect(message).toContain("Vibrato did not delete or replace the entry");
	...
	it("reclassifies a broken symlink racing the bucket mkdir instead of leaking raw EEXIST", async () => {
		const repo = await createRepo("vib-launch-bucket-mkdir-race-");
		const bucket = path.join(repo, ".worktrees");
		...
		try {
			expect(() => prepareLaunchWorktree(repo, ["--worktree", "feature/demo"])).toThrow(
				/worktree_bucket_broken_symlink[\s\S]*Vibrato did not delete or replace the entry/,
			);
		} finally {
	...
	it("does not treat non-ENOENT bucket inspection failures as a missing directory", async () => {
		const repo = await createRepo("vib-launch-bucket-inspection-failure-");
		const bucket = path.join(repo, ".worktrees");
		...
		try {
			expect(() => prepareLaunchWorktree(repo, ["--worktree", "feature/demo"])).toThrow(
				/worktree_bucket_inspection_failed[\s\S]*EACCES[\s\S]*Vibrato did not modify the entry/,
			);
		} finally {
	...
	it("allows a valid directory symlink or Windows junction as the worktree bucket", async () => {
		const repo = await createRepo("vib-launch-valid-bucket-symlink-");
		const bucket = path.join(repo, ".worktrees");
		const target = await fs.mkdtemp(path.join(os.tmpdir(), "vib-launch-bucket-target-"));
	...
	it("reports a symlink to a non-directory target without disclosing or deleting the target", async () => {
		const repo = await createRepo("vib-launch-bucket-file-symlink-");
		const bucket = path.join(repo, ".worktrees");
	...
	it("reports a regular-file bucket without shell text or deletion side effects", async () => {
		const repo = await createRepo("vib-launch-bucket-not-directory-");
		const bucket = path.join(repo, ".worktrees");
	...
	if (process.platform !== "win32") {
		it("reports a FIFO bucket as a non-directory without deleting it", async () => {
			const repo = await createRepo("vib-launch-bucket-fifo-");
			const bucket = path.join(repo, ".worktrees");
		...
		it("reports a Unix socket bucket as a non-directory without deleting it", async () => {
			const repo = await createRepo("vib-launch-bucket-socket-");
			const bucket = path.join(repo, ".worktrees");
	...
	it("creates named launch worktrees from reusable branch names", async () => {
		const repo = await createRepo("vib-launch-named-worktree-");
	...
	it("rejects an occupied nested path instead of adopting the source repository", async () => {
		const repo = await createRepo("vib-launch-occupied-worktree-");
		const planned = planLaunchWorktree(repo, { enabled: true, detached: false, name: "feature/demo" });
	...
	it("rejects a missing locked worktree instead of running status in its absent path", async () => {
		const repo = await createRepo("vib-launch-locked-worktree-");
		const planned = planLaunchWorktree(repo, { enabled: true, detached: false, name: "feature/demo" });
		const ensured = ensureLaunchWorktree(planned);
	...
	it("keeps launch worktree slugs collision-resistant for similar branch names", async () => {
		const repo = await createRepo("vib-launch-collision-worktree-");
	...
	it("adopts an existing sibling bucket named by the VIB_WORKTREE_DIR template", async () => {
		const repo = await createRepo("vib-launch-bucket-env-");
	...
	it("keeps the template repo-scoped so two repos never share one worktree path", async () => {
		const first = await createRepo("vib-launch-bucket-shared-a-");
		const second = await createRepo("vib-launch-bucket-shared-b-");
		const shared = await fs.mkdtemp(path.join(os.tmpdir(), "vib-launch-bucket-shared-root-"));
	...
	it("expands a home-relative override and falls back to the default bucket when unset", async () => {
		const repo = await createRepo("vib-launch-bucket-home-");
		const homePlan = withWorktreeBucketDir("~/vib-worktrees", () =>
		...
		expect(homePlan.enabled && path.dirname(homePlan.worktreePath)).toBe(path.join(os.homedir(), "vib-worktrees"));
	...
	it("rejects the repository-local bucket when Git does not ignore it", async () => {
		const repo = await createRepo("vib-launch-bucket-unignored-");
	...
	it("uses the launch worktree as the generated tmux cwd", async () => {
		const repo = await createRepo("vib-session-worktree-");
...
describe("VIB_WORKTREE_DIR path red-team", () => {
	it("fails closed when a {repo}-less template points two repos at one worktree path", async () => {
		const first = await createRepo("vib-launch-bucket-collision-a-");
		const second = await createRepo("vib-launch-bucket-collision-b-");
		const shared = await fs.mkdtemp(path.join(os.tmpdir(), "vib-launch-bucket-collision-root-"));
		cleanupPaths.push(shared);
	...
	it("fails closed when same-basename repos in different parents share a {repo} root", async () => {
		const parentA = await fs.mkdtemp(path.join(os.tmpdir(), "vib-launch-bucket-twin-a-"));
		const parentB = await fs.mkdtemp(path.join(os.tmpdir(), "vib-launch-bucket-twin-b-"));
		...
		const shared = await fs.mkdtemp(path.join(os.tmpdir(), "vib-launch-bucket-twin-root-"));
		cleanupPaths.push(shared);
	...
	it("pins the traversal contract: ../ segments resolve against the repository parent", async () => {
		const repo = await createRepo("vib-launch-bucket-traversal-");
	...
	it("treats a backslash home prefix literally on POSIX", async () => {
		const repo = await createRepo("vib-launch-bucket-backslash-");
		const plan = withWorktreeBucketDir("~\\vib-worktrees\\{repo}", () =>
			planLaunchWorktree(repo, { enabled: true, detached: false, name: "feature/demo" }),
		);
		...
		expect(plan.enabled && path.dirname(plan.worktreePath)).toBe(
			path.join(path.dirname(repo), `~\\vib-worktrees\\${path.basename(repo)}`),
		);
	});
});
