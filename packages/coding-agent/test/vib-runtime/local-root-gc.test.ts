...
const MARKER = ".vib-local-legacy-migrated-v1";
const PAST_GRACE_MS = 25 * 60 * 60 * 1000;

const tempDirs: string[] = [];

function makeTmpRoot(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vib-local-gc-test-"));
	tempDirs.push(dir);
	fs.mkdirSync(path.join(dir, "vib-local"), { recursive: true });
	return dir;
}

function seedRoot(tmpRoot: string, id: string, opts: { extra?: string; ageMs?: number } = {}): string {
	const dir = path.join(tmpRoot, "vib-local", id);
...
describe("local root GC", () => {
	it("reports nothing when the parent directory does not exist", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vib-local-gc-empty-"));
		tempDirs.push(dir);
