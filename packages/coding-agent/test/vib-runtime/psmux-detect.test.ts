...
import {
	...
	resolveVibTmuxExecutableIdentity,
	VIB_PSMUX_COMMAND_ENV,
	VIB_PSMUX_DETECTION_ENV,
	VIB_PSMUX_FORCE_DETECT_ENV,
...
} from "../../src/vib-runtime/tmux-provider-context";
...
describe("PSMUX_BINARY_NAMES", () => {
	...
	describe("Windows provider context", () => {
		it("uses one identity-anchored namespace for psmux aliases and emits structured argv", () => {
			...
			expect(namespace).toMatch(/^vib-[a-f0-9]{32}$/);
		...
		it("rejects an explicitly classified psmux command on non-Windows platforms", () => {
			...
			).toThrow("vib_tmux_provider_ambiguous: selected psmux command requires Windows");
		});
		...
		it("persists authority and rejects invalid sidecars before mutation", () => {
			const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-psmux-provider-"));
			try {
				__setBinaryResolverForTests(candidate => (candidate === "psmux" ? "C:\\psmux\\psmux.exe" : null));
				__setExecutableIdentityResolverForTests(() => "volume:42");
				const authority = bindVibTmuxProviderAuthority(
					resolveVibTmuxProviderContext({
						platform: "win32",
				...
				expect(() => assertVibTmuxMutationAuthoritySync(authority)).toThrow(
					"vib_tmux_provider_authority_generation_mismatch",
				);
				publishGeneration(root, authority.sessionId, authority.generation);
				...
				expect(() => assertVibTmuxMutationAuthoritySync(authority)).toThrow(
					"vib_tmux_provider_authority_invalid_record",
				);
				expect(() =>
					hasVibTmuxProviderAuthoritySync({
						stateDir: root,
				...
				).toThrow("vib_tmux_provider_authority_invalid_record");
			} finally {
				fs.rmSync(root, { recursive: true, force: true });
			}
		});
		it("treats Windows authority as absent for native probing while direct reads reject off Windows", () => {
			const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-psmux-off-windows-"));
			try {
				...
				).toThrow("vib_tmux_provider_authority_windows_required");
			} finally {
				fs.rmSync(root, { recursive: true, force: true });
			}
		});
		...
		it("rejects stale authority generations without mutating the successor pointer", () => {
			...
			try {
				...
				const first = bindVibTmuxProviderAuthority(context, {
					stateDir: root,
					sessionId: "team",
					generation: "generation-a",
				});
				const second = bindVibTmuxProviderAuthority(context, {
					stateDir: root,
					sessionId: "team",
					generation: "generation-b",
				});
				persistVibTmuxProviderAuthoritySync(first);
				persistVibTmuxProviderAuthoritySync(second);
				assertVibTmuxStagedMutationAuthoritySync(second);
				expect(() => persistVibTmuxProviderAuthoritySync(first)).toThrow();
				publishGeneration(root, second.sessionId, second.generation);
				expect(() => assertVibTmuxMutationAuthoritySync(first)).toThrow(
					"vib_tmux_provider_authority_generation_mismatch",
				);
				assertVibTmuxMutationAuthoritySync(second);
				const sidecar = path.join(
		...
		it("does not displace an expired lock whose independent owner is still live", () => {
			...
			try {
				...
				const successor = bindVibTmuxProviderAuthority(context, {
					stateDir: root,
					sessionId: "team",
					generation: "generation-b",
				});
				...
				expect(() => persistVibTmuxProviderAuthoritySync(successor)).toThrow("migration_busy");
		...
		it("rejects a same-path executable replacement before a provider mutation", () => {
			...
			try {
				...
				expect(() => assertVibTmuxMutationAuthoritySync(authority)).toThrow(
					"vib_tmux_provider_authority_executable_changed",
				);
			} finally {
				fs.rmSync(root, { recursive: true, force: true });
			}
		...
		it("rejects a pointer swap to another valid owner generation", () => {
			...
			try {
				...
				expect(() => assertVibTmuxMutationAuthoritySync(authority)).toThrow("vib_tmux_provider_authority_mismatch");
			} finally {
				fs.rmSync(root, { recursive: true, force: true });
			}
		});
...
describe("executable identity", () => {
	it("captures file identity rather than accepting a same-path replacement", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-psmux-identity-"));
		const executable = path.join(root, "psmux.exe");
		try {
			fs.writeFileSync(executable, "first");
			__setExecutableIdentityResolverForTests(null);
			const first = resolveVibTmuxExecutableIdentity(executable);
			fs.writeFileSync(executable, "replacement");
			const second = resolveVibTmuxExecutableIdentity(executable);
...
describe("detectPsmux", () => {
	...
	it("honors VIB_PSMUX_DETECTION=off and never reports psmux", () => {
		const detected = detectPsmux("psmux", {
			env: { [VIB_PSMUX_DETECTION_ENV]: "off" },
			runner: buildRunner(psmuxVersionOutput()),
			force: true,
		});
		expect(detected).toBe(false);
	...
	it("re-probes every call when VIB_PSMUX_FORCE_DETECT is set", () => {
		let calls = 0;
		...
		detectPsmux("psmux", {
			env: { [VIB_PSMUX_FORCE_DETECT_ENV]: "1" },
			runner,
			force: true,
		});
		detectPsmux("psmux", {
	...
	it("treats an explicit VIB_PSMUX_COMMAND override as authoritative", () => {
		...
		__setBinaryResolverForTests(() => null);
		const detected = detectPsmux("psmux", {
			env: { [VIB_PSMUX_COMMAND_ENV]: "psmux" },
			runner: failingRunner(),
			force: true,
		});
		expect(detected).toBe(true);
...
describe("resolveVibTmuxBinary", () => {
	it("returns the explicit VIB_TMUX_COMMAND override when set", () => {
		const resolved = resolveVibTmuxBinary({
			platform: "linux",
			env: { VIB_TMUX_COMMAND: "/custom/tmux" },
			runner: failingRunner(),
		});
	...
	it("rejects psmux when its executable identity is unavailable", () => {
		...
		).toThrow("vib_tmux_provider_ambiguous: Windows psmux executable identity is unavailable");
	});
	...
	it("rejects a psmux-marked tmux alias when its identity is unavailable", () => {
		...
		).toThrow("vib_tmux_provider_ambiguous: Windows tmux executable identity is unavailable");
	});
	...
	it("fails closed when an explicit Windows tmux.exe identity cannot be established", () => {
		...
		).toThrow("vib_tmux_provider_ambiguous");
	});
	...
	it("fails closed when VIB_PSMUX_COMMAND selects a different executable", () => {
		__setBinaryResolverForTests(candidate => `C:\\tools\\${candidate}.exe`);
		__setExecutableIdentityResolverForTests(path => path.toLowerCase());
		...
		expect(() =>
			resolveVibTmuxBinary({
				platform: "win32",
				env: { VIB_TMUX_COMMAND: "tmux", VIB_PSMUX_COMMAND: "C:\\other\\psmux-wrapper.exe" },
				runner: buildRunner(tmuxVersionOutput()),
			}),
		).toThrow("VIB_PSMUX_COMMAND selects a different executable");
	});
	...
	it("classifies a generic wrapper when VIB_PSMUX_COMMAND matches its executable identity", () => {
		__setBinaryResolverForTests(candidate => {
		...
		const resolved = resolveVibTmuxBinary({
			platform: "win32",
			env: { VIB_TMUX_COMMAND: "wrapper-tmux", VIB_PSMUX_COMMAND: "wrapper-psmux" },
			runner: buildRunner(tmuxVersionOutput()),
		});
	...
	it("treats an explicit Windows psmux path as psmux without relying on the version banner", () => {
		const resolved = resolveVibTmuxBinary({
			platform: "win32",
			env: { VIB_TMUX_COMMAND: "C:\\tools\\psmux.exe" },
			runner: buildRunner(tmuxVersionOutput()),
		});
...
describe("resolveVibTmuxCommand (shared session/team resolver)", () => {
	it("returns psmux on native Windows when psmux resolves and tmux.exe alias does not", () => {
		...
		// pick psmux so vib session ... commands talk to the same
		// multiplexer that vib --tmux just created.
		__setBinaryResolverForTests(candidate =>
			candidate === "psmux" || candidate === "pmux"
				? `C:\\Users\\runner\\AppData\\Local\\Microsoft\\WinGet\\Links\\${candidate}.exe`
				: null,
		);
		const command = resolveVibTmuxCommand({}, "win32");
	...
	it("honors VIB_TMUX_COMMAND override on every platform", () => {
		__setBinaryResolverForTests(() => null);
		const command = resolveVibTmuxCommand({ VIB_TMUX_COMMAND: "psmux" }, "win32");
		expect(command).toBe("psmux");
	});

	it("falls back to literal tmux on POSIX when no binary resolves", () => {
		__setBinaryResolverForTests(() => null);
		const command = resolveVibTmuxCommand({}, "linux");
		expect(command).toBe("tmux");
	});
});
