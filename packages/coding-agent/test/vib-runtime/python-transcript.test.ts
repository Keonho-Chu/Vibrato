...
import { openPythonKernelTranscript, type PythonTranscriptRecord } from "../../src/vib-runtime/python-transcript";
import { auditPath, pythonKernelTranscriptPath, sessionRoot } from "../../src/vib-runtime/session-layout";

const TEST_SESSION_ID = "python-transcript-session";
const tempRoots: string[] = [];

async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-python-transcript-"));
	tempRoots.push(dir);
	return dir;
}
