...
import { readVisibleSkillActiveState } from "@vib-rato/coding-agent/skill-state/active-state";
import {
...
} from "@vib-rato/coding-agent/vib-runtime/ralplan-runtime";
import {
	VIB_RALPLAN_ARTIFACT_ENV,
	VIB_RESTRICTED_ROLE_AGENT_BASH_ENV,
} from "@vib-rato/coding-agent/vib-runtime/restricted-role-agent-bash";
import {
	activeEntryPath,
	activeSnapshotPath,
	modeStatePath,
...
async function seedProjectRalplanMaxIterations(root: string, maxIterations = 5): Promise<void> {
	...
	// scenarios hermetic regardless of the developer's ~/.vib config.
	await fs.mkdir(path.join(root, ".vib"), { recursive: true });
	await fs.writeFile(
		path.join(root, ".vib", "config.yml"),
		YAML.stringify({ vib: { ralplan: { maxIterations } } }, null, 2),
		"utf-8",
	);
}
...
describe("native vib ralplan runtime — consensus handoff", () => {
...
describe("native vib ralplan runtime — --write artifact path", () => {
	it("persists an inline artifact under .vib/plans/ralplan/<run-id>/", async () => {
		const root = await tempDir();
		const result = await runNativeRalplanCommand(
			[
				"--write",
	...
	it("keeps role-subagent writes in the seeded owner session", async () => {
		...
		process.env.VIB_SESSION_ID = childSessionId;
		try {
			const result = await runNativeRalplanCommand(
				[
					"--write",
			...
			process.env.VIB_SESSION_ID = previousSessionId;
		}
	...
	it("restricted role-agent bash treats --artifact paths as inline text", async () => {
		...
		const previous = process.env[VIB_RESTRICTED_ROLE_AGENT_BASH_ENV];
		process.env[VIB_RESTRICTED_ROLE_AGENT_BASH_ENV] = "1";
		try {
			const result = await runNativeRalplanCommand(
				[
					"--write",
			...
			if (previous === undefined) {
				delete process.env[VIB_RESTRICTED_ROLE_AGENT_BASH_ENV];
			} else {
				process.env[VIB_RESTRICTED_ROLE_AGENT_BASH_ENV] = previous;
			}
		}
	});
	...
	it("--artifact-env reads artifact markdown from the sanctioned env var", async () => {
		const root = await tempDir();
		const previous = process.env[VIB_RALPLAN_ARTIFACT_ENV];
		process.env[VIB_RALPLAN_ARTIFACT_ENV] =
		...
		try {
			...
			if (previous === undefined) {
				delete process.env[VIB_RALPLAN_ARTIFACT_ENV];
			} else {
				process.env[VIB_RALPLAN_ARTIFACT_ENV] = previous;
			}
		}
	});
	...
	it("--artifact-env rejects arbitrary env variable names", async () => {
		...
		expect(result.status).toBe(2);
		expect(result.stderr).toContain("--artifact-env must be VIB_RALPLAN_ARTIFACT");
	});
...
describe("native vib ralplan runtime — run-state phase coherence", () => {
...
describe("native vib ralplan runtime — duplicate --write guard", () => {
...
describe("native vib ralplan runtime — persisted role-agent state", () => {
	...
	it("records fallback metadata on a critic write", async () => {
		...
		const result = await runNativeRalplanCommand(
			[
				...
				".vib/plans/ralplan/critic-fallback/stage-02-critic.md",
				"--json",
			],
			root,
		);
		...
		expect(JSON.parse(result.stdout ?? "{}").critic_state).toEqual({
			...
			critic_fallback_receipt_path: ".vib/plans/ralplan/critic-fallback/stage-02-critic.md",
		});
		const state = await readState(root);
		...
		expect(state.critic_fallback_receipt_path).toBe(".vib/plans/ralplan/critic-fallback/stage-02-critic.md");
	});
	...
	it("records fallback metadata together with a fresh planner id", async () => {
		...
		const result = await runNativeRalplanCommand(
			[
				...
				"3",
				"--fallback-receipt-path",
				".vib/plans/ralplan/pp-fb/stage-03-revision.md",
				"--json",
			],
			root,
		);
		...
		expect(state.planner_fallback_receipt_path).toBe(".vib/plans/ralplan/pp-fb/stage-03-revision.md");
...
describe("native vib ralplan runtime — post-clear re-activation (#644)", () => {
	...
	it("re-asserts active:true and resets phase out of terminal lock when a new run_id is written after a clear", async () => {
		...
		// Simulate `vib state ralplan clear`: active -> false, phase -> complete.
...
describe("ralplan automatic handoff admission (#3398)", () => {
	it("defaults to off when project and user config.yml files are absent", async () => {
		...
		const previousAgentOverride = process.env.VIB_CODING_AGENT_DIR;
		try {
			setAgentDir(userDir);
			...
			if (previousAgentOverride === undefined) delete process.env.VIB_CODING_AGENT_DIR;
			else process.env.VIB_CODING_AGENT_DIR = previousAgentOverride;
		}
	});
	...
	it("rejects malformed project config.yml rather than falling through to user config.yml", async () => {
		const root = await tempDir();
		const userDir = await tempDir();
		const projectPath = path.join(root, ".vib", "config.yml");
		const previousAgentDir = getAgentDir();
		const previousAgentOverride = process.env.VIB_CODING_AGENT_DIR;
		try {
			setAgentDir(userDir);
			await fs.writeFile(
				...
				YAML.stringify({ vib: { ralplan: { autoHandoff: "ultragoal" } } }, null, 2),
				"utf-8",
			);
	...
	it("rejects malformed agent config.yml instead of treating automatic handoff as off", async () => {
		...
		const proc = Bun.spawn([process.execPath, CONFIG_ROOT_CONFIG_PROBE, "--ralplan-auto-handoff"], {
			cwd: root,
			env: { ...process.env, HOME: home, VIB_CONFIG_DIR: configDir, VIB_CODING_AGENT_DIR: agentDir },
			stdout: "pipe",
			stderr: "pipe",
		});
	...
	it("rejects an invalid configured automatic handoff target in project config.yml", async () => {
		...
		const projectPath = path.join(root, ".vib", "config.yml");
		await fs.mkdir(path.dirname(projectPath), { recursive: true });
		await fs.writeFile(projectPath, YAML.stringify({ vib: { ralplan: { autoHandoff: "later" } } }, null, 2), "utf-8");

		await expect(resolveRalplanAutoHandoff(root)).rejects.toThrow(
			`invalid ralplan settings at ${projectPath}: expected vib.ralplan.autoHandoff to be one of off, ultragoal`,
		);
	});
	it("rejects invalid final admission config before writing final artifacts", async () => {
		...
		await fs.writeFile(
			path.join(root, ".vib", "config.yml"),
			YAML.stringify({ vib: { ralplan: { autoHandoff: "later" } } }, null, 2),
			"utf-8",
		);
	...
	it("resolves ultragoal and a usable team target from project config.yml", async () => {
		...
		expect(await resolveRalplanAutoHandoff(root)).toMatchObject({
			...
			source: path.join(root, ".vib", "config.yml"),
		});
	});
	...
	it("overlays a later durable PLANNING-STUCK marker on final dedupe", async () => {
		...
		await fs.writeFile(
			path.join(root, ".vib", "config.yml"),
			YAML.stringify({ vib: { ralplan: { autoHandoff: "ultragoal", maxIterations: 1 } } }, null, 2),
			"utf-8",
		);
	...
	it("uses the final ledger admission after state loss and config changes", async () => {
		...
		await fs.writeFile(
			path.join(root, ".vib", "config.yml"),
			YAML.stringify({ vib: { ralplan: { autoHandoff: "off" } } }, null, 2),
			"utf-8",
		);
...
describe("ralplan consensus iteration cap (#3165)", () => {
	...
	it("honors project config.yml maxIterations=2 and resets budget on new run_id", async () => {
		...
		await fs.writeFile(
			path.join(root, ".vib", "config.yml"),
			YAML.stringify({ vib: { ralplan: { maxIterations: 2 } } }, null, 2),
			"utf-8",
		);
...
describe("ralplan crash-gap dedupe repair", () => {
	...
	it("does not apply stale critic metadata while repairing a crash gap", async () => {
		...
		const repaired = await runNativeRalplanCommand(
			[
				...
				"2",
				"--fallback-receipt-path",
				".vib/plans/ralplan/stale-critic-metadata/stage-02-critic.md",
				"--json",
			],
			root,
		);
	...
	it("does not consume a raised-budget second slot while repairing a crash gap", async () => {
		...
		await fs.writeFile(
			path.join(root, ".vib", "config.yml"),
			YAML.stringify({ vib: { ralplan: { maxReviewPassesPerLane: 2 } } }, null, 2),
			"utf-8",
		);
		await fs.mkdir(runDir, { recursive: true });
...
describe("ralplan review lane budget config.yml", () => {
	it("resolves nested config.yml with project-over-user precedence", async () => {
		...
		try {
			...
			await fs.writeFile(
				projectPath,
				YAML.stringify({ vib: { ralplan: { maxReviewPassesPerLane: 3 } } }, null, 2),
				"utf-8",
			);
			...
			await fs.writeFile(
				projectPath,
				YAML.stringify({ vib: { ralplan: { maxReviewPassesPerLane: 4 } } }, null, 2),
				"utf-8",
			);
			expect(await resolveRalplanMaxReviewPassesPerLane(root)).toEqual({
	...
	it("rejects an invalid present project value rather than defaulting", async () => {
		...
		await fs.writeFile(
			projectPath,
			YAML.stringify({ vib: { ralplan: { maxReviewPassesPerLane: 99 } } }, null, 2),
			"utf-8",
		);
...
describe("ralplan review lane budget rigor and receipts", () => {
	...
	it("warns only on a raised-budget final slot and returns lane-specific stuck receipts", async () => {
		...
		await fs.mkdir(path.join(textRoot, ".vib"), { recursive: true });
		await fs.writeFile(
			path.join(textRoot, ".vib", "config.yml"),
			YAML.stringify({ vib: { ralplan: { maxReviewPassesPerLane: 2 } } }, null, 2),
			"utf-8",
		);
...
describe("native vib ralplan runtime — session agent directory threading", () => {
	it("the session agent directory reaches the strict settings resolution", async () => {
		...
		await fs.writeFile(
			...
			YAML.stringify({ vib: { ralplan: { maxIterations: 7, maxReviewPassesPerLane: 5 } } }, null, 2),
		);
		await fs.writeFile(
			path.join(tenantAgentDir, "config.yml"),
			YAML.stringify({ vib: { ralplan: { maxIterations: 9, maxReviewPassesPerLane: 3 } } }, null, 2),
		);
