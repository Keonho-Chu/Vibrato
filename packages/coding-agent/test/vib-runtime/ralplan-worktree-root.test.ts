...
import { runNativeRalplanCommand } from "@vib-rato/coding-agent/vib-runtime/ralplan-runtime";
...
beforeAll(() => {
	priorSessionId = process.env.VIB_SESSION_ID;
});
afterAll(async () => {
	if (priorSessionId !== undefined) process.env.VIB_SESSION_ID = priorSessionId;
	else delete process.env.VIB_SESSION_ID;
	await Promise.all(tempRoots.splice(0).map(dir => fs.rm(dir, recursiveForce)));
});

async function tempDir(prefix = "vib-ralplan-wt-"): Promise<string> {
...
async function initRepo(prefix?: string): Promise<string> {
	...
	await git(["init"], root);
	await git(
		["-c", "user.email=test@vib.local", "-c", "user.name=vib-test", "commit", "--allow-empty", "-m", "init"],
		root,
	);
	return root;
}
...
function statePath(root: string, sessionId: string): string {
	return path.join(root, ".vib", `_session-${sessionId}`, "state", "ralplan-state.json");
}

function runDir(root: string, sessionId: string, runId: string): string {
	return path.join(root, ".vib", `_session-${sessionId}`, "plans", "ralplan", runId);
}

function hudPath(root: string, sessionId: string): string {
	return path.join(root, ".vib", `_session-${sessionId}`, "state", "skill-active-state.json");
}

function activeEntryPath(root: string, sessionId: string): string {
	return path.join(root, ".vib", `_session-${sessionId}`, "state", "active", "ralplan.json");
}
...
describe("ralplan --worktree-root explicit target binding (#4693)", () => {
	...
	it("routes seed and child-role writes from a different cwd into one explicit target tree", async () => {
		...
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		...
		expect(await pathExists(path.join(dispatcher, ".vib"))).toBe(false);
	});
	...
	it("validates a committed target with Windows-safe git argv", async () => {
		...
		expect(await pathExists(path.join(target, ".vib"))).toBe(true);
		expect(await pathExists(path.join(dispatcher, ".vib"))).toBe(false);
	});
	...
	it("rejects missing, non-directory, non-git, and subdirectory targets before any mutation", async () => {
		...
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const plainDir = await tempDir("vib-ralplan-plain-");
		...
		for (const { label, root, match } of cases) {
			...
			expect(await pathExists(path.join(root, ".vib")), label).toBe(false);
		}
		expect(await pathExists(path.join(dispatcher, ".vib"))).toBe(false);
		expect(await pathExists(path.join(target, ".vib"))).toBe(false);
	});
	...
	it("rejects a linked worktree sharing commonDir when the run is bound to a different worktreeRoot", async () => {
		...
		const target = await initRepo("vib-ralplan-target-");
		const linked = path.join(await tempDir("vib-ralplan-linked-parent-"), "linked");
		...
		expect(await pathExists(path.join(linked, ".vib"))).toBe(false);
	});
	...
	it("resets the per-iteration lane budget from the target ledger after a revision", async () => {
		...
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		await fs.mkdir(path.join(target, ".vib"), { recursive: true });
		await fs.writeFile(path.join(target, ".vib", "config.yml"), "vib:\n  ralplan:\n    maxReviewPassesPerLane: 1\n");
	...
	it("canonicalizes absolute, nested-relative, and symlink --worktree-root onto one realpath", async () => {
		...
		const targetLink = await symlinkDir(target, "vib-ralplan-target-link-");
		...
		expect(await pathExists(path.join(dispatcher, ".vib"))).toBe(false);
		expect(await pathExists(path.join(path.dirname(targetLink), ".vib"))).toBe(false);
	});
	...
	it("keeps the bound run across a symlink writer cwd and a later dispatcher restart", async () => {
		...
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const dispatcherLink = await symlinkDir(dispatcher, "vib-ralplan-dispatcher-link-");
		const laterDispatcher = await initRepo("vib-ralplan-later-dispatcher-");
		const targetLink = await symlinkDir(target, "vib-ralplan-restart-target-link-");
		const canonical = await realpath(target);
		...
		expect(await pathExists(path.join(dispatcher, ".vib"))).toBe(false);
		expect(await pathExists(path.join(laterDispatcher, ".vib"))).toBe(false);
		expect(await pathExists(path.join(path.dirname(dispatcherLink), ".vib"))).toBe(false);
	});
	...
	it("rejects blank --worktree-root and records stuck markers only on the bound target", async () => {
		...
		expect(await pathExists(path.join(dispatcher, ".vib"))).toBe(false);
		expect(await pathExists(path.join(target, ".vib", `_session-${session}`))).toBe(false);
	...
	it("rejects a fake .git directory that is not a valid worktree before mutation", async () => {
		...
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const fake = await tempDir("vib-ralplan-fake-git-");
		...
		expect(await pathExists(path.join(fake, ".vib"))).toBe(false);
		expect(await pathExists(path.join(dispatcher, ".vib"))).toBe(false);
	});
	...
	it("rejects an explicit-target --artifact file that escapes the invoking cwd", async () => {
		...
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const outsider = await tempDir("vib-ralplan-outside-artifact-");
	...
	it("renders --worktree-root on the bundled role persistence command", () => {
		expect(ralplanPersistenceTemplate).toMatch(
			/vib ralplan --write --worktree-root <repository_binding\.worktreeRoot>/,
		);
	});

	it("public CLI validates --worktree-root before migrating dispatcher settings", async () => {
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		await fs.mkdir(path.join(dispatcher, ".vib"), { recursive: true });
		const settingsPath = path.join(dispatcher, ".vib", "settings.json");
		await fs.writeFile(settingsPath, JSON.stringify({ "vib.ralplan.maxIterations": 7 }));
		...
		const proc = Bun.spawn(
			[process.execPath, probe, "--worktree-root", missing, "--session-id", "cli-nomut", "--json", "task"],
			{
				...
				stdout: "pipe",
				stderr: "pipe",
				env: { ...process.env, VIB_SESSION_ID: undefined },
			},
		);
		...
		expect(await pathExists(path.join(dispatcher, ".vib", "config.yml"))).toBe(false);
	...
	it("rejects a forged detached HEAD that is not a git commit object", async () => {
		...
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const fake = await tempDir("vib-ralplan-forged-detached-");
		await fs.mkdir(path.join(fake, ".git"));
	...
	it.each(["blob", "tree"] as const)("rejects a %s object in HEAD before seed or write mutation", async kind => {
		...
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const target = await initRepo(`vib-ralplan-${kind}-head-`);
		const objectId =
			kind === "blob"
	...
	it("accepts a valid SHA-256 commit worktree", async () => {
		...
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const target = await tempDir("vib-ralplan-sha256-");
	...
	it("rejects a forged ref HEAD that does not resolve in the object database", async () => {
		...
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const fake = await tempDir("vib-ralplan-forged-ref-");
	...
	it("rejects a target whose .vib is a symlink outside the worktree", async () => {
		const session = "wt-vib-symlink";
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const outside = await tempDir("vib-ralplan-vib-outside-");
		await fs.symlink(outside, path.join(target, ".vib"));
		const seed = await runNativeRalplanCommand(
			["--worktree-root", target, "--session-id", session, "--json", "symlink vib"],
			dispatcher,
		);
		expect(seed.status).toBe(2);
		expect(seed.stderr ?? "").toMatch(/symlinked \.vib|escapes the target worktree/);
	...
	it("rejects an in-cwd artifact symlink so a swapped path cannot be followed", async () => {
		...
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const outsider = await tempDir("vib-ralplan-outside-swap-");
	...
	it("public CLI refuses a symlinked target .vib before settings migration", async () => {
		const target = await initRepo("vib-ralplan-target-");
		const dispatcher = await initRepo("vib-ralplan-dispatcher-");
		const outside = await tempDir("vib-ralplan-vib-outside-cli-");
		await fs.writeFile(path.join(outside, "settings.json"), JSON.stringify({ "vib.ralplan.maxIterations": 7 }));
		await fs.symlink(outside, path.join(target, ".vib"));
		const probe = path.join(import.meta.dir, "../fixtures/ralplan-cli-worktree-root-probe.ts");
		const proc = Bun.spawn(
			[process.execPath, probe, "--worktree-root", target, "--session-id", "cli-vib-symlink", "--json", "task"],
			{
				cwd: dispatcher,
				stdout: "pipe",
				stderr: "pipe",
		...
		const stderr = await new Response(proc.stderr).text();
		expect(status).toBe(2);
		expect(stderr).toMatch(/symlinked \.vib|escapes the target worktree/);
