...
import { runNativeRalplanCommand } from "../../src/vib-runtime/ralplan-runtime";
import {
...
describe("repository binding (#2901)", () => {
	it("captures and matches the same repository identity", async () => {
		const root = await fsp.mkdtemp(path.join(os.tmpdir(), "vib-repo-bind-"));
		tempRoots.push(root);
	...
	it("fails closed when active cwd is a sibling repository", async () => {
		const parent = await fsp.mkdtemp(path.join(os.tmpdir(), "vib-repo-siblings-"));
	...
	it("stamps omitted task bindings from session cwd (no implicit unbound lane)", async () => {
		const root = await fsp.mkdtemp(path.join(os.tmpdir(), "vib-repo-stamp-"));
		tempRoots.push(root);
		await initGitRepo(root);
	...
	it("rejects declared task binding that points at a sibling repo", async () => {
		const parent = await fsp.mkdtemp(path.join(os.tmpdir(), "vib-repo-task-sib-"));
		tempRoots.push(parent);
		const left = path.join(parent, "left");
		const right = path.join(parent, "right");
		await fsp.mkdir(left);
	...
	it("preserves source repository identity in a linked git worktree", async () => {
		const root = await fsp.mkdtemp(path.join(os.tmpdir(), "vib-repo-wt-"));
		tempRoots.push(root);
	...
	it("enforces ralplan repository binding on stage write after seed", async () => {
		const parent = await fsp.mkdtemp(path.join(os.tmpdir(), "vib-ralplan-bind-"));
		tempRoots.push(parent);
		const left = path.join(parent, "left");
		const right = path.join(parent, "right");
		await fsp.mkdir(left);
		...
		const envSession = process.env.VIB_SESSION_ID;
		...
		try {
			...
			const leftState = path.join(left, ".vib", `_session-${sessionId}`, "state", "ralplan-state.json");
			...
			if (envSession === undefined) delete process.env.VIB_SESSION_ID;
			else process.env.VIB_SESSION_ID = envSession;
		}
	});
});
