...
import { CANONICAL_VIB_WORKFLOW_SKILLS } from "@vib-rato/coding-agent/skill-state/canonical-skills";
import { renderCommandRefBlock } from "@vib-rato/coding-agent/vib-runtime/workflow-command-ref";
...
const skillFiles = CANONICAL_VIB_WORKFLOW_SKILLS.map(skill =>
	path.join(repoRoot, "packages", "coding-agent", "src", "defaults", "vib", "skills", skill, "SKILL.md"),
);
...
describe("workflow command-reference proof spike", () => {
	it("renders deterministic command-reference blocks", () => {
		...
		expect(first.bytes).toContain("<!-- vib:cmdref:start state -->");
		expect(first.bytes).toContain("vib state ralplan write --input");
	...
	it("reports byte-equal or gap status without mutating SKILL.md files", async () => {
		...
		const result = await runBun(["scripts/generate-vib-skill-command-refs.ts", "--check", "--json"]);
		...
		expect(parsed.results).toHaveLength(CANONICAL_VIB_WORKFLOW_SKILLS.length);
		for (const item of parsed.results) {
			expect([...CANONICAL_VIB_WORKFLOW_SKILLS] as string[]).toContain(item.skill);
	...
	it("prints per-skill token budget report", async () => {
		...
		expect(parsed.skills).toHaveLength(CANONICAL_VIB_WORKFLOW_SKILLS.length);
