...
async function runDoctorUnchanged(
	...
	const before = await snapshotFiles(path.join(root, ".vib"));
	const result = await runNativeStateCommand(args, root);
	const after = await snapshotFiles(path.join(root, ".vib"));
...
describe("vib state doctor", () => {
	...
	it("uses VIB_SESSION_ID for session-scoped active state when --session-id is absent", async () => {
		const root = await tempDir();
		...
		const prior = process.env.VIB_SESSION_ID;
		...
		try {
			...
			expect(result.stdout).not.toContain("vib state ralplan clear");
		} finally {
			if (prior === undefined) process.env.VIB_SESSION_ID = TEST_SESSION_ID;
			else process.env.VIB_SESSION_ID = prior;
		}
	});
	...
	it("detects orphan transaction journals and prints the hard prune fix command", async () => {
		...
		expect(text.stdout).toContain("fix=vib state prune --hard");

		const json = await runDoctorUnchanged(root, ["doctor", "--json"]);
		const parsed = JSON.parse(json.stdout ?? "{}");
		expect(parsed.ok).toBe(false);
		expect(parsed.problems).toEqual([
			expect.objectContaining({ type: "orphan_journal", path: journalPath, fixCommand: "vib state prune --hard" }),
		]);
	});
	...
	it("detects checksum mismatches and prints the migrate fix command", async () => {
		...
		expect(parsed.problems).toEqual([
			expect.objectContaining({
				...
				skill: "ralplan",
				path: statePath,
				fixCommand: "vib state ralplan migrate",
			}),
		]);
	});
	...
	it("detects schema violations and prints the migrate fix command", async () => {
		...
		expect(parsed.problems).toEqual([
			expect.objectContaining({
				...
				skill: "ultragoal",
				path: statePath,
				fixCommand: "vib state ultragoal migrate",
			}),
		]);
	});
	...
	it("detects stale active-state from raw snapshot and per-skill active entries", async () => {
		...
		expect(text.stdout).toContain("fix=vib state autoresearch clear");
		expect(text.stdout).toContain("fix=vib state ralplan clear");
		...
		expect(parsed.problems).toEqual([
			expect.objectContaining({
				...
				fixCommand: "vib state autoresearch clear",
			}),
			expect.objectContaining({
				type: "stale_active_state",
				skill: "ralplan",
				path: path.join(stateRoot, "skill-active-state.json"),
				fixCommand: "vib state ralplan clear",
			}),
		]);
	});
});
