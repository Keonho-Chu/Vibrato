...
import { renderStateGraph } from "@vib-rato/coding-agent/vib-runtime/state-graph";
import { runNativeStateCommand } from "@vib-rato/coding-agent/vib-runtime/state-runtime";

describe("Vibrato state graph rendering", () => {
	...
	it("renders one skill as dot", () => {
		...
		expect(output).toContain("digraph vib_state {");
