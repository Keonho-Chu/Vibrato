...
import { appendOrMergeDeepInterviewRound } from "@vib-rato/coding-agent/vib-runtime/deep-interview-recorder";
import { runNativeDeepInterviewCommand } from "@vib-rato/coding-agent/vib-runtime/deep-interview-runtime";
...
import { createUltragoalPlan, runNativeUltragoalCommand } from "@vib-rato/coding-agent/vib-runtime/ultragoal-runtime";

const TEST_SESSION_ID = "test-session";
const INITIAL_SESSION_ID = process.env.VIB_SESSION_ID;

function restoreSessionId(sessionId: string | undefined): void {
	if (sessionId === undefined) delete process.env.VIB_SESSION_ID;
	else process.env.VIB_SESSION_ID = sessionId;
}
...
async function tempDir(prefix = "vib-handoff-thrift-"): Promise<string> {
...
const activeTempArtifact = new RegExp(`${escapedTempRoot}/(?:skill-tool|vib)-[^\\n"]+`, "g");
...
describe("CONSUMER/KEY-FIELD MATRIX for compact handoff payloads", () => {
	it("goldens and asserts every preserved consumer key field", async () => {
		...
		expect(scrub(ralplanSeed.stdout ?? "")).toMatchInlineSnapshot(`
			"{"ok":true,"session_id":"test-session","skill":"ralplan","mode":"short","state_path":"/tmp/SCRUBBED","run_id":"run-b","handoff":"/skill:ralplan","repository_binding":{"schema":"vib.repository_binding.v1","worktreeRoot":"/tmp/SCRUBBED","commonDir":null,"displayPath":"/tmp/SCRUBBED"}}
			"
			`);
		...
		expect(scrub(ultragoalHandoff.stdout ?? "")).toMatchInlineSnapshot(`
			...
			vib-objective=Complete the durable ultragoal plan in .vib/ultragoal/goals.json, including later accepted/appended stories, under the original brief constraints; use .vib/ultragoal/ledger.jsonl as the audit trail.
	...
	it("documents the ralplan receipt-only guideline for role agents", async () => {
		...
		expect(skillDoc).toContain("vib ralplan --write");
