...
import { writeJsonAtomic } from "../../src/vib-runtime/state-writer";

const TEST_SESSION_ID = "test-session";

async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-state-integrity-"));
...
describe("vib state integrity", () => {
	it("blocks out-of-band edits on the next mode-state write unless forced", async () => {
		await withTempCwd(async cwd => {
			...
			expect(mismatch).toMatchObject({ skill: "ralplan", category: "state", owner: "vib-state-cli" });
	...
	it("an injected mid-handoff failure leaves a same-mutation journal while unrelated writes still proceed", async () => {
		await withTempCwd(async cwd => {
			...
			process.env.VIB_STATE_HANDOFF_FAIL_AFTER_CALLER = "__never__";
			const originalNow = Date.prototype.toISOString;
			Date.prototype.toISOString = () => "2026-06-03T00:00:00.000Z";
			process.env.VIB_STATE_HANDOFF_FAIL_AFTER_CALLER = "deep-interview:handoff:ralplan:2026-06-03T00:00:00.000Z";
			try {
				...
				delete process.env.VIB_STATE_HANDOFF_FAIL_AFTER_CALLER;
			}
