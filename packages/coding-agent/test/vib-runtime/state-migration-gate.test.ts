...
import { normalizeLegacyState } from "../../src/vib-runtime/state-migrations";
import { runNativeStateCommand } from "../../src/vib-runtime/state-runtime";

const TEST_SESSION_ID = "test-session";

async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-state-migration-"));
...
describe("G7 vib state migration gate", () => {
	...
	it("migrates tampered state with --force and audits the forced mismatch", async () => {
		await withTempCwd(async cwd => {
			...
			expect(persisted.receipt).toMatchObject({ owner: "vib-state-cli", status: "fresh" });
