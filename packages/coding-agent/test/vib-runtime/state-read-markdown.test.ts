...
import { readWorkflowStateJson, runNativeStateCommand } from "@vib-rato/coding-agent/vib-runtime/state-runtime";
...
describe("vib state read markdown", () => {
	it("defaults read output to markdown and keeps --json parseable", async () => {
		...
		await runNativeStateCommand(
			[
				...
				"deep-interview",
				"--input",
				JSON.stringify({ active: true, current_phase: "interviewing", artifact_path: ".vib/specs/draft.md" }),
			],
			root,
		);
		...
		expect(markdown.stdout).toContain(".vib/specs/draft.md");
	...
	it("rejects unknown vib state flags", async () => {
		const root = await tempDir();
		const result = await runNativeStateCommand(["read", "--mode", "deep-interview", "--bogus"], root);
		expect(result.status).toBe(2);
		expect(result.stderr).toContain("unknown vib state flag: --bogus");
	});
});
