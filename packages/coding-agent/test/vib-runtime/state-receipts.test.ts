...
import { runNativeStateCommand } from "../../src/vib-runtime/state-runtime";

const TEST_SESSION_ID = "test-session";

async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-state-receipts-"));
...
function expectValidReceipt(state: Record<string, unknown>, skill: string): void {
	...
	expect(["vib-state-cli", "vib-runtime", "vib-hook"]).toContain(receipt?.owner as string);
...
function findAuditEntry(
	...
	return entries.find(entry => entry.category === "state" && entry.verb === verb && entry.owner === "vib-state-cli");
}

describe("G5 vib state receipts", () => {
	it("persists receipts and audit entries for write, clear, and handoff", async () => {
		await withTempCwd(async cwd => {
			...
			const handoffEntries = entries.filter(
				entry => entry.category === "state" && entry.verb === "handoff" && entry.owner === "vib-state-cli",
			);
	...
	it("rejects missing receipt sessions instead of constructing a default path", async () => {
		expect(() =>
			buildWorkflowStateReceipt({
				...
				skill: "ralplan",
				owner: "vib-state-cli",
				command: "vib state ralplan write",
				sessionId: " ",
			}),
		).toThrow("non-empty Vibrato session id");

		await withTempCwd(async cwd => {
			const sessionId = process.env.VIB_SESSION_ID;
			...
			try {
				...
				if (sessionId !== undefined) process.env.VIB_SESSION_ID = sessionId;
			}
		});
	});
});
