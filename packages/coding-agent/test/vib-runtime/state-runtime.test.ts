...
describe("native vib state runtime", () => {
	...
	it("rejects write when no session id is resolvable", async () => {
		...
		try {
			...
			if (prior !== undefined) process.env.VIB_SESSION_ID = prior;
			else process.env.VIB_SESSION_ID = TEST_SESSION_ID;
		}
	});
	...
	it("errors read/status when no session directories exist", async () => {
		...
		try {
			...
			expect(read.stderr).toContain("no active Vibrato session found");
			const status = await runNativeStateCommand(["status", "--mode", "deep-interview", "--json"], root);
			expect(status.status).toBe(2);
			expect(status.stderr).toContain("no active Vibrato session found");
		} finally {
			if (prior !== undefined) process.env.VIB_SESSION_ID = prior;
			else process.env.VIB_SESSION_ID = TEST_SESSION_ID;
		}
	...
	it("infers the active workflow for write when --mode/positional/input.skill are absent", async () => {
		...
		// Bundled prompt shape: vib state write --input '<json>' (no --mode)
		const result = await runNativeStateCommand(
