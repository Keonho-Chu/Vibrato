...
} from "@vib-rato/coding-agent/vib-runtime/state-schema";

const roots: string[] = [];

async function tempFile(name: string, content: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-state-schema-"));
	roots.push(dir);
...
describe("state-schema (A1)", () => {
	...
	it("RequiredOnWriteEnvelopeSchema requires anchored fields incl. stamped receipt", () => {
		const validReceipt = {
			...
			state_path: "/x/.vib/state/ralplan-state.json",
			storage_path: "/x/.vib/state/ralplan-state.json",
...
describe("readVibJson (A2)", () => {
	it("returns null for a missing file", async () => {
		const result = await readVibJson(path.join(os.tmpdir(), "vib-nope-xyz.json"), WorkflowStateEnvelopeSchema);
	...
	it("returns ok for valid content and preserves raw", async () => {
		...
		const result = await readVibJson(file, WorkflowStateEnvelopeSchema);
