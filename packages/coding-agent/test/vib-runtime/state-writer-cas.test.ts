...
import { sessionStateDir } from "@vib-rato/coding-agent/vib-runtime/session-layout";
import {
	updateJsonAtomic,
...
async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-state-writer-cas-"));
	tempRoots.push(dir);
	return dir;
}
