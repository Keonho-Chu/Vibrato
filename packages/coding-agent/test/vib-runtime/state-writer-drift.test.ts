...
import { WORKFLOW_STATE_VERSION } from "@vib-rato/coding-agent/skill-state/workflow-state-contract";
...
import { auditPath, modeStatePath } from "@vib-rato/coding-agent/vib-runtime/session-layout";
import { migrateAndPersistLegacyState } from "@vib-rato/coding-agent/vib-runtime/state-migrations";
import { runNativeStateCommand } from "@vib-rato/coding-agent/vib-runtime/state-runtime";
import { RequiredOnWriteEnvelopeSchema } from "@vib-rato/coding-agent/vib-runtime/state-schema";
import { writeWorkflowEnvelopeAtomic } from "@vib-rato/coding-agent/vib-runtime/state-writer";
...
async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-state-writer-drift-"));
	tempRoots.push(dir);
	return dir;
}
