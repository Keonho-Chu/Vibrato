...
import { appendJsonlIdempotent } from "@vib-rato/coding-agent/vib-runtime/state-writer";

const tempRoots: string[] = [];

async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-idempotent-append-"));
	tempRoots.push(dir);
	return dir;
}
...
describe("appendJsonlIdempotent (issue #660)", () => {
	it("appends a fresh entry and reports appended: true", async () => {
		...
		const target = ".vib/ledger/index.jsonl";
	...
	it("throws when neither key nor equals is supplied", async () => {
		const root = await tempDir();
		await expect(
			appendJsonlIdempotent(".vib/ledger/index.jsonl", { id: "a" }, { cwd: root } as never),
