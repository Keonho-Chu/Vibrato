...
import { replaceOwnerGeneration } from "@vib-rato/coding-agent/vib-runtime/tmux-owner-isolation";
import {
...
function run(args: string[], env: NodeJS.ProcessEnv): void {
	const result = Bun.spawnSync([env.VIB_TMUX_COMMAND!, ...args], { stdout: "pipe", stderr: "pipe", env });
...
describe.skipIf(!isLinux || !tmux || !userScopeAvailable)("tmux exact owner close integration", () => {
	afterEach(async () => {
		for (const server of isolatedServers.splice(0)) {
			Bun.spawnSync([server.env.VIB_TMUX_COMMAND!, "kill-server"], {
				stdout: "pipe",
				stderr: "pipe",
				env: server.env,
			});
	...
	it("uses TERM, publishes the expected verdict, then cleans up the real tmux session", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-close-integration-"));
		const tmuxTmpDir = path.join(stateDir, "tmux");
		const socketName = `vib-close-${crypto.randomUUID().slice(0, 8)}`;
		const scopeName = `vib-owner-test-${crypto.randomUUID().slice(0, 8)}.scope`;
		const tmuxWrapper = path.join(stateDir, "isolated-tmux");
		const sessionName = `vib_close_${crypto.randomUUID().slice(0, 8)}`;
		const siblingSessionName = `vib_sibling_${crypto.randomUUID().slice(0, 8)}`;
...
import { registerCoordinatorRuntimeStateFinalizer } from ${JSON.stringify(path.resolve(import.meta.dir, "../../src/vib-runtime/session-state-sidecar.ts"))};
registerCoordinatorRuntimeStateFinalizer({ sessionId: ${JSON.stringify(sessionId)}, cwd: ${JSON.stringify(stateDir)}, sessionFile: null });
await writeFile(${JSON.stringify(childReadyFile)}, JSON.stringify({ launched: process.env.VIB_TMUX_LAUNCHED, generation: process.env.VIB_TMUX_OWNER_GENERATION, stateDir: process.env.VIB_TMUX_OWNER_STATE_DIR, socketKey: process.env.VIB_TMUX_OWNER_SERVER_KEY }));
setInterval(() => {}, 1_000);
`,
		);
		await fs.writeFile(
			supervisorScript,
			`import { writeFile } from "node:fs/promises";
import { runManagedOwnerSupervisor } from ${JSON.stringify(path.resolve(import.meta.dir, "../../src/vib-runtime/managed-owner-supervisor.ts"))};
try {
	...
	throw error;
		...
		await fs.writeFile(tmuxWrapper, `#!/usr/bin/env sh\nexec ${tmux} -L "$VIB_TEST_TMUX_SOCKET" "$@"\n`, {
			mode: 0o700,
		});
		const env = {
			...process.env,
			VIB_TMUX_COMMAND: tmuxWrapper,
			VIB_TEST_TMUX_SOCKET: socketName,
			TMUX_TMPDIR: tmuxTmpDir,
			VIB_TMUX_LAUNCHED: "1",
			VIB_TMUX_OWNER_GENERATION: generation,
			VIB_TMUX_OWNER_STATE_DIR: stateDir,
			VIB_TMUX_OWNER_SERVER_KEY: sessionName,
			...
			VIB_MANAGED_OWNER_COMMAND_JSON: JSON.stringify([process.execPath, childScript]),
		};
		...
		const hasSession = (name: string) =>
			Bun.spawnSync([env.VIB_TMUX_COMMAND!, "has-session", "-t", `=${name}`], {
				stdout: "pipe",
				stderr: "pipe",
				env,
			});
		...
		expect(statusVibTmuxSession(sessionName, env)).toMatchObject({
			profile: "1",
			sessionId,
		...
		expect(listVibTmuxSessions(env).find(session => session.name === sessionName)).toMatchObject({
			profile: "1",
			sessionId,
			sessionStateFile: stateFile,
		});
		const panePid = Number(
			Bun.spawnSync([env.VIB_TMUX_COMMAND!, "display-message", "-p", "-t", target, "#{pane_pid}"], {
				stdout: "pipe",
				stderr: "pipe",
				env,
			})
		...
		await forceCloseVibTmuxSession(sessionName, env, sessionId, stateFile, {
			resolveOwner: async () => ({
				sessionId,
				stateDir,
				socketKey: sessionName,
