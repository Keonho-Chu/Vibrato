...
import { buildWindowsPowerShellInnerCommand } from "@vib-rato/coding-agent/vib-runtime/launch-tmux";
import {
...
import {
	...
	buildVibTmuxUntaggedSessionHint,
} from "@vib-rato/coding-agent/vib-runtime/tmux-common";
import {
...
} from "@vib-rato/coding-agent/vib-runtime/tmux-provider-context";
import {
...
function injectSafeAbsentToSafeOwnerProof(): void {
	...
	__setCreateOwnerIsolationForTests({
		probe: {
			readCallerCgroup: () => "0::/user.slice/user-1000.slice/user@1000.service/vib-owner-test.scope\n",
			probeServer: () => {
				probeCount += 1;
				return probeCount === 1
					? { state: "absent" }
...
function installPsmuxAuthorityFixture(
	...
	const context = resolveVibTmuxProviderContext({
	...
	persistVibTmuxProviderAuthoritySync(
		bindVibTmuxProviderAuthority(context, {
			stateDir,
			sessionId,
			generation,
		}),
...
describe("Vibrato tmux session management", () => {
	beforeEach(() => {
	...
	it("lists only Vibrato-managed tmux sessions", () => {
		spyOn(Bun, "spawnSync").mockReturnValue(
			spawnResult(
				0,
				[
					"vib_rato_abc\t1\t0\t1770000000\t1\troot\t2\t12345\tfeature/demo\tfeature-demo\t/repo-a\t\t\t\t\t\t$1",
					"unrelated	2	1	1770000060		root	3	23456		",
					"vib_rato	1	1	1770000120		root	1	34567		",
				].join("\n"),
			),
		);

		clearPsmuxDetectionCache();
		const sessions = listVibTmuxSessions({ VIB_TMUX_COMMAND: "tmux-test" });

		expect(sessions.map(session => session.name)).toEqual(["vib_rato_abc"]);
		...
		expect(Bun.spawnSync).toHaveBeenCalledWith(
			[
				...
				"list-sessions",
				"-F",
				"#{session_name}\t#{session_windows}\t#{session_attached}\t#{session_created}\t#{@vib-profile}\t#{session_key_table}\t#{session_panes}\t#{pane_pid}\t#{@vib-branch}\t#{@vib-branch-slug}\t#{@vib-project}\t#{@vib-session-id}\t#{@vib-session-state-file}\t#{@vib-owner-generation}\t#{@vib-version}\t#{@vib-psmux-incarnation}\t#{session_id}",
			],
			expect.any(Object),
		);
	});
	...
	it("returns an empty list when tmux has no server", () => {
		...
		expect(listVibTmuxSessions()).toEqual([]);
	});
	...
	it("returns an empty list when the default socket reports an ENOENT connection error", () => {
		...
		expect(listVibTmuxSessions({ VIB_TMUX_COMMAND: "tmux-test" })).toEqual([]);
	});
	...
	it("propagates permission and transport failures that use failed-connect diagnostics", () => {
		...
		expect(() => listVibTmuxSessions({ VIB_TMUX_COMMAND: "tmux-test" })).toThrow(stderr);
	...
	// so vib used to report zero sessions while sessions were live on the
	// default socket.
	it("retries on the default socket when the inherited $TMUX socket is unreachable", () => {
		...
		const sessions = listVibTmuxSessions({ VIB_TMUX_COMMAND: "tmux-test", TMUX: `${inheritedSocket},0,1` });
	...
	it("retries when an inherited socket uses the alternate failed-connect ENOENT diagnostic", () => {
		...
		spyOn(Bun, "spawnSync").mockImplementation(((command: unknown, options: unknown) => {
			...
			return spawnResult(0, "vib_rato_alt\t1\t0\t1770000000\t1\troot\t1\t12345\t\t\t\t\t\t\t\t$2");
		}) as never);
		clearPsmuxDetectionCache();

		expect(listVibTmuxSessions({ VIB_TMUX_COMMAND: "tmux-test", TMUX: `${inheritedSocket},0,1` })).toHaveLength(1);
	...
	it("does not retry a case-mismatched socket path on case-sensitive hosts", () => {
		...
		expect(listVibTmuxSessions({ VIB_TMUX_COMMAND: "tmux-test", TMUX: "/tmp/host-emulated/agent-team,0,1" })).toEqual(
			[],
		);
	...
	it("still reports an empty list when the default socket has no server either", () => {
		...
		expect(listVibTmuxSessions({ VIB_TMUX_COMMAND: "tmux-test", TMUX: `${inheritedSocket},0,1` })).toEqual([]);
	});
	...
	it("denies mutations for sessions discovered through an automatic socket fallback", () => {
		...
		spyOn(Bun, "spawnSync").mockImplementation(((command: unknown, options: unknown) => {
			...
			const inherited = (options as { env?: NodeJS.ProcessEnv } | undefined)?.env?.TMUX;
			if (inherited) return spawnResult(1, "", `error connecting to ${inheritedSocket} (No such file or directory)`);
			return spawnResult(0, "vib_rato_fallback\t1\t0\t1770000000\t1\troot\t0\t\t\t\t\t\t\t\t$3");
		}) as never);
		clearPsmuxDetectionCache();

		expect(() =>
			removeVibTmuxSession("vib_rato_fallback", {
				VIB_TMUX_COMMAND: "tmux-test",
				TMUX: `${inheritedSocket},0,1`,
			}),
		).toThrow("vib_tmux_fallback_authority_unconfirmed");
	});

	it("reports provider-aware diagnostics before spawning when Windows has no multiplexer", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-sessions-test-"));
		...
		expect(() => listVibTmuxSessions({ VIB_TMUX_OWNER_STATE_DIR: stateDir })).toThrow(
			"vib_tmux_provider_unavailable — Vibrato searched for psmux, pmux, and tmux on PATH.",
		);
		expect(spawnSyncSpy).not.toHaveBeenCalled();
	});

	it("guards status and remove to Vibrato-managed sessions", () => {
		...
		// resolveVibTmuxCommand now picks the first available multiplexer on
		// Windows; we explicitly opt into literal tmux for this guard test.
		const env = { VIB_TMUX_COMMAND: "tmux" };
		const calls: string[][] = [];
		...
		spawnSyncSpy.mockImplementation(command => {
			...
			if (cmd.includes("list-sessions")) {
				return spawnResult(0, "vib_rato_work	1	0	1770000000	1	root	1			\n");
			}
			if (cmd.includes("show-options")) return spawnResult(0, "1\n");
			if (cmd.includes("if-shell")) return spawnResult(0, "__vib_tmux_guarded_mutation_ok__\n");
			if (cmd.includes("display-message")) return spawnResult(0, "$0\n");
			return spawnResult(0, "");
		});

		expect(statusVibTmuxSession("vib_rato_work", env).name).toBe("vib_rato_work");
		expect(() => statusVibTmuxSession("unrelated", env)).toThrow("vib_tmux_session_not_found:unrelated");
		injectSafeMutationProof();
		expect(removeVibTmuxSession("vib_rato_work", env).name).toBe("vib_rato_work");
	...
	it("refuses a same-name replacement before the final guarded remove", () => {
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation(command => {
			...
			if (commandArgv.includes("list-sessions")) {
				return spawnResult(
					0,
					"vib_rato_work\t1\t0\t1770000000\t1\troot\t0\t\tmain\tmain\t/repo\tsession-1\t/tmp/runtime-state.json\tgeneration-one\t\t$1\n",
				);
			}
			if (commandArgv.includes("display-message")) return spawnResult(0, "$2\n");
			if (commandArgv.includes("show-options")) {
				return spawnResult(0, commandArgv.at(-1) === "@vib-owner-generation" ? "generation-one\n" : "1\n");
			}
			return spawnResult(0, "");
		});

		expect(() =>
			removeVibTmuxSession("vib_rato_work", env, {
				nativeSessionId: "$1",
		...
		).toThrow("vib_tmux_owner_changed:vib_rato_work");
	...
	it("refuses unsafe or unverifiable server proof before any remove, attach, or force-close mutation", async () => {
		for (const proofError of [
			"vib_tmux_owner_isolation_server_unsafe",
			"vib_tmux_owner_isolation_server_unverifiable",
		]) {
			const calls: string[][] = [];
			...
			(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation((rawSpawn: unknown) => {
				...
				if (command.includes("list-sessions"))
					return spawnResult(
						0,
						"vib_rato_work\t1\t0\t1770000000\t1\troot\t0\t\t\t\tsession\t/state/marker\tgeneration\tvib_rato_work\n",
					);
				...
				if (command.includes("show-options")) {
					...
					return spawnResult(
						...
						option === "@vib-profile"
							? "1\n"
							: option === "@vib-session-id"
								? "session\n"
								: option === "@vib-owner-generation"
									? "generation\n"
									: option === "@vib-owner-server-key"
										? "vib_rato_work\n"
										: "/state/marker\n",
					);
				}
			...
			expect(() => removeVibTmuxSession("vib_rato_work", { VIB_TMUX_COMMAND: "tmux" })).toThrow(proofError);
			expect(() => attachVibTmuxSession("vib_rato_work", { VIB_TMUX_COMMAND: "tmux" })).toThrow(proofError);
			await expect(
				forceCloseVibTmuxSession("vib_rato_work", { VIB_TMUX_COMMAND: "tmux" }, undefined, undefined, {
					resolveOwner: async () => ({
						sessionId: "session",
						stateDir: "/state",
						socketKey: "vib_rato_work",
	...
	it("does not kill when final live profile check fails", () => {
		...
		expect(() => removeVibTmuxSession("vib_rato_work")).toThrow("vib_tmux_session_not_managed:vib_rato_work");
	...
	it("explains ProviderAuthority recovery for an untagged multiplexer session", () => {
		const hint = buildVibTmuxUntaggedSessionHint("psmux");
		...
		expect(hint).toContain("VIB_TMUX_COMMAND is a binary override, not a shell command line");
	});
	...
	it("hydrates native Windows tmux sessions from exact option reads when list-sessions omits user options", () => {
		...
		spawnSyncSpy.mockImplementation((rawSpawn: unknown) => {
			...
			if (cmd.includes("show-options")) {
				...
				if (option === "@vib-branch") return spawnResult(0, "issue-882-windows-tmux\n");
				return spawnResult(0, "\n");
			}
			return spawnResult(0, "");
		});

		const session = statusVibTmuxSession("win_session", {
		...
		expect(calls).toContainEqual(["tmux", "show-options", "-qv", "-t", "=win_session:", "@vib-profile"]);
	});
	...
	it("still reports plain not-found when the multiplexer does not list the session", () => {
		...
		expect(() => statusVibTmuxSession("ghost")).toThrow("vib_tmux_session_not_found:ghost");
	});
	...
	it("builds a window-qualified exact target for tmux option commands", () => {
		...
		expect(buildVibTmuxExactOptionTarget("vib_rato_work")).toBe("=vib_rato_work:");
	});
	...
	it("queries the profile option with a window-qualified exact target", () => {
		...
		const showOptions = calls.find(call => call.includes("show-options") && call.includes("@vib-profile"));
		expect(showOptions).toEqual(["tmux", "show-options", "-qv", "-t", "=vib_rato_work:", "@vib-profile"]);
	...
	it("builds psmux-aware targets for session-scoped commands", () => {
		...
		try {
			expect(
				buildVibTmuxExactSessionTarget("work", {
			...
			expect(
				buildVibTmuxExactSessionTarget("work", {
					env: { VIB_TMUX_COMMAND: "pmux", VIB_PSMUX_COMMAND: "pmux" },
				}),
			).toBe("work");
		} finally {
			__setBinaryResolverForTests(null);
	...
	it("drops the tmux `=NAME` exact-session prefix on psmux for option commands", () => {
		...
		// BinaryResolver test seam + VIB_PSMUX_COMMAND override so the
		...
		try {
			expect(
				buildVibTmuxExactOptionTarget("work", {
					env: { VIB_TMUX_COMMAND: "tmux" },
				}),
			).toBe("=work:");
			expect(
	...
	it("hydrates native psmux sessions even when -F is silently ignored", async () => {
		...
		try {
			...
			// recover the @vib-profile tag via follow-up show-options calls.
			//
			...
			spawnSyncSpy.mockImplementation((rawSpawn: unknown) => {
				...
				if (cmd.includes("show-options")) {
					const option = cmd.at(-1);
					if (option === "@vib-profile") return spawnResult(0, "@vib-profile 1");
					return spawnResult(0, "");
				}
				return spawnResult(0, "");
			});
			...
			const sessions = listVibTmuxSessions({
				...
				VIB_COORDINATOR_SESSION_STATE_FILE: path.join(stateDir, "runtime-state.json"),
			});
			...
			expect(sessions[0].providerAuthority?.namespace).toMatch(/^vib-[a-f0-9]{32}$/);
			// Follow-up reads use the durable authority's psmux executable and namespace.
			expect(calls).toContainEqual([
				"/fake/psmux",
				"-L",
				expect.stringMatching(/^vib-[a-f0-9]{32}$/),
				"show-options",
				"-qv",
				"-t",
				"psmux_session",
	...
	it("rejects same-name psmux sessions discovered through distinct provider authorities", async () => {
		...
		try {
			...
			spawnSyncSpy.mockImplementation((rawSpawn: unknown) => {
				...
				if (cmd.includes("list-sessions"))
					return spawnResult(0, "psmux_session: 1 windows (created Sat Jun 27 17:00:00 2026)\n");
				if (cmd.includes("show-options") && cmd.at(-1) === "@vib-profile") return spawnResult(0, "@vib-profile 1");
				return spawnResult(0, "");
			});
			expect(() =>
				listVibTmuxSessions({
			...
			).toThrow("vib_tmux_provider_authority_ambiguous:psmux_session");
		} finally {
			__setBinaryResolverForTests(null);
			__setExecutableIdentityResolverForTests(null);
		}
	...
	it("fails closed before tagging when native session identity is unavailable", () => {
		...
		expect(() => createVibTmuxSession({ VIB_TMUX_COMMAND: "tmux" })).toThrow(
			"vib_tmux_owner_isolation_native_session_identity_unavailable",
		);
				...
				createVibTmuxSession(
					{
						VIB_TMUX_COMMAND: "psmux",
						VIB_PSMUX_COMMAND: "psmux",
						VIB_TMUX_SESSION: "psmux_session",
						VIB_COORDINATOR_SESSION_STATE_FILE: path.join(stateDir, "runtime-state.json"),
					} as NodeJS.ProcessEnv,
					{ platform: "win32" },
				),
			).toThrow("vib_tmux_provider_ambiguous: selected Windows psmux executable identity is unavailable");
	...
	it("builds a BOM-free encoded command for psmux with literal PowerShell values and arguments", () => {
		const encoded = buildWindowsPowerShellInnerCommand({
			command: ["C:\\Program Files\\Vibrato\\O'Brien\\vib.exe"],
			args: ["--resume", "operator's session", "--label=O'Brien"],
			environment: {
				VIB_PSMUX_COMMAND: "C:\\Program Files\\O'Brien\\psmux.exe",
				VIB_TEST_VALUE: "operator's value",
			},
		});
		...
		expect(script).toContain("$env:VIB_PSMUX_COMMAND = 'C:\\Program Files\\O''Brien\\psmux.exe'");
		expect(script).toContain("$env:VIB_TEST_VALUE = 'operator''s value'");
		expect(script).toContain(
			"& 'C:\\Program Files\\Vibrato\\O''Brien\\vib.exe' '--resume' 'operator''s session' '--label=O''Brien'",
		);
	});
	...
	it("passes the shared encoded command to injected win32 session creation", () => {
			...
			createVibTmuxSession(
				{
					...
					VIB_TMUX_COMMAND: "tmux",
					VIB_TMUX_SESSION: "psmux-session",
					VIB_COORDINATOR_SESSION_ID: "operators-session",
					VIB_COORDINATOR_SESSION_STATE_FILE: stateFile,
				},
				{ platform: "win32" },
			),
		).toThrow("vib_tmux_owner_isolation_scope_bootstrap_failed:test-stop");
		...
		const generation = script.match(/\$env:VIB_TMUX_OWNER_GENERATION = '([^']+)'/)?.[1];
		...
		expect(innerCommand).toBe(
			buildWindowsPowerShellInnerCommand({
				...
				environment: {
					...
					VIB_TMUX_OWNER_STATE_DIR: "C:\\Users\\O'Brien",
					VIB_TMUX_OWNER_SERVER_KEY: "tmux",
					VIB_COORDINATOR_SESSION_ID: "operators-session",
					VIB_COORDINATOR_SESSION_STATE_FILE: stateFile,
				},
			}),
	...
	it("refuses psmux before attach-session mutation", () => {
		...
		try {
			...
			expect(() =>
				attachVibTmuxSession("managed", {
					VIB_TMUX_COMMAND: "psmux",
					VIB_PSMUX_COMMAND: "psmux",
				}),
			).toThrow("vib_tmux_provider_authority_unavailable");
	...
	it("refuses an unbound native creation receipt before tagging or cleanup", () => {
		...
		expect(() =>
			createVibTmuxSession({
				...
				VIB_COORDINATOR_SESSION_STATE_FILE: path.join(os.tmpdir(), `vib-unbound-${crypto.randomUUID()}.json`),
			}),
		).toThrow("vib_tmux_owner_changed_after_create");
		expect(calls.some(cmd => cmd.includes("set-option") || cmd.includes("set-window-option"))).toBe(false);
	...
	it("preserves a created session without publishing when metadata readback drops a required value", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-metadata-readback-"));
		fixtureDirectories.push(stateDir);
		injectSafeMutationProof();
		__setCreateOwnerIsolationForTests({
			probe: {
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation((rawCommand: unknown) => {
			...
			if (command.includes("if-shell")) {
				const match = command.join(" ").match(/@vib-owner-generation" "([^"]+)"/);
		...
		).toThrow("vib_tmux_created_metadata_mismatch");
	...
	it("refuses CAS-failure cleanup when a replacement server appears before the guarded kill", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-generation-cas-"));
		fixtureDirectories.push(stateDir);
		const calls: string[][] = [];
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation((rawSpawn: unknown) => {
			...
			if (command.includes("if-shell")) {
				...
				return spawnResult(
					0,
					guardedMutationCount === 1
						? "__vib_tmux_guarded_mutation_ok__\n"
						: "__vib_tmux_guarded_mutation_refused__\n",
				);
			}
			if (command.includes("display-message")) {
		...
		expect((failure as AggregateError).message).toBe("vib_tmux_precommit_failed_cleanup_failed");
		expect((failure as AggregateError).errors.map(String)).toEqual([
			expect.stringContaining("vib_tmux_created_metadata_mismatch"),
			expect.stringContaining("vib_tmux_cleanup_target_changed"),
		]);
		expect(calls).toContainEqual([
			"tmux",
			"if-shell",
	...
	it("preserves a created session without publishing when metadata readback changes a required value", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-metadata-readback-change-"));
		fixtureDirectories.push(stateDir);
		injectSafeMutationProof();
		__setCreateOwnerIsolationForTests({
			probe: {
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation((rawCommand: unknown) => {
			const command = normalizeSpawnSyncCommand(rawCommand);
			calls.push(command);
			if (command.includes("if-shell")) return spawnResult(0, "__vib_tmux_guarded_mutation_ok__\n");
			if (command.includes("display-message"))
				return spawnResult(0, command.includes("#{session_id}\t#{session_name}") ? "$1\tmanaged\n" : "$1\n");
			if (command.includes("show-options"))
				return spawnResult(0, command.at(-1) === "@vib-profile" ? "1\n" : "changed\n");
			return spawnResult(0, "");
		});
		expect(() =>
			createVibTmuxSession({
	...
	it("refuses profile tagging when a replacement server appears at the receipt-to-tag boundary", () => {
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation((rawCommand: unknown) => {
			const command = normalizeSpawnSyncCommand(rawCommand);
			calls.push(command);
			if (command.includes("if-shell")) return spawnResult(0, "__vib_tmux_guarded_mutation_refused__\n");
			if (command.includes("display-message")) {
				if (command.includes("#{session_id}\t#{session_name}")) return spawnResult(0, "$1\tmanaged\n");
				return spawnResult(0, command.includes("#{session_name}") ? "managed\n" : "$1\n");
			}
		...
		expect(() =>
			createVibTmuxSession({
				VIB_TMUX_COMMAND: "tmux",
				VIB_TMUX_SESSION: "managed",
				VIB_COORDINATOR_SESSION_STATE_FILE: path.join(os.tmpdir(), `vib-replacement-${crypto.randomUUID()}.json`),
			}),
		).toThrow("vib_tmux_precommit_failed_cleanup_failed");
		...
		expect(guardedTag?.[6]).toContain('"@vib-profile" "1"');
		expect(guardedTag?.[7]).toBe("display-message -p __vib_tmux_guarded_mutation_refused__");
	});
			...
			createVibTmuxSession(
				{
					VIB_TMUX_COMMAND: "tmux",
					VIB_TMUX_SESSION: "managed",
					VIB_COORDINATOR_SESSION_STATE_FILE: path.join(os.tmpdir(), `vib-unproven-${crypto.randomUUID()}.json`),
				},
				{ platform: "darwin" },
			),
		).toThrow("vib_tmux_precommit_failed_cleanup_failed");
	...
	it("rejects psmux force-close before signal or cleanup", async () => {
		...
		try {
			await expect(
				forceCloseVibTmuxSession(
					"managed",
					{ VIB_TMUX_COMMAND: "psmux", VIB_PSMUX_COMMAND: "psmux" },
					undefined,
					undefined,
					{
						signalTerm,
			...
			).rejects.toThrow("vib_tmux_provider_authority_unavailable");
			expect(signalTerm).not.toHaveBeenCalled();
			expect(cleanupSession).not.toHaveBeenCalled();
		} finally {
			__setBinaryResolverForTests(null);
	...
	it("refuses a psmux replacement that differs only by incarnation before cleanup", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-psmux-close-incarnation-"));
		fixtureDirectories.push(stateDir);
		...
		).rejects.toThrow("vib_tmux_owner_changed:managed");
		expect(cleanupSession).not.toHaveBeenCalled();
	});
	it("recovers a matching durable SIGTERM verdict when the owner-exit observer fails", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-close-"));
		...
		await forceCloseVibTmuxSession(
			"managed",
			{ VIB_TMUX_COMMAND: "tmux" },
			sessionId,
			path.join(stateDir, "marker"),
		...
		expect(calls).toEqual(
			expect.arrayContaining([
				...
				["tmux", "show-options", "-qv", "-t", "$0", "@vib-profile"],
				[
					"tmux",
					"if-shell",
					"-t",
	...
	it("does not extend durable verdict polling beyond the original intent expiry", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-close-expiry-"));
		const sessionId = "session";
		const generation = "generation";
		const marker = path.join(stateDir, "marker");
		...
		await expect(
			forceCloseVibTmuxSession("managed", { VIB_TMUX_COMMAND: "tmux" }, sessionId, marker, {
				resolveOwner: async () => ({
					sessionId,
					stateDir,
					socketKey: "managed",
	...
	it("terminates a real owner process through the default signal and start-time proofs", async () => {
		...
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-close-default-deps-"));
		fixtureDirectories.push(stateDir);
		const sessionId = "session";
		const generation = "generation";
		const marker = path.join(stateDir, "marker");
		...
		injectSafeMutationProof();
		try {
			await forceCloseVibTmuxSession("managed", { VIB_TMUX_COMMAND: "tmux" }, sessionId, marker);
			await owner.exited;
	...
	it("surfaces an exact compatibility cleanup failure after a matching SIGTERM verdict", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-close-cleanup-failure-"));
		const sessionId = "session";
		const generation = "generation";
		const marker = path.join(stateDir, "marker");
		await fs.mkdir(path.join(stateDir, sessionId, "owner-lifecycle"), {
	...
	it("does not kill a same-name replacement when the original native session ID disappears during verdict observation", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-close-replacement-"));
		const sessionId = "session";
		const generation = "generation";
		const marker = path.join(stateDir, "marker");
		await fs.mkdir(path.join(stateDir, sessionId, "owner-lifecycle"), {
	...
	it("rejects a missing native session ID before SIGTERM or cleanup", async () => {
		...
		await expect(
			forceCloseVibTmuxSession("managed", { VIB_TMUX_COMMAND: "tmux" }, undefined, undefined, {
				resolveOwner: async () => ({
					sessionId: "session",
					stateDir: "/state",
					socketKey: "managed",
		...
		).rejects.toThrow("vib_tmux_owner_unverifiable:managed");
		expect(signalTerm).not.toHaveBeenCalled();
		expect(cleanupSession).not.toHaveBeenCalled();
	});
	...
	it("cancels an intent when the owner PID changes after the initial start-time proof", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-tmux-close-race-"));
		const sessionId = "session";
		const generation = "generation";
		const marker = path.join(stateDir, "marker");
		await fs.mkdir(path.join(stateDir, sessionId, "owner-lifecycle"), {
