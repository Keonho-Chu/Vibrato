...
import { verifyUltragoalDurableCompletionState } from "@vib-rato/coding-agent/vib-runtime/ultragoal-guard";
import {
...
} from "@vib-rato/coding-agent/vib-runtime/ultragoal-receipt-freshness";
import {
	appendLedger,
...
beforeEach(() => {
	...
	process.env.CI_DEV_CHANGED_PATHS = "packages/coding-agent/test/vib-runtime/ultragoal-critic-gate.test.ts";
});
...
describe("ultragoal terminal critic gate", () => {
	it("requires a clean OKAY criticReview for a single-goal final aggregate", async () => {
		...
		for (const [name, mutate] of cases) {
			...
			await fs.rm(path.join(root, ".vib"), { recursive: true, force: true });
		}
	...
	it("rejects final aggregate completion at the hard stop until a gate override is recorded", async () => {
		...
		expect(blocked.stderr).toContain(
			"terminal-critic ceiling reached; requires human/leader vib ultragoal record-critic-gate-override",
		);
