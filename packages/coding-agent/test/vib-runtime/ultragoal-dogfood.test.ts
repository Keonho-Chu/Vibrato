...
beforeEach(() => {
	...
	// parallel shard load.
	process.env.CI_DEV_CHANGED_PATHS = "packages/coding-agent/test/vib-runtime/ultragoal-dogfood.test.ts";
});
...
function qualityGate(): Record<string, unknown> {
	return {
		...
		executorQa: {
			...
			e2eCommands: ["vib ultragoal checkpoint --status complete dogfood"],
