...
import { computeUltragoalPlanGeneration } from "@vib-rato/coding-agent/vib-runtime/ultragoal-receipt-freshness";
import {
	addUltragoalSubgoal,
...
beforeEach(() => {
	...
	process.env.CI_DEV_CHANGED_PATHS =
		"packages/coding-agent/test/vib-runtime/ultragoal-durable-completion-release.test.ts";
});
...
async function createTwoGoalPlan(root: string, mode: "aggregate" | "per-story" = "aggregate"): Promise<void> {
	await createUltragoalPlan({ cwd: root, brief: "Ship the feature", vibGoalMode: mode });
...
function reviewedBatchReplacementFixture(options: { multiple?: boolean; staleReplacement?: boolean } = {}): {
	...
	const plan: UltragoalPlan = {
		...
		vibObjective: "Complete reviewed batch",
