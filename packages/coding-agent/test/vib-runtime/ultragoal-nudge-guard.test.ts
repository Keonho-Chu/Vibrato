...
import { assertUltragoalAskAllowed } from "@vib-rato/coding-agent/tools/ultragoal-ask-guard";
import { DEFAULT_ULTRAGOAL_OBJECTIVE } from "@vib-rato/coding-agent/vib-runtime/goal-mode-request";
import {
...
const ORIGINAL_VIB_CONFIG_DIR = process.env.VIB_CONFIG_DIR;
...
async function setProjectBudget(cwd: string, budget: number): Promise<void> {
	const vibDir = path.join(cwd, ".vib");
	await fs.mkdir(vibDir, { recursive: true });
	await fs.writeFile(
		path.join(vibDir, "config.yml"),
		YAML.stringify({ vib: { ultragoal: { nudgeBudget: budget } } }, null, 2),
	);
}
...
afterEach(async () => {
	...
	if (ORIGINAL_VIB_CONFIG_DIR === undefined) delete process.env.VIB_CONFIG_DIR;
	else process.env.VIB_CONFIG_DIR = ORIGINAL_VIB_CONFIG_DIR;
...
describe("ultragoal nudge guard", () => {
	...
	it("AC2: exhausted premature complete still requires a valid completion receipt", async () => {
		...
		await expect(
			assertCanCompleteCurrentGoal({ cwd, currentGoal: DEFAULT_OBJECTIVE_GOAL, sessionId: TEST_SESSION_ID }),
		).rejects.toThrow(/`vib ultragoal checkpoint --status complete --quality-gate-json <file>`/);
		const ledger = await readUltragoalLedger(cwd, TEST_SESSION_ID);
	...
	it("AC1: the session agent directory selects the nudge budget profile", async () => {
		...
		await fs.writeFile(
			...
			YAML.stringify({ vib: { ultragoal: { nudgeBudget: 0 } } }, null, 2),
		);
	...
	it("AC3: per-story isolation and ledger-reset zeroing", async () => {
		const cwd = await tempDir();
		process.env.VIB_SESSION_ID = TEST_SESSION_ID;
		await createUltragoalPlan({ cwd, brief: MULTI_BRIEF, vibGoalMode: "per-story" });
	...
	//
	// The user-settings half runs in a child process. `VIB_CONFIG_DIR` is a config
	...
	it("AC4: nudge budget default is 10 and project overrides user", async () => {
		...
		await fs.mkdir(path.join(home, ".vib", "agent"), { recursive: true });
		await fs.writeFile(
			path.join(home, ".vib", "agent", "config.yml"),
			YAML.stringify({ vib: { ultragoal: { nudgeBudget: 3 } } }, null, 2),
		);
		...
		const userOnly = Bun.spawn([process.execPath, probe], {
			...
			env: {
				...
				HOME: home,
				VIB_CONFIG_DIR: ".vib",
				VIB_CODING_AGENT_DIR: path.join(home, ".vib", "agent"),
				PI_CODING_AGENT_DIR: path.join(home, ".vib", "agent"),
			},
			stdout: "pipe",
			stderr: "pipe",
		});
	...
	it("AC6: vib ultragoal status reports the consumed nudge target and count", async () => {
		const cwd = await tempDir();
		process.env.VIB_SESSION_ID = TEST_SESSION_ID;
		await createUltragoalPlan({ cwd, brief: SINGLE_BRIEF });
