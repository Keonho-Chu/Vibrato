...
async function createActiveRun(): Promise<string> {
	...
	await fs.writeFile(path.join(cwd, ".vib", "settings.json"), JSON.stringify({ "vib.ultragoal.nudgeBudget": 0 }));
...
function ultragoalPath(cwd: string, file: "goals.json" | "ledger.jsonl"): string {
	return path.join(cwd, ".vib", `_session-${TEST_SESSION_ID}`, "ultragoal", file);
}
