...
import { reconcileWorkflowSkillState } from "@vib-rato/coding-agent/vib-runtime/state-runtime";
import {
	validateCompletionReceipt,
	verifyUltragoalDurableCompletionState,
} from "@vib-rato/coding-agent/vib-runtime/ultragoal-guard";
import {
	addUltragoalSubgoal,
...
const NON_COMPUTER_TEST_PATH = "packages/coding-agent/test/vib-runtime/ultragoal-runtime.test.ts";

beforeEach(() => {
	savedSessionId = process.env.VIB_SESSION_ID;
	savedSessionFile = process.env.VIB_SESSION_FILE;
	process.env.VIB_SESSION_ID = TEST_SESSION_ID;
	delete process.env.VIB_SESSION_FILE;
...
afterEach(async () => {
	if (savedSessionId === undefined) delete process.env.VIB_SESSION_ID;
	else process.env.VIB_SESSION_ID = savedSessionId;
	if (savedSessionFile === undefined) delete process.env.VIB_SESSION_FILE;
	else process.env.VIB_SESSION_FILE = savedSessionFile;
...
function batchChangeSetPaths(): Array<{ path: string; status: string }> {
	return [
		{ path: "packages/coding-agent/src/vib-runtime/ultragoal-runtime.ts", status: "unknown" },
		{ path: "packages/coding-agent/src/vib-runtime/ultragoal-guard.ts", status: "unknown" },
		{ path: "packages/coding-agent/src/vib-runtime/ultragoal-receipt-freshness.ts", status: "unknown" },
		{ path: "packages/coding-agent/test/vib-runtime/ultragoal-runtime.test.ts", status: "unknown" },
		{ path: "packages/coding-agent/src/defaults/vib/skills/ultragoal/SKILL.md", status: "unknown" },
		{ path: "packages/coding-agent/src/defaults/vib/skills/ralplan/SKILL.md", status: "unknown" },
		{ path: "packages/coding-agent/src/prompts/system/system-prompt.md", status: "unknown" },
		{ path: "packages/coding-agent/test/default-vib-definitions.test.ts", status: "unknown" },
		{ path: "packages/coding-agent/src/vib-runtime/workflow-manifest.generated.json", status: "unknown" },
		{ path: "packages/coding-agent/src/vib-runtime/workflow-manifest.ts", status: "unknown" },
	];
}
...
describe("ultragoal CLI replay validation", () => {
	...
	it("fails closed when process.execPath is the compiled Vibrato application", () => {
		expect(() => resolveCliReplayCommand(["bun", "-e", 'console.log("safe")'], { compiled: true })).toThrow(
			"compiled Vibrato runtime",
		);
...
describe("native Vibrato ultragoal runtime", () => {
	...
	it("creates a durable aggregate plan and ledger", async () => {
		...
		expect(plan.vibGoalMode).toBe("aggregate");
		expect(plan.vibObjective).toContain(".vib/ultragoal/goals.json");
	...
	it("validation batch deferred: explicit paths must exactly match the computed cumulative diff", async () => {
		const root = await batchTempDir();
		const actualPath = "packages/coding-agent/src/vib-runtime/ultragoal-runtime.ts";
		const secondPath = "packages/coding-agent/test/vib-runtime/ultragoal-runtime.test.ts";
	...
	it("review cohort: a later generation must be delta-only over a new frozen source", async () => {
		...
		const secondGeneration = (overrides: Record<string, unknown>) => {
			...
			gate.iteration.reviewCohort = {
				...
				deltaOnly: true,
				deltaPaths: ["packages/coding-agent/src/vib-runtime/ultragoal-runtime.ts"],
	...
	it("quality-gate validate: is read-only and rule-identical to checkpoint complete", async () => {
		...
		const goalsPath = path.join(root, ".vib", `_session-${process.env.VIB_SESSION_ID}`, "ultragoal", "goals.json");
		const ledgerPath = path.join(root, ".vib", `_session-${process.env.VIB_SESSION_ID}`, "ultragoal", "ledger.jsonl");
	...
	it("validation batch deferred: uses CI changed paths when git diff is unavailable", async () => {
		...
		try {
			...
			uncovered.deferredToBatch.changeSet.paths = [
				{ path: "packages/coding-agent/src/vib-runtime/ultragoal-runtime.ts", status: "modified" },
			];
	...
	it("prints receipt-only json for complete-goals", async () => {
		...
		expect(receipt).toMatchObject({
			...
			goal_id: "G001",
			goal_status: "active",
			vib_objective: created.vibObjective,
			goals_path: path.join(sessionUltragoalDir(root, TEST_SESSION_ID), "goals.json"),
		});
	...
	it("prints checkpoint-specific help with receipt guidance", async () => {
		...
		expect(result.status).toBe(0);
		expect(result.stdout).toContain("vib ultragoal checkpoint --goal-id");
	...
	it("prints top-level and command-specific help for classify-blocker", async () => {
		...
		expect(topLevel.stdout).toContain("vib ultragoal classify-blocker --help");
	...
	it("accepts per-story durable active goals for per-story plans", async () => {
		const root = await tempDir();
		const created = await createUltragoalPlan({ cwd: root, brief: "Ship the fix", vibGoalMode: "per-story" });
		await startNextUltragoalGoal({ cwd: root });
	...
	it("does not require computer-use adversarial cases for prompt-only wording changes", async () => {
		...
		await validateExecutorQaRedTeamEvidenceForReview(root, webExecutorQa(), {
			...
			changeSet: {
				...
				paths: [
					{
						path: "packages/coding-agent/src/defaults/vib/skills/ultragoal/SKILL.md",
						status: "modified",
					},
				],
			},
	...
	it("allows blocked checkpoints without completion quality gates", async () => {
		...
		const result = await runNativeUltragoalCommand(
			[
				...
				"blocked",
				"--evidence",
				"legacy completed Vibrato goal blocks goal create in this thread",
			],
			root,
		);
		const status = await getUltragoalStatus(root);
		...
		expect(ledgerRaw).toContain("legacy completed Vibrato goal blocks");
	});
...
describe("ultragoal @goal decomposition", () => {
	...
	it("reconciles completed runs with mode-state and HUD active-state", async () => {
		...
		expect(modeState.receipt).toMatchObject({ skill: "ultragoal", owner: "vib-runtime" });
...
describe("ultragoal mode-state + HUD reconciliation (#342)", () => {
	...
	async function withSessionId<T>(id: string | undefined, fn: () => Promise<T>): Promise<T> {
		const prev = process.env.VIB_SESSION_ID;
		if (id === undefined) delete process.env.VIB_SESSION_ID;
		else process.env.VIB_SESSION_ID = id;
		try {
			return await fn();
		} finally {
			if (prev === undefined) delete process.env.VIB_SESSION_ID;
			else process.env.VIB_SESSION_ID = prev;
		}
	}
	...
	it("writes session-scoped state when VIB_SESSION_ID is set (AC1)", async () => {
		const root = await tempDir();
	...
	it("fails cleanly instead of crashing when no session id is resolvable", async () => {
		...
		await withSessionId(undefined, async () => {
			...
			expect(result.stderr).toContain("VIB_SESSION_ID");
		});
	});
	...
	it("stamps reconcile provenance distinguishable from a user write (AC5)", async () => {
		...
		await withSessionId(TEST_SESSION_ID, async () => {
			...
			expect(receipt.owner).toBe("vib-runtime");
