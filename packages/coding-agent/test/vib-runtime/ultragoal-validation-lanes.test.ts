...
import type { UltragoalChangeSetPath } from "@vib-rato/coding-agent/vib-runtime/ultragoal-change-set";
import {
...
import { isHighRiskChangePath } from "@vib-rato/coding-agent/vib-runtime/ultragoal-validation-policy";
...
async function seedGitRepo(root: string, files: Record<string, string>): Promise<void> {
	...
	await Bun.write(path.join(root, ".gitignore"), ".vib/\nartifacts/\n");
...
describe("ultragoal validation lane selection gate (#4560)", () => {
	...
	afterEach(async () => {
		...
		if (priorSessionId === undefined) delete process.env.VIB_SESSION_ID;
		else process.env.VIB_SESSION_ID = priorSessionId;
	});
	...
	async function seedPriorCohort(sourceHash: string): Promise<void> {
		const ledgerPath = path.join(root, ".vib", `_session-${TEST_SESSION_ID}`, "ultragoal", "ledger.jsonl");
