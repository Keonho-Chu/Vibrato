...
} from "../../src/vib-runtime/workflow-settings";

const KEY: WorkflowSettingKey = "vib.ralplan.maxIterations";
...
async function tempDir(): Promise<string> {
	const directory = await fs.mkdtemp(path.join(os.tmpdir(), "vib-workflow-settings-"));
...
async function writeProjectConfig(cwd: string, document: unknown): Promise<string> {
	const projectDir = path.join(cwd, ".vib");
...
describe("workflow-settings resolver", () => {
	test("falls back to the retained project settings.json while project config.yml is absent", async () => {
		...
		// The migration could not publish (e.g. a read-only .vib): only the
		// retained legacy settings.json exists and its override stays effective.
		await writeProjectSettings(cwd, { "vib.ralplan.maxIterations": 7 });

		const result = await resolveIn(cwd, { VIB_CODING_AGENT_DIR: agentDir });
		expect(result.value).toBe(7);
		expect(result.source.endsWith(path.join(".vib", "settings.json"))).toBe(true);
	});
	...
	test("the legacy fallback stays active while no ownership marker records the migrated key", async () => {
		...
		// `.vib` or config root rejected the write, so the source was retained),
	...
	test("a present migration target with a durable ownership marker suppresses the legacy fallback", async () => {
		...
		// later `vib config unset` must stick even while the target stays present.
		await writeProjectConfig(cwd, { theme: { dark: "red" } });
		await writeProjectSettings(cwd, { "vib.ralplan.maxIterations": 7 });
		await fs.mkdir(path.join(cwd, ".vib", "state"), { recursive: true });
		await fs.writeFile(
			path.join(cwd, ".vib", "state", "settings.json.migrated-keys"),
			JSON.stringify(["vib.ralplan.maxIterations"]),
		);
		await fs.writeFile(path.join(agentDir, "config.yml"), YAML.stringify({ theme: { dark: "red" } }, null, 2));

		const result = await resolveIn(cwd, { VIB_CODING_AGENT_DIR: agentDir, HOME: home, VIB_CONFIG_DIR: ".vib" });
	...
	test("owned keys do not exit 2 on a malformed retained source", async () => {
		...
		// `vib config unset` removed the key from config.yml; the retained source
		...
		const result = await resolveIn(cwd, { VIB_CODING_AGENT_DIR: agentDir, HOME: home, VIB_CONFIG_DIR: ".vib" }, KEY, {
			strict: true,
		});
		expect(result.value).toBe("default");
	...
	test("the session agent directory overrides the process-global agent layer", async () => {
		...
		const result = await resolveIn(
			cwd,
			{ VIB_CODING_AGENT_DIR: defaultAgentDir, HOME: home, VIB_CONFIG_DIR: ".vib" },
			KEY,
			{
	...
	test("an unreadable legacy config-root source fails closed under strict policy", async () => {
		...
		await writeProjectConfig(cwd, { theme: { dark: "red" } });
		const configRoot = path.join(home, ".vib");
		...
		try {
			const result = await resolveIn(
				cwd,
				{ VIB_CODING_AGENT_DIR: agentDir, HOME: home, VIB_CONFIG_DIR: ".vib" },
				KEY,
				{ strict: true },
			);
	...
	test("an isolated session profile does not inherit the machine-global strict evidence", async () => {
		...
		await fs.writeFile(
			...
			JSON.stringify({
				version: 2,
				keys: [{ key: "vib.ralplan.maxIterations", value: "invalid" }],
	...
	test("an unreadable ownership marker fails closed instead of resurrecting the retained value", async () => {
		...
		await writeProjectConfig(cwd, { theme: { dark: "red" } });
		await writeProjectSettings(cwd, { "vib.ralplan.maxIterations": "invalid" });
		await fs.mkdir(path.join(cwd, ".vib", "state"), { recursive: true });
		await fs.writeFile(
			path.join(cwd, ".vib", "state", "settings.json.migrated-keys"),
			JSON.stringify(["vib.ralplan.maxIterations"]),
		...
		await fs.chmod(path.join(cwd, ".vib", "state"), 0o000);
		try {
			const result = await resolveIn(
				cwd,
				{ VIB_CODING_AGENT_DIR: agentDir, HOME: home, VIB_CONFIG_DIR: ".vib" },
			...
			await fs.chmod(path.join(cwd, ".vib", "state"), 0o700);
		}
	});
	...
	test("parses a legacy settings.json fallback as JSON, not YAML", async () => {
		...
		await fs.writeFile(path.join(cwd, ".vib", "settings.json"), "{ vib: { ralplan: { maxIterations: 7 } } }");
		...
		const strict = await resolveIn(cwd, { VIB_CODING_AGENT_DIR: agentDir }, KEY, { strict: true });
	...
	test("a future-schema agent config.yml keeps the config-root legacy fallback active", async () => {
		...
		const result = await resolveIn(cwd, { HOME: home, VIB_CONFIG_DIR: ".myconfig" });
		expect(result.value).toBe(7);
	});
	...
	test("the config-root legacy fallback is suppressed for project-owned keys in a collision", async () => {
		...
		await fs.mkdir(path.join(home, ".vib", "state"), { recursive: true });
		await fs.writeFile(
			path.join(home, ".vib", "state", "settings.json.migrated-keys"),
			JSON.stringify(["vib.ralplan.maxIterations"]),
		);

		const result = await resolveIn(home, { HOME: home, VIB_CONFIG_DIR: ".vib" });
		expect(result.value).toBe("default");
		expect(result.source).toBe("default");
	});
	...
	test("the config-root legacy fallback is suppressed for project-owned keys via symlink aliases", async () => {
		...
		await fs.mkdir(path.join(cwd, ".vib"), { recursive: true });
		await fs.symlink(path.join(home, ".myconfig", "settings.json"), path.join(cwd, ".vib", "settings.json"));
	...
	test("project .vib/config.yml wins over the built-in default", async () => {
		const cwd = await tempDir();
		// A non-numeric string is preserved (no schema number coercion applies).
		await writeProjectConfig(cwd, { vib: { ralplan: { maxIterations: "seven" } } });

		const result = await resolveWorkflowSetting(cwd, KEY, { defaultValue: "default", parse: stringParse });
		expect(result.value).toBe("seven");
		expect(result.source).toBe(path.join(cwd, ".vib", "config.yml"));
	});

	test("project .vib/config.yml beats user agent config.yml", async () => {
		const cwd = await tempDir();
		const agentDir = await tempDir();
		await writeProjectConfig(cwd, { vib: { ralplan: { maxIterations: "project-wins" } } });
		await fs.writeFile(
			path.join(agentDir, "config.yml"),
			YAML.stringify({ vib: { ralplan: { maxIterations: "agent-loses" } } }, null, 2),
		);
	...
	test("user agent config.yml wins over the built-in default", async () => {
		...
		await fs.writeFile(
			path.join(agentDir, "config.yml"),
			YAML.stringify({ vib: { ralplan: { maxIterations: "agent" } } }, null, 2),
		);
	...
	test("a legacy project settings.json applies while project config.yml is absent", async () => {
		...
		await writeProjectSettings(cwd, { "vib.ralplan.maxIterations": "legacy-json" });
	...
	test("a legacy config-root settings.json applies while the agent config.yml is absent", async () => {
		...
		await fs.writeFile(
			path.join(home, ".myconfig", "settings.json"),
			JSON.stringify({ "vib.ralplan.maxIterations": "root" }),
		);
	...
	test("strict resolution throws on retained strict-invalid evidence from config-root", async () => {
		...
		const tolerant = await resolveIn(cwd, { HOME: home, VIB_CONFIG_DIR: ".myconfig" });
		...
		const strict = await resolveIn(cwd, { HOME: home, VIB_CONFIG_DIR: ".myconfig" }, KEY, { strict: true });
		expect(strict.threw).toBe(true);
		expect(strict.message).toContain("retained invalid vib.ralplan.maxIterations");
		...
		await fs.writeFile(
			...
			JSON.stringify({
				version: 1,
				key: "vib.ralplan.autoHandoff",
				value: "bad",
				source: path.join(home, ".myconfig", "settings.json"),
			}),
		);
		const otherKey = await resolveIn(cwd, { HOME: home, VIB_CONFIG_DIR: ".myconfig" }, KEY, { strict: true });
	...
	test("a valid project config.yml override wins over retained agent-layer evidence", async () => {
		...
		await writeProjectConfig(cwd, { vib: { ralplan: { maxIterations: 7 } } });
	...
	test("strict resolution throws on retained project evidence after the project layer yields nothing", async () => {
		...
		await fs.writeFile(
			...
			JSON.stringify({
				...
				key: "vib.ralplan.maxIterations",
				value: "bad",
				source: path.join(cwd, ".vib", "settings.json"),
			}),
		);
		...
		await writeProjectConfig(cwd, { vib: { ralplan: { maxIterations: 9 } } });
		const override = await resolveIn(cwd, { HOME: home, VIB_CONFIG_DIR: ".myconfig" }, KEY, { strict: true });
	...
	test("strict resolution throws on malformed project evidence for every strict key", async () => {
		...
		await fs.writeFile(
			path.join(cwd, ".vib", "state", "settings.json.strict-invalid"),
			JSON.stringify({ version: 2, malformed: true, source: path.join(cwd, ".vib", "settings.json") }),
		);

		const strict = await resolveIn(cwd, { HOME: home, VIB_CONFIG_DIR: ".myconfig" }, KEY, { strict: true });
		expect(strict.threw).toBe(true);
		expect(strict.message).toContain("retained malformed project .vib/settings.json");
	...
	test("flat dotted and nested shapes are both extracted, flat wins", async () => {
		...
		expect(extractWorkflowSetting({ "vib.ralplan.maxIterations": 7 }, KEY)).toEqual({ present: true, value: 7 });
		expect(extractWorkflowSetting({ vib: { ralplan: { maxIterations: 8 } } }, KEY)).toEqual({
			present: true,
			value: 8,
		});
		expect(
			extractWorkflowSetting({ "vib.ralplan.maxIterations": 7, vib: { ralplan: { maxIterations: 8 } } }, KEY),
		).toEqual({
			present: true,
			value: 7,
		});
		...
		expect(extractWorkflowSetting({ vib: { other: 1 } }, KEY)).toEqual({ present: false, value: undefined });
	...
	test("flat dotted keys are not honored in config.yml (nested only)", async () => {
		...
		await writeProjectConfig(cwd, { "vib.ralplan.maxIterations": "yaml-flat-ignored" });
	...
	test("empty documents continue; a null root is a malformed shape (strict throws)", async () => {
		const cwd = await tempDir();
		await fs.mkdir(path.join(cwd, ".vib"), { recursive: true });
		await fs.writeFile(path.join(cwd, ".vib", "config.yml"), "", "utf8"); // empty YAML -> no explicit settings
	...
	test("malformed YAML is invalid syntax, continue by default, throw under strict", async () => {
		const cwd = await tempDir();
		await fs.mkdir(path.join(cwd, ".vib"), { recursive: true });
		await fs.writeFile(path.join(cwd, ".vib", "config.yml"), "{ broken: [yaml", "utf8");
		...
		expect(thrown.path).toBe(path.join(cwd, ".vib", "config.yml"));
