...
import {
	...
	sessionSpecsDir,
	sessionStateDir,
} from "../src/vib-runtime/session-layout";
import { reconcileWorkflowSkillState } from "../src/vib-runtime/state-runtime";
import { RequiredOnWriteEnvelopeSchema } from "../src/vib-runtime/state-schema";
import {
...
} from "../src/vib-runtime/state-writer";
import {
	addUltragoalSubgoal,
	checkpointUltragoalGoal,
	createUltragoalPlan,
	startNextUltragoalGoal,
} from "../src/vib-runtime/ultragoal-runtime";

describe("Vibrato native skill-state hooks", () => {
	...
	beforeAll(() => {
		...
		process.env.CI_DEV_CHANGED_PATHS = "packages/coding-agent/test/vib-skill-state-hooks.test.ts";
	...
	async function cwd(): Promise<string> {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vib-skill-hooks-"));
		return tempDir;
	}
	...
	it("detects only the public Vibrato workflow skill surface", () => {
	...
	it("fails open and logs when custom skill-active state is corrupt", async () => {
		...
		try {
			...
			expect(String(warn.mock.calls[0]?.[0] ?? "")).toContain("vib skill-state: invalid skill-active-state at");
	...
	it("UserPromptSubmit fails open with recovery guidance when skill-active state is corrupt", async () => {
		...
		try {
			...
			expect(result.outputJson).not.toMatchObject({ decision: "block" });
			expect(context).toContain("Vibrato state recovery");
			expect(context).toContain(statePath);
			expect(context).toContain("vib state doctor");
			expect(context).toContain("vib state clear");
	...
	it("Stop reads valid custom mode state unchanged", async () => {
		...
		const allowed = await dispatchVibNativeSkillHook(
			{
				hookEventName: "Stop",
				cwd: root,
	...
	it("Stop blocks corrupt handoff-required mode state with concrete recovery guidance", async () => {
		...
		const warn = vi.spyOn(logger, "warn").mockImplementation(() => {});
		try {
			const blocked = await dispatchVibNativeSkillHook(
				{
					hookEventName: "Stop",
					cwd: root,
			...
			expect(message).toContain("vib state clear");
	...
	it("deep-interview plaintext ask leak blocks stop with ask-tool recovery", async () => {
		...
		for (const sessionFile of [
			...
			const genericBlocked = await dispatchVibNativeSkillHook({
				hookEventName: "Stop",
				cwd: root,
				sessionId,
				threadId: sessionId,
			...
			expect(genericBlocked.outputJson?.stopReason).not.toBe("vib_skill_deep_interview_plaintext_ask_leak");
		...
		const crystallizedAllowed = await dispatchVibNativeSkillHook({
			hookEventName: "Stop",
			cwd: root,
			sessionId: crystallizedSessionId,
			threadId: crystallizedSessionId,
		...
		const cancelledAllowed = await dispatchVibNativeSkillHook({
			hookEventName: "Stop",
			cwd: root,
			sessionId: cancelledSessionId,
			threadId: cancelledSessionId,
		...
		const ralplanBlocked = await dispatchVibNativeSkillHook({
			hookEventName: "Stop",
			cwd: root,
			sessionId: ralplanSessionId,
			threadId: ralplanSessionId,
		...
		expect(ralplanBlocked.outputJson?.stopReason).not.toBe("vib_skill_deep_interview_plaintext_ask_leak");
	});
	...
	it("UserPromptSubmit treats schema-invalid active ultragoal mode state as inactive and logs", async () => {
		...
		try {
			...
			).toContain("Vibrato state recovery");
	...
	it("UserPromptSubmit combines recovery diagnostics with active Ultragoal guidance", async () => {
		...
		try {
			...
			expect(context).toContain("vib ultragoal steer");
			expect(context).not.toContain("do not expose");
			expect(warn).toHaveBeenCalledTimes(1);
		} finally {
			warn.mockRestore();
	...
	it("rich deep-interview prompt activation blocks product mutation and direct spec artifacts", async () => {
		...
		const blockedSpec = await getWorkflowMutationDecision({
			...
			sessionId: "session-rich",
			tool: { name: "write" } as never,
			args: { path: ".vib/specs/deep-interview-sample.md", content: "spec" },
		});
		expect(blockedSpec.blocked).toBe(true);
		expect(blockedSpec.reason).toBe("vib-target");
		expect(blockedSpec.message).toContain("runtime-owned");

		// Per #951 the mutation guard never blocks `bash`, even for `.vib/**` targets;
		// `.vib/**` is gated only through the dedicated write/edit/ast_edit tools.
		const allowedVibBash = await getWorkflowMutationDecision({
			cwd: root,
			sessionId: "session-rich",
			tool: { name: "bash" } as never,
			args: { command: "cat sample.md > .vib/specs/deep-interview-sample.md" },
		});
		expect(allowedVibBash.blocked).toBe(false);

		const blocked = await getWorkflowMutationDecision({
			cwd: root,
			sessionId: "session-rich",
			tool: { name: "write" } as never,
			args: { path: ".vib/state/sessions/session-rich/deep-interview-state.json", content: "{}" },
		});
		expect(blocked.blocked).toBe(true);
		expect(blocked.reason).toBe("workflow-state-target");
	});

	it("blocks direct workflow state JSON writes and points to vib state", async () => {
		const root = await cwd();
		const blocked = await getWorkflowMutationDecision({
			cwd: root,
			tool: { name: "write" } as never,
			args: { path: ".vib/state/ralplan-state.json", content: "{}" },
		});
		expect(blocked.blocked).toBe(true);
		expect(blocked.reason).toBe("workflow-state-target");
		expect(blocked.message).toContain("vib state ralplan");
		...
		const allowedPlan = await getWorkflowMutationDecision({
			cwd: root,
			tool: { name: "write" } as never,
			args: { path: ".vib/plans/sample.md", content: "plan" },
		});
	...
	it("encodes hook session ids before writing skill and mode state paths", async () => {
		...
		await expect(fs.stat(path.join(root, ".vib", "escape"))).rejects.toThrow();
	});
	...
	it("UserPromptSubmit injects sanitized effective skill config without raw paths or settings-file instructions", async () => {
		...
		expect(context).not.toContain("~/.vib");
		expect(context).not.toContain(".vib/settings.json");
...
`,
	...
	it("Stop blocks while matching skill state is active and allows terminal mode state", async () => {
		...
		expect(blocked.outputJson).toMatchObject({ decision: "block", stopReason: "vib_skill_ralplan_planner" });
	...
	it("Stop forces deep-interview crystallization when an ordinary stop would terminalize without a spec", async () => {
		...
		expect(String(blocked.outputJson?.reason ?? "")).toContain("vib deep-interview --write");
	});
	...
	it("UserPromptSubmit blocks completion from durable state (no transcript recovery)", async () => {
		...
		await Bun.write(
			sessionFile,
			`${JSON.stringify({ type: "session", id: "session-ultra-transcript", timestamp: new Date().toISOString(), cwd: root })}\n${JSON.stringify({ type: "mode_change", id: "1", parentId: null, timestamp: new Date().toISOString(), mode: "goal", data: { goal: { objective: plan.vibObjective, status: "active" } } })}\n`,
		);
		await dispatchVibNativeSkillHook(
			{
				hookEventName: "UserPromptSubmit",
	...
	it("Stop blocks when stale Ultragoal mode-state would release but the plan still has pending goals", async () => {
		...
		expect(blocked.outputJson).toMatchObject({
			decision: "block",
			stopReason: "vib_skill_ultragoal_stale_mode_state",
		});
	...
	it("merges managed Codex UserPromptSubmit/Stop hooks without dropping user hooks", () => {
		const existing = JSON.stringify({
			hooks: {
				...
				Stop: [
					{ hooks: [{ type: "command", command: "vib codex-native-hook" }] },
					{ hooks: [{ type: "command", command: "echo user-stop" }] },
				],
			},
		});

		const merged = mergeVibManagedCodexHooksConfig(existing);
		...
		expect(readVibManagedCodexHooksStatus(merged.content, "/tmp/hooks.json")).toMatchObject({
			installed: true,
			missingEvents: [],
			managedHookCount: 2,
		});
