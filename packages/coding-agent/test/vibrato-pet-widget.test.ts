...
import * as tui from "@vib-rato/tui";
import {
...
import { PetFramedEditor, VibratoPetWidget } from "../src/modes/components/vibrato-pet-widget";
...
function makeWidget(
	...
	const widget = new VibratoPetWidget({
		ui: stubs.ui,
		editor: stubs.editor,
...
describe("VibratoPetWidget", () => {
	...
	it("renders a bounded text-cell fallback without terminal image escapes", () => {
		...
		const protocol = vi.spyOn(VibratoPetWidget, "pixelProtocol").mockReturnValue("text");
		try {
			widget.setMode("red");
	...
	it("replaces an active pixel pet with text cells when graphics disappear", () => {
		const { widget, editorContainer, getEmitter } = makeWidget(80, 30, { protocol: null });
		const protocol = vi.spyOn(VibratoPetWidget, "pixelProtocol").mockReturnValue("sixel");
		try {
			widget.setMode("red");
	...
	it("does not reuse a Kitty image ID while a protocol-switch delete is pending", () => {
		...
		vi.spyOn(VibratoPetWidget, "pixelProtocol").mockImplementation(() => protocol);
		...
		const replacement = new VibratoPetWidget({
			ui: stubs.ui,
			editor: stubs.editor,
			editorContainer: stubs.editorContainer,
			floorContainer: stubs.floorContainer,
	...
	it("runs a para-para-then-sob burst for working BlueVibrato", () => {
		vi.useFakeTimers();
	...
	it("stays off when no pixel protocol is available", () => {
		vi.spyOn(VibratoPetWidget, "pixelProtocol").mockReturnValue(null);
		const stubs = makeStubs();
		const widget = new VibratoPetWidget({
			ui: stubs.ui,
			editor: stubs.editor,
	...
	it("uses only Ouroboros frames for raster overlays and iTerm GIFs", async () => {
		...
		const expectedSixel = buildVibratoPixelFrames({
			protocol: "sixel",
			skin: "ouroboros",
		...
		try {
			...
			const expectedGif = tui.getVibratoPetGifCached({
				skin: "ouroboros",
