...
describe("getVariantOverride", () => {
	it("honors the documented VIB_NATIVE_VARIANT name", () => {
		// The three natives docs name `VIB_NATIVE_VARIANT`, and
		...
		expect(getVariantOverride({ VIB_NATIVE_VARIANT: "baseline" })).toBe("baseline");
		expect(getVariantOverride({ VIB_NATIVE_VARIANT: "modern" })).toBe("modern");
	});
	...
	it("prefers the canonical name over the legacy alias", () => {
		expect(getVariantOverride({ VIB_NATIVE_VARIANT: "modern", PI_NATIVE_VARIANT: "baseline" })).toBe("modern");
	});

	it("treats an empty or blank canonical value as absent", () => {
		expect(getVariantOverride({ VIB_NATIVE_VARIANT: "", PI_NATIVE_VARIANT: "baseline" })).toBe("baseline");
		expect(getVariantOverride({ VIB_NATIVE_VARIANT: "   ", PI_NATIVE_VARIANT: "baseline" })).toBe("baseline");
		expect(getVariantOverride({ VIB_NATIVE_VARIANT: "" })).toBeNull();
	});

	it("ignores invalid values under either name", () => {
		// docs/natives-architecture.md: "invalid values are ignored".
		expect(getVariantOverride({ VIB_NATIVE_VARIANT: "bogus" })).toBeNull();
