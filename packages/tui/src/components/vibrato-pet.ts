 ...
 *
 * FRAME CATALOG (VibratoPixelFrameName → PIXEL_GRIDS):
 *   base    idle rest; also the dance "drop/settle" beat
 *   gazeL   eyes glance left    ┐ idle loop (see vibrato-pet-widget IDLE_LOOP)
 ...
 *
 * RENDERING: buildVibratoPixelFrames({ protocol, cellWidthPx, cellHeightPx,
 ...
 * packages/coding-agent/src/modes/components/vibrato-pet-widget.ts.
 *
 * ADD A FRAME: draw the grid → add its name to VibratoPixelFrameName → register it in
...
const RED_PALETTE: Palette = {
	...
	A: [196, 60, 30], // antenna
	w: [200, 230, 255], // tear (BlueVibrato sob)
};
// BlueVibrato recolors the crab to match the "blue-crab" theme (crabShell body, claw
...
// BlueVibrato's idle sob: a squeezed `>< ` visor (arms down, unlike RedVibrato's flex)
...
export type VibratoPixelFrameName =
	| "base"
	| "gazeL"
	| "gazeR"
	| "flicker"
	...
	| "cry2"
	| "cry3";
export type PetFrameName = VibratoPixelFrameName | OuroborosFrameName;

const PIXEL_GRIDS: Record<VibratoPixelFrameName, string[]> = {
	base: F0,
	gazeL: FL,
	gazeR: FR,
	flicker: FF,
...
export type VibratoGifFrameTuple = readonly [VibratoPixelFrameName, number];
export const PARA_PARA_STEPS: readonly VibratoGifFrameTuple[] = [
	["danceL", 300],
	["danceR", 300],
	["base", 260],
	["flex", 480],
...
export const VIBRATO_IDLE_STEPS: ReadonlyArray<readonly [VibratoPixelFrameName, number]> = [
	["base", 1100],
	["gazeL", 350],
	["base", 500],
	["gazeR", 350],
...
export interface PetSkin {
	id: PetSkinId;
	/** Selector/settings label, e.g. "RedVibrato". */
	label: string;
...
export const __vibratoPetTestHooks = {
...
export interface VibratoGifFrame {
	readonly name: PetFrameName;
	readonly delayMs: number;
}
export type VibratoGifTimeline = readonly VibratoGifFrame[];
export interface VibratoGifRectangle {
	readonly width?: number;
	readonly height?: number;
}
export interface VibratoGifDisplaySize {
...
export interface VibratoGifContentInset {
	/** Transparent top padding in source pixels. */
	readonly topPx?: number;
	/** Transparent bottom padding in source pixels. */
	readonly bottomPx?: number;
}
export interface VibratoPetGifArtifact {
	readonly bytes: Uint8Array;
	readonly base64: string;
	readonly width: number;
	readonly height: number;
	readonly frames: readonly VibratoGifFrame[];
	readonly skin: PetSkinId;
	readonly multipart: readonly string[];
	readonly tmuxDcs: readonly string[];
}
export interface VibratoPetGifOptions {
	readonly skin?: PetSkinId;
	readonly timeline?: VibratoGifTimeline;
	readonly cellWidthPx?: number;
	readonly cellHeightPx?: number;
	readonly targetRows?: number;
	readonly rectangle?: VibratoGifRectangle;
	readonly displaySize?: VibratoGifDisplaySize;
	readonly contentInset?: VibratoGifContentInset;
}
const GIF_CLEAR = 256,
	GIF_END = 257;
...
export const idleTimeline = (): VibratoGifTimeline => [
...
export const workingTimeline = (): VibratoGifTimeline => PARA_PARA_STEPS.map(([name, delayMs]) => ({ name, delayMs }));
export const burstTimeline = (skin: PetSkinId = "red"): VibratoGifTimeline => {
	const burst = PET_SKINS[skin].burst;
	const frames: VibratoGifFrame[] = burst.intro.map(([name, delayMs]) => ({ name, delayMs }));
	const tail = burst.tail;
...
export const previewTimeline = (skin: PetSkinId = "red"): VibratoGifTimeline => burstTimeline(skin);
function isGifTimeline(input: VibratoPetGifOptions | VibratoGifTimeline): input is VibratoGifTimeline {
	return Array.isArray(input);
}
function gifOptions(input: VibratoPetGifOptions | VibratoGifTimeline): Required<
	Pick<VibratoPetGifOptions, "skin" | "timeline" | "cellWidthPx" | "cellHeightPx" | "targetRows">
> & {
	rectangle?: VibratoGifRectangle;
	displaySize?: VibratoGifDisplaySize;
	contentInset?: VibratoGifContentInset;
} {
...
export function encodeVibratoPetGif(input: VibratoPetGifOptions | VibratoGifTimeline = {}): VibratoPetGifArtifact {
...
const gifCache = new Map<string, VibratoPetGifArtifact>();
let gifCacheBytes = 0;
...
export function getVibratoPetGifCached(input: VibratoPetGifOptions | VibratoGifTimeline = {}): VibratoPetGifArtifact {
	const o = gifOptions(input),
		key = JSON.stringify([
			o.skin,
			o.timeline,
	...
	const value = encodeVibratoPetGif(o);
...
export function getVibratoPetGifCacheStats(): {
	size: number;
	bytes: number;
	gifBytes: number;
	base64Bytes: number;
...
export function resetVibratoPetGifCache(): void {
	gifCache.clear();
	gifCacheBytes = 0;
...
export const clearVibratoPetGifCache = resetVibratoPetGifCache;
...
export function encodeGridIterm2(
	...
	const params = `name=dmlicmF0by1wZXQucG5n;width=${columns};height=${rows};preserveAspectRatio=0;inline=1`;
...
export interface VibratoPixelFrames {
 ...
 */
export function buildVibratoPixelFrames(options: {
	protocol: "sixel" | "kitty" | "iterm2";
	cellWidthPx: number;
	cellHeightPx: number;
	targetRows?: number;
