...
import {
	__vibratoPetTestHooks,
	buildVibratoPixelFrames,
	burstTimeline,
	encodeGridIterm2,
	encodeGridSixel,
	encodeVibratoPetGif,
	getVibratoPetGifCached,
	getVibratoPetGifCacheStats,
	idleTimeline,
	PET_SKIN_IDS,
	PET_SKINS,
	petBurstDurationMs,
...
describe("vibrato pixel frames", () => {
	it("falls back removed persisted skins to RedVibrato without overriding explicit off", () => {
	...
	it("encodes bottom-aligned sixel frames with a transparent background", () => {
		const built = buildVibratoPixelFrames({ protocol: "sixel", cellWidthPx: 9, cellHeightPx: 18, targetRows: 2 });
	...
	it("adds transparent sixel padding for a nine-pixel sub-cell drop", () => {
		const built = buildVibratoPixelFrames({
			protocol: "sixel",
			cellWidthPx: 9,
			cellHeightPx: 18,
			targetRows: 2,
	...
	it("carries the >< effort face on danceL and the ^^ victory face on flex", () => {
		const effort = __vibratoPetTestHooks
			.getPixelGrid("danceL")
			.slice(6, 9)
			.map(row => row.slice(5, 11));
		const victory = __vibratoPetTestHooks
			.getPixelGrid("flex")
			.slice(6, 9)
			.map(row => row.slice(5, 11));
	...
	it("encodes kitty frames as chunked raw-RGBA transmits with delete-first", () => {
		const built = buildVibratoPixelFrames({ protocol: "kitty", cellWidthPx: 9, cellHeightPx: 18, targetRows: 2 });
	...
	it("horizontally pads the kitty image so a non-2:1 cell ratio does not stretch the sprite", () => {
		...
		const built = buildVibratoPixelFrames({ protocol: "kitty", cellWidthPx: 14, cellHeightPx: 18, targetRows: 2 });
	...
	it("encodes deterministic iTerm2 PNG dimensions, CRCs, colors, and transparent padding", () => {
		...
		expect(sequence).toContain("name=dmlicmF0by1wZXQucG5n;width=2;height=2;preserveAspectRatio=0;inline=1");
	...
	it("rejects malformed or unbounded iTerm2 PNG inputs", () => {
		...
		expect(() => buildVibratoPixelFrames({ protocol: "iterm2", cellWidthPx: 0, cellHeightPx: 18 })).toThrow(
			"cell width",
		);
		expect(() => buildVibratoPixelFrames({ protocol: "kitty", cellWidthPx: 1, cellHeightPx: 10_000 })).toThrow(
			"frame dimensions are out of bounds",
		);
	});

	it("keeps Kitty and Sixel fixtures unchanged while iTerm2 uses the reserved cell block", () => {
		const sixel = buildVibratoPixelFrames({ protocol: "sixel", cellWidthPx: 9, cellHeightPx: 18 });
		const kitty = buildVibratoPixelFrames({ protocol: "kitty", cellWidthPx: 9, cellHeightPx: 18 });
		...
		for (const fixture of ITERM_PET_GEOMETRY_FIXTURES) {
			const iterm2 = buildVibratoPixelFrames({
				protocol: "iterm2",
...
describe("GIF artifacts and helpers", () => {
	it("encodes deterministic GIF89a geometry, metadata, delays, and distinct skin palettes", () => {
		...
		const red = encodeVibratoPetGif({ skin: "red", timeline, cellWidthPx: 9, cellHeightPx: 18, targetRows: 2 });
		const blue = encodeVibratoPetGif({ skin: "blue", timeline, cellWidthPx: 9, cellHeightPx: 18, targetRows: 2 });
		...
		expect(red.bytes).toEqual(
			encodeVibratoPetGif({ skin: "red", timeline, cellWidthPx: 9, cellHeightPx: 18, targetRows: 2 }).bytes,
		);
		...
		expect(red.multipart[0]).toBe(
			`\x1b]1337;MultipartFile=;name=dmlicmF0by1wZXQuZ2lm;size=${red.bytes.byteLength};width=${red.width}px;height=${red.height}px;inline=1;preserveAspectRatio=0:\x07`,
		);
	...
	it("supports rectangle geometry and all public timeline helpers", () => {
		const rectangle = encodeVibratoPetGif({ rectangle: { width: 7, height: 5 }, timeline: idleTimeline() });
	...
	it("keeps GIF cache bounded and resettable", () => {
		resetVibratoPetGifCache();
		expect(getVibratoPetGifCacheStats()).toMatchObject({
			size: 0,
			bytes: 0,
			evictions: 0,
			gifBytes: 0,
		...
		const first = getVibratoPetGifCached({ rectangle: { width: 1, height: 1 } });
		const second = getVibratoPetGifCached({ rectangle: { width: 2, height: 1 } });
		for (let i = 3; i <= 32; i++) getVibratoPetGifCached({ rectangle: { width: i, height: 1 } });
		expect(getVibratoPetGifCacheStats().size).toBe(32);
		expect(getVibratoPetGifCacheStats().evictions).toBe(0);
		expect(getVibratoPetGifCached({ rectangle: { width: 1, height: 1 } })).toBe(first);
		getVibratoPetGifCached({ rectangle: { width: 33, height: 1 } });
		expect(getVibratoPetGifCacheStats().size).toBe(32);
		expect(getVibratoPetGifCacheStats().evictions).toBe(1);
		expect(getVibratoPetGifCached({ rectangle: { width: 1, height: 1 } })).toBe(first);
		expect(getVibratoPetGifCached({ rectangle: { width: 2, height: 1 } })).not.toBe(second);
		resetVibratoPetGifCache();
		expect(getVibratoPetGifCacheStats()).toMatchObject({
			size: 0,
			bytes: 0,
	...
	it("keys cached GIF artifacts by terminal display size", () => {
		...
		const pixelSized = getVibratoPetGifCached({ rectangle: pixels });
		const cellSized = getVibratoPetGifCached({
			rectangle: pixels,
	...
	it("evicts deterministically at the retained-byte ceiling independently of the entry cap", () => {
		...
		resetVibratoPetGifCache();
		const first = getVibratoPetGifCached({
		...
		const stats = getVibratoPetGifCacheStats();
		...
		const repeated = getVibratoPetGifCacheStats();
