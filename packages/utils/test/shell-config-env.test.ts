...
/**
 * `docs/environment-variables.md` documents `VIB_BASH_NO_CI` / `VIB_BASH_NO_LOGIN`,
 ...
 * do NOT enable), Vibrato-first precedence (an explicit falsey Vibrato beats a truthy
...
describe("spawn env CI suppression (VIB_BASH_NO_CI)", () => {
	it("injects CI=true when no knob is set", () => {
		expect(getShellConfig().env.CI).toBe("true");
	});

	it("honors the documented VIB_BASH_NO_CI", () => {
		expect(configWith({ VIB_BASH_NO_CI: "1" }).env.CI).toBeUndefined();
	});
	...
	it.each(["0", "false", "off", "no"])("treats VIB_BASH_NO_CI=%s as not set", value => {
		expect(configWith({ VIB_BASH_NO_CI: value }).env.CI).toBe("true");
	});

	it("resolves Vibrato-first: an explicit VIB_BASH_NO_CI=0 beats a truthy legacy alias", () => {
		expect(configWith({ VIB_BASH_NO_CI: "0", PI_BASH_NO_CI: "1" }).env.CI).toBe("true");
	});

	it("skips a blank VIB_BASH_NO_CI and falls through to the legacy alias", () => {
		expect(configWith({ VIB_BASH_NO_CI: "   ", PI_BASH_NO_CI: "1" }).env.CI).toBeUndefined();
	});
});

describe("login shell args (VIB_BASH_NO_LOGIN)", () => {
	it("uses a login shell when no knob is set", () => {
		expect(getShellConfig().args).toEqual(["-l", "-c"]);
	});

	it("honors the documented VIB_BASH_NO_LOGIN", () => {
		expect(configWith({ VIB_BASH_NO_LOGIN: "1" }).args).toEqual(["-c"]);
	});
	...
	it.each(["0", "false", "off", "no"])("treats VIB_BASH_NO_LOGIN=%s as not set", value => {
		expect(configWith({ VIB_BASH_NO_LOGIN: value }).args).toEqual(["-l", "-c"]);
	});

	it("resolves Vibrato-first: an explicit VIB_BASH_NO_LOGIN=0 beats a truthy legacy alias", () => {
		expect(configWith({ VIB_BASH_NO_LOGIN: "0", PI_BASH_NO_LOGIN: "1" }).args).toEqual(["-l", "-c"]);
	});

	it("accepts case-insensitive truthy spellings", () => {
		expect(configWith({ VIB_BASH_NO_LOGIN: "true" }).args).toEqual(["-c"]);
		expect(configWith({ VIB_BASH_NO_LOGIN: "YES" }).args).toEqual(["-c"]);
		expect(configWith({ VIB_BASH_NO_LOGIN: "on" }).args).toEqual(["-c"]);
	});
});
