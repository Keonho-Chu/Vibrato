---
name: execute
description: Delegate execution to Vibrato (runs /skill:ultragoal to completion with verification).
---

Call the `vib_delegate_execute` coordinator MCP tool to delegate this work to vib-rato.
...
Vibrato starts a session and runs `/skill:ultragoal` to completion, returning a
