...
description: Operate Vibrato SDK sessions from the CLI (`vib sdk session list|inspect|send|status|tail|raw` plus the explicit raw control|query|global hatch). Advisory reference: broker-bound, credential-free output; mutating verbs run only when explicitly invoked.
---
...
`vib sdk session raw control|query|global` dispatches one SDK operation with `--op`
