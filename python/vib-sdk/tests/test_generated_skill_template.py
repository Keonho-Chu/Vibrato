...
TEMPLATE = ROOT / "sdk-skills" / "vib-sdk-author" / "templates" / "direct-sdk.py"
CORE_QUERIES = (
    "session.metadata",
    "context.get",
    "goal.list/get",
...
def test_python_template_composes_queries_through_broker_cli_and_redacts(
    ...
    assert all(
        args == ["vib", "sdk", "session", "raw", "query", "session-1", "--query", query]
...
def test_python_template_requires_explicit_session_and_rejects_bypass_inputs(
    ...
    cli = FakeCli()
    configure(monkeypatch, cli)
    endpoint_path = tmp_path / ".vib" / "state" / "sdk" / "session-1.json"
...
def test_python_template_contains_no_endpoint_authority_bypass() -> None:
    ...
    assert '["vib", "sdk", "session", *arguments]' in source
