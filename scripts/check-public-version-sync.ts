...
const EXPECTED_EVIDENCE_ASSET = "vibrato-release-packages-expected-v1.json";
const FINAL_EVIDENCE_ASSET = "vibrato-release-packages-v1.json";
...
const VERSIONED_MARKETING_RE = /\b(?:New in|Also new in|Vibrato|Vibrato|Feature card for the)\s+(\d+\.\d+\.\d+)\b/gi;
const MARKETING_VERSION_FILES = ["README.md", "docs/**/*.md", "packages/*/README.md"];
const LIVE_FETCH_TIMEOUT_MS = 5_000;
const PUBLIC_FETCH_HEADERS = { "User-Agent": "vib-rato-public-version-sync/2.0" };
...
export async function buildDocsIndexOutput(repoRoot: string): Promise<string> {
	...
	return [
		...
		`Reflect.set(globalThis, Symbol.for("vib.docs-index.generated.loaded"), true);`,
		"",
...
export async function checkPublicVersionSync(repoRoot = path.join(import.meta.dir, "..")): Promise<SyncViolation[]> {
	...
	const canonicalVersion = catalog["@vib-rato/coding-agent"];

	if (!canonicalVersion) {
		violations.push({ path: "package.json", message: "Missing canonical @vib-rato/coding-agent catalog version." });
		return violations;
	}
	...
	if (await pathExists(bunLockPath)) {
		...
		for (const match of header.matchAll(new RegExp(`\"(@vib-rato\\/[^\"]+)\":\\s*\"(${exactVersion})\"`, "g"))) {
