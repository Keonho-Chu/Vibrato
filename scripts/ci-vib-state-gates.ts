...
delete process.env.VIB_STATE_SESSION_ID;

const relevantPathPrefixes = [
	"packages/coding-agent/",
	".github/workflows/dev-ci.yml",
	".github/workflows/ci.yml",
	"scripts/ci-vib-state-gates.ts",
	"scripts/verify-vib-state-writers.ts",
	"scripts/generate-vib-workflow-manifest.ts",
	"scripts/verify-vib-skill-docs.ts",
...
const boundedGateGroups: Record<string, readonly (readonly string[])[]> = {
	...
	["bun", "scripts/generate-vib-workflow-manifest.ts", "--check"],
	["bun", "scripts/verify-vib-skill-docs.ts", "--fail"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-schema.test.ts"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-migrations.test.ts"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-schema-corpus.test.ts"],
	],
	runtime: [
	// NOTE: state-writer-drift.test.ts imports recordSkillActivation (hooks),
	// which loads the @vib-rato/natives addon transitively, so it runs in the
	// heavier "Affected path validation" job, not this native-free gate.
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-runtime.test.ts"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-handoff.test.ts"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-receipts.test.ts"],
	],
	integrity: [
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-integrity.test.ts"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-write-hardening.test.ts"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-graph.test.ts"],
	],
	read: [
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-read-markdown.test.ts"],
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-token-thrift.test.ts"],
	// Lane H read-only doctor: imports only the native-free state-runtime module.
	["bun", "test", "packages/coding-agent/test/vib-runtime/state-doctor.test.ts"],
	// NOTE: workflow-mutation-guard, vib-skill-state-hooks, and skill-active-state
	// load the @vib-rato/natives addon transitively via the tool/hook runtime, so they
...
async function changedFiles(): Promise<string[]> {
	...
	console.log("vib-state-gates: no comparable base SHA found; running bounded gates.");
...
if (selectedGroup !== "all" && !boundedGateGroups[selectedGroup]) {
	console.error(`vib-state-gates: unknown group '${selectedGroup}'. Known groups: ${groupNames.join(", ")}`);
	process.exit(2);
}
...
if (relevantFiles.length === 0) {
	console.log("vib-state-gates: no relevant paths changed; gate commands skipped.");
	console.log(`vib-state-gates: inspected ${files.length} changed path(s).`);
	process.exit(0);
}

console.log(`vib-state-gates: relevant paths changed; running group ${selectedGroup}.`);
for (const file of relevantFiles) {
	console.log(`vib-state-gates: relevant ${file}`);
}

const commands = selectedGroup === "all" ? groupNames.flatMap(group => boundedGateGroups[group]) : boundedGateGroups[selectedGroup];
for (const command of commands) {
	console.log(`vib-state-gates: running ${command.join(" ")}`);
	await $`${command}`.cwd(repoRoot);
}

console.log(`vib-state-gates: group ${selectedGroup} passed.`);
