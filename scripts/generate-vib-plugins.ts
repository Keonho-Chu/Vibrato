...
/**
 * Generate the canonical vib-rato host plugin bundles under `plugins/`.
 *
 ...
 *
 * Usage:
 *   bun scripts/generate-vib-plugins.ts            # write files
 *   bun scripts/generate-vib-plugins.ts --check    # compare complete file set + bytes, exit 1 on drift
 */
...
const DELEGATE_TOOLS = COORDINATOR_MCP_TOOL_NAMES.filter(name => name.startsWith("vib_delegate_"));

const PLUGIN_NAME = "vib-rato";
...
const DELEGATE_META: DelegateMeta[] = [
	{
		tool: "vib_delegate_plan",
		workflow: "plan",
		skill: "ralplan",
		summary: "Delegate consensus planning to Vibrato (runs /skill:ralplan to a pending-approval plan).",
	},
	{
		tool: "vib_delegate_execute",
		workflow: "execute",
		skill: "ultragoal",
		summary: "Delegate execution to Vibrato (runs /skill:ultragoal to completion with verification).",
	},
];
/**
 * Inventory of `vib sdk session` semantic verbs plus the raw hatch kinds,
 ...
 * `vib-sdk-session` skill is rendered from this inventory and
 * `scripts/verify-vib-skill-docs.ts` checks every skill reference against it,
...
function coordinatorServer(projectDirToken: string): Record<string, unknown> {
	return {
		"vib-coordinator": {
			...
			env: {
				VIB_COORDINATOR_MCP_WORKDIR_ROOTS: projectDirToken,
				VIB_COORDINATOR_MCP_REPO: NAMESPACE_LABEL,
...
Call the \`${meta.tool}\` coordinator MCP tool to delegate this work to vib-rato.
...
Vibrato starts a session and runs \`/skill:${meta.skill}\` to completion, returning a
...
function sdkSessionSkillDoc(): string {
	...
	return `---
name: vib-sdk-session
description: Operate Vibrato SDK sessions from the CLI (\`vib sdk session ${verbs}\` plus the explicit raw ${rawKinds} hatch). Advisory reference: broker-bound, credential-free output; mutating verbs run only when explicitly invoked.
---

# Vibrato SDK session CLI (advisory)

Advisory reference for interacting with live Vibrato SDK sessions through the
broker-bound \`vib sdk session\` command family. This skill is informational:
...
- \`vib sdk session status <sessionId> <opRef>\` — lossless \`turn.result\` with
  \`kind: "prompt"\` for a previously submitted operation reference.

- \`vib sdk session tail <sessionId>\` — retained transcript replay from the
...
\`vib sdk session raw ${rawKinds}\` dispatches one SDK operation with \`--op\`
...
function readmeDoc(): string {
	return `# vib-rato plugin (generated)

These files are generated by \`scripts/generate-vib-plugins.ts\` from the
...
- \`.codex.mcp.json\` — Codex coordinator MCP wiring (host-neutral; \`vib setup codex\` rewrites concrete roots).
- \`commands/\`, \`skills/\` — host-facing delegate command + skill docs, including
  the advisory \`vib-sdk-session\` (SDK session CLI reference) and
  \`vib-sdk-guides\` (trusted SDK guide index) skills.

Install: \`codex plugin marketplace add ./plugins\` (Codex) or \`/plugin marketplace add ./plugins\` (Claude Code), then install the \`vib-rato\` plugin.
`;
}
...
export function renderPluginFiles(): Map<string, string> {
	...
	// `codex plugin marketplace add ./plugins` reads this and loads ./vib-rato.
	files.set(
		path.join(".agents", "plugins", "marketplace.json"),
		json({
			name: `${PLUGIN_NAME}-local`,
			interface: { displayName: "Vibrato" },
			plugins: [
				{
					name: PLUGIN_NAME,
	...
	files.set(
		...
		json({
			name: PLUGIN_NAME,
			owner: { name: "Vibrato" },
			metadata: { description: "Vibrato delegation plugin with advisory SDK skills", version },
			plugins: [
				{
					name: PLUGIN_NAME,
					source: `./${dir}`,
					description:
						"Delegate Vibrato planning/execution workflows via coordinator MCP, plus advisory SDK session CLI and guide skills.",
					version,
					author: { name: "Vibrato" },
					keywords: ["vib", "delegation", "mcp", "planning", "agents", "sdk"],
				},
			],
		}),
	);
	...
	files.set(
		...
		json({
			name: PLUGIN_NAME,
			description:
				"Delegate planning and execution workflows to Vibrato through the coordinator MCP server, plus advisory SDK session CLI and guide skills.",
			version,
			commands: "./commands",
			skills: "./skills",
			mcpServers: "./.mcp.json",
	...
	files.set(
		...
		json({
			...
			version,
			description:
				"Delegate Codex tasks to Vibrato workflows through coordinator MCP, plus advisory SDK session CLI and guide skills.",
			skills: "./skills/",
	...
	// host-neutral file that `vib setup codex` rewrites with a concrete workdir root.
	...
	files.set(path.join(dir, "skills", "vib-delegation", "SKILL.md"), skillDoc());
	files.set(path.join(dir, "skills", "vib-sdk-session", "SKILL.md"), sdkSessionSkillDoc());
	files.set(path.join(dir, "skills", "vib-sdk-guides", "SKILL.md"), sdkGuidesSkillDoc());
...
function runSelfTest(): void {
	...
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "vib-plugin-self-test-"));
	try {
		for (const [rel, content] of files) {
			const target = path.join(root, rel);
			fs.mkdirSync(path.dirname(target), { recursive: true });
