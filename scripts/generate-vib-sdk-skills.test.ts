...
async function materialize(): Promise<{ files: Map<string, string>; root: string }> {
	...
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-sdk-skills-test-"));
	roots.push(root);
...
async function cliFixture(): Promise<CliFixture> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-sdk-template-cli-test-"));
	roots.push(root);
	...
	const executable = path.join(binDir, "vib");
	await Bun.write(
		executable,
		`#!/usr/bin/env bun
const callsPath = Bun.env.VIB_TEMPLATE_CALLS;
const secret = Bun.env.VIB_TEMPLATE_SECRET;
...
async function runTypeScriptTemplate(
	...
	const child = Bun.spawn(["bun", path.join(repoRoot, "sdk-skills", "vib-sdk-author", "templates", "direct-sdk.ts"), ...args], {
		env: {
			...process.env,
			PATH: `${fixture.binDir}${path.delimiter}${process.env.PATH ?? ""}`,
			VIB_TEMPLATE_CALLS: fixture.callsPath,
			VIB_TEMPLATE_SECRET: fixture.secret,
		},
		stdin: "pipe",
		stdout: "pipe",
		stderr: "pipe",
...
describe("generated external Vibrato SDK skills", () => {
	it("renders the exact namespaced six-file versioned contract", () => {
		...
		expect(manifest.bundle).toBe("vib-sdk-skills");
		expect(manifest.formatVersion).toBe(BUNDLE_FORMAT_VERSION);
		expect(manifest.files).toEqual(bundleContentFiles(files));
		expect(files.get("vib-sdk-discover/SKILL.md")).toContain("name: vib-sdk-discover");
		expect(files.get("vib-sdk-operate/SKILL.md")).toContain("name: vib-sdk-operate");
		expect(files.get("vib-sdk-author/SKILL.md")).toContain("name: vib-sdk-author");
	});
	...
	it("keeps the four default workflow skills closed and adds no extra skills", async () => {
		...
		expect(skills).toEqual(["vib-sdk-author/SKILL.md", "vib-sdk-discover/SKILL.md", "vib-sdk-operate/SKILL.md"]);
		for (const key of files.keys()) {
			expect(key.startsWith("packages/coding-agent/")).toBe(false);
			expect(key.startsWith(".vib/")).toBe(false);
			const topLevel = key.split("/")[0];
			if (topLevel !== BUNDLE_MANIFEST_NAME) {
				expect(["vib-sdk-author", "vib-sdk-discover", "vib-sdk-operate"]).toContain(topLevel);
			}
		}
		const defaultsRoot = path.join(repoRoot, "packages", "coding-agent", "src", "defaults", "vib", "skills");
		...
		await fs.mkdir(path.join(extra.root, "vib-sdk-bash"));
		await Bun.write(path.join(extra.root, "vib-sdk-bash", "SKILL.md"), "---\nname: vib-sdk-bash\n---\n");
		expect(await findUnexpectedSdkSkillFiles(extra.files, extra.root)).toEqual([path.join("vib-sdk-bash", "SKILL.md")]);
	...
	it("fails closed on missing, malformed, and unsupported manifest versions", async () => {
		...
		expect(
			validateBundleManifest(JSON.stringify({ bundle: "vib-sdk-skills", formatVersion: 999, files: contentFiles }), contentFiles),
		).toContain("unsupported");
		expect(
			validateBundleManifest(
				JSON.stringify({ bundle: "vib-sdk-skills", formatVersion: BUNDLE_FORMAT_VERSION, files: ["vib-sdk-bash/SKILL.md"] }),
				contentFiles,
			),
	...
	it("upgrades installed v1 bundles and fails closed on legacy or future layouts", async () => {
		...
		await Bun.write(
			...
			JSON.stringify({ bundle: "vib-sdk-skills", formatVersion: 2, files: contentFiles }, null, 2) + "\n",
		);
	...
	it("passes the generator drift check as a subprocess", async () => {
		const child = Bun.spawn(["bun", path.join(repoRoot, "scripts", "generate-vib-sdk-skills.ts"), "--check"], {
			stdout: "pipe",
			stderr: "pipe",
		});
		const [exitCode, stdout, stderr] = await Promise.all([
	...
	it("rejects missing, drifted, and unexpected generated files", async () => {
		...
		await fs.rm(path.join(missing.root, "vib-sdk-discover", "SKILL.md"));
		expect(await checkSdkSkillFiles(missing.files, missing.root, false)).toBe(1);

		const drifted = await materialize();
		await Bun.write(path.join(drifted.root, "vib-sdk-operate", "SKILL.md"), `${drifted.files.get(path.join("vib-sdk-operate", "SKILL.md")) ?? ""}drift\n`);
		expect(await checkSdkSkillFiles(drifted.files, drifted.root, false)).toBe(1);

		const symlinked = await materialize();
		const target = path.join(symlinked.root, "vib-sdk-operate", "SKILL.md");
		...
		if (process.platform !== "win32") {
			...
			const fifoPath = path.join(fifo.root, "vib-sdk-discover", "SKILL.md");
	...
	it("keeps broker-bound templates and guidance fail-closed and outside endpoint authority", () => {
		const files = renderSdkSkillFiles();
		const typescript = files.get("vib-sdk-author/templates/direct-sdk.ts") ?? "";
		const python = files.get("vib-sdk-author/templates/direct-sdk.py") ?? "";
		const discover = files.get("vib-sdk-discover/SKILL.md") ?? "";
		const operate = files.get("vib-sdk-operate/SKILL.md") ?? "";
		const author = files.get("vib-sdk-author/SKILL.md") ?? "";
		for (const source of [typescript, python]) {
			...
			expect(source).not.toContain(".vib/state/sdk");
		...
		expect(typescript).toContain('Bun.spawn(["vib", "sdk", "session"');
		expect(python).toContain('["vib", "sdk", "session", *arguments]');
		...
		expect(discover).toContain("vib sdk session list");
		expect(operate).toContain("vib sdk session raw");
		expect(author).toContain("vib sdk session");
	...
	it("requires an explicit session ID and rejects secret-shaped input before invoking the CLI", async () => {
		const fixture = await cliFixture();
		const stateDirectory = path.join(fixture.repo, ".vib", "state", "sdk");
