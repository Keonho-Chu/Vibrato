...
import authorPrompt from "./vib-sdk-skills/prompts/author.md" with { type: "text" };
import discoverPrompt from "./vib-sdk-skills/prompts/discover.md" with { type: "text" };
import operatePrompt from "./vib-sdk-skills/prompts/operate.md" with { type: "text" };

const repoRoot = path.join(import.meta.dir, "..");
const bundleDir = path.join(repoRoot, "sdk-skills");

export const BUNDLE_NAME = "vib-sdk-skills";
...
// scripts/vib-sdk-skills/prompts/ and are imported verbatim. They are the
...
export function validatePromptAllowlistConsistency(): string | null {
	...
	if (!operatePrompt.includes(controlsBlock)) {
		return "prompt drift: static scripts/vib-sdk-skills/prompts/operate.md per-session controls do not match ALLOWED_CONTROLS";
	}
	if (!operatePrompt.includes(globalsBlock)) {
		return "prompt drift: static scripts/vib-sdk-skills/prompts/operate.md lifecycle operations do not match ALLOWED_GLOBALS";
	}
	return null;
}
...
async function runVibSession(repo: string, arguments_: readonly string[]): Promise<Record<string, unknown>> {
	const child = Bun.spawn(["vib", "sdk", "session", ...arguments_], {
		cwd: repo,
		stdin: "ignore",
		stdout: "pipe",
		stderr: "pipe",
...
async function inspect(repo: string, sessionId: string): Promise<Record<string, unknown>> {
	...
	for (const query of CORE_QUERIES) {
		try {
			const response = await runVibSession(repo, ["raw", "query", sessionId, "--query", query]);
...
async function raw(repo: string, args: string[]): Promise<unknown> {
	const process = Bun.spawn(["vib", "sdk", "session", "raw", ...args, "--repo", repo], { stdout: "pipe", stderr: "pipe" });
...
async function main(): Promise<void> {
	...
	const result = await runVibSession(args.repo, [
		"raw",
		"control",
		args.sessionId,
		"--op",
...
main().catch(() => { process.stderr.write("Vibrato SDK request failed safely.\\n"); process.exitCode = 1; });
`;
}
...
def parse_args() -> argparse.Namespace:
    parser = SafeArgumentParser(description="Trusted local broker-bound Vibrato session template", allow_abbrev=False)
...
def run_vib_session(repo: str, arguments: list[str]) -> dict[str, Any]:
    try:
        completed = subprocess.run(
            ["vib", "sdk", "session", *arguments],
            cwd=repo,
...
def inspect(repo: str, session_id: str) -> dict[str, Any]:
    ...
    for query in CORE_QUERIES:
        try:
            response = run_vib_session(repo, ["raw", "query", session_id, "--query", query])
...
try:
    ...
    print("Vibrato SDK request failed safely.", file=sys.stderr)
    raise SystemExit(1)
`;
}
...
export function renderSdkSkillFiles(): Map<string, string> {
	const files = new Map<string, string>([
		[path.join("vib-sdk-discover", "SKILL.md"), discoverPrompt],
		[path.join("vib-sdk-operate", "SKILL.md"), operatePrompt],
		[path.join("vib-sdk-author", "SKILL.md"), authorPrompt],
		[path.join("vib-sdk-author", "templates", "direct-sdk.ts"), typeScriptTemplate()],
		[path.join("vib-sdk-author", "templates", "direct-sdk.py"), pythonTemplate()],
	]);
...
async function runSelfTest(): Promise<void> {
	const files = renderSdkSkillFiles();
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-sdk-skills-self-test-"));
	try {
		for (const [rel, content] of files) {
			const target = path.join(root, rel);
			await fs.mkdir(path.dirname(target), { recursive: true });
		...
		const stale = path.join(root, "vib-sdk-author", "stale.md");
		await Bun.write(stale, "stale\n");
		if ((await checkSdkSkillFiles(files, root, false)) !== 1 || !(await findUnexpectedSdkSkillFiles(files, root)).includes(path.join("vib-sdk-author", "stale.md")))
