...
import { serializeManifestProjection } from "../packages/coding-agent/src/vib-runtime/workflow-manifest";
...
function usage(): never {
	console.error("Usage: bun scripts/generate-vib-workflow-manifest.ts [--write|--check]");
	process.exit(2);
}
...
function main(): void {
	...
	console.error(
		`G3 DRIFT: ${path.relative(repoRoot, GENERATED_MANIFEST)} does not match serializeManifestProjection(). Run bun scripts/generate-vib-workflow-manifest.ts --write.`,
	);
	process.exit(1);
}
