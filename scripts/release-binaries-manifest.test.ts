...
describe("release binary checksum manifest", () => {
	test("hashes every published standalone asset and round-trips the sums file", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vib-release-binaries-"));
		try {
			...
			expect(manifest.schema).toBe("vibrato-release-binaries-v1");
			...
			expect(parseSha256Sums(sums, "vib-linux-x64")).toBe(manifest.binaries[0]?.sha256);
