...
/**
 * Static verification for the generated vib-rato plugin bundles.
 *
...
import { findUnexpectedPluginFiles, renderPluginFiles } from "./generate-vib-plugins";

const PLUGIN_DIR = "vib-rato";
...
function coordinatorMcpServer(config: Record<string, unknown>): Record<string, unknown> | undefined {
	...
	return record(servers["vib-coordinator"]);
}
...
const delegateTools = COORDINATOR_MCP_TOOL_NAMES.filter(name => name.startsWith("vib_delegate_"));
const delegateToolSet = new Set<string>(delegateTools);
gate(
	"delegate tools in contract",
	delegateTools.length === 2 &&
		["vib_delegate_plan", "vib_delegate_execute"].every(tool => delegateToolSet.has(tool)),
...
const skill = path.join(PLUGIN_DIR, "skills", "vib-delegation", "SKILL.md");
...
const codexMcpOk = "mcp_servers" in codexMcpObj || "vib-coordinator" in codexMcpObj;
...
for (const rel of mcpFiles) {
	const text = read(rel);
	if (!text.includes("VIB_COORDINATOR_MCP_WORKDIR_ROOTS")) workdirRootsOk = false;
	if (/VIB_COORDINATOR_MCP_ROOTS[^_]/.test(text)) noBadRoots = false;
	if (text.includes("VIB_COORDINATOR_MCP_MUTATIONS")) noMutations = false;
}
gate("MCP config uses WORKDIR_ROOTS", mcpFiles.length > 0 && workdirRootsOk, `mcp files: ${mcpFiles.length}`);
gate("MCP config omits invalid ROOTS var", noBadRoots, "no VIB_COORDINATOR_MCP_ROOTS present");
...
const directMachineTmuxRoutePattern =
	/(?:\btmux\s+watch\b|(?:scripts\/)?vib-session\/(?:prompt|tail|watch)(?:\.sh)?\b|\bvib-session\s+(?:prompt|tail|watch)\b|(?:\.\/)?(?:scripts\/)?vib-session\/create\.sh(?:[ \t]+(?:"[^"\n]*"|'[^'\n]*'|[^\s]+)){3}|\bvib-session\s+create(?:[ \t]+(?:"[^"\n]*"|'[^'\n]*'|[^\s]+)){3})/g;
...
const machineTmuxRouteRegressionFixtures = [
	...
	'["capture", "pane"].join("-")',
	'["pipe", "pane"].join("-")',
	'./scripts/vib-session/create.sh bot /repo --print "task"',
	"scripts/vib-session/create.sh bot /repo positional-prompt",
	"vib-session create bot /repo --file task.md",
	"vib-session create bot /repo resume",
	"vib-session create bot /repo \\\n  positional-prompt",
