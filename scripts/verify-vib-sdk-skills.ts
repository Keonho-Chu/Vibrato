...
const discoverPath = path.join("vib-sdk-discover", "SKILL.md");
const operatePath = path.join("vib-sdk-operate", "SKILL.md");
const authorPath = path.join("vib-sdk-author", "SKILL.md");
const typeScriptPath = path.join("vib-sdk-author", "templates", "direct-sdk.ts");
const pythonPath = path.join("vib-sdk-author", "templates", "direct-sdk.py");
...
gate(
	...
	"scripts/vib-sdk-skills/prompts/operate.md",
);
...
for (const [name, contents, expectedName] of [
	["discover", discover, "vib-sdk-discover"],
	["operate", operate, "vib-sdk-operate"],
	["author", author, "vib-sdk-author"],
] as const) {
...
gate(
	...
	discover.includes("vib sdk session list") &&
		author.includes("vib sdk session") &&
		operate.includes("vib sdk session raw") &&
...
gate(
	...
	typeScript.includes('Bun.spawn(["vib", "sdk", "session"') &&
		python.includes('["vib", "sdk", "session", *arguments]') &&
		typeScript.includes("cwd: repo") &&
		python.includes("cwd=repo"),
	"vib sdk session through repository cwd",
);
gate(
