...
/**
 * G4 verifier for bundled Vibrato skill documentation.
 *
 *   --report  (default)  list command references and direct `.vib` shell mutations, exit 0
 *   --fail               exit non-zero on manifest drift or direct `.vib` shell mutations
 */

import * as fs from "node:fs";
import * as path from "node:path";

import { listVerbs } from "../packages/coding-agent/src/vib-runtime/workflow-manifest";
import { CANONICAL_VIB_WORKFLOW_SKILLS, type CanonicalVibWorkflowSkill } from "../packages/coding-agent/src/skill-state/canonical-skills";
import { SDK_SESSION_CLI_VERBS, SDK_SESSION_RAW_KINDS } from "./generate-vib-plugins";

const repoRoot = path.join(import.meta.dir, "..");
const skillsRoot = path.join(repoRoot, "packages", "coding-agent", "src", "defaults", "vib", "skills");
const skills = new Set<string>(CANONICAL_VIB_WORKFLOW_SKILLS);

type AdvisorySkill = "vib-sdk-session" | "vib-sdk-guides";
type DocumentedSkill = CanonicalVibWorkflowSkill | AdvisorySkill;
...
function isSkill(value: string): value is CanonicalVibWorkflowSkill {
...
/** Generated advisory plugin skills rendered by `scripts/generate-vib-plugins.ts`. */
...
const FORBIDDEN_DAEMON_SESSION_ROUTE = /\bvib\s+daemon\s+session\b/gu;
...
const FORBIDDEN_MUTATION_CMD =
	/\b(?:vib\s+config\b|vib\s+settings\b|vib\s+setup\b|vib\s+gc\b|vib\s+plugin\s+(?:install|remove|enable|disable)\b|vib\s+notify\s+setup\b|vib\s+update\b)/gu;
const FORBIDDEN_GUIDE_EXECUTION =
	/\b(?:run|execute|invoke|start|launch|dispatch|run\s+through|work\s+through)\s+(?:the\s+)?(?:advisory\s+)?(?:guide|guide\s+skills?|vib-sdk-guides?)\b/giu;
...
function collectSdkSessionVerbRefs(file: string, content: string): CommandRef[] {
	...
	const commandPattern = new RegExp(`\\bvib\\s+sdk\\s+session\\s+(${verbs})\\s*([a-z][a-z0-9-]*)?\\b`, "gu");
...
function collectSdkSkillContentGates(file: string, content: string): string[] {
	...
	if (FORBIDDEN_DAEMON_SESSION_ROUTE.test(content))
		violations.push(`${relative}: references the removed \`vib daemon session\` route`);
...
function collectCommandRefs(file: string, content: string): CommandRef[] {
	...
	for (let i = 0; i < lines.length; i++) {
		const line = lines[i] ?? "";
		const commandPattern = /\bvib\s+(?:state\s+)?(deep-interview|ralplan|ultragoal|autoresearch)\s+([a-z][a-z0-9-]*)\b/gu;
		for (const match of line.matchAll(commandPattern)) {
			const skill = match[1];
...
function collectDirectVibMutations(file: string, content: string): MutationRef[] {
	const refs: MutationRef[] = [];
	const relative = path.relative(repoRoot, file);
	const lines = content.split("\n");
	const mutationPattern = /(?:^|[;&|]\s*)(?:rm\s+(?:-[A-Za-z]*\s+)*|rmdir\s+|mkdir\s+(?:-[A-Za-z]*\s+)*|touch\s+|mv\s+|cp\s+|install\s+|tee\s+(?:-[A-Za-z]*\s+)*|printf\b[^|;>]*>|echo\b[^|;>]*>|cat\b[^|;>]*>|>+\s*)\.?\.vib(?:\b|\/)/u;
	for (let i = 0; i < lines.length; i++) {
...
function main(): void {
	...
	const missingAdvisory = ADVISORY_SKILL_DIRS.filter(
		dir => !fs.existsSync(path.join(repoRoot, "plugins", "vib-rato", "skills", dir, "SKILL.md")),
	);

	const drift = commandRefs.filter(ref => !ref.valid);
	console.log(`vib skill docs verifier - scanned ${path.relative(repoRoot, skillsRoot)}/*/SKILL.md`);
	console.log(`Found ${commandRefs.length} vib command reference(s).`);
	console.log(`Found ${mutationRefs.length} direct .vib shell mutation example(s).\n`);
	...
	console.log(
		`\nSummary: ${drift.length} command drift issue(s), ${mutationRefs.length} direct .vib shell mutation example(s), ` +
	...
	if (failMode && (drift.length > 0 || mutationRefs.length > 0 || missingAdvisory.length > 0 || advisoryContentGates.length > 0)) {
		console.error(
			`\nG4 FAIL: skill docs must reference manifest verbs only, avoid direct .vib shell mutation examples, ` +
