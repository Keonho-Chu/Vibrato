...
const script = path.join(import.meta.dir, "verify-vib-state-writers.ts");
...
test("--root scans the supplied PR-head tree rather than the verifier source tree", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "vib-writer-root-"));
	try {
		const file = path.join(root, "packages", "coding-agent", "src", "bad.ts");
		await fs.mkdir(path.dirname(file), { recursive: true });
		await Bun.write(file, 'const target = ".vib/state.json";\nawait Bun.write(target, "bad");\n');
...
test("detects aliased node filesystem writes into .vib", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "vib-writer-alias-"));
	try {
		const file = path.join(root, "packages", "coding-agent", "src", "evil.ts");
		await fs.mkdir(path.dirname(file), { recursive: true });
		await Bun.write(
			file,
			'import { writeFile as write } from "node:fs/promises";\nconst target = ".vib/redteam.json";\nawait write(target, "pwn");\n',
		);
		const result = await run(root);
		expect(result.exitCode).toBe(1);
...
test("detects dynamic fs bindings and indirect Bun.write aliases into .vib", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "vib-writer-dynamic-"));
	try {
		const directory = path.join(root, "packages", "coding-agent", "src");
		await fs.mkdir(directory, { recursive: true });
...
test("detects constructed constant .vib paths", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "vib-writer-constructed-"));
	try {
		const file = path.join(root, "packages", "coding-agent", "src", "constructed.ts");
		await fs.mkdir(path.dirname(file), { recursive: true });
		await Bun.write(file, 'const target = "." + "vib/pwned.json";\nawait Bun.write(target, "pwn");\n');
		const result = await run(root);
		expect(result.exitCode).toBe(1);
