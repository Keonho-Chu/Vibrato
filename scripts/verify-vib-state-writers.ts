 ...
 *
 * The locked design requires that EVERY native write to `.vib/**` flows through one sanctioned
 * writer module. This verifier reports filesystem mutation call sites whose path argument is
 * locally tied to a `.vib` path literal or a known `.vib` path helper.
 *
 *   --report  (default)  list every candidate `.vib/**` write site, exit 0
...
// The one module allowed to perform raw `.vib/**` filesystem mutations once routing is complete.
const ALLOWED_WRITER_RELATIVE = path.join("packages", "coding-agent", "src", "vib-runtime", "state-writer.ts");
...
// `.vib` is referenced directly, or via a known path-helper symbol that resolves under `.vib`.
const VIB_REFERENCE_PATTERNS: readonly RegExp[] = [
	/["'`]\.vib(?:[\\/]|["'`])/u,
	/\bstateDirFor\b/u,
	/\bmodeStateFile\b/u,
...
function nearbyAssignmentTargetsThisLine(lines: readonly string[], index: number): boolean {
	...
	for (let i = start; i < index; i++) {
		...
		if (!sameLineReferencesVib(prior)) continue;
...
function sameLineReferencesVib(line: string): boolean {
	if (VIB_REFERENCE_PATTERNS.some(re => re.test(line))) return true;
	if (lineLooksLikeGeneratedStringLiteral(line)) return false;
	const stringParts = [...line.matchAll(/["'`]([^"'`]*)["'`]/gu)].map(match => match[1]!).join("");
	return /(?:^|[\\/])?\.vib(?:[\\/]|$)/u.test(stringParts);
}


function isGuardedOutsideProjectVibFallback(lines: readonly string[], index: number): boolean {
	const start = Math.max(0, index - 8);
	const context = lines.slice(start, index + 1).join("\n");
	return /isUnderProjectVib\([\s\S]*?\}\s*else\s*\{[\s\S]*$/u.test(context);
}

function isGuardedOutsideProjectVibTernaryFallback(lines: readonly string[], index: number): boolean {
	const start = Math.max(0, index - 6);
	const context = lines.slice(start, index + 1).join("\n");
	return /isUnderProjectVib\([\s\S]*?\?[\s\S]*?:\s*[^\n]*$/u.test(context);
}
...
function collectFindings(): Finding[] {
	...
	for (const file of listTsFiles(SCAN_ROOT)) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (!allowed && !sameLineReferencesVib(line) && !nearbyAssignmentTargetsThisLine(lines, i)) continue;
			if (!allowed && (isGuardedOutsideProjectVibFallback(lines, i) || isGuardedOutsideProjectVibTernaryFallback(lines, i))) continue;
...
function main(): void {
	...
	console.log(`vib .vib/** writer inventory - scanned ${path.relative(repoRoot, SCAN_ROOT)}`);
	...
	if (failMode && offending.length > 0) {
		console.error(`\nG1 FAIL: ${offending.length} direct .vib/** write site(s) must route through ${ALLOWED_WRITER_RELATIVE}.`);
		process.exit(1);
	}
}
