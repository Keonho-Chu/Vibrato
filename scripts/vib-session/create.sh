#!/usr/bin/env bash
# Create a public-safe Vibrato tmux session through the owner-isolation protocol.
...
SESSION="$1"
WORKDIR="$2"
VIB_BIN="${VIB_BIN-$(command -v vib || true)}"
TMUX_BIN="${VIB_SESSION_TMUX_BIN:-tmux}"
STATE_DIR="${VIB_SESSION_STATE_DIR:-$WORKDIR/.vib-session-state/$SESSION}"
RUNTIME_STATE_JSON="$STATE_DIR/runtime-state.json"
SOCKET_KEY="vib-${SESSION//[^A-Za-z0-9_.-]/_}"
...
if [[ -z "$VIB_BIN" ]] || { [[ "$VIB_BIN" == */* ]] && [[ ! -x "$VIB_BIN" ]]; } || { [[ "$VIB_BIN" != */* ]] && ! command -v "$VIB_BIN" >/dev/null 2>&1; }; then
  echo "vib not found in PATH; set VIB_BIN" >&2
  exit 1
fi
...
flock -x 9
VIB_SESSION_TRANSITION_LOCK_HELD=1
...
PY
  then
    local deadline=$((SECONDS + ${VIB_SESSION_PENDING_VERDICT_GRACE_SECONDS:-3}))
...
PY
  ...
  if ! vib_session_validate_raw_verdict "$LIFECYCLE_DIR/verdict-$prior_generation.json" "$GENERATION_JSON" "$SESSION" "$prior_generation" "$SOCKET_KEY"; then
...
PY
  vib_session_publish_current_alias "$LIFECYCLE_DIR/verdict-$prior_generation.json" "$STATE_DIR/verdict.json" "$GENERATION_JSON" "$SESSION" "$prior_generation"
  if [[ -f "$LIFECYCLE_DIR/incident-$prior_generation.json" ]]; then
    vib_session_publish_current_alias "$LIFECYCLE_DIR/incident-$prior_generation.json" "$STATE_DIR/incident.json" "$GENERATION_JSON" "$SESSION" "$prior_generation" owner_incident
  fi
  for marker in vanished terminal final; do
    if [[ -f "$LIFECYCLE_DIR/$marker-$prior_generation.json" ]]; then
      ...
      vib_session_publish_current_alias "$LIFECYCLE_DIR/$marker-$prior_generation.json" "$STATE_DIR/$marker.json" "$GENERATION_JSON" "$SESSION" "$prior_generation" "$marker_kind"
    fi
  done
}
...
WORKTREE_BASELINE_DIRTY="$(vib_session_git_dirty_boolean "$WORKDIR")"
...
PY
  ...
  if ! response="$("$TMUX_BIN" -L "$SOCKET_KEY" if-shell -t "$native_id" -F "$condition" "kill-session -t '$native_id' ; display-message -p __vib_creation_rollback_ok__" "display-message -p __vib_creation_rollback_refused__" 2>/dev/null)"; then
    ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_rollback_indeterminate"
    return 1
  fi
  case "$response" in
    __vib_creation_rollback_ok__) return 0 ;;
    __vib_creation_rollback_refused__) ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_rollback_refused" ;;
    *) ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_rollback_indeterminate" ;;
  esac
  return 1
}
...
PY
  if generation_is_current; then
    vib_session_publish_current_alias "$LIFECYCLE_DIR/creation-failed-$OWNER_GENERATION.json" "$STATE_DIR/creation-state.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" creation_failed
  fi
}
...
PY
  if generation_is_current; then
    vib_session_publish_current_alias "$canonical" "$STATE_DIR/creation-cleanup-failure.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" creation_cleanup_failed
  fi
}
...
python3 - "$METADATA_CANONICAL" "$SESSION" "$WORKDIR" "$BRANCH" "$CREATED_AT" "$VIB_BIN" "$STATE_DIR" "$RUNTIME_STATE_JSON" "$WORKTREE_BASELINE_DIRTY" "$OWNER_GENERATION" "$SOCKET_KEY" <<'PY'
import json
import sys
path, session, workdir, branch, created_at, vib_bin, state_dir, runtime_state, baseline_dirty, generation, socket_key = sys.argv[1:]
with open(path, "x", encoding="utf-8") as handle:
    json.dump({
        "schema_version": 1, "session_id": session, "workdir": workdir,
        "branch": branch, "created_at": created_at, "vib_bin": vib_bin,
...
vib_session_write_public_marker "$CREATION_CANONICAL" creation_started "$SESSION" "$OWNER_GENERATION"
cat >"$STATE_DIR/runner.sh" <<'RUNNER'
#!/usr/bin/env bash
set +e
source "$VIB_SESSION_POSTMORTEM_SH"
write_terminal() {
  python3 - "$VIB_SESSION_FINAL_CANONICAL_JSON" "$VIB_SESSION_TERMINAL_CANONICAL_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" "$1" "$2" "$3" "$VIB_COORDINATOR_SESSION_STATE_FILE" "$VIB_SESSION_WORKDIR" "$VIB_SESSION_WORKTREE_BASELINE_DIRTY" "$VIB_SESSION_PROMPT_ACCEPTED_JSON" <<'PY'
import json
import datetime
import os
import subprocess
...
if runtime_path and os.path.isfile(runtime_path) and os.path.getsize(runtime_path):
    ...
    try:
        ...
        fresh_after = os.environ.get("VIB_SESSION_RUNTIME_FRESH_AFTER", "")
...
if os.path.isfile(prompt_path) and os.path.getsize(prompt_path) > 0:
    try:
        ...
        with open(os.path.join(os.environ["VIB_SESSION_STATE_DIR"], session, "owner-lifecycle", "generation.json"), encoding="utf-8") as handle:
...
PY
  vib_session_publish_current_alias "$VIB_SESSION_TERMINAL_CANONICAL_JSON" "$VIB_SESSION_TERMINAL_JSON" "$VIB_SESSION_GENERATION_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" terminal
  vib_session_publish_current_alias "$VIB_SESSION_FINAL_CANONICAL_JSON" "$VIB_SESSION_FINAL_JSON" "$VIB_SESSION_GENERATION_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" terminal
}
...
def write_immutable(path, record, inject=None):
    if inject and os.environ.get("VIB_SESSION_TEST_FAIL_RECEIPT_WRITE") in {inject, inject.removesuffix("_canonical")}:
...
def record_publication_failure(boundary):
    state_dir = os.environ["VIB_SESSION_STATE_DIR"]
...
def publish_current_alias(canonical_path, alias_path, kind, inject=None):
    if inject and os.environ.get("VIB_SESSION_TEST_FAIL_RECEIPT_WRITE") == inject:
        raise OSError("injected receipt write failure")
    completed = subprocess.run(
        ["bash", "-c", 'source "$1"; vib_session_publish_current_alias "$2" "$3" "$4" "$5" "$6" "$7"', "vib-supervisor-failure-alias", os.environ["VIB_SESSION_POSTMORTEM_SH"], canonical_path, alias_path, os.path.join(os.environ["VIB_SESSION_STATE_DIR"], os.environ["VIB_SESSION_NAME"], "owner-lifecycle", "generation.json"), os.environ["VIB_SESSION_NAME"], os.environ["VIB_SESSION_OWNER_GENERATION"], kind],
...
def observe(signum):
    ...
    request = {
        "schema_version": 1,
        "op": "observe_terminal",
        "session_id": os.environ["VIB_SESSION_NAME"],
        "owner_generation": os.environ["VIB_SESSION_OWNER_GENERATION"],
        "state_dir": os.environ["VIB_SESSION_STATE_DIR"],
        "socket_key": os.environ["VIB_TMUX_OWNER_SERVER_KEY"],
    ...
    if signum == signal.SIGTERM:
        intent_path = os.path.join(
            os.environ["VIB_SESSION_STATE_DIR"],
            os.environ["VIB_SESSION_NAME"],
            "owner-lifecycle",
            f'intent-{os.environ["VIB_SESSION_OWNER_GENERATION"]}.json',
        )
        try:
            ...
            valid_intent = (
                ...
                and intent["session_id"] == os.environ["VIB_SESSION_NAME"]
                and intent["generation"] == os.environ["VIB_SESSION_OWNER_GENERATION"]
                and intent["server_key"] == os.environ["VIB_TMUX_OWNER_SERVER_KEY"]
    ...
    try:
        ...
        validated = subprocess.run(["bash", "-c", 'source "$1"; vib_session_validate_raw_verdict "$2" "$3" "$4" "$5" "$6"', "vib-supervisor-verdict", os.environ["VIB_SESSION_POSTMORTEM_SH"], canonical_path, generation_path, session, generation, os.environ["VIB_TMUX_OWNER_SERVER_KEY"]], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=3, check=False)
        ...
        subprocess.run(
            [
                ...
                "vib-supervisor-alias",
                os.environ["VIB_SESSION_POSTMORTEM_SH"],
                canonical_path,
...
try:
    child = subprocess.Popen(command, cwd=os.environ["VIB_SESSION_WORKDIR"])
    status = child.wait()
except OSError:
    status = 127
exit_code = status if status >= 0 else 128 - status
finalized = subprocess.run(
    ["bash", os.environ["VIB_SESSION_RUNNER_SH"], "--finalize", str(exit_code), started_at, now()],
    check=False,
)
...
LAUNCH=(env "VIB_SESSION_NAME=$SESSION" "VIB_SESSION_WORKDIR=$WORKDIR" "VIB_SESSION_BRANCH=$BRANCH" "VIB_SESSION_STATE_DIR=$STATE_DIR" "VIB_SESSION_OWNER_GENERATION=$OWNER_GENERATION" "VIB_SESSION_RUNTIME_FRESH_AFTER=$CREATED_AT" "VIB_SESSION_STARTED_JSON=$STATE_DIR/started.json" "VIB_SESSION_TERMINAL_JSON=$STATE_DIR/terminal.json" "VIB_SESSION_TERMINAL_CANONICAL_JSON=$LIFECYCLE_DIR/terminal-$OWNER_GENERATION.json" "VIB_SESSION_FINAL_JSON=$STATE_DIR/final.json" "VIB_SESSION_FINAL_CANONICAL_JSON=$LIFECYCLE_DIR/final-$OWNER_GENERATION.json" "VIB_SESSION_GENERATION_JSON=$GENERATION_JSON" "VIB_COORDINATOR_SESSION_ID=$SESSION" "VIB_COORDINATOR_SESSION_BRANCH=$BRANCH" "VIB_COORDINATOR_SESSION_STATE_FILE=$RUNTIME_STATE_JSON" "VIB_TMUX_OWNER_GENERATION=$OWNER_GENERATION" "VIB_TMUX_OWNER_STATE_DIR=$STATE_DIR" "VIB_TMUX_OWNER_SERVER_KEY=$SOCKET_KEY" "VIB_SESSION_PROMPT_ACCEPTED_JSON=$STATE_DIR/prompt-accepted.json" "VIB_SESSION_WORKTREE_BASELINE_DIRTY=$WORKTREE_BASELINE_DIRTY" "VIB_SESSION_VIB_BIN=$VIB_BIN" "VIB_SESSION_RUNNER_SH=$STATE_DIR/runner.sh" "VIB_SESSION_POSTMORTEM_SH=$SCRIPT_DIR/postmortem.sh" python3 "$STATE_DIR/supervisor.py")
LAUNCH_SHELL="$(shell_join "${LAUNCH[@]}")"
TMUX_ARGV=("$TMUX_BIN" -L "$SOCKET_KEY" new-session -d -P -F '#{session_id}' -s "$SESSION" -c "$WORKDIR" -n vib "$LAUNCH_SHELL")
...
PLAN_RESPONSE="$(printf '%s\n' "$PLAN_LINE" | "$VIB_BIN" --internal-tmux-owner-isolation)" || { echo "owner-isolation plan protocol failed" >&2; exit 1; }
...
try:
    ...
    if execution.get("mode") == "direct":
        ...
        expected_scope = f"vib-owner-{token}.scope" if isinstance(token, str) and token else None
...
POST_SPAWN_RESPONSE="$(printf '%s\n' "$PLAN_LINE" | "$VIB_BIN" --internal-tmux-owner-isolation)" || { echo "owner-isolation post-spawn proof failed" >&2; show_recovery_hint; exit 1; }
...
for option_value in \
  "@vib-profile=1" \
  "@vib-session-id=$SESSION" \
  "@vib-session-state-file=$RUNTIME_STATE_JSON" \
  "@vib-owner-generation=$OWNER_GENERATION" \
  "@vib-owner-server-key=$SOCKET_KEY"; do
  option="${option_value%%=*}"
  value="${option_value#*=}"
...
PY
  ...
  tag_response="$("$TMUX_BIN" -L "$SOCKET_KEY" if-shell -t "$ROLLBACK_OWNER_NATIVE_ID" -F "$OWNER_TAG_CONDITION" "$tag_command" "display-message -p __vib_owner_tag_refused__" 2>/dev/null)" || {
...
GENERATION_PUBLISH_RESPONSE="$(printf '%s\n' "$GENERATION_PUBLISH_REQUEST" | "$VIB_BIN" --internal-tmux-owner-isolation)" || { echo "generation publication protocol failed" >&2; exit 1; }
...
raise SystemExit(0 if valid else 1)
PY
vib_session_publish_current_alias "$METADATA_CANONICAL" "$STATE_DIR/metadata.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION"
vib_session_publish_current_alias "$CREATION_CANONICAL" "$STATE_DIR/creation-state.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" creation_started
STARTED_CANONICAL="$LIFECYCLE_DIR/started-$OWNER_GENERATION.json"
vib_session_write_public_marker "$STARTED_CANONICAL" started "$SESSION" "$OWNER_GENERATION"
vib_session_publish_current_alias "$STARTED_CANONICAL" "$STATE_DIR/started.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" started
LIFECYCLE_DIR="$STATE_DIR/$SESSION/owner-lifecycle"
GENERATION_JSON="$LIFECYCLE_DIR/generation.json"
...
if [[ "$RECOVERY_RESULT" == 1:* ]]; then
  vib_session_publish_current_alias "$LIFECYCLE_DIR/recovery-$OWNER_GENERATION.json" "$STATE_DIR/recovery.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" owner_recovered || { echo "failed to publish recovery lifecycle alias" >&2; exit 1; }
fi
...
set -euo pipefail
source "$VIB_SESSION_POSTMORTEM_SH"
interval="${VIB_SESSION_MONITOR_INTERVAL:-5}"
case "$interval" in ''|*[!0-9]*) interval=5 ;; esac
(( interval >= 1 )) || interval=1
last_seen_ms="$(date +%s%3N)"
while true; do
  probe_started_ms="$(date +%s%3N)"
  if timeout 1s "$VIB_SESSION_TMUX_BIN" -L "$VIB_SESSION_SOCKET_KEY" has-session -t "=$VIB_SESSION_NAME" >/dev/null 2>&1; then
...
request="$(python3 - "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" "$VIB_SESSION_STATE_DIR" "$VIB_SESSION_SOCKET_KEY" "$observed_at" <<'PY'
import json, sys
...
while true; do
  ...
  if verdict="$(printf '%s\n' "$request" | timeout "${remaining_seconds}s" "$VIB_SESSION_VIB_BIN" --internal-tmux-owner-isolation)"; then break; fi
  verdict=""; sleep 1
done
now_ms="$(date +%s%3N)"
if [[ -z "$verdict" || "$now_ms" -ge "$deadline_at_ms" ]]; then
  monitor_failure_canonical="$VIB_SESSION_STATE_DIR/$VIB_SESSION_NAME/owner-lifecycle/monitor-failure-$VIB_SESSION_OWNER_GENERATION.json"
  python3 - "$monitor_failure_canonical" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" <<'PY'
import json, os, sys
...
PY
  vib_session_publish_current_alias "$monitor_failure_canonical" "$VIB_SESSION_STATE_DIR/monitor-failure.json" "$VIB_SESSION_GENERATION_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" monitor_failure || exit 1
  exit 1
fi
within_recovery_deadline() { [[ "$(date +%s%3N)" -lt "$deadline_at_ms" ]]; }

within_recovery_deadline || exit 1
vib_session_validate_raw_verdict "$VIB_SESSION_VERDICT_CANONICAL_JSON" "$VIB_SESSION_GENERATION_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" "$VIB_SESSION_SOCKET_KEY" || exit 1
within_recovery_deadline || exit 1
classification="$(python3 - "$VIB_SESSION_VERDICT_CANONICAL_JSON" <<'PY'
import json, sys
with open(sys.argv[1], encoding="utf-8") as handle: print(json.load(handle)["classification"])
PY
)"
within_recovery_deadline || exit 1
vib_session_publish_current_alias "$VIB_SESSION_VERDICT_CANONICAL_JSON" "$VIB_SESSION_VERDICT_JSON" "$VIB_SESSION_GENERATION_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" || exit 1
within_recovery_deadline || exit 1
if [[ "$classification" == unexpected_owner_loss ]]; then
  within_recovery_deadline || exit 1
  vib_session_write_vanished_json "$VIB_SESSION_VANISHED_CANONICAL_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_WORKDIR" tmux_session_missing owner_lost failure false false "$VIB_SESSION_OWNER_GENERATION"
  within_recovery_deadline || exit 1
  vib_session_publish_current_alias "$VIB_SESSION_VANISHED_CANONICAL_JSON" "$VIB_SESSION_VANISHED_JSON" "$VIB_SESSION_GENERATION_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" || exit 1
  within_recovery_deadline || exit 1
  vib_session_publish_current_alias "$VIB_SESSION_INCIDENT_CANONICAL_JSON" "$VIB_SESSION_INCIDENT_JSON" "$VIB_SESSION_GENERATION_JSON" "$VIB_SESSION_NAME" "$VIB_SESSION_OWNER_GENERATION" owner_incident || exit 1
  within_recovery_deadline || exit 1
fi
MONITOR
...
if [[ "${VIB_SESSION_MONITOR_DISABLE:-0}" != 1 ]]; then
  MONITOR_LAUNCH=(env "VIB_SESSION_NAME=$SESSION" "VIB_SESSION_WORKDIR=$WORKDIR" "VIB_SESSION_OWNER_GENERATION=$OWNER_GENERATION" "VIB_SESSION_STATE_DIR=$STATE_DIR" "VIB_SESSION_SOCKET_KEY=$SOCKET_KEY" "VIB_SESSION_TMUX_BIN=$TMUX_BIN" "VIB_SESSION_VIB_BIN=$VIB_BIN" "VIB_SESSION_POSTMORTEM_SH=$SCRIPT_DIR/postmortem.sh" "VIB_SESSION_GENERATION_JSON=$GENERATION_JSON" "VIB_SESSION_VERDICT_JSON=$STATE_DIR/verdict.json" "VIB_SESSION_VERDICT_CANONICAL_JSON=$LIFECYCLE_DIR/verdict-$OWNER_GENERATION.json" "VIB_SESSION_VANISHED_JSON=$STATE_DIR/vanished.json" "VIB_SESSION_VANISHED_CANONICAL_JSON=$LIFECYCLE_DIR/vanished-$OWNER_GENERATION.json" "VIB_SESSION_INCIDENT_JSON=$STATE_DIR/incident.json" "VIB_SESSION_INCIDENT_CANONICAL_JSON=$LIFECYCLE_DIR/incident-$OWNER_GENERATION.json" "VIB_SESSION_MONITOR_INTERVAL=${VIB_SESSION_MONITOR_INTERVAL:-5}" bash "$STATE_DIR/monitor.sh")
...
flock -u 9
exec 9>&-
unset VIB_SESSION_TRANSITION_LOCK_HELD
printf 'created Vibrato session: %s\n' "$SESSION"
