...
function env(overrides: Record<string, string>) {
	return { ...process.env, VIB_SESSION_MONITOR_DISABLE: "1", ...overrides };
}

async function fixture(root: string, mode = "direct", runner = "sleep 30") {
	const bin = path.join(root, "bin", "vib");
	await executable(bin, `#!/usr/bin/env bash
set -euo pipefail
if [[ "${"$"}{1:-}" == --internal-tmux-owner-isolation ]]; then
  request=$(cat)
  if [[ -n "${"$"}{VIB_FIXTURE_ISOLATION_LOG:-}" ]]; then printf '%s\n' "$request" >>"${"$"}VIB_FIXTURE_ISOLATION_LOG"; fi
...
if request["op"] == "publish_generation":
    ...
    if mode == "scoped" and not post_spawn:
        ...
        expected_scope = f"vib-owner-{token}.scope"
        generation_path = os.path.join(request["state_dir"], request["session_id"], "owner-lifecycle", "generation.json")
        if os.path.exists(generation_path):
    ...
    if post_spawn:
        ...
        server_start_time = os.environ.get("VIB_FIXTURE_POSTSPAWN_START_TIME", server_start_time)
...
runner_status=$?
sleep "${"$"}{VIB_SESSION_FIXTURE_HOLD_SECONDS:-1}"
exit "$runner_status"
`);
	return bin;
}
...
async function supervisorAdapter(root: string) {
	const bin = path.join(root, "bin", "vib-supervisor-adapter");
...
if len(sys.argv) > 1 and sys.argv[1] == "--internal-tmux-owner-isolation":
    ...
    session = os.environ["VIB_SESSION_NAME"]
    generation = os.environ["VIB_SESSION_OWNER_GENERATION"]
    server_key = os.environ["VIB_TMUX_OWNER_SERVER_KEY"]
    intent_path = os.path.join(os.environ["VIB_SESSION_STATE_DIR"], session, "owner-lifecycle", f"intent-{generation}.json")
    try:
    ...
    with open(os.environ["VIB_FIXTURE_SAFE_OBSERVATION_LOG"], "a", encoding="utf-8") as handle:
...
open(os.environ["VIB_FIXTURE_RAW_READY"], "w", encoding="utf-8").close()
...
describe("vib-session create public owner lifecycle", () => {
	test("rejects missing binaries, directories, git worktrees, and detached branches", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-validation-")); roots.push(root);
		...
		const missing = Bun.spawnSync(["bash", createScript, "x", root], { env: env({ VIB_BIN: "/definitely-not-a-vib-executable" }), stderr: "pipe" });
		expect(missing.exitCode).toBe(1); expect(missing.stderr.toString()).toContain("vib not found");
		const binary = await fixture(root);
		const nongit = Bun.spawnSync(["bash", createScript, "x", root], { env: env({ VIB_BIN: binary }), stderr: "pipe" });
		expect(nongit.exitCode).toBe(1); expect(nongit.stderr.toString()).toContain("not a git worktree");
		const absent = Bun.spawnSync(["bash", createScript, "x", path.join(root, "absent")], { env: env({ VIB_BIN: binary }), stderr: "pipe" });
		...
		const detachedResult = Bun.spawnSync(["bash", createScript, "detached", detached], { env: env({ VIB_BIN: binary }), stderr: "pipe" });
	...
	test("writes worktree baseline and public creation metadata before planning", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-metadata-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root);
		const name = `metadata-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	...
	test("records an initially dirty worktree without claiming it as new recovery work", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-dirty-")); roots.push(root);
		...
		const name = `dirty-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	...
	test("accepts direct canonical plans, re-proves the isolated server, and tags the owner session", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-direct-")); roots.push(root);
		...
		const name = `direct-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
		const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state, VIB_FIXTURE_ISOLATION_LOG: proofLog }), stderr: "pipe" });
		if (result.exitCode !== 0) throw new Error(result.stderr.toString());
		expect(result.stdout.toString()).toContain(`created Vibrato session: ${name}`);
		...
		for (const [option, value] of [["@vib-profile", "1"], ["@vib-session-id", name], ["@vib-session-state-file", path.join(state, "runtime-state.json")]] as const) {
			expect(Bun.spawnSync(["tmux", "-L", socket, "show-options", "-t", `=${name}:`, "-v", option], { stdout: "pipe" }).stdout.toString().trim()).toBe(value);
		}
		expect(Bun.spawnSync(["tmux", "-L", socket, "show-options", "-t", `=${name}:`, "-v", "@vib-owner-generation"], { stdout: "pipe" }).stdout.toString().trim()).not.toBe("");
		expect(Bun.spawnSync(["tmux", "-L", socket, "show-options", "-t", `=${name}:`, "-v", "@vib-owner-server-key"], { stdout: "pipe" }).stdout.toString().trim()).toBe(socket);
		...
		expect(supervisor).toContain("vib_session_publish_current_alias");
	...
	test("rejects a post-spawn server start-time mismatch before tagging or generation publication", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-postspawn-start-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root);
		const name = `postspawn-start-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
		const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state, VIB_FIXTURE_POSTSPAWN_START_TIME: "1" }), stdout: "pipe", stderr: "pipe" });
	...
	test("accepts scoped plans with exactly the owner bootstrap stdin line", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-scoped-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root, "scoped");
		const name = `scoped-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
...
test("fails closed for malformed plans before creating an owner", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-invalid-plan-")); roots.push(root);
	const dir = await worktree(root); const bad = path.join(root, "bad-vib"); const name = `invalid-plan-${Date.now()}`;
...
if request.get("op") == "publish_generation":
    ...
    print("{}")
`);
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bad }), stderr: "pipe" });
	expect(result.exitCode).toBe(1); expect(result.stderr.toString()).toContain("plan response rejected");
	expect(Bun.spawnSync(["tmux", "-L", `vib-${name}`, "has-session", "-t", `=${name}`], { stdout: "pipe", stderr: "pipe" }).exitCode).not.toBe(0);
});

	test("runner classifies terminal runtime completion as normal cleanup", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-completed-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state");
		const runner = `python3 - <<'PY'\nimport json, os\njson.dump({"session_id": os.environ["VIB_SESSION_NAME"], "cwd": os.environ["VIB_SESSION_WORKDIR"], "owner_generation": os.environ["VIB_SESSION_OWNER_GENERATION"], "state":"completed", "final_response":{"source":"agent_end"}}, open(os.environ["VIB_COORDINATOR_SESSION_STATE_FILE"], "w"))\nPY\nexit 0`;
		const bin = await fixture(root, "direct", runner); const name = `completed-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
	...
	test("runner ignores arbitrary runtime-state payload classifications", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-postmortem-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state");
		const hostile = "private runtime payload must never persist";
		const runner = `python3 - <<'PY'\nimport json, os\njson.dump({"session_id": os.environ["VIB_SESSION_NAME"], "cwd": os.environ["VIB_SESSION_WORKDIR"], "owner_generation": os.environ["VIB_SESSION_OWNER_GENERATION"], "state":"errored", "source":"${hostile}", "event":"${hostile}", "reason":"${hostile}", "previous_runtime_state":"${hostile}", "final_response":{"source":"${hostile}"}}, open(os.environ["VIB_COORDINATOR_SESSION_STATE_FILE"], "w"))\nPY\nexit 1`;
		const bin = await fixture(root, "direct", runner); const name = `postmortem-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
...
test("runner ignores a stale prompt acceptance marker from another generation", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-recovery-")); roots.push(root);
	...
	const bin = await fixture(root, "direct", "printf changed > README.md; exit 0"); const name = `recovery-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
...
test("ships only human-visible tmux lifecycle helpers", async () => {
	...
	for (const forbidden of ["clawhip", "router", "VIB_SESSION_ROUTER", "VIB_SESSION_CHANNEL", "VIB_SESSION_KEYWORDS", "tmux watch", "--channel", "--mention", "--keywords"]) {
		expect(create).not.toContain(forbidden);
	}
	...
	expect(postmortem).toContain("vib_session_write_public_marker");
	expect(harnessOwner).toContain("MACHINE_CONTROL=Coordinator MCP, ACP, or Vibrato SDK");
});

test("passes branch and coordinator identity to the raw owner without private lifecycle payloads", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-owner-env-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state");
	const runner = `python3 - <<'PY'
import json, os
json.dump({key: os.environ.get(key) for key in ["VIB_COORDINATOR_SESSION_ID", "VIB_COORDINATOR_SESSION_BRANCH", "VIB_COORDINATOR_SESSION_STATE_FILE", "VIB_SESSION_STATE_DIR"]}, open(os.path.join(os.environ["VIB_SESSION_STATE_DIR"], "owner-env.json"), "w"))
PY
sleep 2`;
	const bin = await fixture(root, "direct", runner); const name = `owner-env-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	await waitFor(path.join(state, "owner-env.json"));
	expect(await Bun.file(path.join(state, "owner-env.json")).json()).toEqual({ VIB_COORDINATOR_SESSION_ID: name, VIB_COORDINATOR_SESSION_BRANCH: "session-test", VIB_COORDINATOR_SESSION_STATE_FILE: path.join(state, "runtime-state.json"), VIB_SESSION_STATE_DIR: state });
});


test("records one generation-bound recovery only after a prior incident replacement is started", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-replacement-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root);
	const name = `replacement-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
	...
	const created = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }), stderr: "pipe" });
...
test("reconciles an immediately replaced missing owner before publishing the next generation", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-immediate-replace-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `immediate-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	...
	const replacement = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }), stdout: "pipe", stderr: "pipe" });
...
test("publishes one coherent generation-bound creation failure and removes stale completion aliases", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-failure-receipt-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const name = `failure-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	const bin = path.join(root, "bin", "vib");
	await executable(bin, `#!/usr/bin/env python3
...
if __import__("os").path.exists(__import__("os").environ["VIB_FIXTURE_POSTSPAWN"]): print("{}")
else:
 open(__import__("os").environ["VIB_FIXTURE_POSTSPAWN"], "w").close()
...
`);
	...
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state, VIB_FIXTURE_POSTSPAWN: path.join(root, "postspawn") }), stdout: "pipe", stderr: "pipe" });
	expect(result.exitCode).toBe(1);
	const lifecycle = path.join(state, name, "owner-lifecycle");
...
test("retries a generation-bound failed create without synthesizing owner loss or recovery", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-failed-retry-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `failed-retry-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
	...
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state, VIB_SESSION_TMUX_BIN: failingTmux }) }).exitCode).toBe(1);
	const lifecycle = path.join(state, name, "owner-lifecycle");
	const failureName = (await fs.readdir(lifecycle)).find(file => file.startsWith("creation-failed-"))!;
...
test("does not recover an incident again in a third generation when a canonical recovery exists", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-third-recovery-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root);
	const name = `third-recovery-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` }); const lifecycle = path.join(state, name, "owner-lifecycle");
	...
	test("does not accept a terminal runtime record from another session", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-mismatch-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state");
		const runner = `python3 - <<'PY'\nimport json, os\njson.dump({"session_id":"other", "state":"completed"}, open(os.environ["VIB_COORDINATOR_SESSION_STATE_FILE"], "w"))\nPY\nexit 0`;
		const bin = await fixture(root, "direct", runner); const name = `mismatch-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
	...
	test("does not classify a stale terminal runtime receipt as a replacement generation cleanup", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-stale-runtime-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); await fs.mkdir(state, { recursive: true });
		const name = `stale-runtime-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	...
	test("writes terminal lifecycle state even when the runner exits unsuccessfully", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-terminal-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root, "direct", "exit 23");
		const name = `terminal-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "terminal.json"));
	...
	test("monitor consumes one absolute seven-second recovery budget across polling and publication", async () => {
		...
		expect(create).toContain('timeout "${remaining_seconds}s" "$VIB_SESSION_VIB_BIN"');
	...
	test("anchors recovery to the pre-probe timestamp and rejects a crossed publication deadline", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-monitor-deadline-"));
		roots.push(root);
...
`,
		...
		const adapter = path.join(root, "vib");
		await executable(adapter, "#!/usr/bin/env bash\nprintf '{\"ok\":true}\\n'\n");
		const postmortem = path.join(root, "postmortem.sh");
		await executable(
			postmortem,
			`#!/usr/bin/env bash
vib_session_publish_current_alias() { cp "$1" "$2"; }
vib_session_validate_raw_verdict() { return 0; }
vib_session_write_vanished_json() { : >"$1"; }
`,
		);
		...
		const result = Bun.spawnSync(["bash", monitor], {
			env: {
				...
				VIB_SESSION_MONITOR_INTERVAL: "1",
				VIB_SESSION_POSTMORTEM_SH: postmortem,
				VIB_SESSION_TMUX_BIN: tmux,
				VIB_SESSION_SOCKET_KEY: "private-socket",
				VIB_SESSION_NAME: name,
				VIB_SESSION_OWNER_GENERATION: "11111111-1111-4111-8111-111111111111",
				VIB_SESSION_STATE_DIR: state,
				VIB_SESSION_VIB_BIN: adapter,
				VIB_SESSION_VERDICT_CANONICAL_JSON: path.join(lifecycle, "verdict.json"),
				VIB_SESSION_GENERATION_JSON: path.join(lifecycle, "generation.json"),
				VIB_SESSION_VERDICT_JSON: path.join(state, "verdict.json"),
				VIB_SESSION_WORKDIR: root,
				VIB_SESSION_VANISHED_CANONICAL_JSON: path.join(lifecycle, "vanished.json"),
				VIB_SESSION_VANISHED_JSON: path.join(state, "vanished.json"),
				VIB_SESSION_INCIDENT_CANONICAL_JSON: path.join(lifecycle, "incident.json"),
				VIB_SESSION_INCIDENT_JSON: path.join(state, "incident.json"),
			},
			stdout: "pipe",
			stderr: "pipe",
		});
	...
	test("monitor writes immutable verdict and incident markers after owner loss without claiming recovery", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-monitor-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root);
		const name = `monitor-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: { ...env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }), VIB_SESSION_MONITOR_DISABLE: "0", VIB_SESSION_MONITOR_INTERVAL: "1" } }).exitCode).toBe(0);
		...
		const recovered = Bun.spawnSync(["bash", createScript, name, dir], {
			env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }),
			stdout: "pipe",
			stderr: "pipe",
		});
...
test("serializes current aliases and refuses a stale generation overwrite", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-alias-generation-")); roots.push(root);
	...
	const stale = Bun.spawnSync(
		[
			"bash",
			"-c",
			'source "$1"; vib_session_publish_current_alias "$2" "$3" "$4" "$5" "$6"',
			"vib-alias-test",
			postmortemScript,
			canonicalPath,
			aliasPath,
...
test("keeps creation-cleanup and monitor failure canonicals immutable and rejects stale aliases", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-failure-publication-")); roots.push(root);
	...
	const stale = Bun.spawnSync(["bash", "-c", 'source "$1"; vib_session_publish_current_alias "$2" "$3" "$4" "$5" "$6" creation_cleanup_failed', "vib-cleanup-alias-test", postmortemScript, canonicalPath, aliasPath, generationPath, "session", "old"], { stdout: "pipe", stderr: "pipe" });
	expect(stale.exitCode).not.toBe(0);
	...
	const staleMonitor = Bun.spawnSync(["bash", "-c", 'source "$1"; vib_session_publish_current_alias "$2" "$3" "$4" "$5" "$6" monitor_failure', "vib-monitor-alias-test", postmortemScript, monitorCanonicalPath, monitorAliasPath, generationPath, "session", "old"], { stdout: "pipe", stderr: "pipe" });
	...
	expect(create).toContain('monitor-failure-$VIB_SESSION_OWNER_GENERATION.json');
	expect(create).toContain('os.link(temporary, canonical)');
	expect(create).toContain('vib_session_publish_current_alias "$monitor_failure_canonical"');
});
...
test("keeps canonical generation receipts immutable while aliases publish the current generation", async () => {
	...
	expect(create).toContain("VIB_SESSION_GENERATION_JSON");
	expect(postmortem).toContain("vib_session_publish_current_alias");
...
test("rejects a non-UUID prior generation before reconciliation or alias publication", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-invalid-generation-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `invalid-generation-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	...
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }), stdout: "pipe", stderr: "pipe" });
	expect(result.exitCode).toBe(1);
...
test("validates expected raw verdicts only against the matching consumed intent", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-consumed-intent-"));
	roots.push(root);
	...
	const validate = () =>
		Bun.spawnSync(
			[
				"bash",
				"-c",
				'source "$1"; vib_session_validate_raw_verdict "$2" "$3" "$4" "$5" "$6" "$7"',
...
test("accepts only canonical UTC calendar timestamps for raw expected and unexpected verdicts", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-raw-verdict-timestamps-")); roots.push(root);
	...
	const validate = () => Bun.spawnSync(["bash", "-c", 'source "$1"; vib_session_validate_raw_verdict "$2" "$3" "$4" "$5" "$6" "$7"', "validate-raw-timestamp", postmortemScript, verdictPath, generationPath, session, generation, serverKey, intentPath], { stdout: "pipe", stderr: "pipe" });
...
test("forwards SIGTERM dispatch only for an exact live owner intent receipt", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-supervisor-intent-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const setupBin = await fixture(root);
	const name = `intent-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: setupBin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	...
	for (const [, intent] of cases) {
		...
		const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, VIB_SESSION_NAME: name, VIB_SESSION_WORKDIR: dir, VIB_SESSION_STATE_DIR: state, VIB_SESSION_OWNER_GENERATION: generation, VIB_TMUX_OWNER_SERVER_KEY: socket, VIB_SESSION_VIB_BIN: adapter, VIB_SESSION_POSTMORTEM_SH: postmortemScript, VIB_SESSION_RUNNER_SH: "/bin/true", VIB_FIXTURE_RAW_READY: ready, VIB_FIXTURE_SAFE_OBSERVATION_LOG: observationLog }, stdout: "pipe", stderr: "pipe" });
		await Promise.race([
			waitFor(ready),
...
test("records a nonzero terminal observer adapter failure while forwarding SIGTERM", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-supervisor-adapter-failure-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const setupBin = await fixture(root); const name = `adapter-failure-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: setupBin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	const generation = ((await Bun.file(path.join(state, name, "owner-lifecycle", "generation.json")).json()) as { generation: string }).generation;
...
open(os.environ["VIB_FIXTURE_RAW_READY"], "w").close()
signal.signal(signal.SIGTERM, lambda *_: raise_exit())
def raise_exit(): raise SystemExit(0)
signal.pause()
`);
	const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, VIB_SESSION_NAME: name, VIB_SESSION_WORKDIR: dir, VIB_SESSION_STATE_DIR: state, VIB_SESSION_OWNER_GENERATION: generation, VIB_TMUX_OWNER_SERVER_KEY: `vib-${name}`, VIB_SESSION_VIB_BIN: adapter, VIB_SESSION_POSTMORTEM_SH: postmortemScript, VIB_SESSION_RUNNER_SH: "/bin/true", VIB_FIXTURE_RAW_READY: ready }, stdout: "pipe", stderr: "pipe" });
	await waitFor(ready);
	expect(Bun.spawnSync(["kill", "-TERM", String(supervisor.pid)]).exitCode).toBe(0);
	expect(await supervisor.exited).toBe(0);
...
test("records finalizer failure without replacing the owner exit status", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-finalizer-failure-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const setupBin = await fixture(root); const name = `finalizer-failure-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: setupBin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	const generation = ((await Bun.file(path.join(state, name, "owner-lifecycle", "generation.json")).json()) as { generation: string }).generation;
	...
	const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, VIB_SESSION_NAME: name, VIB_SESSION_WORKDIR: dir, VIB_SESSION_STATE_DIR: state, VIB_SESSION_OWNER_GENERATION: generation, VIB_TMUX_OWNER_SERVER_KEY: `vib-${name}`, VIB_SESSION_VIB_BIN: child, VIB_SESSION_POSTMORTEM_SH: postmortemScript, VIB_SESSION_RUNNER_SH: finalizer }, stdout: "pipe", stderr: "pipe" });
...
test("records rollback failures and immutable collision faults", async () => {
	for (const kind of ["rollback", "probe", "canonical-collision"] as const) {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), `vib-creation-cleanup-${kind}-`)); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `creation-cleanup-${kind}-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		const tmux = path.join(root, "tmux-fault");
		await executable(tmux, `#!/usr/bin/env bash
if [[ "$*" == *"set-option"* ]]; then
  if [[ "${"$"}{VIB_FIXTURE_CLEANUP_KIND}" == canonical-collision ]]; then generation="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["generation"])' "${"$"}{VIB_FIXTURE_STATE}/${"$"}{VIB_FIXTURE_SESSION}/owner-lifecycle/generation.json")"; canonical="${"$"}{VIB_FIXTURE_STATE}/${"$"}{VIB_FIXTURE_SESSION}/owner-lifecycle/creation-cleanup-failure-${"$"}{generation}.json"; alias="${"$"}{VIB_FIXTURE_STATE}/creation-cleanup-failure.json"; printf '{"schema_version":1,"kind":"creation_cleanup_failed","session_id":"%s","owner_generation":"%s","exit_code":99,"rollback_failures":["owner_session"],"failure_publication_failed":false}\n' "${"$"}{VIB_FIXTURE_SESSION}" "${"$"}{generation}" >"${"$"}{canonical}"; printf '{"schema_version":1,"kind":"creation_cleanup_failed","session_id":"%s","owner_generation":"%s","exit_code":98,"rollback_failures":["owner_session"],"failure_publication_failed":false}\n' "${"$"}{VIB_FIXTURE_SESSION}" "${"$"}{generation}" >"${"$"}{alias}"; fi
  touch "${"$"}{VIB_FIXTURE_PROBE_MARKER}"
  exit 23
fi
if [[ ( "${"$"}{VIB_FIXTURE_CLEANUP_KIND}" == rollback || "${"$"}{VIB_FIXTURE_CLEANUP_KIND}" == canonical-collision ) && "$*" == *"kill-session"* ]]; then exit 24; fi
if [[ "${"$"}{VIB_FIXTURE_CLEANUP_KIND}" == probe && -e "${"$"}{VIB_FIXTURE_PROBE_MARKER}" && "$*" == *"if-shell"* ]]; then exit 25; fi
exec tmux "$@"
`);
		const created = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state, VIB_SESSION_TMUX_BIN: tmux, VIB_FIXTURE_CLEANUP_KIND: kind, VIB_FIXTURE_STATE: state, VIB_FIXTURE_SESSION: name, VIB_FIXTURE_PROBE_MARKER: path.join(root, "rollback-probe") }), stdout: "pipe", stderr: "pipe" });
...
test("refuses rollback after same-name replacement of the captured native owner", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-creation-replacement-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `creation-replacement-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
	const tmux = path.join(root, "tmux-replacement");
	await executable(tmux, `#!/usr/bin/env bash
if [[ "$*" == *"set-option"* ]] && [[ ! -e "${"$"}{VIB_FIXTURE_REPLACED}" ]]; then
  touch "${"$"}{VIB_FIXTURE_REPLACED}"
  tmux -L "${"$"}{VIB_FIXTURE_SOCKET}" kill-session -t "=${"$"}{VIB_FIXTURE_SESSION}"
  tmux -L "${"$"}{VIB_FIXTURE_SOCKET}" new-session -d -s "${"$"}{VIB_FIXTURE_SESSION}" 'sleep 30'
  exit 23
fi
exec tmux "$@"
`);
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state, VIB_SESSION_TMUX_BIN: tmux, VIB_FIXTURE_REPLACED: path.join(root, "replaced"), VIB_FIXTURE_SOCKET: socket, VIB_FIXTURE_SESSION: name }), stdout: "pipe", stderr: "pipe" });
	expect(result.exitCode).toBe(1);
...
test("holds stale publishers behind replacement creation and preserves every current alias", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-alias-transition-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `transition-${Date.now()}`; const socket = `vib-${name}`; sessions.push({ name, socket });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	...
	await executable(tmux, `#!/usr/bin/env bash
if [[ "$*" == *"new-session"* ]]; then touch "${"$"}{VIB_FIXTURE_TRANSITION_GATE}"; read -r <"${"$"}{VIB_FIXTURE_TRANSITION_RELEASE}"; fi
exec tmux "$@"
`);
	const replacement = Bun.spawn(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state, VIB_SESSION_TMUX_BIN: tmux, VIB_FIXTURE_TRANSITION_GATE: gate, VIB_FIXTURE_TRANSITION_RELEASE: release }), stdout: "pipe", stderr: "pipe" });
	await waitFor(gate);
	...
	const publisher = Bun.spawn(["bash", "-c", 'source "$1"; lifecycle="$2"; state="$3"; generation_path="$4"; session="$5"; generation="$6"; started="$7"; done_marker="$8"; touch "$started"; status=0; for pair in "metadata metadata.json" "creation-state creation-state.json" "started started.json"; do set -- $pair; vib_session_publish_current_alias "$lifecycle/$1-$generation.json" "$state/$2" "$generation_path" "$session" "$generation" || status=1; done; touch "$done_marker"; exit "$status"', "publisher", postmortemScript, lifecycle, state, path.join(lifecycle, "generation.json"), name, oldGeneration, publisherStarted, publisherDone], { stdout: "pipe", stderr: "pipe" });
...
test("rejects a noncanonical prior generation before owner creation", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-create-prestart-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `prestart-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
	...
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }), stderr: "pipe" });
	expect(result.exitCode).toBe(1);
...
test("fails closed and byte-preserves malformed, corrupt, or mismatched current generation state", async () => {
	for (const [label, body] of [
		...
		const root = await fs.mkdtemp(path.join(os.tmpdir(), `vib-generation-${label}-`)); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `generation-${label}-${Date.now()}-${Math.random()}`; sessions.push({ name, socket: `vib-${name}` });
		const lifecycle = path.join(state, name, "owner-lifecycle"); await fs.mkdir(lifecycle, { recursive: true });
...
test("requires every terminal runtime identity receipt field", async () => {
	for (const missing of ["session_id", "cwd", "owner_generation"] as const) {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), `vib-runtime-${missing}-`)); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const name = `runtime-${missing}-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		const runner = `python3 - <<'PY'\nimport json, os\nrecord={"session_id":os.environ["VIB_SESSION_NAME"],"cwd":os.environ["VIB_SESSION_WORKDIR"],"owner_generation":os.environ["VIB_SESSION_OWNER_GENERATION"],"state":"completed"}\ndel record["${missing}"]\njson.dump(record, open(os.environ["VIB_COORDINATOR_SESSION_STATE_FILE"], "w"))\nPY\nexit 0`;
...
test("does not reconcile malformed or unknown canonical verdicts into incidents", async () => {
	for (const verdict of [
		...
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "vib-verdict-invalid-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `invalid-${Date.now()}-${Math.random()}`; sessions.push({ name, socket: `vib-${name}` });
		const lifecycle = path.join(state, name, "owner-lifecycle"); await fs.mkdir(lifecycle, { recursive: true });
		...
		const storedVerdict = "observed_at" in verdict
			? { schema_version: 1, generation: "prior", session_id: name, server_key: `vib-${name}`, observed_at: verdict.observed_at, signal: "UNKNOWN", exit_code: null, result: "owner_lost", observer: "replacement_reconciler", classification: "unexpected_owner_loss", reason: "tmux_session_missing", dedupe_key: `owner-loss:${name}:prior` }
		...
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: bin, VIB_SESSION_STATE_DIR: state }) }).exitCode).not.toBe(0);
...
test("keeps owner status while exposing injected supervisor and finalization receipt publication failures", async () => {
	for (const kind of ["supervisor_failure", "finalization_failure"] as const) {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), `vib-receipt-publication-${kind}-`)); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const setupBin = await fixture(root); const name = `receipt-${kind}-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: setupBin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
		const generation = ((await Bun.file(path.join(state, name, "owner-lifecycle", "generation.json")).json()) as { generation: string }).generation;
		...
		if (kind === "supervisor_failure") {
			...
			const ready = path.join(root, "ready"); const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, VIB_SESSION_NAME: name, VIB_SESSION_WORKDIR: dir, VIB_SESSION_STATE_DIR: state, VIB_SESSION_OWNER_GENERATION: generation, VIB_TMUX_OWNER_SERVER_KEY: `vib-${name}`, VIB_SESSION_VIB_BIN: owner, VIB_SESSION_POSTMORTEM_SH: postmortemScript, VIB_SESSION_RUNNER_SH: "/bin/true", VIB_FIXTURE_RAW_READY: ready, VIB_SESSION_TEST_FAIL_RECEIPT_WRITE: kind }, stdout: "pipe", stderr: "pipe" });
			...
			const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, VIB_SESSION_NAME: name, VIB_SESSION_WORKDIR: dir, VIB_SESSION_STATE_DIR: state, VIB_SESSION_OWNER_GENERATION: generation, VIB_TMUX_OWNER_SERVER_KEY: `vib-${name}`, VIB_SESSION_VIB_BIN: owner, VIB_SESSION_POSTMORTEM_SH: postmortemScript, VIB_SESSION_RUNNER_SH: finalizer, VIB_SESSION_TEST_FAIL_RECEIPT_WRITE: kind }, stdout: "pipe", stderr: "pipe" });
			expect(await supervisor.exited).toBe(23);
		}
...
test("keeps owner status while exposing canonical-collision and alias publication faults", async () => {
	for (const kind of ["supervisor_failure", "finalization_failure"] as const) for (const fault of ["canonical-collision", "alias"] as const) {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), `vib-receipt-${kind}-${fault}-`)); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const setupBin = await fixture(root); const name = `receipt-${kind}-${fault}-${Date.now()}`; sessions.push({ name, socket: `vib-${name}` });
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ VIB_BIN: setupBin, VIB_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
		...
		if (kind === "supervisor_failure") {
			await executable(owner, `#!/usr/bin/env python3\nimport os, signal, sys\nif "--internal-tmux-owner-isolation" in sys.argv: raise SystemExit(23)\nopen(os.environ["VIB_FIXTURE_RAW_READY"], "w").close()\nsignal.signal(signal.SIGTERM, lambda *_: raise_exit())\ndef raise_exit(): raise SystemExit(0)\nsignal.pause()\n`);
			const ready = path.join(root, "ready"); const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, VIB_SESSION_NAME: name, VIB_SESSION_WORKDIR: dir, VIB_SESSION_STATE_DIR: state, VIB_SESSION_OWNER_GENERATION: generation, VIB_TMUX_OWNER_SERVER_KEY: `vib-${name}`, VIB_SESSION_VIB_BIN: owner, VIB_SESSION_POSTMORTEM_SH: postmortemScript, VIB_SESSION_RUNNER_SH: "/bin/true", VIB_FIXTURE_RAW_READY: ready, ...(injected ? { VIB_SESSION_TEST_FAIL_RECEIPT_WRITE: injected } : {}) }, stdout: "pipe", stderr: "pipe" });
			await waitFor(ready); expect(Bun.spawnSync(["kill", "-TERM", String(supervisor.pid)]).exitCode).toBe(0); expect(await supervisor.exited).toBe(0);
		} else {
			await executable(owner, "#!/usr/bin/env bash\nexit 23\n"); await executable(finalizer, "#!/usr/bin/env bash\nexit 24\n");
			const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, VIB_SESSION_NAME: name, VIB_SESSION_WORKDIR: dir, VIB_SESSION_STATE_DIR: state, VIB_SESSION_OWNER_GENERATION: generation, VIB_TMUX_OWNER_SERVER_KEY: `vib-${name}`, VIB_SESSION_VIB_BIN: owner, VIB_SESSION_POSTMORTEM_SH: postmortemScript, VIB_SESSION_RUNNER_SH: finalizer, ...(injected ? { VIB_SESSION_TEST_FAIL_RECEIPT_WRITE: injected } : {}) }, stdout: "pipe", stderr: "pipe" });
			expect(await supervisor.exited).toBe(23);
		}
