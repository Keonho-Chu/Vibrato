#!/usr/bin/env bash
# Disposable, public-safe evidence for issue #1938. It never targets a Vibrato
...
EVIDENCE_DIR="$REPO_ROOT/.vib/_session-$SESSION_ID/runtime/evidence/issue-1938"
EVIDENCE_PATH="$EVIDENCE_DIR/$PHASE.json"


now() {
  if [[ "${VIB_ISSUE1938_TEST_DISABLE_PYTHON:-}" != 1 ]] && command -v python3 >/dev/null 2>&1; then
  ...
  elif [[ "${VIB_ISSUE1938_TEST_DISABLE_BUN:-}" != 1 ]] && command -v bun >/dev/null 2>&1; then
...
if [[ "${VIB_ISSUE1938_TEST_FORCE_UNSUPPORTED:-}" == 1 ]]; then SYSTEMCTL_USER=false; DISPOSABLE_UNIT=false; fi
[[ "${VIB_ISSUE1938_TEST_DISABLE_PYTHON:-}" == 1 ]] && PYTHON3_AVAILABLE=false
[[ "${VIB_ISSUE1938_TEST_DISABLE_BUN:-}" == 1 ]] && BUN_AVAILABLE=false
...
publish_evidence_temp() {
  local temporary="$1"
  [[ "${VIB_ISSUE1938_TEST_FAIL_EVIDENCE_RENAME:-}" != 1 ]] || { rm -f "$temporary"; return 1; }
...
if os.environ.get("VIB_ISSUE1938_TEST_FAIL_PYTHON_SERIALIZER") == "1": raise SystemExit(1)
PY
...
private_tmux_session_pid() {
  ...
  [[ "${VIB_ISSUE1938_TEST_TMUX_CLEANUP_PROBE:-}" != error ]] || return 2
...
unit_is_explicitly_gone() {
  ...
  [[ "${VIB_ISSUE1938_TEST_SYSTEMD_CLEANUP_PROBE:-}" != error ]] || return 2
...
if [[ -n "${VIB_ISSUE1938_TEST_CLEANUP_PROBE_ONLY:-}" ]]; then
  case "$VIB_ISSUE1938_TEST_CLEANUP_PROBE_ONLY" in
    tmux) TRACKED_SERVERS+=("/nonexistent|test|\$1|$$|$(proc_start_time $$)|$(proc_cgroup $$)") ;;
    monitor)
      TMUX_TMPDIR_PRIVATE="$(mktemp -d)" || exit 1
      socket="vib-issue1938-held-monitor-$$"; monitor_session="issue1938-held-owner-monitor"
      ...
      ;;
    systemd) TRACKED_UNITS+=("vib-issue1938-test.service") ;;
    *) exit 2 ;;
  esac
  exit 1
fi
...
if ! git -C "$REPO_ROOT" diff --quiet -- . ':!/.vib/**' || ! git -C "$REPO_ROOT" diff --cached --quiet -- . ':!/.vib/**' || [[ -n "$(git -C "$REPO_ROOT" ls-files --others --exclude-standard -- ':!/.vib/**')" ]]; then
...
SERVICE_UNIT="vib-issue1938-$RUN_PREFIX-$$-$RANDOM.service"
TMUX_SOCKET="vib-issue1938-$RUN_PREFIX-$$-$RANDOM"
...
independent_scope() { [[ "$1" =~ (^|/)vib-owner-[A-Za-z0-9-]+\.scope(/|;|$) ]]; }
owner_scope() { [[ "$1" =~ (^|/)vib-owner-([A-Za-z0-9-]+)\.scope ]] && printf 'vib-owner-%s.scope\n' "${BASH_REMATCH[2]}"; }
...
run_pre_code() {
  ...
  SCOPE_UNIT="vib-issue1938-$SESSION_ID-$$-$RANDOM.scope"; TRACKED_UNITS+=("$SCOPE_UNIT")
...
run_post_code() {
  ...
  WORKTREE="$(mktemp -d)"; WORKTREE_BRANCH="vib-issue1938-$RUN_PREFIX-$$-$RANDOM"; started="$(now)"
  ...
  mkdir -p "$WORKTREE/tmux" "$EVIDENCE_DIR/artifacts"; TMUX_SOCKET="vib-issue1938-$RUN_PREFIX-$$-$RANDOM"
  ...
  product_bin="$WORKTREE/issue-1938-vib"; tmux_wrapper="$WORKTREE/issue-1938-tmux"
  printf '#!/usr/bin/env bash\nexec %q %q "$@"\n' "$bun_bin" "$WORKTREE/packages/coding-agent/src/cli.ts" >"$product_bin"
  printf '#!/usr/bin/env bash\nexec %q -L "$VIB_ISSUE1938_TMUX_SOCKET" "$@"\n' "$tmux_bin" >"$tmux_wrapper"
  chmod 700 "$product_bin" "$tmux_wrapper"

  raw_session="issue1938-raw-$RUN_PREFIX-$$"; raw_state="$WORKTREE/.vib-session-state/$raw_session"; RAW_SOCKET="vib-${raw_session//[^A-Za-z0-9_.-]/_}"

  if ! run_unsafe_service env TMUX_TMPDIR="$WORKTREE/tmux" VIB_ISSUE1938_TMUX_SOCKET="$RAW_SOCKET" VIB_BIN="$product_bin" VIB_SESSION_TMUX_BIN="$tmux_wrapper" VIB_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS" VIB_SESSION_SKIP_ROUTER=1 bash "$WORKTREE/scripts/vib-session/create.sh" "$raw_session" "$WORKTREE"; then append_case raw_proof_before_exec failed "$started" "$(now)" '' "$raw_session" '' '' proven '' launch_failed "$SERVICE_UNIT"; return 1; fi
  ...
  managed_session="issue1938-managed-$RUN_PREFIX-$$"; SERVICE_UNIT="vib-issue1938-managed-$RUN_PREFIX-$$-$RANDOM.service"; TRACKED_UNITS+=("$SERVICE_UNIT")


  if ! start_unsafe_service env -u TMUX -u TMUX_PANE -u VIB_TMUX_LAUNCHED TMUX_TMPDIR="$WORKTREE/tmux" VIB_ISSUE1938_TMUX_SOCKET="$TMUX_SOCKET" VIB_TMUX_COMMAND="$tmux_wrapper" VIB_TMUX_SESSION="$managed_session" VIB_LAUNCH_POLICY=tmux script -qefc "$product_bin --tmux" /dev/null; then append_case managed_proof_before_exec failed "$started" "$(now)" '' "$managed_session" '' '' proven '' launch_failed "$SERVICE_UNIT"; return 1; fi
  ...
  if TMUX_TMPDIR="$WORKTREE/tmux" VIB_ISSUE1938_TMUX_SOCKET="$RAW_SOCKET" VIB_TMUX_COMMAND="$tmux_wrapper" "$product_bin" session force-close "$raw_session" >/dev/null; then :; else append_case expected_close_verdict failed "$started" "$(now)" "$raw_pid" "$raw_session" "$raw_cgroup" SIGTERM expected_operator_shutdown SIGTERM unavailable; return 1; fi
  ...
  recovery_session="issue1938-recovery-$RUN_PREFIX-$$"; recovery_state="$WORKTREE/.vib-session-state/$recovery_session"; RECOVERY_SOCKET="vib-${recovery_session//[^A-Za-z0-9_.-]/_}"; SERVICE_UNIT="vib-issue1938-recovery-$RUN_PREFIX-$$-$RANDOM.service"; TRACKED_UNITS+=("$SERVICE_UNIT")


  if ! run_unsafe_service env TMUX_TMPDIR="$WORKTREE/tmux" VIB_ISSUE1938_TMUX_SOCKET="$RECOVERY_SOCKET" VIB_BIN="$product_bin" VIB_SESSION_TMUX_BIN="$tmux_wrapper" VIB_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS" VIB_SESSION_SKIP_ROUTER=1 bash "$WORKTREE/scripts/vib-session/create.sh" "$recovery_session" "$WORKTREE"; then append_case unexpected_incident_recovery failed "$started" "$(now)" '' "$recovery_session" '' SIGTERM unexpected_owner_loss UNKNOWN launch_failed; return 1; fi
  ...
  SERVICE_UNIT="vib-issue1938-recovered-$RUN_PREFIX-$$-$RANDOM.service"; TRACKED_UNITS+=("$SERVICE_UNIT")
