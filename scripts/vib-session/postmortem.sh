#!/usr/bin/env bash
# Public-safe Vibrato session postmortem helpers. Markers contain only lifecycle
# state and paths; they never read or persist pane, prompt, or runtime payloads.


vib_session_git_dirty_boolean() {
  local workdir="${1:-}"
  local status
...
vib_session_write_public_marker() {
...
vib_session_validate_raw_verdict() {
...
PY
 ...
 vib_session_publish_current_alias() {
  ...
  if [[ "${VIB_SESSION_TRANSITION_LOCK_HELD:-0}" != 1 ]]; then
    exec {vib_alias_lock_fd}>"$transition_lock"
    flock -x "$vib_alias_lock_fd"
  fi
...
PY
  local rc=$?
  if [[ "${VIB_SESSION_TRANSITION_LOCK_HELD:-0}" != 1 ]]; then
    flock -u "$vib_alias_lock_fd"
    eval "exec ${vib_alias_lock_fd}>&-"
  fi
  return "$rc"
}

 vib_session_write_vanished_json() {
