---
name: vib-sdk-author
description: Author trusted-local TypeScript and Python scripts that operate Vibrato sessions through the broker-bound CLI.
---

# Author broker-bound Vibrato session scripts
...
- Invoke the documented `vib sdk session` CLI as a child process. It routes live controls through `SessionRouter` and lifecycle mutations through `SessionLifecycleService` and the Broker.
...
- Never scan `.vib/state/sdk`, parse endpoint records, read endpoint credentials, or open a raw per-session WebSocket.
- Use only `vib sdk session raw query <sessionId> --query <query>` for fixed inspection queries and `vib sdk session raw control <sessionId> --op <operation> --json-input <object> --confirm` for fixed controls.
...
- Keep broader lifecycle orchestration behind `vib sdk session raw global` with a stable idempotency key or an SDK-core lifecycle facade; the canonical templates do not invent lifecycle routing.
